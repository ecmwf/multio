...
!>
!> This module defines the `FILTER_METHOD_T` type, along with its associated
...
#define PP_FILE_NAME 'filter_method_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_METHOD_MOD'
MODULE FILTER_METHOD_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_METHOD_T
  ...
  !> @details If `.TRUE.`, the `METHODS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: METHODS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_METHOD_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_METHOD_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_METHOD_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_METHOD_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_METHOD_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_METHOD_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_METHOD_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_METHOD_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_METHOD_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_METHOD_FREE
...
PUBLIC :: FILTER_METHOD_T

CONTAINS


!>
!> @brief Initializes the `FILTER_METHOD_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_METHOD_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_METHOD_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_METHOD_INIT'
FUNCTION FILTER_METHOD_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_METHOD_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%METHODS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%METHODS_, HOOKS )
...
END FUNCTION FILTER_METHOD_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_METHOD_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_METHOD_MATCH'
FUNCTION FILTER_METHOD_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Match the filter
  IF ( MSG%METHOD .EQ. UNDEF_PARAM_E ) THEN
  ...
  ELSE
    ...
    CASE( FLT_INT_MATCH_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%METHODS_%MATCH( MSG%METHOD, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%METHOD_ .EQ. MSG%METHOD)
      ENDIF
    ...
    CASE( FLT_INT_IGNORE_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%METHOD_ .EQ. MSG%METHOD)
      ENDIF

    CASE( FLT_INT_GT_E )

      MATCH = (MSG%METHOD .GT. THIS%METHOD_ )

    CASE( FLT_INT_GE_E )

      MATCH = (MSG%METHOD .GE. THIS%METHOD_ )

    CASE( FLT_INT_LE_E )

      MATCH = (MSG%METHOD .LE. THIS%METHOD_ )

    CASE( FLT_INT_LT_E )

      MATCH = (MSG%METHOD .LT. THIS%METHOD_ )

    CASE( FLT_INT_HAS_E )

      MATCH = .NOT.(MSG%METHOD .EQ. THIS%METHOD_ )

    CASE( FLT_INT_LACKS_E )

      MATCH = (MSG%METHOD .EQ. THIS%METHOD_ )
...
!      PP_DEBUG_PUSH_MSG_TO_FRAME( '"method" filter can be used only with "levtype"="pl"' )
...
END FUNCTION FILTER_METHOD_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_METHOD_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_METHOD_PRINT'
FUNCTION FILTER_METHOD_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'METHOD FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_METHOD_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_METHOD_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_METHOD_FREE'
FUNCTION FILTER_METHOD_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%METHODS_%FREE( HOOKS )
...
END FUNCTION FILTER_METHOD_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_METHOD_MOD
