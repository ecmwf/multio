...
!>
!> This module defines the `FILTER_ANOFFSET_T` type, along with its associated
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_ANOFFSET_T
  ...
  !> @details If `.TRUE.`, the `ANOFFSETS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: ANOFFSETS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_ANOFFSET_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_ANOFFSET_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_ANOFFSET_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_ANOFFSET_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_ANOFFSET_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_ANOFFSET_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_ANOFFSET_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_ANOFFSET_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_ANOFFSET_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_ANOFFSET_FREE
...
PUBLIC :: FILTER_ANOFFSET_T

CONTAINS


!>
!> @brief Initializes the `FILTER_ANOFFSET_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_ANOFFSET_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_ANOFFSET_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_ANOFFSET_INIT'
FUNCTION FILTER_ANOFFSET_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_ANOFFSET_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%ANOFFSETS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%ANOFFSETS_, HOOKS )
...
END FUNCTION FILTER_ANOFFSET_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_ANOFFSET_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ANOFFSET_MATCH'
FUNCTION FILTER_ANOFFSET_MATCH( THIS, MSG, PAR, MATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Match the filter
  ! IF ( MSG%ANOFFSET .EQ. UNDEF_PARAM_E ) THEN
  !   SELECT CASE( THIS%FILTER_TYPE_ )
  !       CASE( FLT_INT_LACKS_E )

  !           MATCH = (MSG%ANOFFSET .EQ. THIS%ANOFFSET_ )
  !       CASE DEFAULT
  ...
  ! ELSE
    ...
    CASE( FLT_INT_MATCH_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%ANOFFSETS_%MATCH( MSG%ANOFFSET, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%ANOFFSET_ .EQ. MSG%ANOFFSET)
      ENDIF
    ...
    CASE( FLT_INT_IGNORE_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%ANOFFSET_ .EQ. MSG%ANOFFSET)
      ENDIF

    CASE( FLT_INT_GT_E )

      MATCH = (MSG%ANOFFSET .GT. THIS%ANOFFSET_ )

    CASE( FLT_INT_GE_E )

      MATCH = (MSG%ANOFFSET .GE. THIS%ANOFFSET_ )

    CASE( FLT_INT_LE_E )

      MATCH = (MSG%ANOFFSET .LE. THIS%ANOFFSET_ )

    CASE( FLT_INT_LT_E )

      MATCH = (MSG%ANOFFSET .LT. THIS%ANOFFSET_ )

    CASE( FLT_INT_HAS_E )
    
      MATCH = .NOT.(MSG%ANOFFSET .EQ. THIS%ANOFFSET_ )
...
!      PP_DEBUG_PUSH_MSG_TO_FRAME( '"anoffset" filter can be used only with "levtype"="pl"' )
...
END FUNCTION FILTER_ANOFFSET_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_ANOFFSET_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ANOFFSET_PRINT'
FUNCTION FILTER_ANOFFSET_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'ANOFFSET FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_ANOFFSET_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_ANOFFSET_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ANOFFSET_FREE'
FUNCTION FILTER_ANOFFSET_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%ANOFFSETS_%FREE( HOOKS )
...
END FUNCTION FILTER_ANOFFSET_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_ANOFFSET_MOD
