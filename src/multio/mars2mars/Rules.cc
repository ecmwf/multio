#include "multio/mars2mars/Rules.h"
#include "multio/datamod/MarsMiscGeo.h"
#include "multio/mars2grib/rules/Matcher.h"
#include "multio/mars2grib/rules/ParamMatcher.h"
#include "multio/mars2mars/rules/Rule.h"
#include "multio/mars2mars/rules/Setter.h"


namespace multio::mars2mars {

using namespace rules;
using namespace multio::mars2grib::rules;
using namespace datamod;

//-----------------------------------------------------------------------------
// SOL fixes
//-----------------------------------------------------------------------------

// To param 262024

auto fixParam35ToSol1() {
    return rule(all(matchParams(35), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(1),  //
                setKey<MarsKeys::PARAM>(262024));                                        //
}
auto fixParam36ToSol2() {
    return rule(all(matchParams(36), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(2),  //
                setKey<MarsKeys::PARAM>(262024));                                        //
}
auto fixParam37ToSol3() {
    return rule(all(matchParams(37), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(3),  //
                setKey<MarsKeys::PARAM>(262024));                                        //
}
auto fixParam38ToSol4() {
    return rule(all(matchParams(38), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(4),  //
                setKey<MarsKeys::PARAM>(262024));                                        //
}

// To param 262199

auto fixParam39ToSol1() {
    return rule(all(matchParams(39), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(1),  //
                setKey<MarsKeys::PARAM>(262199));                                        //
}
auto fixParam40ToSol2() {
    return rule(all(matchParams(40), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(2),  //
                setKey<MarsKeys::PARAM>(262199));                                        //
}
auto fixParam41ToSol3() {
    return rule(all(matchParams(41), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(3),  //
                setKey<MarsKeys::PARAM>(262199));                                        //
}
auto fixParam42ToSol4() {
    return rule(all(matchParams(42), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),          //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(4),  //
                setKey<MarsKeys::PARAM>(262199));                                        //
}


// To param 262360

auto fixParam139ToSol1() {
    return rule(all(matchParams(139), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),         //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(1),  //
                setKey<MarsKeys::PARAM>(262360));                                        //
}
auto fixParam170ToSol2() {
    return rule(all(matchParams(170), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),         //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(2),  //
                setKey<MarsKeys::PARAM>(262360));                                        //
}
auto fixParam183ToSol3() {
    return rule(all(matchParams(183), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),         //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(3),  //
                setKey<MarsKeys::PARAM>(262360));                                        //
}
auto fixParam236ToSol4() {
    return rule(all(matchParams(236), OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}),         //
                setKey<MarsKeys::LEVTYPE>(LevType::SOL), setKey<MarsKeys::LEVELIST>(4),  //
                setKey<MarsKeys::PARAM>(262360));                                        //
}


//-----------------------------------------------------------------------------
// Tables Version fixes
//-----------------------------------------------------------------------------

auto fixTablesVersionChemAccum() {
    return rule(all(matchParams(228080, 228081, 228082, 233032, 233033, 233034, 233035, 235062, 235063, 235064)),  //
                setKey<MiscKeys::TablesVersion>(30));                                                              //
}
auto fixTablesVersionChemInstant() {
    return rule(all(matchParams(228083, 228084, 228085)),  //
                setKey<MiscKeys::TablesVersion>(30));      //
}


//-----------------------------------------------------------------------------
// Cloud cover
//-----------------------------------------------------------------------------

auto fixCloudParam164() {
    return rule(all(matchParams(164)),                                    //
                setKey<MarsKeys::PARAM>(228164), SetScaleFactor{100.0});  //
}
auto fixCloudParam186() {
    return rule(all(matchParams(186)),                                  //
                setKey<MarsKeys::PARAM>(3073), SetScaleFactor{100.0});  //
}
auto fixCloudParam187() {
    return rule(all(matchParams(187)),                                  //
                setKey<MarsKeys::PARAM>(3074), SetScaleFactor{100.0});  //
}
auto fixCloudParam188() {
    return rule(all(matchParams(188)),                                  //
                setKey<MarsKeys::PARAM>(3075), SetScaleFactor{100.0});  //
}


//-----------------------------------------------------------------------------
// Precipitation
//-----------------------------------------------------------------------------

auto fixConvectivePrecip143() {
    return rule(all(matchParams(143)),                                     //
                setKey<MarsKeys::PARAM>(228143), SetScaleFactor{1000.0});  //
}

auto fixTotalPrecip228() {
    return rule(all(matchParams(228)),                                     //
                setKey<MarsKeys::PARAM>(228228), SetScaleFactor{1000.0});  //
}


//-----------------------------------------------------------------------------
// Height above
//-----------------------------------------------------------------------------

auto fixHeightAboveGround2m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, OneOf<MarsKeys::LEVELIST>{{0}},
                    matchParams(167, 168, 174096, 228037)),  //
                setKey<MarsKeys::LEVELIST>(2));              //
}

auto fixHeightAboveGround10m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, OneOf<MarsKeys::LEVELIST>{{0}},
                    matchParams(228029, 228131, 228132, 165, 166, 207)),  //
                setKey<MarsKeys::LEVELIST>(10));                          //
}

auto fixHeightAboveSea() {
    return rule(all(matchParams(140233, 140245, 140249)),                             //
                setKey<MarsKeys::LEVELIST>(10), setKey<MiscKeys::BitsPerValue>(24));  //
}


//-----------------------------------------------------------------------------
// Misc level fixes
//-----------------------------------------------------------------------------

auto fixMapToSol() {
    return rule(
        all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, Has<MarsKeys::LEVELIST>{}, NoneOf<MarsKeys::LEVELIST>{{0}},
            matchParams(33, 238, 228038, 228141, 260360, 262000, 262024, 260199)),  //
        setKey<MarsKeys::LEVTYPE>(LevType::SOL));                                   //
}


auto fixRunOffWaterParam205() {
    return rule(all(matchParams(205)),                                     //
                setKey<MarsKeys::PARAM>(231002), SetScaleFactor{1000.0});  //
}

auto fixSnowfallWaterEquivParam() {
    return rule(all(matchParams(144)),                                     //
                setKey<MarsKeys::PARAM>(228114), SetScaleFactor{1000.0});  //
}


//-----------------------------------------------------------------------------
// Fix Timespan
//-----------------------------------------------------------------------------

auto fixTimespanMax2T() {
    return rule(all(matchParams(122, 228026, 201)),  //
                setKey<MarsKeys::PARAM>(237167));    //
}

auto fixTimespanMaxCape() {
    return rule(all(matchParams(228035)),          //
                setKey<MarsKeys::PARAM>(237117));  //
}

auto fixTimespanMaxMuCapes() {
    return rule(all(matchParams(228036)),          //
                setKey<MarsKeys::PARAM>(237321));  //
}

auto fixTimespanMaxPrecipRate() {
    return rule(all(matchParams(228222, 228224, 228226)),  //
                setKey<MarsKeys::PARAM>(237055));          //
}

auto fixTimespanMaxWindGust() {
    return rule(all(matchParams(123, 228028, 49)),  //
                setKey<MarsKeys::PARAM>(237318));   //
}

auto fixTimespanMeanFlashDensity() {
    return rule(all(matchParams(228051, 228057, 228058)),  //
                setKey<MarsKeys::PARAM>(235326));          //
}

auto fixTimespanMin2T() {
    return rule(all(matchParams(121, 228027, 202)),  //
                setKey<MarsKeys::PARAM>(238167));    //
}

auto fixTimespanMinPrecipRate() {
    return rule(all(matchParams(228223, 228225, 228227)),  //
                setKey<MarsKeys::PARAM>(238055));          //
}

auto fixTimespanModePrecip() {
    return rule(all(matchParams(260320, 260321, 260339)),  //
                setKey<MarsKeys::PARAM>(260683));          //
}

auto fixTimespanSeverityPrecip() {
    return rule(all(matchParams(260318, 260319, 260338)),  //
                setKey<MarsKeys::PARAM>(260682));          //
}


//-----------------------------------------------------------------------------
// Fix Windspeed
//-----------------------------------------------------------------------------

auto fixWindspeedU100m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228246)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(100),
                setKey<MarsKeys::PARAM>(131));  //
}

auto fixWindspeedU200m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228239)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(200),
                setKey<MarsKeys::PARAM>(131));  //
}

auto fixWindspeedV100m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228247)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(100),
                setKey<MarsKeys::PARAM>(132));  //
}

auto fixWindspeedV200m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228240)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(200),
                setKey<MarsKeys::PARAM>(132));  //
}

auto fixWindspeed100m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228249)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(100),
                setKey<MarsKeys::PARAM>(10));  //
}

auto fixWindspeed200m() {
    return rule(all(OneOf<MarsKeys::LEVTYPE>{{LevType::SFC}}, matchParams(228241)),  //
                setKey<MarsKeys::LEVTYPE>(LevType::HL), setKey<MarsKeys::LEVELIST>(200),
                setKey<MarsKeys::PARAM>(131));  //
}


//-----------------------------------------------------------------------------
// Wave rules bits per value
//-----------------------------------------------------------------------------


auto ruleWaveBitsPerValueS1() {
    return rule(all(GreaterThan<MarsKeys::STEP>{std::chrono::hours{1}},
                    matchParams(22824)),              //
                setKey<MiscKeys::BitsPerValue>(24));  //
}

auto ruleWaveBitsPerValue() {
    return rule(all(matchParams(paramRange(140098, 140105), paramRange(140112, 140129), paramRange(140207, 140209),
                                paramRange(140211, 140212), paramRange(140214, 140232), paramRange(140234, 140239),
                                140244, paramRange(140251, 140254))),  //
                setKey<MiscKeys::BitsPerValue>(24));                   //
}


//-----------------------------------------------------------------------------
// All rules
//-----------------------------------------------------------------------------

const RuleList& allRules() {
    static auto all_ = ruleList(        //
        fixParam35ToSol1(),             //
        fixParam36ToSol2(),             //
        fixParam37ToSol3(),             //
        fixParam38ToSol4(),             //
        fixParam39ToSol1(),             //
        fixParam40ToSol2(),             //
        fixParam41ToSol3(),             //
        fixParam42ToSol4(),             //
        fixParam139ToSol1(),            //
        fixParam170ToSol2(),            //
        fixParam183ToSol3(),            //
        fixParam236ToSol4(),            //
        fixTablesVersionChemAccum(),    //
        fixTablesVersionChemInstant(),  //
        fixCloudParam164(),             //
        fixCloudParam186(),             //
        fixCloudParam187(),             //
        fixCloudParam188(),             //
        fixConvectivePrecip143(),       //
        fixTotalPrecip228(),            //
        fixHeightAboveGround2m(),       //
        fixHeightAboveGround10m(),      //
        fixHeightAboveSea(),            //
        fixMapToSol(),                  //
        fixRunOffWaterParam205(),       //
        fixSnowfallWaterEquivParam(),   //
        fixTimespanMax2T(),             //
        fixTimespanMaxCape(),           //
        fixTimespanMaxMuCapes(),        //
        fixTimespanMaxPrecipRate(),     //
        fixTimespanMaxWindGust(),       //
        fixTimespanMeanFlashDensity(),  //
        fixTimespanMin2T(),             //
        fixTimespanMinPrecipRate(),     //
        fixTimespanModePrecip(),        //
        fixTimespanSeverityPrecip(),    //
        fixWindspeedU100m(),            //
        fixWindspeedU200m(),            //
        fixWindspeedV100m(),            //
        fixWindspeedV200m(),            //
        fixWindspeed100m(),             //
        fixWindspeed200m(),             //
        ruleWaveBitsPerValueS1(),       //
        ruleWaveBitsPerValue()          //
    );
    return all_;
}

std::optional<MappingResult> applyMappings(MarsKeyValueSet& mars, MiscKeyValueSet& misc) {
    return allRules()(mars, misc);
}

}  // namespace multio::mars2mars
