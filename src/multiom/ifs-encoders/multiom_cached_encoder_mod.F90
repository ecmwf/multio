...
TYPE :: MULTIOM_CACHED_ENCODERS_T
  ...
  !> Flag indicating if the encoder is initialized
...
CONTAINS
  ...
  PROCEDURE, PUBLIC, NON_OVERRIDABLE, PASS :: GET_ENCODER_OPTIONS   => MULTIO_ENCODER_GET_ENCODER_OPTIONS
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: JSON1
  CHARACTER(LEN=:), ALLOCATABLE :: JSON2
  ...
  PP_TRYCALL(ERRFLAG_MARS_TO_JSON) MSG%TO_JSON( JSON1, HOOKS )
...
&     MSG, PAR, MAPPED_MSG, MAPPED_PAR, MAPPING_TAG, MAPPING_NAME, HOOKS )
    ...
    ! Extract mapped json
    PP_TRYCALL(ERRFLAG_MARS_TO_JSON) MAPPED_MSG%TO_JSON( JSON2, HOOKS )
    IF ( THIS%ENCODER_OPTIONS%PRINT_DICTIONARIES ) THEN
      !> Print Mapped message
      IF ( ALLOCATED(JSON2) ) THEN
        WRITE(*,'(A,A)') ' + MAPPED   - MARS to JSON: ', JSON2
        DEALLOCATE( JSON2, STAT=DEALLOC_STAT, ERRMSG=ERRMSG )
...
&    THIS%CACHE_OPTIONS, THIS%ENCODER_OPTIONS, THIS%FILTER_OPTIONS, HOOKS )
    ...
    DO J = 1, ENCODER_SZ
      ...
      IF ( THIS%ENCODER_OPTIONS%DUMP_DICTIONARIES ) THEN
        !> Perform the encoding
...
&            MAPPING_TAG, MAPPING_NAME, ENCODER_TAG, ENCODER_NAME, METADATA, HOOKS )
  ...
  IF ( ALLOCATED(JSON1) ) THEN
    DEALLOCATE( JSON1, STAT=DEALLOC_STAT, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STAT .NE. 0, ERRFLAG_UNABLE_TO_DEALLOCATE )
  ENDIF
  IF ( ALLOCATED(JSON2) ) THEN
    DEALLOCATE( JSON2, STAT=DEALLOC_STAT, ERRMSG=ERRMSG )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_MAPPING_COLLECTION_EVAL)
      ...
      IF ( ALLOCATED(JSON1) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Input  mars: '//TRIM(ADJUSTL(JSON1)) )
        DEALLOCATE( JSON1, STAT=DEALLOC_STAT )
      ENDIF
    CASE (ERRFLAG_ENCODER_COLLECTION_ENCODE)
      ...
      ENDIF
      IF ( ALLOCATED(JSON2) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Mapped mars: '//TRIM(ADJUSTL(JSON2)) )
        DEALLOCATE( JSON2, STAT=DEALLOC_STAT )
      ENDIF
...
IMPLICIT NONE
  ...
  IF ( THIS%INITIALIZED ) THEN
    ...
    IF ( ASSOCIATED(THIS%METADATA) ) THEN
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MULTIO_ENCODER_GET_ENCODER_OPTIONS'
PP_THREAD_SAFE FUNCTION MULTIO_ENCODER_GET_ENCODER_OPTIONS( THIS, OPTIONS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(MULTIOM_CACHED_ENCODERS_T), TARGET, INTENT(INOUT) :: THIS
  TYPE(GRIB_ENCODER_OPTIONS_T), POINTER,    INTENT(INOUT) :: OPTIONS
  TYPE(HOOKS_T),                            INTENT(INOUT) :: HOOKS
  ...
  ! Error handlign
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.THIS%INITIALIZED, ERRFLAG_NOT_INITIALIZED )

  ! Associate the pointer
  OPTIONS => THIS%ENCODER_OPTIONS
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_INITIALIZED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'The encoder is not initialized' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION MULTIO_ENCODER_GET_ENCODER_OPTIONS
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
