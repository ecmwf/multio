...
#define PP_FILE_NAME 'encoding_rules_factory_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'ENCODING_RULES_FACTORY_MOD'
MODULE ENCODING_RULES_FACTORY_MOD
IMPLICIT NONE

PRIVATE

PUBLIC :: MAKE_ENCODING_RULES
PUBLIC :: DESTROY_ENCODING_RULES

CONTAINS

#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_ENCODING_RULES'
PP_THREAD_SAFE FUNCTION MAKE_ENCODING_RULES( RULES, &
&   RULES_CONFIGURATION, FILTER_OPT, ENCODER_OPT, HOOKS, NESTING_LEVEL ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(ENCODING_RULE_CONTAINER_T), POINTER, DIMENSION(:), INTENT(INOUT) :: RULES
  ...
  DO I = 1, N_RULES
    ...
    IF ( HAS_RULE .AND. .NOT.HAS_FILE ) THEN
      ...
      IF ( PRESENT(NESTING_LEVEL) ) THEN
        PP_TRYCALL(ERRFLAG_READ_RULE_FROM_CFG) MAKE_RULE_FROM_CFG( RULES(I), I, RULE_CONFIGURATION, &
&                       FILTER_OPT, ENCODER_OPT, HOOKS, NESTING_LEVEL=NESTING_LEVEL )
      ELSE
        PP_TRYCALL(ERRFLAG_READ_RULE_FROM_CFG) MAKE_RULE_FROM_CFG( RULES(I), I, RULE_CONFIGURATION, &
&                       FILTER_OPT, ENCODER_OPT, HOOKS )
      ENDIF
    ...
    ELSE IF ( HAS_FILE .AND. .NOT. HAS_RULE) THEN
      ...
      !> Read rule from file
      IF ( PRESENT(NESTING_LEVEL) ) THEN
        PP_TRYCALL(ERRFLAG_READ_RULE_FROM_FILE) MAKE_RULE_FROM_FILE( RULES(I), I, RULE_CONFIGURATION, &
&                       FILTER_OPT, ENCODER_OPT, HOOKS, NESTING_LEVEL=NESTING_LEVEL )
      ELSE
        PP_TRYCALL(ERRFLAG_READ_RULE_FROM_FILE) MAKE_RULE_FROM_FILE( RULES(I), I, RULE_CONFIGURATION, &
&                       FILTER_OPT, ENCODER_OPT, HOOKS )
...
END FUNCTION MAKE_ENCODING_RULES
...
RECURSIVE FUNCTION MAKE_RULE_FROM_CFG( RULE, ID, CFG, FILTER_OPT, ENCODER_OPT, HOOKS, NESTING_LEVEL ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(ENCODING_RULE_CONTAINER_T), INTENT(INOUT) :: RULE
  ...
  !> Read the rule
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONSTRUCT_THE_RULE) RULE%RULE_%INIT( ID, CFG, FILTER_OPT, ENCODER_OPT, HOOKS )
...
RECURSIVE FUNCTION MAKE_RULE_FROM_FILE( RULE, ID, CFG, FILTER_OPT, ENCODER_OPT, HOOKS, NESTING_LEVEL ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Logging
  IF ( PRESENT(NESTING_LEVEL) ) THEN
    IF (ENCODER_OPT%LOG_RULES_FILES) THEN
      WRITE(*,'(A)') REPEAT(' ',NESTING_LEVEL*2)//'Load rule from file: "'//TRIM(ADJUSTL(FNAME))//'"'
    ENDIF
  ENDIF
  ...
  !> Read the rule
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONSTRUCT_THE_RULE) RULE%RULE_%INIT( ID, LOC_CFG, FILTER_OPT, ENCODER_OPT, HOOKS )
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_ENCODING_RULES'
RECURSIVE FUNCTION DESTROY_ENCODING_RULES( RULES, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  DO I = 1, SIZE(RULES)
    ...
    PP_TRYCALL(ERRFLAG_RULE_FREE_ERROR) RULES(I)%RULE_%FREE( OPT, HOOKS )
...
END FUNCTION DESTROY_ENCODING_RULES
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE ENCODING_RULES_FACTORY_MOD
