...
TYPE :: FORTRAN_MESSAGE_T
  ...
  INTEGER(KIND=JPIB_K) :: TIMESPAN = UNDEF_PARAM_E
  CHARACTER(LEN=32)    :: STATTYPE = REPEAT('*',32)
  ...
  !> DestinE mars specific
  INTEGER(KIND=JPIB_K) :: DATASET = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: ACTIVITY = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: EXPERIMENT = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: GENERATION = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: REALIZATION = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: RESOLUTION = UNDEF_PARAM_E

  ! multi model
  INTEGER(KIND=JPIB_K) :: METHOD = UNDEF_PARAM_E
  INTEGER(KIND=JPIB_K) :: SYSTEM = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  THIS%DATASET     = UNDEF_PARAM_E
  THIS%ACTIVITY    = UNDEF_PARAM_E
  THIS%EXPERIMENT   = UNDEF_PARAM_E
  THIS%GENERATION  = UNDEF_PARAM_E
  THIS%REALIZATION = UNDEF_PARAM_E
  THIS%RESOLUTION  = UNDEF_PARAM_E

  THIS%METHOD = UNDEF_PARAM_E
  THIS%SYSTEM  = UNDEF_PARAM_E

  !> String members
  THIS%TIMESPAN = UNDEF_PARAM_E
  THIS%STATTYPE = REPEAT('*',32)
...
PP_THREAD_SAFE FUNCTION FORTRAN_MESSAGE_PRINT( THIS, UNIT, HOOKS ) RESULT(RET)
  ...
  USE :: ENUMERATORS_MOD,          ONLY: IACTIVITY2CACTIVITY
  USE :: ENUMERATORS_MOD,          ONLY: IDATASET2CDATASET
  USE :: ENUMERATORS_MOD,          ONLY: IEXPERIMENT2CEXPERIMENT
  USE :: ENUMERATORS_MOD,          ONLY: IRESOLUTION2CRESOLUTION
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ILEVTYPE2CLEVTYPE=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IREPRES2CREPRES=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IACTIVITY2CACTIVITY=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IDATASET2CDATASET=9_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IEXPERIMENT2CEXPERIMENT=10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IRESOLUTION2CRESOLUTION=11_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_DATE_TO_STRING=12_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_TIME_TO_STRING=13_JPIB_K
  ...
  ! Print TIMESPAN
  IF ( THIS%TIMESPAN .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%TIMESPAN
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - TIMESPAN....: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print STATTYPE
  IF ( THIS%STATTYPE .NE. REPEAT('*',32) ) THEN
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - STATTYPE...: ', TRIM(ADJUSTL(THIS%STATTYPE))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  ! Print DATASET
  IF ( THIS%DATASET .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IDATASET2CDATASET) IDATASET2CDATASET( THIS%DATASET, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - DATASET..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print ACTIVITY
  IF ( THIS%ACTIVITY .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IACTIVITY2CACTIVITY) IACTIVITY2CACTIVITY( THIS%ACTIVITY, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - ACTIVITY..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print GENERATION
  IF ( THIS%GENERATION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%GENERATION
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - GENERATION..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print EXPERIMENT
  IF ( THIS%EXPERIMENT .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IEXPERIMENT2CEXPERIMENT) IEXPERIMENT2CEXPERIMENT( THIS%EXPERIMENT, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - EXPERIMENT..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print REALIZATION
  IF ( THIS%REALIZATION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%REALIZATION
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - REALIZATION..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print RESOLUTION
  IF ( THIS%RESOLUTION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IRESOLUTION2CRESOLUTION) IRESOLUTION2CRESOLUTION( THIS%RESOLUTION, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - RESOLUTION..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  ! Print method
  IF ( THIS%METHOD .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%METHOD
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - METHO..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print system
  IF ( THIS%SYSTEM .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%SYSTEM
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - SYSTEM..: ', TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_IACTIVITY2CACTIVITY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'IACTIVITY2CACTIVITY failed' )
    CASE (ERRFLAG_IDATASET2CDATASET)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'IDATASET2CDATASET failed' )
    CASE (ERRFLAG_IEXPERIMENT2CEXPERIMENT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'IEXPERIMENT2CEXPERIMENT failed' )
    CASE (ERRFLAG_IRESOLUTION2CRESOLUTION)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'IRESOLUTION2CRESOLUTION failed' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IEXPERIMENT2CEXPERIMENT=10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_IRESOLUTION2CRESOLUTION=11_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_OUT_OF_BOUNDS=12_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_ALLOCATE=13_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_DATE_TO_STRING=14_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_TIME_TO_STRING=15_JPIB_K
  ...
  IF ( THIS%TIMESPAN .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"timespan":"'//TRIM(ADJUSTL(CTMP))//'"'
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%STATTYPE .NE. REPEAT( '*',32) ) THEN
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"stattype":"'//TRIM(ADJUSTL(THIS%STATTYPE))//'"'
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%DATASET .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%DATASET
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    PP_TRYCALL(ERRFLAG_IDATASET2CDATASET) IDATASET2CDATASET( THIS%DATASET, CTMP(1:16), HOOKS )
    JSON(CNT) = '"dataset":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%ACTIVITY .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"activity":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%GENERATION .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"generation":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%EXPERIMENT .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"experment":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%REALIZATION .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"realization":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%RESOLUTION .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"resolution":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%METHOD .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"method":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
  ...
  IF ( THIS%SYSTEM .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"system":'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
...
IMPLICIT NONE
  ...
  IF ( THIS%STATTYPE .NE. REPEAT( '*',32) ) THEN
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = 'stattype-'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 1
  ENDIF
...
IMPLICIT NONE
  ...
  CTMP32         = THIS%STATTYPE
  THIS%STATTYPE  = OTHER%STATTYPE
  OTHER%STATTYPE = CTMP32
  ...
  ITMP            = THIS%EXPERIMENT
  THIS%EXPERIMENT  = OTHER%EXPERIMENT
  OTHER%EXPERIMENT = ITMP

  ITMP            = THIS%GENERATION
  THIS%GENERATION  = OTHER%GENERATION
  OTHER%GENERATION = ITMP

  ITMP            = THIS%REALIZATION
  THIS%REALIZATION  = OTHER%REALIZATION
  OTHER%REALIZATION = ITMP

  ITMP            = THIS%RESOLUTION
  THIS%RESOLUTION  = OTHER%RESOLUTION
  OTHER%RESOLUTION = ITMP
...
PP_THREAD_SAFE FUNCTION FORTRAN_MESSAGE_SET_ENUM_INT( THIS, ID, VALUE, HOOKS ) RESULT(RET)
  ...
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_TIMESPAN_E
  ...
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_DATASET_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_ACTIVITY_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_EXPERIMENT_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_GENERATION_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_REALIZATION_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_RESOLUTION_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_METHOD_E
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGINTFLD_SYSTEM_E
...
PP_THREAD_SAFE FUNCTION FORTRAN_MESSAGE_SET_ENUM_STRING( THIS, ID, VALUE, HOOKS ) RESULT(RET)
  ...
  USE :: DATAKINDS_DEF_MOD, ONLY: JPIB_K
  USE :: HOOKS_MOD,         ONLY: HOOKS_T
  USE :: FORTRAN_MESSAGE_ENUMERATORS_MOD, ONLY: MSGSTRFLD_STATTYPE_E
...
PP_THREAD_SAFE FUNCTION FORTRAN_MESSAGE_READ_FROM_YAML( MSG, CONFIG, HOOKS ) RESULT(RET)
  ...
  USE :: ENUMERATORS_MOD, ONLY: CDATASET2IDATASET
  USE :: ENUMERATORS_MOD, ONLY: CACTIVITY2IACTIVITY
  USE :: ENUMERATORS_MOD, ONLY: CEXPERIMENT2IEXPERIMENT
  USE :: ENUMERATORS_MOD, ONLY: CRESOLUTION2IRESOLUTION
...
IMPLICIT NONE
  ...
  IF ( HAS_MESSAGE  ) THEN
    ...
    !> Read the "dataset"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'dataset', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      P_FILTER => CDATASET2IDATASET
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER_WITH_FILTER( MSG_CONFIG, 'dataset', MSG%DATASET, P_FILTER, HOOKS )
    ELSE
      MSG%DATASET = UNDEF_PARAM_E
    ENDIF

    !> Read the "activity"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'activity', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      P_FILTER => CACTIVITY2IACTIVITY
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER_WITH_FILTER( MSG_CONFIG, 'activity', MSG%ACTIVITY, P_FILTER, HOOKS )
    ELSE
      MSG%ACTIVITY = UNDEF_PARAM_E
    ENDIF

    !> Read the "experiment"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'experiment', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      P_FILTER => CEXPERIMENT2IEXPERIMENT
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER_WITH_FILTER( MSG_CONFIG, 'experiment', MSG%EXPERIMENT, P_FILTER, HOOKS )
    ELSE
      MSG%EXPERIMENT = UNDEF_PARAM_E
    ENDIF

    !> Read the "generation"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'generation', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( MSG_CONFIG, 'generation', MSG%GENERATION, HOOKS )
    ELSE
      MSG%GENERATION = UNDEF_PARAM_E
    ENDIF

    !> Read the "realization"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'realization', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( MSG_CONFIG, 'realization', MSG%REALIZATION, HOOKS )
    ELSE
      MSG%REALIZATION = UNDEF_PARAM_E
    ENDIF

    !> Read the "resolution"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'resolution', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      P_FILTER => CRESOLUTION2IRESOLUTION
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER_WITH_FILTER( MSG_CONFIG, 'resolution', MSG%RESOLUTION, P_FILTER, HOOKS )
    ELSE
      MSG%RESOLUTION = UNDEF_PARAM_E
    ENDIF

    !> Read the "method"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'method', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( MSG_CONFIG, 'method', MSG%METHOD, HOOKS )
    ELSE
      MSG%METHOD = UNDEF_PARAM_E
    ENDIF

    !> Read the "system"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'system', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( MSG_CONFIG, 'system', MSG%SYSTEM, HOOKS )
    ELSE
      MSG%SYSTEM = UNDEF_PARAM_E
    ENDIF
...
IMPLICIT NONE
  ...
  IF ( THIS%TIMESPAN .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%TIMESPAN
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'timespan: "'//TRIM(ADJUSTL(CTMP))//'"'
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF

  ! Print STATTYPE
  IF ( THIS%STATTYPE .NE. REPEAT('*',32) ) THEN
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'stattype: "'//TRIM(ADJUSTL(THIS%STATTYPE))//'"'
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%DATASET .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IDATASET2CDATASET) IDATASET2CDATASET( THIS%DATASET, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'dataset: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%ACTIVITY .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IACTIVITY2CACTIVITY) IACTIVITY2CACTIVITY( THIS%ACTIVITY, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'activity: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%EXPERIMENT .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IEXPERIMENT2CEXPERIMENT) IEXPERIMENT2CEXPERIMENT( THIS%EXPERIMENT, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'experiment: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%GENERATION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%GENERATION
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'generation: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%REALIZATION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%REALIZATION
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'realization: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%RESOLUTION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    PP_TRYCALL(ERRFLAG_IRESOLUTION2CRESOLUTION) IRESOLUTION2CRESOLUTION( THIS%RESOLUTION, CTMP(1:16), HOOKS )
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'resolution: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%METHOD .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%METHOD
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'method: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  IF ( THIS%SYSTEM .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%SYSTEM
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'system: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
