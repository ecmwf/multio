...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_CREATE( MULTIO_GRIB2, DICT_TYPE, LEN ) &
 BIND(C,NAME='multio_grib2_dict_create_f') RESULT(RET)
  ...
  USE :: MAP_INT64_HEALPIX_DICT_MOD,       ONLY: MAP_FUNCTION_INT64_HEALPIX_DICT_IF
  USE :: MAP_INT64_REDUCED_GG_DICT_MOD,    ONLY: MAP_FUNCTION_INT64_REDUCED_GG_DICT_IF
  USE :: MAP_INT64_REGULAR_GG_DICT_MOD,    ONLY: MAP_FUNCTION_INT64_REGULAR_GG_DICT_IF
  USE :: MAP_INT64_REGULAR_LL_DICT_MOD,    ONLY: MAP_FUNCTION_INT64_REGULAR_LL_DICT_IF
  ...
  USE :: API_SHARED_DATA_MOD,              ONLY: REGULAR_LL_DICT_TYPE_E
  ...
  USE :: API_SHARED_DATA_MOD,              ONLY: SHARED_HEALPIX_DICT_MAP
  USE :: API_SHARED_DATA_MOD,              ONLY: SHARED_REDUCED_GG_DICT_MAP
  USE :: API_SHARED_DATA_MOD,              ONLY: SHARED_REGULAR_GG_DICT_MAP
  USE :: API_SHARED_DATA_MOD,              ONLY: SHARED_REGULAR_LL_DICT_MAP
  ...
  USE :: API_SHARED_DATA_MOD,              ONLY: FREE_HEALPIX_DICT
  USE :: API_SHARED_DATA_MOD,              ONLY: FREE_REDUCED_GG_DICT
  USE :: API_SHARED_DATA_MOD,              ONLY: FREE_REGULAR_GG_DICT
  USE :: API_SHARED_DATA_MOD,              ONLY: FREE_REGULAR_LL_DICT
  USE :: API_SHARED_DATA_MOD,              ONLY: FREE_HEALPIX_DICT
...
IMPLICIT NONE
  ...
  TYPE(HEALPIX_T),    POINTER      :: HEALPIX
  TYPE(REDUCED_GG_T), POINTER      :: REDUCED_GG
  TYPE(REGULAR_GG_T), POINTER      :: REGULAR_GG
  TYPE(REGULAR_LL_T), POINTER      :: REGULAR_LL
  ...
  PROCEDURE(MAP_FUNCTION_INT64_HEALPIX_DICT_IF),    POINTER :: HEALPIX_DESTRUCTOR
  PROCEDURE(MAP_FUNCTION_INT64_REDUCED_GG_DICT_IF), POINTER :: REDUCED_GG_DESTRUCTOR
  PROCEDURE(MAP_FUNCTION_INT64_REGULAR_GG_DICT_IF), POINTER :: REGULAR_GG_DESTRUCTOR
  PROCEDURE(MAP_FUNCTION_INT64_REGULAR_LL_DICT_IF), POINTER :: REGULAR_LL_DESTRUCTOR
...
!$omp end critical(API_DICTIONARY_MAP_INSERT)
  ...
  CASE ( 'healpix', 'healpix_ring', 'healpix_nested' )
    ...
    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_UNABLE_TOCHECK_INITIALIZATION_STATUS) SHARED_HEALPIX_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )

    !> Conditionally initialized the dictionarys map
!$omp single
    IF ( .NOT. INITIALIZED ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_INIT_MAP) SHARED_HEALPIX_DICT_MAP%INIT( HOOKS )
    ENDIF
!$omp end single

!$omp critical(API_DICTIONARY_MAP_INSERT)
    !> Get the size of the dictionary map
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_HEALPIX_DICT_MAP%SIZE( SIZE, HOOKS )

    IF ( SIZE .EQ. 0_JPIB_K ) THEN
      F_MULTIO_GRIB2(1) = HEALPIX_DICT_TYPE_E
      F_MULTIO_GRIB2(2) = 1_C_LONG_LONG
    ELSE
      !> Check if the dictionary is already in the dictionarys map
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAXIMUM) SHARED_HEALPIX_DICT_MAP%MAX( MAX_DICT_HANDLE, HOOKS )
    ...
    !> Allocate the mars dictionary
    ALLOCATE( HEALPIX, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_ALLOC_FAILURE )

    HEALPIX_DESTRUCTOR => FREE_HEALPIX_DICT
    PP_TRYCALL(ERRFLAG_UNABLE_TO_ADD_DICTIONARY) SHARED_HEALPIX_DICT_MAP%INSERT( F_MULTIO_GRIB2, HEALPIX, HEALPIX_DESTRUCTOR, HOOKS )
!$omp end critical(API_DICTIONARY_MAP_INSERT)
...
!$omp end critical(API_DICTIONARY_MAP_INSERT)
  ...
  CASE ( 'regulargg', 'regular-gg', 'geometryregulargg', 'geometry-regular-gg' )
    ...
    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_UNABLE_TOCHECK_INITIALIZATION_STATUS) SHARED_REGULAR_GG_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )

    !> Conditionally initialized the dictionarys map
!$omp single
    IF ( .NOT. INITIALIZED ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_INIT_MAP) SHARED_REGULAR_GG_DICT_MAP%INIT( HOOKS )
    ENDIF
!$omp end single

!$omp critical(API_DICTIONARY_MAP_INSERT)
    !> Get the size of the dictionary map
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_REGULAR_GG_DICT_MAP%SIZE( SIZE, HOOKS )

    IF ( SIZE .EQ. 0_JPIB_K ) THEN
      F_MULTIO_GRIB2(1) = REGULAR_GG_DICT_TYPE_E
      F_MULTIO_GRIB2(2) = 1_C_LONG_LONG
    ELSE
      !> Check if the dictionary is already in the dictionarys map
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAXIMUM) SHARED_REGULAR_GG_DICT_MAP%MAX( MAX_DICT_HANDLE, HOOKS )
    ...
    !> Allocate the mars dictionary
    ALLOCATE( REGULAR_GG, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_ALLOC_FAILURE )

    REGULAR_GG_DESTRUCTOR => FREE_REGULAR_GG_DICT
    PP_TRYCALL(ERRFLAG_UNABLE_TO_ADD_DICTIONARY) SHARED_REGULAR_GG_DICT_MAP%INSERT( F_MULTIO_GRIB2, REGULAR_GG, REGULAR_GG_DESTRUCTOR, HOOKS )
!$omp end critical(API_DICTIONARY_MAP_INSERT)
  ...
  CASE ( 'regularll', 'regular-ll', 'geometryregularll', 'geometry-regular-ll' )
    ...
    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_UNABLE_TOCHECK_INITIALIZATION_STATUS) SHARED_REGULAR_LL_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )

    !> Conditionally initialized the dictionarys map
!$omp single
    IF ( .NOT. INITIALIZED ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_INIT_MAP) SHARED_REGULAR_LL_DICT_MAP%INIT( HOOKS )
    ENDIF
!$omp end single

!$omp critical(API_DICTIONARY_MAP_INSERT)
    !> Get the size of the dictionary map
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_REGULAR_LL_DICT_MAP%SIZE( SIZE, HOOKS )

    IF ( SIZE .EQ. 0_JPIB_K ) THEN
      F_MULTIO_GRIB2(1) = REGULAR_LL_DICT_TYPE_E
      F_MULTIO_GRIB2(2) = 1_C_LONG_LONG
    ELSE
      !> Check if the dictionary is already in the dictionarys map
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAXIMUM) SHARED_REGULAR_LL_DICT_MAP%MAX( MAX_DICT_HANDLE, HOOKS )
    ...
    !> Allocate the mars dictionary
    ALLOCATE( REGULAR_LL, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_ALLOC_FAILURE )

    REGULAR_LL_DESTRUCTOR => FREE_REGULAR_LL_DICT
    PP_TRYCALL(ERRFLAG_UNABLE_TO_ADD_DICTIONARY) SHARED_REGULAR_LL_DICT_MAP%INSERT( F_MULTIO_GRIB2, REGULAR_LL, REGULAR_LL_DESTRUCTOR, HOOKS )
!$omp end critical(API_DICTIONARY_MAP_INSERT)
...
!$omp critical(API_DICTIONARY_MAP_REMOVE)
    ...
    !> Print the parametrization dictionary map
    !> NOTE: To be uncommented for debugging the API
    ! PP_TRYCALL(ERRFLAG_MAP_LIST) SHARED_HEALPIX_DICT_MAP%LIST( 6_JPIB_K, 'PARAMETRIZATION_DICTIONARY_MAP: ', HOOKS )

    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_MAP_NOT_INITIALIZED) SHARED_HEALPIX_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. INITIALIZED, ERRFLAG_MAP_NOT_INITIALIZED )

    !> Check if the handle is associated to an dictionary
    PP_TRYCALL(ERRFLAG_MATCH_DICTIONARY) SHARED_HEALPIX_DICT_MAP%HAS( F_MULTIO_GRIB2, MAP_HAS_DICTIONARY, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. MAP_HAS_DICTIONARY, ERRFLAG_MATCH_DICTIONARY )

    !> Remove the dictionary from the map
    PP_TRYCALL(ERRFLAG_REMOVE_DICTIONARY) SHARED_HEALPIX_DICT_MAP%REMOVE( F_MULTIO_GRIB2, DICTIONARY_REMOVED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. DICTIONARY_REMOVED, ERRFLAG_REMOVE_DICTIONARY )
    ...
    !> Get the map size
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_HEALPIX_DICT_MAP%SIZE( MAP_SIZE, HOOKS )

    !> To avoid the need of init/exit API, every time the map is empty we deallocate the map
    IF ( MAP_SIZE .EQ. 0_JPIB_K ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_MAP) SHARED_HEALPIX_DICT_MAP%FREE( HOOKS )
    ENDIF
!$omp end critical(API_DICTIONARY_MAP_REMOVE)
...
!$omp critical(API_DICTIONARY_MAP_REMOVE)
    ...
    !> Print the parametrization dictionary map
    !> NOTE: To be uncommented for debugging the API
    ! PP_TRYCALL(ERRFLAG_MAP_LIST) SHARED_REGULAR_GG_DICT_MAP%LIST( 6_JPIB_K, 'PARAMETRIZATION_DICTIONARY_MAP: ', HOOKS )

    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_MAP_NOT_INITIALIZED) SHARED_REGULAR_GG_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. INITIALIZED, ERRFLAG_MAP_NOT_INITIALIZED )

    !> Check if the handle is associated to an dictionary
    PP_TRYCALL(ERRFLAG_MATCH_DICTIONARY) SHARED_REGULAR_GG_DICT_MAP%HAS( F_MULTIO_GRIB2, MAP_HAS_DICTIONARY, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. MAP_HAS_DICTIONARY, ERRFLAG_MATCH_DICTIONARY )

    !> Remove the dictionary from the map
    PP_TRYCALL(ERRFLAG_REMOVE_DICTIONARY) SHARED_REGULAR_GG_DICT_MAP%REMOVE( F_MULTIO_GRIB2, DICTIONARY_REMOVED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. DICTIONARY_REMOVED, ERRFLAG_REMOVE_DICTIONARY )
    ...
    !> Get the map size
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_REGULAR_GG_DICT_MAP%SIZE( MAP_SIZE, HOOKS )

    !> To avoid the need of init/exit API, every time the map is empty we deallocate the map
    IF ( MAP_SIZE .EQ. 0_JPIB_K ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_MAP) SHARED_REGULAR_GG_DICT_MAP%FREE( HOOKS )
    ENDIF
!$omp end critical(API_DICTIONARY_MAP_REMOVE)

  CASE ( REGULAR_LL_DICT_TYPE_E )

!$omp critical(API_DICTIONARY_MAP_REMOVE)

    !> Print the parametrization dictionary map
    !> NOTE: To be uncommented for debugging the API
    ! PP_TRYCALL(ERRFLAG_MAP_LIST) SHARED_REGULAR_LL_DICT_MAP%LIST( 6_JPIB_K, 'PARAMETRIZATION_DICTIONARY_MAP: ', HOOKS )

    !> Check if the dictionary is already in the dictionarys map
    PP_TRYCALL(ERRFLAG_MAP_NOT_INITIALIZED) SHARED_REGULAR_LL_DICT_MAP%INITIALIZED( INITIALIZED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. INITIALIZED, ERRFLAG_MAP_NOT_INITIALIZED )

    !> Check if the handle is associated to an dictionary
    PP_TRYCALL(ERRFLAG_MATCH_DICTIONARY) SHARED_REGULAR_LL_DICT_MAP%HAS( F_MULTIO_GRIB2, MAP_HAS_DICTIONARY, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. MAP_HAS_DICTIONARY, ERRFLAG_MATCH_DICTIONARY )

    !> Remove the dictionary from the map
    PP_TRYCALL(ERRFLAG_REMOVE_DICTIONARY) SHARED_REGULAR_LL_DICT_MAP%REMOVE( F_MULTIO_GRIB2, DICTIONARY_REMOVED, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT. DICTIONARY_REMOVED, ERRFLAG_REMOVE_DICTIONARY )
    ...
    !> Get the map size
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SIZE) SHARED_REGULAR_LL_DICT_MAP%SIZE( MAP_SIZE, HOOKS )

    !> To avoid the need of init/exit API, every time the map is empty we deallocate the map
    IF ( MAP_SIZE .EQ. 0_JPIB_K ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_MAP) SHARED_REGULAR_LL_DICT_MAP%FREE( HOOKS )
    ENDIF
!$omp end critical(API_DICTIONARY_MAP_REMOVE)
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_SET( DICT, KEY, KLEN, VALUE, VLEN ) &
 BIND(C,NAME='multio_grib2_dict_set_f') RESULT(RET)
  ...
  USE :: API_SHARED_DATA_MOD,                 ONLY: EXTRACT_REGULAR_LL_DICTIONARY
  ...
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_SET_VALUE_FROM_STRING
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_SET_VALUE_FROM_STRING
...
IMPLICIT NONE
  ...
  CASE ( HEALPIX_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
  ...
  CASE ( REGULAR_GG_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
  ...
  CASE ( REGULAR_LL_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_SET_INT64( DICT, KEY, KLEN, VALUE ) &
 BIND(C,NAME='multio_grib2_dict_set_int64_f') RESULT(RET)
  ...
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_NAME2ITERATOR
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_SET_VALUE_FROM_INT64
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_NAME2ITERATOR
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_SET_VALUE_FROM_INT64
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_SET_VALUE_FROM_INT64
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_SET_VALUE_FROM_INT64
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_SET_VALUE_FROM_INT64
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_SET_VALUE_FROM_INT64
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SET_VAL=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_HANDLE=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_KEY_LENGTH_INVALID=8_JPIB_K
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(DICT), ERRFLAG_DICTIONARY_NOT_ASSOCIATED )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(KEY),  ERRFLAG_KEY_NOT_ASSOCIATED )
  PP_DEBUG_CRITICAL_COND_THROW( KLEN.LE.0, ERRFLAG_KEY_LENGTH_INVALID )
  ...
  ! Convert the value to the proper integer
  LOC_VALUE = INT( VALUE, KIND=JPIB_K )
  ...
  CASE ( MARS_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_MARS_DICTIONARY( F_DICT, MARS_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) MARS_DICTIONARY_NAME2ITERATOR( MARS_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) MARS_DICTIONARY_SET_VALUE_FROM_INT64( MARS_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( PAR_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_PAR_DICTIONARY( F_DICT, PAR_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) PAR_DICTIONARY_NAME2ITERATOR( PAR_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) PAR_DICTIONARY_SET_VALUE_FROM_INT64( PAR_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( HEALPIX_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) HEALPIX_DICTIONARY_SET_VALUE_FROM_INT64( HEALPIX_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REDUCED_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REDUCED_GG_DICTIONARY( F_DICT, REDUCED_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REDUCED_GG_DICTIONARY_NAME2ITERATOR( REDUCED_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REDUCED_GG_DICTIONARY_SET_VALUE_FROM_INT64( REDUCED_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_GG_DICTIONARY_SET_VALUE_FROM_INT64( REGULAR_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_LL_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_LL_DICTIONARY_SET_VALUE_FROM_INT64( REGULAR_LL_DICT, ITERATOR, LOC_VALUE, HOOKS )
  ...
  CASE ( SH_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_SH_DICTIONARY( F_DICT, SH_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) SH_DICTIONARY_NAME2ITERATOR( SH_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) SH_DICTIONARY_SET_VALUE_FROM_INT64( SH_DICT, ITERATOR, LOC_VALUE, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_WRONG_HANDLE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Wrong handle (handle is not from an dictionary)' )
    CASE (ERRFLAG_KEY_LENGTH_INVALID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Key length is invalid' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown error' )
    END SELECT
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_SET_REAL64( DICT, KEY, KLEN, VALUE ) &
 BIND(C,NAME='multio_grib2_dict_set_real64_f') RESULT(RET)
  ...
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_NAME2ITERATOR
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_SET_VALUE_FROM_REAL64
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_NAME2ITERATOR
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_SET_VALUE_FROM_REAL64
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_SET_VALUE_FROM_REAL64
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_SET_VALUE_FROM_REAL64
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_SET_VALUE_FROM_REAL64
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_SET_VALUE_FROM_REAL64
...
IMPLICIT NONE
  ...
  ! Convert the value to the proper integer
  LOC_VALUE = REAL( VALUE, KIND=JPRD_K )
  ...
  CASE ( MARS_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_MARS_DICTIONARY( F_DICT, MARS_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) MARS_DICTIONARY_NAME2ITERATOR( MARS_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) MARS_DICTIONARY_SET_VALUE_FROM_REAL64( MARS_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( PAR_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_PAR_DICTIONARY( F_DICT, PAR_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) PAR_DICTIONARY_NAME2ITERATOR( PAR_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) PAR_DICTIONARY_SET_VALUE_FROM_REAL64( PAR_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( HEALPIX_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) HEALPIX_DICTIONARY_SET_VALUE_FROM_REAL64( HEALPIX_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REDUCED_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REDUCED_GG_DICTIONARY( F_DICT, REDUCED_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REDUCED_GG_DICTIONARY_NAME2ITERATOR( REDUCED_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REDUCED_GG_DICTIONARY_SET_VALUE_FROM_REAL64( REDUCED_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_GG_DICTIONARY_SET_VALUE_FROM_REAL64( REGULAR_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_LL_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_LL_DICTIONARY_SET_VALUE_FROM_REAL64( REGULAR_LL_DICT, ITERATOR, LOC_VALUE, HOOKS )
  ...
  CASE ( SH_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_SH_DICTIONARY( F_DICT, SH_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) SH_DICTIONARY_NAME2ITERATOR( SH_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) SH_DICTIONARY_SET_VALUE_FROM_REAL64( SH_DICT, ITERATOR, LOC_VALUE, HOOKS )
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_SET_INT64_ARRAY( DICT, KEY, KLEN, VALUE, VLEN ) &
 BIND(C,NAME='multio_grib2_dict_set_int64_array_f') RESULT(RET)
  ...
  USE :: API_OPTIONS_DICTIONARY_UTILS_MOD,    ONLY: OPTIONS_DICTIONARY_SET_VALUE_FROM_STRING
  USE :: API_MARS_DICTIONARY_UTILS_MOD,       ONLY: MARS_DICTIONARY_NAME2ITERATOR
  USE :: API_MARS_DICTIONARY_UTILS_MOD,       ONLY: MARS_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_NAME2ITERATOR
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_NAME2ITERATOR
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K) :: ITERATOR
  INTEGER(KIND=JPIB_K) :: I
  INTEGER(KIND=JPIB_K), DIMENSION(:), POINTER :: LOC_VALUE
  TYPE(HOOKS_T) :: HOOKS

  !> Local error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_DICTIONARY_NOT_ASSOCIATED=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_KEY_NOT_ASSOCIATED=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_VAL_NOT_ASSOCIATED=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_EXTRACT_DICTIONARY=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SET_VAL=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_HANDLE=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_KEY_LENGTH_INVALID=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_VAL_LENGTH_INVALID=9_JPIB_K
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(DICT), ERRFLAG_DICTIONARY_NOT_ASSOCIATED )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(KEY),  ERRFLAG_KEY_NOT_ASSOCIATED )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(VALUE),  ERRFLAG_VAL_NOT_ASSOCIATED )

  PP_DEBUG_CRITICAL_COND_THROW( KLEN.LE.0, ERRFLAG_KEY_LENGTH_INVALID )
  PP_DEBUG_CRITICAL_COND_THROW( VLEN.LE.0, ERRFLAG_VAL_LENGTH_INVALID )
  ...
  !> Get th fortran handle from the c handle
  LOC_VALUE => NULL()
  CALL C_F_POINTER( VALUE, LOC_VALUE, [VLEN] )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(LOC_VALUE), ERRFLAG_VAL_NOT_ASSOCIATED )
  ...
  CASE ( MARS_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_MARS_DICTIONARY( F_DICT, MARS_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) MARS_DICTIONARY_NAME2ITERATOR( MARS_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) MARS_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( MARS_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( PAR_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_PAR_DICTIONARY( F_DICT, PAR_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) PAR_DICTIONARY_NAME2ITERATOR( PAR_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) PAR_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( PAR_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( HEALPIX_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) HEALPIX_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( HEALPIX_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REDUCED_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REDUCED_GG_DICTIONARY( F_DICT, REDUCED_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REDUCED_GG_DICTIONARY_NAME2ITERATOR( REDUCED_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REDUCED_GG_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( REDUCED_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_GG_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( REGULAR_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_LL_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_LL_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( REGULAR_LL_DICT, ITERATOR, LOC_VALUE, HOOKS )
  ...
  CASE ( SH_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_SH_DICTIONARY( F_DICT, SH_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) SH_DICTIONARY_NAME2ITERATOR( SH_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) SH_DICTIONARY_SET_VALUE_FROM_INT64_ARRAY( SH_DICT, ITERATOR, LOC_VALUE, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_KEY_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Key not associated' )
    CASE (ERRFLAG_VAL_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Value not associated' )
    CASE (ERRFLAG_EXTRACT_DICTIONARY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to extract the dictionary' )
    CASE (ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR)
    ...
    CASE (ERRFLAG_KEY_LENGTH_INVALID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Key length is invalid' )
    CASE (ERRFLAG_VAL_LENGTH_INVALID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Value length is invalid' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown error' )
    END SELECT
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_SET_REAL64_ARRAY( DICT, KEY, KLEN, VALUE, VLEN ) &
 BIND(C,NAME='multio_grib2_dict_set_real64_array_f') RESULT(RET)
  ...
  USE :: API_OPTIONS_DICTIONARY_UTILS_MOD,    ONLY: OPTIONS_DICTIONARY_SET_VALUE_FROM_STRING
  USE :: API_MARS_DICTIONARY_UTILS_MOD,       ONLY: MARS_DICTIONARY_NAME2ITERATOR
  USE :: API_MARS_DICTIONARY_UTILS_MOD,       ONLY: MARS_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_NAME2ITERATOR
  USE :: API_PAR_DICTIONARY_UTILS_MOD,        ONLY: PAR_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_NAME2ITERATOR
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K) :: ITERATOR
  INTEGER(KIND=JPIB_K) :: I
  REAL(KIND=JPRD_K), DIMENSION(:), POINTER :: LOC_VALUE
  TYPE(HOOKS_T) :: HOOKS
  ...
  CASE ( MARS_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_MARS_DICTIONARY( F_DICT, MARS_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) MARS_DICTIONARY_NAME2ITERATOR( MARS_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) MARS_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( MARS_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( PAR_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_PAR_DICTIONARY( F_DICT, PAR_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) PAR_DICTIONARY_NAME2ITERATOR( PAR_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) PAR_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( PAR_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( HEALPIX_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) HEALPIX_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( HEALPIX_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REDUCED_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REDUCED_GG_DICTIONARY( F_DICT, REDUCED_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REDUCED_GG_DICTIONARY_NAME2ITERATOR( REDUCED_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REDUCED_GG_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( REDUCED_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_GG_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_GG_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( REGULAR_GG_DICT, ITERATOR, LOC_VALUE, HOOKS )

  CASE ( REGULAR_LL_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) REGULAR_LL_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( REGULAR_LL_DICT, ITERATOR, LOC_VALUE, HOOKS )
  ...
  CASE ( SH_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_SH_DICTIONARY( F_DICT, SH_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) SH_DICTIONARY_NAME2ITERATOR( SH_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_VAL) SH_DICTIONARY_SET_VALUE_FROM_REAL64_ARRAY( SH_DICT, ITERATOR, LOC_VALUE, HOOKS )
...
IMPLICIT NONE
  ...
  CASE ( PAR_DICT_TYPE_E )
    ...
    CASE ( HEALPIX_DICT_TYPE_E )
      PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_SUBDICT, HEALPIX_DICT, HOOKS )
      REPRES => HEALPIX_DICT
    ...
    CASE ( REGULAR_LL_DICT_TYPE_E )
      PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_SUBDICT, REGULAR_LL_DICT, HOOKS )
      REPRES => REGULAR_LL_DICT
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_GET( DICT, KEY, KLEN, VALUE ) &
 BIND(C,NAME='multio_grib2_dict_get_f') RESULT(RET)
  ...
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD,    ONLY: HEALPIX_DICTIONARY_GET_VALUE_AS_STRING
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_GET_VALUE_AS_STRING
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_GET_VALUE_AS_STRING
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_GET_VALUE_AS_STRING
  USE :: API_SH_DICTIONARY_UTILS_MOD,         ONLY: SH_DICTIONARY_NAME2ITERATOR
...
IMPLICIT NONE
  ...
  CASE ( HEALPIX_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_VALUE) HEALPIX_DICTIONARY_GET_VALUE_AS_STRING( HEALPIX_DICT, ITERATOR, VALUE, LHAS, HOOKS )
  ...
  CASE ( REGULAR_GG_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_VALUE) REGULAR_GG_DICTIONARY_GET_VALUE_AS_STRING( REGULAR_GG_DICT, ITERATOR, VALUE, LHAS, HOOKS )

  CASE ( REGULAR_LL_DICT_TYPE_E )

    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_VALUE) REGULAR_LL_DICTIONARY_GET_VALUE_AS_STRING( REGULAR_LL_DICT, ITERATOR, VALUE, LHAS, HOOKS )
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_HAS( DICT, KEY, KLEN, HAS ) &
 BIND(C,NAME='multio_grib2_dict_has_f') RESULT(RET)
  ...
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_NAME2ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_HAS
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REDUCED_GG_DICTIONARY_UTILS_MOD, ONLY: REDUCED_GG_DICTIONARY_HAS
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_NAME2ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_HAS
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_NAME2ITERATOR
...
IMPLICIT NONE
  ...
  CASE ( HEALPIX_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) HEALPIX_DICTIONARY_NAME2ITERATOR( HEALPIX_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_STATUS) HEALPIX_DICTIONARY_HAS( HEALPIX_DICT, ITERATOR, LHAS, HOOKS )
  ...
  CASE ( REGULAR_GG_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_GG_DICTIONARY_NAME2ITERATOR( REGULAR_GG_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_STATUS) REGULAR_GG_DICTIONARY_HAS( REGULAR_GG_DICT, ITERATOR, LHAS, HOOKS )
  ...
  CASE ( REGULAR_LL_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_ITERATOR) REGULAR_LL_DICTIONARY_NAME2ITERATOR( REGULAR_LL_DICT, F_KEY, ITERATOR, HOOKS )
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_STATUS) REGULAR_LL_DICTIONARY_HAS( REGULAR_LL_DICT, ITERATOR, LHAS, HOOKS )
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_DICT_ITERATE( DICT, IT, KEY, VALUE ) &
 BIND(C,NAME='multio_grib2_dict_iterate') RESULT(RET)
  ...
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_MAX_ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_INIT_ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_GET_NEXT_ITERATOR
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_GET_KEY_AS_STRING
  USE :: API_HEALPIX_DICTIONARY_UTILS_MOD, ONLY: HEALPIX_DICTIONARY_GET_VALUE_AS_STRING
  ...
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_MAX_ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_INIT_ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_GET_NEXT_ITERATOR
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_GET_KEY_AS_STRING
  USE :: API_REGULAR_GG_DICTIONARY_UTILS_MOD, ONLY: REGULAR_GG_DICTIONARY_GET_VALUE_AS_STRING


  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_MAX_ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_INIT_ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_GET_NEXT_ITERATOR
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_GET_KEY_AS_STRING
  USE :: API_REGULAR_LL_DICTIONARY_UTILS_MOD, ONLY: REGULAR_LL_DICTIONARY_GET_VALUE_AS_STRING
...
IMPLICIT NONE
  ...
  CASE ( HEALPIX_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_PAR_DICTIONARY) EXTRACT_HEALPIX_DICTIONARY( F_DICT, HEALPIX_DICT, HOOKS )

    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAX_ITERATOR) HEALPIX_DICTIONARY_MAX_ITERATOR( HEALPIX_DICT, PAR_ITERATOR_SIZE, HOOKS )
    IF ( .NOT.C_ASSOCIATED(IT) ) THEN
      F_ITERATOR => NULL()
      PP_TRYCALL(ERRFLAG_ALLOC_FAILURE) ALLOCATE_ITERATOR( F_ITERATOR, HOOKS )
      ITERATOR = F_ITERATOR(1)
    ...
    ELSE
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_KEY) HEALPIX_DICTIONARY_GET_KEY_AS_STRING( HEALPIX_DICT, ITERATOR, F_KEY, HOOKS )
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_VAL) HEALPIX_DICTIONARY_GET_VALUE_AS_STRING( HEALPIX_DICT, ITERATOR, VALUE, HAS, HOOKS )
      PP_TRYCALL(ERRFLAG_CONVERT_TO_C_STRING) CONVERT_TO_C_STRING( F_KEY, KEY, HOOKS )
  ...
  CASE ( REGULAR_GG_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_PAR_DICTIONARY) EXTRACT_REGULAR_GG_DICTIONARY( F_DICT, REGULAR_GG_DICT, HOOKS )

    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAX_ITERATOR) REGULAR_GG_DICTIONARY_MAX_ITERATOR( REGULAR_GG_DICT, PAR_ITERATOR_SIZE, HOOKS )
    IF ( .NOT.C_ASSOCIATED(IT) ) THEN
      F_ITERATOR => NULL()
      PP_TRYCALL(ERRFLAG_ALLOC_FAILURE) ALLOCATE_ITERATOR( F_ITERATOR, HOOKS )
      ITERATOR = F_ITERATOR(1)
    ...
    ELSE
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_KEY) REGULAR_GG_DICTIONARY_GET_KEY_AS_STRING( REGULAR_GG_DICT, ITERATOR, F_KEY, HOOKS )
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_VAL) REGULAR_GG_DICTIONARY_GET_VALUE_AS_STRING( REGULAR_GG_DICT, ITERATOR, VALUE, HAS, HOOKS )
      PP_TRYCALL(ERRFLAG_CONVERT_TO_C_STRING) CONVERT_TO_C_STRING( F_KEY, KEY, HOOKS )
  ...
  CASE ( REGULAR_LL_DICT_TYPE_E )
    ...
    PP_TRYCALL(ERRFLAG_EXTRACT_PAR_DICTIONARY) EXTRACT_REGULAR_LL_DICTIONARY( F_DICT, REGULAR_LL_DICT, HOOKS )

    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_MAX_ITERATOR) REGULAR_LL_DICTIONARY_MAX_ITERATOR( REGULAR_LL_DICT, PAR_ITERATOR_SIZE, HOOKS )
    IF ( .NOT.C_ASSOCIATED(IT) ) THEN
      F_ITERATOR => NULL()
      PP_TRYCALL(ERRFLAG_ALLOC_FAILURE) ALLOCATE_ITERATOR( F_ITERATOR, HOOKS )
      ITERATOR = F_ITERATOR(1)
    ...
    ELSE
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_KEY) REGULAR_LL_DICTIONARY_GET_KEY_AS_STRING( REGULAR_LL_DICT, ITERATOR, F_KEY, HOOKS )
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PAR_VAL) REGULAR_LL_DICTIONARY_GET_VALUE_AS_STRING( REGULAR_LL_DICT, ITERATOR, VALUE, HAS, HOOKS )
      PP_TRYCALL(ERRFLAG_CONVERT_TO_C_STRING) CONVERT_TO_C_STRING( F_KEY, KEY, HOOKS )
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_HANDLE=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_EXTRACT_DICTIONARY=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_MARS_TO_JSON=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CONVERT_STRING=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_PAR_TO_JSON=7_JPIB_K
  ...
  CASE ( MARS_DICT_TYPE_E )
      ...
      PP_TRYCALL(ERRFLAG_CONVERT_STRING) CONVERT_TO_C_STRING(JSON, VALUE, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_PAR_TO_JSON)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to convert parametriaziton to json' )
