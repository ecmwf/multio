...
PRIVATE
  ...
  TYPE :: PARAMETRIZATION_T
    ...
    !> Destine tables version
    INTEGER(KIND=JPIB_K) :: DESTINE_LOCAL_VERSION=UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  ! Initialize the destine tables version
  THIS%DESTINE_LOCAL_VERSION=UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  THIS%DESTINE_LOCAL_VERSION = OTHER%DESTINE_LOCAL_VERSION
...
PP_THREAD_SAFE FUNCTION PARAMETRIZATION_SET_INT( THIS, ID, VALUE, HOOKS ) RESULT(RET)
  ...
  USE :: PARAMETRIZATION_ENUMERATORS_MOD, ONLY: PARINTFLD_DESTINE_LOCAL_VERSION_E
...
IMPLICIT NONE
  ...
  CASE ( PARINTFLD_DESTINE_LOCAL_VERSION_E )
    THIS%DESTINE_LOCAL_VERSION = VALUE
  CASE DEFAULT
...
IMPLICIT NONE
  ...
  CASE ( PARINTFLD_DESTINE_LOCAL_VERSION_E )
    VALUE = THIS%DESTINE_LOCAL_VERSION
  CASE DEFAULT
    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOWN_PAR )
  END SELECT
...
IMPLICIT NONE
  ...
  IF ( HAS_PARAMETRIZATION  ) THEN
    ...
    ! Read the generating process identifier parameters
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( PARAMETRIZATION_CONFIGURATION, 'destine-local-version', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( PARAMETRIZATION_CONFIGURATION, 'destine-local-version', PARAMETRIZATION%DESTINE_LOCAL_VERSION, HOOKS )
    ELSE
      PARAMETRIZATION%DESTINE_LOCAL_VERSION = UNDEF_PARAM_E
    ENDIF
...
IMPLICIT NONE
  ...
  ! Print "GeneratingProcessIdentifier"
  IF ( THIS%DESTINE_LOCAL_VERSION .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%DESTINE_LOCAL_VERSION
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'destine-local-version: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
