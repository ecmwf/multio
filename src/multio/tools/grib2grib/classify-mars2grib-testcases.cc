...
public:
    ClassifyMars2GribTestcases(int argc, char** argv) : multio::MultioTool(argc, argv) {
        options_.push_back(new eckit::option::SimpleOption<std::string>("input-directory", "Input testcase directory"));
        options_.push_back(
        ...
        options_.push_back(new eckit::option::SimpleOption<bool>("skip-errors", "Report invalid records and continue"));
    }
        ...
        for (const auto& file : inputFiles(inputDirectory)) {
            ...
            while (std::getline(input, line)) {
                ...
                try {
                    ...
                    if (!skipErrors_) {
                        throw eckit::UserError(file.asString() + ":" + std::to_string(lineNumber) + ": " + error.what(),
                                               Here());
                    }
