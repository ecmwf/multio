!>
!> @file assignment_msg_int_mod.F90
!>
!> @brief Module containing definitions and procedures for variableeter intops.
!>
!> This module defines the `ASSIGNMENT_MSG_INT_T` type, along with its associated
...
#define PP_FILE_NAME 'assignment_msg_int_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'ASSIGNMENT_MSG_INT_MOD'
MODULE ASSIGNMENT_MSG_INT_MOD
...
!>
TYPE, EXTENDS(ASSIGNMENT_BASE_A) :: ASSIGNMENT_MSG_INT_T
...
CONTAINS
  ...
  !> @details This procedure sets up the `ASSIGNMENT_MSG_INT_T` type, initializing its components.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_INT_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => ASSIGNMENT_MSG_INT_INIT
  ...
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_INT_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => ASSIGNMENT_MSG_INT_EVAL

  !> @brief Prints the intop's details for debugging or logging.
  !> @details Outputs the intop's configuration and current state.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_INT_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => ASSIGNMENT_MSG_INT_PRINT

  !> @brief Frees resources allocated for the intop.
  !> @details Cleans up the `ASSIGNMENT_MSG_INT_T` type, deallocating any resources used by the intop.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_INT_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => ASSIGNMENT_MSG_INT_FREE
...
PUBLIC :: ASSIGNMENT_MSG_INT_T

CONTAINS


!>
!> @brief Initializes the `ASSIGNMENT_MSG_INT_T` intop from a YAML configuration.
!>
!> This function initializes the `ASSIGNMENT_MSG_INT_T` type based on the provided YAML
...
!>
!> @variable [inout] THIS The intop object (`ASSIGNMENT_MSG_INT_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_INT_INIT'
FUNCTION ASSIGNMENT_MSG_INT_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(ASSIGNMENT_MSG_INT_T),            INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SOURCE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'name', THIS%VARIABLE_ID_, P_CMSGINTFLDS2IMSGINTFLDS, HOOKS )
...
END FUNCTION ASSIGNMENT_MSG_INT_INIT
...
!>
!> @variable [inout] THIS  The intop object (`ASSIGNMENT_MSG_INT_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_INT_EVAL'
FUNCTION ASSIGNMENT_MSG_INT_EVAL( THIS, IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_MSG_INT_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS    The intop object (`ASSIGNMENT_MSG_INT_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_INT_PRINT'
FUNCTION ASSIGNMENT_MSG_INT_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_ENUM_TO_STRING) IMSGINTFLDS2CMSGINTFLDS( THIS%VARIABLE_ID_, VARIABLE_NAME, HOOKS )
...
END FUNCTION ASSIGNMENT_MSG_INT_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS   The intop object (`ASSIGNMENT_MSG_INT_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_INT_FREE'
FUNCTION ASSIGNMENT_MSG_INT_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_MSG_INT_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE ASSIGNMENT_MSG_INT_MOD
