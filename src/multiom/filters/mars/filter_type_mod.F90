...
!>
!> This module defines the `FILTER_TYPE_T` type, along with its associated
...
#define PP_SECTION_NAME 'FILTER_TYPE_MOD'
MODULE FILTER_TYPE_MOD
...
!>
!> @brief Filter type for managing level types (TYPE).
!>
!> This derived type `FILTER_TYPE_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_TYPE_T
  ...
  TYPE(KEYSET_INT64_T) :: TYPES_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_TYPE_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_TYPE_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_TYPE_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_TYPE_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_TYPE_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_TYPE_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_TYPE_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_TYPE_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CTYPE2ITYPE
...
!>
!> @see FILTER_TYPE_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_TYPE_INIT'
FUNCTION FILTER_TYPE_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_TYPE_T),       INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CTYPE2ITYPE
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%TYPES_%INIT( HOOKS )
  THIS%USE_KEYSET_ = .FALSE.

  !> Associate the filter procedure
  P_CTYPE2ITYPE => CTYPE2ITYPE
  ...
  IF ( THIS%FILTER_TYPE_ .NE. FLT_INT_HAS_E .AND. THIS%FILTER_TYPE_ .NE. FLT_INT_LACKS_E ) THEN
    ...
    IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%TYPES_, P_CTYPE2ITYPE, HOOKS )
      THIS%USE_KEYSET_ = .TRUE.

    ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%TYPE_, P_CTYPE2ITYPE, HOOKS )
...
END FUNCTION FILTER_TYPE_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_TYPE_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_TYPE_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_TYPE_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_TYPE_MATCH'
FUNCTION FILTER_TYPE_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = (MSG%TYPE .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = (MSG%TYPE .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )

    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%TYPES_%MATCH( MSG%TYPE, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%TYPE_ .EQ. MSG%TYPE)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%TYPE_ .EQ. MSG%TYPE)
    ENDIF
...
END FUNCTION FILTER_TYPE_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_TYPE_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_TYPE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_TYPE_PRINT'
FUNCTION FILTER_TYPE_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'TYPE FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_TYPE_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_TYPE_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_TYPE_FREE'
FUNCTION FILTER_TYPE_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%TYPES_%FREE( HOOKS )
...
END FUNCTION FILTER_TYPE_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_TYPE_MOD
