...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_MULTIO_PLANS_FILE_FROM_YAML => GRIB_HEADER2MULTIO_READ_MAPPING_FILE_FROM_YAML
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_MAPPING_FILE_FROM_YAML => GRIB_HEADER2MULTIO_READ_ENCODER_FILE_FROM_YAML
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_ENCODER_FILE_FROM_YAML => GRIB_HEADER2MULTIO_READ_MULTIO_PLANS_FILE_FROM_YAML
...
IMPLICIT NONE
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'mapping-file', HAS_KEY, HOOKS )
  ...
  IF ( HAS_KEY ) THEN
    ...
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'mapping-file', MAPPING_FILE, HOOKS )
...
IMPLICIT NONE
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'encoder-file', HAS_KEY, HOOKS )
  ...
  IF ( HAS_KEY ) THEN
    ...
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'encoder-file', ENCODER_FILE, HOOKS )
...
IMPLICIT NONE
  ...
  ! Set stream
  MSG%STREAM = THIS%MODEL_PAR_%SIM_%NSTREAM

  ! Set the message type
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TYPE) CTYPE2ITYPE( THIS%MODEL_PAR_%SIM_%CTYPE, MSG%TYPE, HOOKS )

  ! Set the message class
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_CLASS) CCLASS2ICLASS( THIS%MODEL_PAR_%SIM_%CFCLASS, MSG%CLASS, HOOKS )

  ! Set the experiment version number
  MSG%EXPVER = THIS%MODEL_PAR_%SIM_%CNMEXP(1:4)

  ! Set ensemble number
  CONDITIONS(1) = (MSG%TYPE.EQ.TYPE_CF_E)
  CONDITIONS(2) = (MSG%TYPE.EQ.TYPE_PF_E)
  CONDITIONS(3) = (MSG%TYPE.EQ.TYPE_CV_E)
  CONDITIONS(4) = (MSG%STREAM.EQ.STREAM_ENDA_E)
  CONDITIONS(5) = (MSG%STREAM.EQ.STREAM_ELDA_E)

  IF ( ANY( CONDITIONS ) ) THEN
    MSG%NUMBER = THIS%MODEL_PAR_%SIM_%NENSFNB
    PAR%ENSEMBLE%TYPE_OF_ENSEMBLE_FORECAST_ = 1_JPIB_K
    PAR%ENSEMBLE%NUMBER_OF_FORECASTS_IN_ENSEMBLE_ = THIS%MODEL_PAR_%SIM_%NTOTENS
  ENDIF

  ! Set the analysis data (TODO: improve this check)
  ! This check should be done on the basis of the type/stream/class,
  ! not on the basis of the section 2 that is suppoosed to be removed from here
  IF ( THIS%MODEL_PAR_%SIM_%NLOCGRB .EQ. 36 ) THEN
...
&                                                       PROCESSOR_TOPO%MYPROC_IO, THIS%TID_, HOOKS )
  ...
  PP_TRYCALL(ERRFLAG_LOADING_LOCAL_SAMPLE) MAKE_METADATA( THIS%METADATA_, 'log', 'sample', HOOKS )
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_METADATA_LIST_FREE=14_JPIB_K
