...
#define PP_FILE_NAME 'repres_map_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'REPRES_MAP_MOD'
MODULE REPRES_MAP_MOD
...
INTEGER(KIND=JPIB_K), PARAMETER :: REPRES_KEY_LENGTH=16_JPIB_K

!> @brief Datatype used to to store a node of the tree
TYPE :: REPRES_NODE_T

  !> Key
  CHARACTER(LEN=REPRES_KEY_LENGTH) :: KEY=REPEAT(' ',REPRES_KEY_LENGTH)

  !> Value
  LOGICAL :: TO_BE_DEALLOCATED = .TRUE.
  CLASS(REPRES_A), POINTER :: REPRES_ => NULL()
  ...
  TYPE(REPRES_NODE_T), POINTER :: PARENT => NULL()

  !> Pointer to the lef subtree
  TYPE(REPRES_NODE_T), POINTER :: LEFT   => NULL()

  !> Pointer to the right subtree
  TYPE(REPRES_NODE_T), POINTER :: RIGHT  => NULL()
...
TYPE :: REPRES_MAP_T
  ...
  TYPE(REPRES_NODE_T), POINTER :: ROOT => NULL()
...
CONTAINS
  ...
  !> Public methods
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT   => REPRES_INIT
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE   => REPRES_FREE
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MIN    => REPRES_MINIMUM
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MAX    => REPRES_MAXIMUM
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PUSH   => REPRES_PUSH
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH  => REPRES_MATCH
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: GET    => REPRES_GET
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: DELETE => REPRES_REMOVE
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: KEYS   => REPRES_GET_SORTED_KEYS
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: LIST   => REPRES_LIST
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT  => REPRES_PRINT

END TYPE


!> @brief Node used as terminal symbol
TYPE(REPRES_NODE_T), TARGET :: NIL

!> Whitelist of public symbols
PUBLIC :: REPRES_KEY_LENGTH
PUBLIC :: REPRES_MAP_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY1
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY2
...
PP_THREAD_SAFE FUNCTION INSERT_NODE( ROOT, KEY, REPRES, TO_BE_DEALLOCATED, INSERTED, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER,     INTENT(INOUT) :: ROOT
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY
  CLASS(REPRES_A), POINTER,         INTENT(IN)    :: REPRES
  ...
  ! Local variables
  TYPE(REPRES_NODE_T), POINTER :: INSERTION_POINT
  LOGICAL :: FOUND
  LOGICAL :: KEY_LT
  ...
  ! Logging
  PP_LOG_DEVELOP_STR( 'PUSH :: '//TRIM(ADJUSTL(KEY))//'::<'//TRIM(ADJUSTL(REPRES%DATA_REPRESENTATION_TYPE))//','//TRIM(ADJUSTL(REPRES%NAME))//'>' )
...
&       REPRES, TO_BE_DEALLOCATED, HOOKS )
...
&             KEY, REPRES, TO_BE_DEALLOCATED, HOOKS )
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: CURRENT
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
...
PP_THREAD_SAFE FUNCTION NODE_INIT( THIS, PARENT, KEY, REPRES, TO_BE_DEALLOCATED, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T),              INTENT(INOUT) :: THIS
  TYPE(REPRES_NODE_T), POINTER,     INTENT(IN)    :: PARENT
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY
  CLASS(REPRES_A), POINTER,         INTENT(IN)    :: REPRES
  LOGICAL,                          INTENT(IN)    :: TO_BE_DEALLOCATED
  TYPE(HOOKS_T),                    INTENT(INOUT) :: HOOKS
  ...
  THIS%TO_BE_DEALLOCATED = TO_BE_DEALLOCATED
  THIS%REPRES_ => REPRES
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: X
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
  ...
  IF ( ASSOCIATED( X ) ) THEN
    ...
    X%KEY = REPEAT(' ',REPRES_KEY_LENGTH)

    ! Free mappers
    IF ( ASSOCIATED( X%REPRES_ ) ) THEN
      IF ( X%TO_BE_DEALLOCATED ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_VAL) X%REPRES_%FREE( HOOKS )
        DEALLOCATE( X%REPRES_, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
        PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_UNABLE_TO_FREE_VAL )
      ENDIF
      X%TO_BE_DEALLOCATED = .TRUE.
      NULLIFY(  X%REPRES_ )
    ENDIF
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: CUR
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  TYPE(REPRES_NODE_T), POINTER :: Y
  TYPE(REPRES_NODE_T), POINTER :: X
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(REPRES_NODE_T), TARGET,  INTENT(INOUT) :: X_
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(IN)    :: X
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(IN)    :: X
  TYPE(REPRES_NODE_T), POINTER, INTENT(OUT)   :: Y
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: NODE_1
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: NODE_2
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
  ...
  CHARACTER(LEN=REPRES_KEY_LENGTH) :: KEY
  ...
  KEY = NODE_1%KEY
  TO_BE_DEALLOCATED = NODE_1%TO_BE_DEALLOCATED
  NODE_1%KEY = NODE_2%KEY
  NODE_1%TO_BE_DEALLOCATED = NODE_2%TO_BE_DEALLOCATED
  NODE_2%KEY = KEY
  NODE_2%TO_BE_DEALLOCATED = TO_BE_DEALLOCATED
  KEY = REPEAT(' ',REPRES_KEY_LENGTH)

  ! Swapping values between two nodes
  REPRES => NODE_1%REPRES_
  NODE_1%REPRES_ => NODE_2%REPRES_
  NODE_2%REPRES_ => REPRES
  REPRES => NULL()
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(REPRES_NODE_T), POINTER, INTENT(INOUT) :: Z
  TYPE(HOOKS_T),                INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER, INTENT(IN)    :: ROOT
  TYPE(REPRES_NODE_T), POINTER, INTENT(IN)    :: CURRENT
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T),  INTENT(INOUT) :: ROOT
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(REPRES_NODE_T), INTENT(INOUT) :: ROOT
  INTEGER(KIND=JPIB_K), INTENT(IN)    :: UNIT
  TYPE(HOOKS_T),        INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(REPRES_NODE_T), POINTER,                   INTENT(INOUT) :: NODE
  CHARACTER(LEN=REPRES_KEY_LENGTH), DIMENSION(:), INTENT(INOUT) :: SORTED_KEYS
  INTEGER(KIND=JPIB_K),                           INTENT(INOUT) :: CNT
  TYPE(HOOKS_T),                                  INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  TYPE(REPRES_NODE_T), POINTER :: Y
  TYPE(REPRES_NODE_T), POINTER :: PREV
  LOGICAL :: IS_LEAF
...
PP_THREAD_SAFE FUNCTION REPRES_INIT( ENCODERS_MAP, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(REPRES_MAP_T), INTENT(INOUT) :: ENCODERS_MAP
  TYPE(HOOKS_T),       INTENT(INOUT) :: HOOKS
...
PP_THREAD_SAFE FUNCTION REPRES_FREE( ENCODERS_MAP, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_MINIMUM'
PP_THREAD_SAFE FUNCTION REPRES_MINIMUM( ENCODERS_MAP, KEY, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(REPRES_MAP_T),              INTENT(INOUT) :: ENCODERS_MAP
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(OUT)   :: KEY
  TYPE(HOOKS_T),                    INTENT(INOUT) :: HOOKS
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_MAXIMUM'
PP_THREAD_SAFE FUNCTION REPRES_MAXIMUM( ENCODERS_MAP, KEY, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_GET_SORTED_KEYS'
PP_THREAD_SAFE FUNCTION REPRES_GET_SORTED_KEYS( ENCODERS_MAP, SORTED_KEYS, HOOKS ) RESULT(RET)
...
END FUNCTION REPRES_GET_SORTED_KEYS
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_PUSH'
PP_THREAD_SAFE FUNCTION REPRES_PUSH( THIS, KEY, REPRES, TO_BE_DEALLOCATED, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(REPRES_MAP_T),              INTENT(INOUT) :: THIS
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY
  CLASS(REPRES_A), POINTER,         INTENT(IN)    :: REPRES
  LOGICAL,                          INTENT(IN)    :: TO_BE_DEALLOCATED
  TYPE(HOOKS_T),                    INTENT(INOUT) :: HOOKS
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_INSERT_NODE) INSERT_NODE( &
     THIS%ROOT, KEY, REPRES, TO_BE_DEALLOCATED, INSERTED, HOOKS )
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_MATCH'
PP_THREAD_SAFE FUNCTION REPRES_MATCH( THIS, KEY, LMATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local variables
  LOGICAL :: ROOT_IS_LEAF
  TYPE(REPRES_NODE_T), POINTER :: SEARCHED_NODE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_GET'
PP_THREAD_SAFE FUNCTION REPRES_GET( THIS, KEY, REPRES, LMATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CLASS(REPRES_MAP_T),              INTENT(INOUT) :: THIS
  CHARACTER(LEN=REPRES_KEY_LENGTH), INTENT(IN)    :: KEY
  CLASS(REPRES_A), POINTER,         INTENT(OUT)   :: REPRES
  LOGICAL,                          INTENT(OUT)   :: LMATCH
  TYPE(HOOKS_T),                    INTENT(INOUT) :: HOOKS
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_REMOVE'
PP_THREAD_SAFE FUNCTION REPRES_REMOVE( THIS, KEY, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_LIST'
PP_THREAD_SAFE FUNCTION REPRES_LIST( THIS, UNIT, PREFIX, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'REPRES_PRINT'
PP_THREAD_SAFE FUNCTION REPRES_PRINT( THIS, NAME, IDX, HOOKS ) RESULT(RET)
...
END FUNCTION REPRES_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE REPRES_MAP_MOD
