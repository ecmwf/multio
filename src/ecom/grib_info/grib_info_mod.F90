...
IMPLICIT NONE
  ...
  WRITE(LOGUNIT,'(A,I8)')  ' + Requested encoding....................: ', GI%REQUESTED_ENCODING_
...
SUBROUTINE READ_GRIB_INFO_YAML( CFG, MODEL_PARAMS, CNT, IDX )
  ...
  USE :: GRIB_INFO_DATA_MOD, ONLY: GRIB_INFO_DB
  USE :: GRIB_INFO_DATA_MOD, ONLY: UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  IF ( CFG%HAS( 'requested-encoding' ) ) THEN
    EX = CFG%GET( 'requested-encoding', ITMP )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.EX, 1 )
    GRIB_INFO_DB(CNT)%REQUESTED_ENCODING_ = ITMP
  ELSE
