!>
!> @file grib2_section4_dirfreq_default_mod.F90
!>
!> @brief Module for managing GRIB2 Section 4 time configuration operations.
!>
!> The `GRIB2_SECTION4_DIRFREQ_DEFAULT_MOD` module contains procedures to initialize, allocate,
...
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_ALLOCATE
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_PRESET
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_RUNTIME
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_TO_BE_ENCODED
!>   - @see GRIB2_SECTION4_DIRFREQ_DEFAULT_FREE
!>
...
#define PP_FILE_NAME 'grib2_section4_dirfreq_default_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_MOD'
MODULE GRIB2_SECTION4_DIRFREQ_DEFAULT_MOD
...
!>
!> The `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` type extends the base class `GRIB_SECTION_BASE_A` and
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION4_DIRFREQ_DEFAULT_T

  !> Default symbols visibility
  PRIVATE

  !> Scaled factors for wave direction
  INTEGER(KIND=JPIB_K) :: SCALED_FACTOR_OF_WAVE_DIRECTIONS_=2_JPIB_K

  !> Scaled factors for wave frequencies
  INTEGER(KIND=JPIB_K) :: SCALED_FACTOR_OF_WAVE_FREQUENCIES_=6_JPIB_K
...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOCATE => GRIB2_SECTION4_DIRFREQ_DEFAULT_ALLOCATE
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRESET => GRIB2_SECTION4_DIRFREQ_DEFAULT_PRESET
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RUNTIME => GRIB2_SECTION4_DIRFREQ_DEFAULT_RUNTIME
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION4_DIRFREQ_DEFAULT_TO_BE_ENCODED
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => GRIB2_SECTION4_DIRFREQ_DEFAULT_FREE

  !>
  !> @brief Read resources allocated for the GRIB2 Section 4 time configuration object.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: READ_FROM_CFG => GRIB2_SECTION4_DIRFREQ_READ_FROM_CFG
...
PUBLIC :: GRIB2_SECTION4_DIRFREQ_DEFAULT_T
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section being initialized.
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_CFG( THIS, &
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(GRIB2_SECTION4_DIRFREQ_DEFAULT_T),  INTENT(INOUT) :: THIS
  ...
  THIS%SUBTYPE_ = 'DIRECTION-FREQUENCY'
  THIS%KIND_    = 'DEFAULT'
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_LAZY'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_LAZY( THIS, &
...
#if defined( PP_HAS_GET_GRIB2_DIRFREQ_CONFIGURATION_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_DIRFREQ_CONFIGURATION_FROM_MESSAGE
#endif
...
#if defined( PP_HAS_GET_GRIB2_DIRFREQ_CONFIGURATION_FROM_MESSAGE )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ) GET_GRIB2_ENSEMBLE_CONFIGURATION_FROM_MESSAGE( &
&       MSG, PAR, OPT, THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_, &
&       THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_, HOOKS )
#else
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section to allocate resources for.
!>   @param [in]    MSG      All the mars keywords needed to describe the field `FORTRAN_MESSAGE_T`.
!>   @param [in]    PAR      All information outside mars keywords needed to describe the field `PARAMETRIZATION_T`.
!>   @param [in]    OPT      The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_ALLOCATE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_ALLOCATE( THIS, &
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K) :: N_DIRS
  INTEGER(KIND=JPIB_K) :: N_FREQ
  INTEGER(KIND=JPIB_K) :: ALLOC_STATUS
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  REAL(KIND=JPRD_K), ALLOCATABLE, DIMENSION(:) :: SCALED_VALUES_OF_WAVE_DIRECTIONS
  REAL(KIND=JPRD_K), ALLOCATABLE, DIMENSION(:) :: SCALED_VALUES_OF_WAVE_FREQUENCIES


  !> Error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_METADATA=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WAM_PARAMETRIZATION_NOT_ASSOCIATED=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=3_JPIB_K
  ...
  ! Lazy initialization of scale factors
  IF ( PAR%WAVE%SCALE_FACTOR_OF_WAVE_DIRECTION .NE. UNDEF_PARAM_E ) THEN
    THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_ = PAR%WAVE%SCALE_FACTOR_OF_WAVE_DIRECTION
  ENDIF
  IF ( PAR%WAVE%SCALE_FACTOR_OF_WAVE_FREQUENCY .NE. UNDEF_PARAM_E ) THEN
    THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_ = PAR%WAVE%SCALE_FACTOR_OF_WAVE_FREQUENCY
  ENDIF

  ! Error handling
  PP_DEBUG_CRITICAL_COND_THROW( .NOT. ASSOCIATED(METADATA), ERRFLAG_METADATA )

  !> Gererate the scaled values for the wave directions
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(PAR%WAVE%DIRS_), ERRFLAG_WAM_PARAMETRIZATION_NOT_ASSOCIATED )
  N_DIRS = SIZE(PAR%WAVE%DIRS_)
  ALLOCATE(SCALED_VALUES_OF_WAVE_DIRECTIONS(N_DIRS), STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_ALLOCATION_ERROR )

  !> Fill the scaled values for directions
  DO I = 1, N_DIRS
    SCALED_VALUES_OF_WAVE_DIRECTIONS(I) = NINT(PAR%WAVE%DIRS_(I)*10**THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_*RAD2DEG)
  ENDDO


  !> Gererate the scaled values for the wave frequencies
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(PAR%WAVE%FREQ_), ERRFLAG_WAM_PARAMETRIZATION_NOT_ASSOCIATED )
  N_FREQ = SIZE(PAR%WAVE%FREQ_)
  ALLOCATE(SCALED_VALUES_OF_WAVE_FREQUENCIES(N_FREQ), STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_ALLOCATION_ERROR )

  !> Fill the scaled values for frequencies
  DO I = 1, N_FREQ
    SCALED_VALUES_OF_WAVE_FREQUENCIES(I) = NINT(PAR%WAVE%FREQ_(I)*10**THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_)
  ENDDO

  !> Set the metadata
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'numberOfWaveDirections', N_DIRS )
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaleFactorOfWaveDirections', THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_ )
  IF ( ALLOCATED(SCALED_VALUES_OF_WAVE_DIRECTIONS) ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaledValuesOfWaveDirections', SCALED_VALUES_OF_WAVE_DIRECTIONS )
  ENDIF

  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'numberOfWaveFrequencies', N_FREQ )
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaleFactorOfWaveFrequencies', THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_ )
  IF ( ALLOCATED(SCALED_VALUES_OF_WAVE_FREQUENCIES) ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaledValuesOfWaveFrequencies', SCALED_VALUES_OF_WAVE_FREQUENCIES )
  ENDIF

  !> Set the runtime
  ! IF ( OPT%CACHE_STRATEGY .EQ. OPT_CACHE_FULL_E ) THEN
  !   PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'waveDirectionNumber', MSG%DIRECTION )
  !   PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'waveFrequencyNumber', MSG%FREQUENCY )
  ! ENDIF

  !> Free local memory
  IF ( ALLOCATED(SCALED_VALUES_OF_WAVE_DIRECTIONS) ) THEN
    DEALLOCATE(SCALED_VALUES_OF_WAVE_DIRECTIONS, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS .NE. 0, ERRFLAG_DEALLOCATION_ERROR )
  ENDIF

  IF ( ALLOCATED(SCALED_VALUES_OF_WAVE_FREQUENCIES) ) THEN
    DEALLOCATE(SCALED_VALUES_OF_WAVE_FREQUENCIES, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS .NE. 0, ERRFLAG_DEALLOCATION_ERROR )
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE ( ERRFLAG_WAM_PARAMETRIZATION_NOT_ASSOCIATED )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'WAM parametrization not associated' )
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_ALLOCATE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section to be preset.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_PRESET'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_PRESET( THIS, &
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_PRESET
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS      An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section for runtime execution.
!>   @param [in]    MSG       The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR       The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_RUNTIME'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_RUNTIME( THIS, &
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_RUNTIME
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
!>   @param [in]    TIME_HIST     The time history object of type `TIME_HISTORY_T` providing historical time data.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_TO_BE_ENCODED( THIS, &
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_DIRFREQ_DEFAULT_T` representing the GRIB section to be freed.
!>   @param [in]    OPT   The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_DEFAULT_FREE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_FREE( THIS, OPT, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_DEFAULT_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_DIRFREQ_READ_FROM_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_DIRFREQ_READ_FROM_CFG( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Local variables
  LOGICAL :: HAS_SCALED_FACTOR_OF_WAVE_DIRECTIONS
  LOGICAL :: HAS_SCALED_FACTOR_OF_WAVE_FREQUENCIES
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( CFG, &
&     'scale-factor-of-wave-directions',  HAS_SCALED_FACTOR_OF_WAVE_DIRECTIONS, HOOKS )

  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( CFG, &
&     'scale-factor-of-wave-frequencies', HAS_SCALED_FACTOR_OF_WAVE_FREQUENCIES, HOOKS )

  !> Read the encoder configuration
  IF ( HAS_SCALED_FACTOR_OF_WAVE_DIRECTIONS ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( CFG, &
&      'scale-factor-of-wave-directions', THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_, HOOKS )
  ELSE
    THIS%SCALED_FACTOR_OF_WAVE_DIRECTIONS_ = 2_JPIB_K
  ENDIF

  IF ( HAS_SCALED_FACTOR_OF_WAVE_FREQUENCIES ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( CFG, &
&      'scale-factor-of-wave-frequencies', THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_, HOOKS )
  ELSE
    THIS%SCALED_FACTOR_OF_WAVE_FREQUENCIES_ = 6_JPIB_K
  ENDIF
...
END FUNCTION GRIB2_SECTION4_DIRFREQ_READ_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE GRIB2_SECTION4_DIRFREQ_DEFAULT_MOD
