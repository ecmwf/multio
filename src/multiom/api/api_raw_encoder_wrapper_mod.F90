...
#define PP_FILE_NAME 'API_RAW_ENCODER_wrapper_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'API_RAW_ENCODER_WRAPPER_MOD'
MODULE API_RAW_ENCODER_WRAPPER_MOD
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_RAW_ENCODER_OPEN( OPTIONS, MARS, PAR, RULES, MULTIO_GRIB2, SAMPLE_ID ) &
 BIND(C,NAME='multio_grib2_raw_encoder_open') RESULT(RET)
  ...
  ! USE :: MULTIOM_RAW_ENCODER_MOD,          ONLY: MULTIOM_RAW_ENCODERS_T
...
IMPLICIT NONE
  ...
  TYPE(C_PTR), VALUE,  INTENT(IN)    :: MARS
  TYPE(C_PTR), VALUE,  INTENT(IN)    :: PAR
  TYPE(C_PTR), VALUE,  INTENT(IN)    :: RULES
  TYPE(C_PTR), VALUE,  INTENT(IN)    :: MULTIO_GRIB2
  INTEGER(KIND=C_INT), INTENT(INOUT) :: SAMPLE_ID
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_RULE_NOT_ASSOCIATED=13_JPIB_K
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.C_ASSOCIATED(RULE), ERRFLAG_RULE_NOT_ASSOCIATED )
  ...
  IF ( HAS_OPTIONS ) THEN
    ...
    CASE ( OPT_DICT_TYPE_E )
      ...
      ENCODER_OPTIONS%PRINT_WHOLE_ERROR_STACK = OPT_DICT%PRINT_WHOLE_ERROR_STACK
      ENCODER_OPTIONS%PRINT_DICTIONARIES = OPT_DICT%PRINT_DICTIONARIES
...
!$omp critical(API_RAW_ENCODER_MAP_INSERT)
  ...
  IF ( SIZE .EQ. 0_JPIB_K ) THEN
    F_MULTIO_GRIB2(1) = 2_C_LONG_LONG
...
!$omp end critical(API_RAW_ENCODER_MAP_INSERT)
#endif
...
#if 0
!$omp critical(API_RAW_ENCODER_MAP_REMOVE)
  !> Error handling
...
!$omp end critical(API_RAW_ENCODER_MAP_REMOVE)
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_ENCODER_ENCODE( MULTIO_GRIB2, C_MARS_DICT, C_PAR_DICT, C_CODES_HANDLE_LOC, C_MESSAGE_LEN ) &
 BIND(C,NAME='multio_grib2_raw_encoder_encode_f') RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(MULTIOM_CACHED_ENCODERS_T), POINTER    :: ENCODER
  TYPE(GRIB_ENCODER_OPTIONS_T), POINTER       :: ENCODER_OPTIONS
  LOGICAL                                     :: PRINT_WHOLE_ERROR_STACK
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GET_ENCODER_OPTIONS=22_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_GET_ENCODER_OPTIONS) ENCODER%GET_ENCODER_OPTIONS(ENCODER_OPTIONS, HOOKS)
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(ENCODER_OPTIONS), ERRFLAG_GET_ENCODER_OPTIONS )
  PRINT_WHOLE_ERROR_STACK=ENCODER_OPTIONS%PRINT_WHOLE_ERROR_STACK
...
&       MAPPING_TAG, MAPPING_NAME, ENCODER_TAG, ENCODER_NAME, MD, HOOKS )
  ...
  CLASS IS (GRIB_METADATA_T)
    ...
    PAR_DICT%VALUES_SCALE_FACTOR=OUT_PAR%VALUES_SCALE_FACTOR
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_GET_ENCODER_OPTIONS)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unable to get encoder options' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown error' )
    END SELECT
    ...
    WRITE(ERROR_UNIT,*) ' PRINT ERROR STACK FROM: "'//__FILE__//'":', __LINE__
    CALL HOOKS%DEBUG_HOOK_%PRINT_ERROR_STACK( ERROR_UNIT, .NOT.PRINT_WHOLE_ERROR_STACK)
...
END MODULE API_RAW_ENCODER_WRAPPER_MOD
