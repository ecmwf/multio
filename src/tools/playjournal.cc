...
void ReadJournal::run()
{
    ...
    // Create a MultIO based on exactly the configuration in the journal file
    MultIO msink(journal.config());

    JournalRecord record(journal, JournalRecord::Uninitialised);
    while (journal.readRecord(record)) {
        msink.replayRecord(record);
    };

    Log::info() << "===============================================" << std::endl;
    Log::info() << "Finished journal playback" << std::endl;
