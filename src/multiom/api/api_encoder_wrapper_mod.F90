...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_ENCODER_ENCODE( MULTIO_GRIB2, C_MARS_DICT, C_PAR_DICT, GRIB_HANDLE ) &
 BIND(C,NAME='multio_grib2_encoder_encode') RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GRIB_HANDLE_IS_NULL_POINTER=7_JPIB_K
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_GRIB_HANDLE_IS_NULL_POINTER)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Grib handle is null' )
    END SELECT
