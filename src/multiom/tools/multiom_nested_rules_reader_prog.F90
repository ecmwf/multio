...
IMPLICIT NONE
  ...
  TYPE(NESTED_RULES_TREE_LOADER_T) :: NESTED_RULES_TREE_LOADER
  CHARACTER(LEN=128) :: YAMLFNAME
  TYPE(FILTER_OPTIONS_T) :: FILTER_OPT
  INTEGER(KINd=JPIB_K) :: SZ
  INTEGER(KINd=JPIB_K) :: MAX_SZ
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_RULES_SIZE = 7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_RULES_FREE = 8_JPIB_K
  ...
  ! Open the filename
  PP_TRYCALL(ERRFLAG_UNABLE_TO_OPEN_CFG_FILE) YAML_NEW_CONFIGURATION_FROM_FILE( TRIM(ADJUSTL(YAMLFNAME)), MAIN_CFG, HOOKS )

  ! Initialize the rules loader
  PP_TRYCALL(ERRFLAG_RULES_LOADER) NESTED_RULES_TREE_LOADER%INIT( MAIN_CFG, FILTER_OPT, HOOKS )

  ! Compute the number of rules in the structure
  PP_TRYCALL(ERRFLAG_RULES_PRINT) NESTED_RULES_TREE_LOADER%SIZE( SZ, MAX_SZ, MAX_LEVELS, HOOKS )
  WRITE(*,'(A,3I8)') 'The number of rules in the structure is: ', SZ, MAX_SZ, MAX_LEVELS

  ! Print all the rules
  PP_TRYCALL(ERRFLAG_RULES_PRINT) NESTED_RULES_TREE_LOADER%PRINT( 6_JPIB_K, 0_JPIB_K, HOOKS )


  ! Initialize the rules loader
  PP_TRYCALL(ERRFLAG_RULES_FREE) NESTED_RULES_TREE_LOADER%FREE( HOOKS )
