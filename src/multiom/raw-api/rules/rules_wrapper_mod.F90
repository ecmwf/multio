...
&  F_MARS_DICT, RULE_FNAME, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=*),           INTENT(OUT)   :: RULE_FNAME
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  TYPE(RAW_RULES_LIST_T) :: MATCHING_RULES
  TYPE(NESTED_RULE_COLLECTION_T), POINTER :: RULES
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INIT_LIST_RULES=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_SEARCH_RULES=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_NUM_MATCHES=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_GENERATE_JSON=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_MATCHES_COUNT=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_MULTIPLE_MATCHING_RULES=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GET_RULE_BY_ID=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_NO_MATCHING_RULE=9_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_INIT_LIST_RULES) MATCHING_RULES%INIT( HOOKS )

  ! Search for rules
  PP_TRYCALL(ERRFLAG_SEARCH_RULES) RULES%SEARCH( F_MARS_DICT, MATCHING_RULES, HOOKS )

  ! Count the number of matches
  NUM_MATCHES = -1_JPIB_K
  PP_TRYCALL(ERRFLAG_NUM_MATCHES) MATCHING_RULES%SIZE( NUM_MATCHES, HOOKS )
  ...
  ELSEIF ( NUM_MATCHES .EQ. 1_JPIB_K ) THEN
    ...
    RULE_FNAME = REPEAT(' ', LEN(RULE_FNAME))
    ! Call the encoder
    PP_TRYCALL(ERRFLAG_GET_RULE_BY_ID) MATCHING_RULES%GET_RULE_BY_ID( 1_JPIB_K, &
&     RULE_FNAME, HOOKS )
  ...
  ELSE
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GENERATE_JSON) MATCHING_RULES%TO_JSON( &
&      JSON_RULES, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_MULTIPLE_MATCHING_RULES)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Multiple matching rules' )
      IF ( ALLOCATED(JSON_MSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error message: ' // TRIM(ADJUSTL(JSON_MSG)) )
        DEALLOCATE( JSON_MSG, STAT=ALLOC_STAT )
      END IF
      IF ( ALLOCATED(JSON_RULES) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Rules: ' // TRIM(ADJUSTL(JSON_RULES)) )
        DEALLOCATE( JSON_RULES, STAT=ALLOC_STAT )
      END IF
    CASE (ERRFLAG_GET_RULE_BY_ID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get rule by id' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
