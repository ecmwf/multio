...
PRIVATE
  ...
  TYPE :: WAVE_PAR_T
    ...
    INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_DIRECTION = UNDEF_PARAM_E
    INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_FREQUENCY = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTION = UNDEF_PARAM_E
  WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCY = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  WAVE_PAR_TO%SCALE_FACTOR_OF_WAVE_DIRECTION = WAVE_PAR_FROM%SCALE_FACTOR_OF_WAVE_DIRECTION
  WAVE_PAR_TO%SCALE_FACTOR_OF_WAVE_FREQUENCY = WAVE_PAR_FROM%SCALE_FACTOR_OF_WAVE_FREQUENCY
...
IMPLICIT NONE
  ...
  IF ( HAS_KEY  ) THEN
    ...
    ! Read SCALE_FACTOR_OF_WAVE_DIRECTION
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( WAVE_CONFIGURATION, 'scale-factor-of-wave-direction', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( WAVE_CONFIGURATION, 'scale-factor-of-wave-direction', WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTION, HOOKS )
    ELSE
      WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTION = UNDEF_PARAM_E
    ENDIF

    ! Read SCALE_FACTOR_OF_WAVE_FREQUENCY
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( WAVE_CONFIGURATION, 'scale-factor-of-wave-frequency', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( WAVE_CONFIGURATION, 'scale-factor-of-wave-frequency', WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCY, HOOKS )
    ELSE
      WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCY = UNDEF_PARAM_E
    ENDIF
...
IMPLICIT NONE
  ...
  IF ( ANY(CONDITIONS) ) THEN
    ...
    IF ( WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTION .NE. UNDEF_PARAM_E ) THEN
      ! convert integer to string
      CTMP = REPEAT(' ', MAX_STR_LEN)
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_STRING) TO_STRING(WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTION, CTMP, HOOKS )

      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'scale-factor-of-wave-direction: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF

    IF ( WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCY .NE. UNDEF_PARAM_E ) THEN
      ! convert integer to string
      CTMP = REPEAT(' ', MAX_STR_LEN)
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_STRING) TO_STRING(WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCY, CTMP, HOOKS )

      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'scale-factor-of-wave-frequency: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF
