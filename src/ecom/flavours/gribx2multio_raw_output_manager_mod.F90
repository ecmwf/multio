...
IMPLICIT NONE
  ...
  IF ( CFG%GET( TOLOWER(GRIBX2MULTIO_RAW_OMNAME), GRIBX2MULTIO_RAW_CFG ) ) THEN
    ...
    IF ( ALLOCATED(CLTMP) ) DEALLOCATE(CLTMP)
    IF ( GRIBX2MULTIO_RAW_CFG%GET( 'samples-path', CLTMP  ) ) THEN
      PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CLTMP), 1 )
...
IMPLICIT NONE
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) TOLOWER(GRIBX2MULTIO_RAW_OMNAME)//'-output-manager-', PROCESSOR_TOPO%MYPROC_IO, '.log'
