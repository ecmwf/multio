...
MODULE GRIB_HEADER2MULTIO_MOD
  ...
  USE :: REPRES_MAP_MOD,             ONLY: REPRES_KEY_LENGTH
...
TYPE, EXTENDS(OUTPUT_MANAGER_BASE_A) :: GRIB_HEADER2MULTIO_OUTPUT_MANAGER_T
  ...
  !> Representations map
  TYPE(REPRES_MAP_T) :: REPRES_MAP_
  CHARACTER(LEN=REPRES_KEY_LENGTH) :: GG_NAME=REPEAT(' ',REPRES_KEY_LENGTH)
  CHARACTER(LEN=REPRES_KEY_LENGTH) :: SH_NAME=REPEAT(' ',REPRES_KEY_LENGTH)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_REPRESENTATION_MAP) THIS%REPRES_MAP_%INIT( HOOKS )
  ...
  IF ( THIS%MODEL_PAR_%GEO_%NHTYP .GT. 0_JPIB_K ) THEN
    ...
    KEY='gg_'//TRIM(ADJUSTL(THIS%GG_NAME))
    PP_TRYCALL(ERRFLAG_UNABLE_TO_PUSH_TO_REPRESENTATION_MAP) THIS%REPRES_MAP_%PUSH( KEY, REPRES, .TRUE., HOOKS )
  ...
  IF ( THIS%MODEL_PAR_%GEO_%NSTTYP .GE. 2_JPIB_K ) THEN
    ...
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
    WRITE(THIS%SH_NAME, '(A,A)',IOSTAT=WRITE_STAT) 'SRS',TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
    ...
    !> Add the representation to the map
    KEY=REPEAT(' ',REPRES_KEY_LENGTH)
    KEY='sh_'//TRIM(ADJUSTL(THIS%SH_NAME))
    PP_TRYCALL(ERRFLAG_UNABLE_TO_PUSH_TO_REPRESENTATION_MAP) THIS%REPRES_MAP_%PUSH( KEY, REPRES, .TRUE., HOOKS )
  ...
  ELSEIF ( ABS(THIS%MODEL_PAR_%GEO_%RSTRET-1.0_JPRD_K) .GE. 1.0E-14_JPRD_K ) THEN
    ...
    WRITE(CTMP,'(I16)',IOSTAT=WRITE_STAT) THIS%MODEL_PAR_%GEO_%ISMAX
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
    WRITE(THIS%SH_NAME, '(A,A)',IOSTAT=WRITE_STAT) 'SS',TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
  ...
  ELSE
    ...
    WRITE(CTMP,'(I16)',IOSTAT=WRITE_STAT) THIS%MODEL_PAR_%GEO_%ISMAX
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
    WRITE(THIS%SH_NAME, '(A,A)',IOSTAT=WRITE_STAT) 'S',TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0_JPIB_K, ERRFLAG_UNABLE_TO_WRITE )
...
&    THIS%GG_NAME, THIS%SH_NAME, THIS%REPRES_MAP_, &
&    MSG, PAR, HOOKS )
...
&    THIS%GG_NAME, THIS%SH_NAME, THIS%REPRES_MAP_, &
&    MSG, PAR, HOOKS )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_GEOMETRY) WAM2MARS_SET_DIRFREQ( YDMSG, THIS%MODEL_PAR_, MSG, PAR, HOOKS )
...
&                                                       PROCESSOR_TOPO%MYPROC_IO, THIS%TID_, HOOKS )
  ...
  ! Print the geoemtry map
  IF ( THIS%VERBOSE_ ) THEN
    PP_TRYCALL(ERRFLAG_PRINT_GEOMETRY_MAP) THIS%REPRES_MAP_%LIST( 6_JPIB_K, 'Geometry map: ', HOOKS )
  ENDIF
...
IMPLICIT NONE
  ...
  CTMP = REPEAT(' ',32)
  WRITE(CTMP, '(I16)', IOSTAT=WRITE_STAT ) BYTESIZE
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  PP_LOG_DEVELOP_STR( 'Cache size in bytes is: '//TRIM(ADJUSTL(CTMP)) )
