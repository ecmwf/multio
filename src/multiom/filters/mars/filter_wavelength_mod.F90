...
!>
!> This module defines the `FILTER_WAVELENGTH_T` type, along with its associated
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_WAVELENGTH_T
  ...
  !> @details If `.TRUE.`, the `WAVELENGTHS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  INTEGER(KIND=JPIB_K) :: WAVELENGTH_
  ...
  TYPE(KEYSET_INT64_T) :: WAVELENGTHS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_WAVELENGTH_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_WAVELENGTH_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_WAVELENGTH_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_WAVELENGTH_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_WAVELENGTH_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_WAVELENGTH_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_WAVELENGTH_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_WAVELENGTH_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_WAVELENGTH_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_WAVELENGTH_FREE
...
PUBLIC :: FILTER_WAVELENGTH_T

CONTAINS


!>
!> @brief Initializes the `FILTER_WAVELENGTH_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_WAVELENGTH_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_WAVELENGTH_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_WAVELENGTH_INIT'
FUNCTION FILTER_WAVELENGTH_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_WAVELENGTH_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%WAVELENGTHS_%INIT( HOOKS )
...
&            .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%WAVELENGTHS_, HOOKS )
...
END FUNCTION FILTER_WAVELENGTH_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_WAVELENGTH_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_WAVELENGTH_MATCH'
FUNCTION FILTER_WAVELENGTH_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = .FALSE. !(MSG%WAVELENGTHS_ .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = .TRUE. ! (MSG%WAVELENGTHS_ .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )
    MATCH = .FALSE.
    ! IF ( MSG%WAVELENGTH .EQ. UNDEF_PARAM_E ) THEN
    !   MATCH = .FALSE.
    ! ELSE
    !   IF ( THIS%USE_KEYSET_ ) THEN
    !     PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%WAVELENGTHS_%MATCH( MSG%WAVELENGTH, MATCH, HOOKS )
    !   ELSE
    !     MATCH = (THIS%WAVELENGTH_ .EQ. MSG%WAVELENGTH)
    !   ENDIF
    ! ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    !    MATCH = .NOT.MATCH
    !  ELSE
    !    MATCH = .NOT.(THIS%WAVELENGTH_ .EQ. MSG%WAVELENGTH)
    !  ENDIF
    !ENDIF
  ...
  CASE( FLT_INT_GT_E )
    ...
    !   MATCH = .FALSE.
    ! ELSE
    !   MATCH = (MSG%WAVELENGTH .GT. THIS%WAVELENGTH_ )
    ! ENDIF
  ...
  CASE( FLT_INT_GE_E )
    ...
    !  MATCH = .FALSE.
    !ELSE
    !  MATCH = (MSG%WAVELENGTH .GE. THIS%WAVELENGTH_ )
    !ENDIF
  ...
  CASE( FLT_INT_LE_E )
    ...
    !  MATCH = .FALSE.
    !ELSE
    !  MATCH = (MSG%WAVELENGTH .LE. THIS%WAVELENGTH_ )
    !ENDIF
  ...
  CASE( FLT_INT_LT_E )
    ...
    !  MATCH = .FALSE.
    !ELSE
    !  MATCH = (MSG%WAVELENGTH .LT. THIS%WAVELENGTH_ )
    !ENDIF
...
END FUNCTION FILTER_WAVELENGTH_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_WAVELENGTH_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_WAVELENGTH_PRINT'
FUNCTION FILTER_WAVELENGTH_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'WAVELENGTH FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_WAVELENGTH_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_WAVELENGTH_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_WAVELENGTH_FREE'
FUNCTION FILTER_WAVELENGTH_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%WAVELENGTHS_%FREE( HOOKS )
...
END FUNCTION FILTER_WAVELENGTH_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_WAVELENGTH_MOD
