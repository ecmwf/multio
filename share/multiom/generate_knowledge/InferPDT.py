...
# TODO pgeier remove this file - only c++ generated code is used
    ...
    match matcher:
        ...
        case MatchPDTCategory():
            ...
            if cat not in catValuePairs.keys():
                ...
                raise ValueError(
                    f"Category {cat} is not available in dict {catValuePairs}. Matcher {matcher}"
                )
            ...
            if catVal not in matcher.subMatch.keys():
                if matcher.default:
                    return inferPDT(catValuePairs, matcher.default)
                raise ValueError(
                    f"Category with value {{{cat}: {catVal}}} has no entry in sub matcher and no default is given. Possible values {list(matcher.subMatch.keys())}"
                )
...
def buildDecisionMap(subCatList, selector, mappedSelectorList) -> MatchPDTType:
    if len(subCatList) == 0:
        if len(mappedSelectorList) == 1:
            ...
            raise ValueError(
                f"buildDecisionMap: selector {selector} matches multiple or no pdts: {mappedSelectorList}"
            )
...
pdtMatcher = buildDecisionMap(
    pdtCategoriesHandleOrder, {}, categorySelectorsWithMappedPdt
)
