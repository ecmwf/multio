...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_OPEN_FILE_FAILED=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CLOSE_FILE_FAILED=8_JPIB_K
  ...
  ! OPEN(NEWUNIT=SAMPLEFILE, FILE=TRIM(ADJUSTL(SAMPLE_NAME)), IOSTAT=STAT, IOMSG=IOMSG, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=1)
  ! OPEN(NEWUNIT=SAMPLEFILE, FILE=TRIM(ADJUSTL(SAMPLE_NAME)), STATUS="old", ACTION="read", IOSTAT=STAT, IOMSG=IOMSG, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=1)
  CALL GRIB_OPEN_FILE(SAMPLEFILE, TRIM(ADJUSTL(SAMPLE_NAME)), 'r', KRET)
  PP_DEBUG_DEVELOP_COND_THROW( KRET.NE.0, ERRFLAG_OPEN_FILE_FAILED )
  
  CALL GRIB_NEW_FROM_FILE( SAMPLEFILE, THIS%IGRIB_HANDLE_, STATUS=KRET )
  PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, ERRFLAG_GRIB_NEW_FROM_SAMPLES_FAILED )
  
  CALL GRIB_CLOSE_FILE(SAMPLEFILE, KRET)
  PP_DEBUG_DEVELOP_COND_THROW( KRET.NE.0, ERRFLAG_CLOSE_FILE_FAILED )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_OPEN_FILE_FAILED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to open sample file.' )
    CASE (ERRFLAG_CLOSE_FILE_FAILED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to close sample file.' )
