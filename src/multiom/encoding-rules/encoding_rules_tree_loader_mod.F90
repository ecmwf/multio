...
TYPE :: ENCODING_RULES_TREE_LOADER_NODE_T
  ...
  TYPE(ENCODING_RULE_COLLECTION_T), POINTER :: RULES_COLLECTION => NULL()
...
IMPLICIT NONE
  ...
  IF ( HAS_NESTED_RULES ) THEN
    PP_TRYCALL(ERRFLAG_RULES_INITIALIZE) THIS%HEAD%INIT_CFG( CFG, FILTER_OPT, ENCODER_OPT, THIS%SIZE, LEVEL, HOOKS )
  ELSEIF ( HAS_ENCODING_RULES ) THEN
    PP_TRYCALL(ERRFLAG_RULES_INITIALIZE) THIS%HEAD%INIT_RULES( CFG, FILTER_OPT, ENCODER_OPT, THIS%SIZE, LEVEL, HOOKS )
  ENDIF

  ! Debugging
  CTMP=REPEAT(' ',LEN(CTMP))
  WRITE(CTMP,*) THIS%SIZE
...
PP_THREAD_SAFE FUNCTION ENCODING_RULES_TREE_LOADER_MATCH_SINGLE( THIS, MSG, PAR, METADATA, ENCODER, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(CACHED_ENCODER_T), POINTER,     INTENT(OUT)   :: ENCODER
  ...
  !> Match the rules
  PP_TRYCALL(ERRFLAG_MATCH_RULES) THIS%HEAD%MATCH_SINGLE( MSG, PAR, METADATA, ENCODER, OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION ENCODING_RULES_TREE_LOADER_MATCH_MULTIPLE( THIS, MSG, PAR, METADATA, ENCODERS, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Allocate the encoders
  ALLOCATE( ENCODERS(NUM_MATCHES), STAT=ALLOC_STATUS )
  PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS .NE. 0, ERRFLAG_UNABLE_TO_ALLOCATE )


  !> Match the rules
  ID = 0
  PP_TRYCALL(ERRFLAG_MATCH_RULES) THIS%HEAD%MATCH( ID, MSG, PAR, METADATA, ENCODERS, OPT, HOOKS )
...
&               FILTER_OPT, ENCODER_OPT, SIZE, LEVEL, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Logging info
  ! WRITE(*,*) REPEAT(' ',LEVEL*2)//'ENCODING_RULES_TREE_LOADER_NODE_INIT :: '//TRIM(ADJUSTL(KEY)), ' :: ', THIS%IKEY
  ...
  DO I = 1, NUMBER_OF_OPERATIONS
    ...
    !> Read the operation
    PP_TRYCALL(ERRFLAG_READ_OPERATION) THIS%READ_OPERATION( I, OPERATION, FILTER_OPT, HOOKS )

    IF ( HAS_NESTED_RULES ) THEN
      PP_TRYCALL(ERRFLAG_READ_NESTED_RULES) THIS%CHILDREN(I)%INIT_CFG( OPERATION, FILTER_OPT, ENCODER_OPT, SIZE, LEVEL+1, HOOKS )
    ELSEIF ( HAS_ENCODING_RULES ) THEN
      PP_TRYCALL(ERRFLAG_READ_RULES) THIS%CHILDREN(I)%INIT_RULES( OPERATION, FILTER_OPT, ENCODER_OPT, SIZE, LEVEL+1, HOOKS )
    ENDIF
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_READ_RULES) THIS%RULES_COLLECTION%INIT( CFG, FILTER_OPT, ENCODER_OPT, HOOKS )
...
RECURSIVE FUNCTION ENCODING_RULES_TREE_LOADER_NODE_READ_OPERATION( THIS, ID, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Create the filter
  PP_TRYCALL(ERRFLAG_CREATE_FILTERS) MAKE_FILTER( THIS%FILTERS(ID)%FILTER, TRIM(ADJUSTL(THIS%KEY)), CFG, OPT, HOOKS )
...
RECURSIVE FUNCTION ENCODING_RULES_TREE_LOADER_NODE_MATCH_SINGLE( THIS, MSG, PAR, METADATA, ENCODER, OPT, HOOKS ) RESULT(RET)
...
RECURSIVE FUNCTION ENCODING_RULES_TREE_LOADER_NODE_MATCH_MULTIPLE( THIS, ID, MSG, PAR, METADATA, ENCODERS, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(ENCODING_RULES_TREE_LOADER_NODE_T),      INTENT(INOUT) :: THIS
  INTEGER(KIND=JPIB_K),                          INTENT(INOUT) :: ID
  TYPE(FORTRAN_MESSAGE_T),                       INTENT(IN)    :: MSG
  TYPE(PARAMETRIZATION_T),                       INTENT(IN)    :: PAR
  CLASS(METADATA_BASE_A), POINTER,               INTENT(IN)    :: METADATA
  TYPE(CACHED_ENCODER_T), DIMENSION(:), POINTER, INTENT(OUT)   :: ENCODERS
  ...
  IF ( CONDITIONS(1) ) THEN
    DO I = 1, SIZE(THIS%FILTERS)
      ...
      IF ( LMATCH ) THEN
        ! Recursive call to the children
        PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_ENCODERS) THIS%CHILDREN(I)%MATCH_MULTIPLE( ID, MSG, PAR, METADATA, ENCODERS, OPT, HOOKS )
      ENDIF
    ENDDO
  ELSEIF ( CONDITIONS(2) ) THEN
