...
#define PP_FILE_NAME 'grib2_section4_statistics_utils_mod.F90'
...
&  MSG, PAR, OPT, TIME_SINCE_START_IN_SECONDS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Local variables
  INTEGER(KIND=JPIB_K) :: INITIAL_STEP
  INTEGER(KIND=JPIB_K) :: LENGTH_OF_TIME_STEP_IN_SECONDS

  !> Error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_INITIAL_STEP=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_LENGTH_OF_TIME_STEP=2_JPIB_K
  ...
  ! Get info from parametrization
  PP_TRYCALL(ERRFLAG_INVALID_INITIAL_STEP) PAR%TIME%GET_INITIAL_STEP( INITIAL_STEP, HOOKS )
  PP_TRYCALL(ERRFLAG_INVALID_LENGTH_OF_TIME_STEP) PAR%TIME%GET_LENGTH_OF_TIME_STEP_IN_SECONDS( LENGTH_OF_TIME_STEP_IN_SECONDS, HOOKS )

  ! Error handling
  PP_DEBUG_CRITICAL_COND_THROW( INITIAL_STEP .LT. 0, ERRFLAG_INVALID_INITIAL_STEP )
  PP_DEBUG_CRITICAL_COND_THROW( LENGTH_OF_TIME_STEP_IN_SECONDS .LT. 0, ERRFLAG_INVALID_LENGTH_OF_TIME_STEP )

  ! Compute the time since the start of the simulation
  TIME_SINCE_START_IN_SECONDS = MSG%STEP*LENGTH_OF_TIME_STEP_IN_SECONDS

  ! Add the contribution of the initial step
  TIME_SINCE_START_IN_SECONDS = TIME_SINCE_START_IN_SECONDS + INITIAL_STEP * HOURS2SECONDS

  ! WRITE(*,*) 'STEP: ', MSG%STEP
  ! WRITE(*,*) 'LENGTH_OF_TIME_STEP_IN_SECONDS: ', LENGTH_OF_TIME_STEP_IN_SECONDS
  ! WRITE(*,*) 'INITIAL_STEP: ', INITIAL_STEP
  ! WRITE(*,*) 'TIME_SINCE_START_IN_SECONDS: ', TIME_SINCE_START_IN_SECONDS
  ! WRITE(*,*) 'TIME 0: ', INITIAL_STEP * HOURS2SECONDS
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_INVALID_INITIAL_STEP)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'invalid initial step' )
    CASE (ERRFLAG_INVALID_LENGTH_OF_TIME_STEP)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'invalid length of time step' )
    CASE DEFAULT
...
&  MSG, PAR, TIME_SINCE_START_IN_SECONDS, OPT, METADATA, HOOKS ) RESULT(RET)
