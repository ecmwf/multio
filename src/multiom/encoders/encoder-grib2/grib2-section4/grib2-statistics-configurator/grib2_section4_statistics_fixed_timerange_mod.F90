...
&        MSG, PAR, TIME_HIST, CURR_TIME, OPT, TIME_SINCE_START, HOOKS )
  ...
  ! PP_DEBUG_CRITICAL_COND_THROW( MSG%TIMEPROC.EQ.UNDEF_PARAM_E, ERRFLAG_EXPECTED_TIMEPROC )
#if defined(ENABLE_CHECKS)
  IF ( MSG%TIMEPROC.NE.UNDEF_PARAM_E ) THEN
    PP_DEBUG_CRITICAL_COND_THROW( MSG%TIMEPROC.NE.THIS%OVERALL_LENGTH_OF_TIMERANGE_IN_SECONDS_(1)/3600, ERRFLAG_WRONG_TIMERANGE )
  ENDIF
#endif
  ...
  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'lengthOfTimeRange',  MSG%TIMEPROC )

  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'startStep', MAX(0_JPIB_K,MSG%STEP-MSG%TIMEPROC ) )
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_STATISTICS_FIXED_TIMERANGE_FREE( THIS, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
