...
TYPE :: FORTRAN_MESSAGE_T
  ...
  CHARACTER(LEN=8)     :: TIMEPROC = REPEAT('*',8)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_INT=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_STRING=9_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_FLOAT=10_JPIB_K
  ...
  IF ( N_MSGINTFLDS .GT. 0 ) THEN
    ...
    DO I = 1, N_MSGINTFLDS
      PP_TRYCALL(ERRFLAG_HAS_INT) THIS%HAS_ENUM( I, HAS_FIELD, HOOKS )
      IF ( HAS_FIELD ) THEN
  ...
  IF ( N_MSGSTRFLDS .GT. 0 ) THEN
    ...
    DO I = 1, N_MSGSTRFLDS
      PP_TRYCALL(ERRFLAG_HAS_STRING) THIS%HAS_ENUM( 100_JPIB_K + I, HAS_FIELD, HOOKS )
      IF ( HAS_FIELD ) THEN
        PP_TRYCALL(ERRFLAG_IMSGSTRINGFLDS2CMSGSTRINGFLDS) IMSGSTRINGFLDS2CMSGSTRINGFLDS( 100_JPIB_K + I, CKEY, HOOKS )
        PP_TRYCALL(ERRFLAG_GET_STRING) THIS%GET_ENUM_STRING( 100_JPIB_K + I, CTMP, HOOKS )
  ...
  IF ( N_MSGFLOATFLDS .GT. 0 ) THEN
    ...
    DO I = 1, N_MSGFLOATFLDS
      PP_TRYCALL(ERRFLAG_IMSGFLOATFLDS2CMSGFLOATFLDS) IMSGFLOATFLDS2CMSGFLOATFLDS( 200_JPIB_K + I, CKEY, HOOKS )
      IF ( HAS_FIELD ) THEN
        PP_TRYCALL(ERRFLAG_GET_FLOAT) THIS%GET_ENUM_FLOAT( 200_JPIB_K + I, RTMP, HOOKS )
...
PP_THREAD_SAFE FUNCTION FORTRAN_MESSAGE_TO_JSON( THIS, JSON, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_INT=10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_STRING=11_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_FLOAT=12_JPIB_K
  ...
  IF ( N_MSGSTRFLDS .GT. 0 ) THEN
    DO I = 1, N_MSGSTRFLDS
      ...
      IF ( HAS_FIELD ) THEN
        ...
        PP_TRYCALL(ERRFLAG_IMSGSTRINGFLDS2CMSGSTRINGFLDS) IMSGSTRINGFLDS2CMSGSTRINGFLDS( 100_JPIB_K + I, CKEY, HOOKS )
        PP_TRYCALL(ERRFLAG_GET_STRING) THIS%GET_ENUM_STRING( 100_JPIB_K + I, CTMP, HOOKS )
        SZ = 1 + LEN_TRIM(ADJUSTL(CKEY)) + 1 + LEN_TRIM(ADJUSTL(CTMP)) + 3
        LO = HI + 1
        HI = LO + SZ - 1
      ENDIF
    ENDDO
  ...
  IF ( N_MSGSTRFLDS .GT. 0 ) THEN
    DO I = 1, N_MSGSTRFLDS
      ...
      IF ( HAS_FIELD ) THEN
        ...
        HI = LO + SZ - 1
        JSON_ITEM = REPEAT(' ', 1024)
        WRITE(JSON_ITEM, '(A1,A,A2,A,A1,A1)') ' ', TRIM(ADJUSTL(CKEY)) ,':"', TRIM(ADJUSTL(CTMP)), '"', SEP
        JSON(LO:HI) = TRIM(JSON_ITEM)
      ENDIF
    ENDDO
  ENDIF
  ...
  IF ( N_MSGFLOATFLDS .GT. 0 ) THEN
    DO I = 1, N_MSGFLOATFLDS
      PP_TRYCALL(ERRFLAG_HAS_STRING) THIS%HAS_ENUM( I, HAS_FIELD, HOOKS )
      IF ( HAS_FIELD ) THEN
        CNT = CNT + 1
        IF ( CNT .GE. N_FIELDS ) THEN
          SEP = ' '
...
IMPLICIT NONE
  ...
  ! Perturbation number in ensemble simulation
