...
IMPLICIT NONE
  ...
  DO I = 1, SIZE(STRING_ARRAY)
    ...
    STRING(LO:HI) = TRIM(SEP)//TRIM(ADJUSTL(STRING_ARRAY(I)))
    SEP=', '
  ENDDO
  LO = HI + 1
  HI = LO + 1
...
IMPLICIT NONE
  ...
  WRITE(STR, '(I32)', IOSTAT=WRITE_STATUS) VAL
...
IMPLICIT NONE
  ...
  ! Convert Integer to string
  WRITE(STR, '(F22.8)', IOSTAT=WRITE_STATUS) VAL
...
IMPLICIT NONE
  ...
  DO I = 1, N
    ...
    ! Convert real64 to string
    PP_TRYCALL(ERRFLAG_CONVERT_REAL64_TO_STRING) REAL64_TO_STRING( VAL(I), STR(I), HOOKS )
