...
SUBROUTINE ENCODING_INFO_ACCESS_OR_CREATE( MODEL_PARAMS, PARAM_ID, PREFIX, REPRES, LEVEL, STEP, GRIB_INFO, TIME_HIST )
...
IMPLICIT NONE
  ...
  TYPE(GRIB_INFO_T), POINTER, INTENT(OUT) :: GRIB_INFO
  ...
  TYPE(KEY_T) :: KEY
  TYPE(ENCODING_INFO_T), POINTER :: EI
  CLASS(*), POINTER :: VALUE
  LOGICAL :: EX
  ...
  ! Search keys
  LEV_TYPE = IPREFIX2ILEVTYPE( PREFIX, PARAM_ID )
  KEY%K = LEVEL
  ...
  IF ( EX ) THEN
    ...
    CLASS IS ( ENCODING_INFO_T )
      ...
      ! Update time history
      CALL CB_ENQUEUE( A%TIME_HISTORY, STEP )

      ! Set output variables
      GRIB_INFO => A%GRIB_INFO
      EX = CB_GET_ALL( A%TIME_HISTORY, TIME_HIST%SIZE_, TIME_HIST%HIST_(1:)  )
...
IMPLICIT NONE
  ...
  TYPE(GRIB_INFO_T), INTENT(OUT) :: GRIB_INFO
  ...
  ! Packing type
  IF ( DEFINITIONS%PACKING_TYPE .EQ. UNDEF_PARAM_E ) THEN
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)')    '#  '
  WRITE(UNIT,'(A,A)')  '#',REPEAT('-',120)
  WRITE(UNIT,'(A1,A16,A1,A16,A1,A16,A1,A16,A1,A16,A1,A16,A1,A16,A1)') '|', &
&  ' PARAM ID.      ', '|', &
&  ' REPRES         ', '|', &
&  ' LEV_TYPE       ', '|', &
&  ' LEVEL          ', '|', &
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A1,I16,A1,A16,A1,A16,A1,I16,A1,A16,A1,I16,A1,A16,A1)') '|', &
&  PARAM_ID, '|', &
&  IREPRES2CREPRES(REPRES),   '|', &
&  ILEVTYPE2CLEVTYPE(LEV_TYPE), '|', &
&  LEVEL,    '|', &
