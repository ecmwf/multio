!>
!> @file filter_param_mod.F90
!>
!> @brief Module containing definitions and procedures for parameter filters.
!>
!> This module defines the `FILTER_PARAM_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of parameter filters within the system. Parameter filters allow for
...
#define PP_FILE_NAME 'filter_param_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_PARAM_MOD'
MODULE FILTER_PARAM_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_PARAM_T
  ...
  !> @details If `.TRUE.`, the `PARAMIDS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: PARAMIDS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_PARAM_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_PARAM_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_PARAM_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_PARAM_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_PARAM_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_PARAM_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_PARAM_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_PARAM_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_PARAM_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_PARAM_FREE
...
PUBLIC :: FILTER_PARAM_T

CONTAINS


!>
!> @brief Initializes the `FILTER_PARAM_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_PARAM_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_PARAM_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_PARAM_INIT'
FUNCTION FILTER_PARAM_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_PARAM_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%PARAMIDS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%PARAMIDS_, HOOKS )
...
END FUNCTION FILTER_PARAM_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_PARAM_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PARAM_MATCH'
FUNCTION FILTER_PARAM_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%PARAMIDS_%MATCH( MSG%PARAM, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%PARAMID_ .EQ. MSG%PARAM)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%PARAMID_ .EQ. MSG%PARAM)
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%PARAM .GT. THIS%PARAMID_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%PARAM .GE. THIS%PARAMID_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%PARAM .LE. THIS%PARAMID_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%PARAM .LT. THIS%PARAMID_ )
...
END FUNCTION FILTER_PARAM_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_PARAM_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PARAM_PRINT'
FUNCTION FILTER_PARAM_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'PARAM FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_PARAM_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_PARAM_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PARAM_FREE'
FUNCTION FILTER_PARAM_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%PARAMIDS_%FREE( HOOKS )
...
END FUNCTION FILTER_PARAM_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_PARAM_MOD
