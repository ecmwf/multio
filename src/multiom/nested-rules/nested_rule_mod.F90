...
TYPE :: NESTED_RULE_T
  ...
  !> Sample to be loaded
  CHARACTER(LEN=32) :: SAMPLE_NAME_ = REPEAT( ' ', 32 )
...
CONTAINS
  ...
  PROCEDURE, PASS, PUBLIC, NON_OVERRIDABLE :: GET_ENCODER_CFG => RULE_GET_ENCODER_CFG
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: NESTED_RULE_TAG
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_DEALLOCATE_NESTED_RULE_TAG=15_JPIB_K
  ...
  !>
  !> Read the name of the rule
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_STRING( CFG, 'tag', NESTED_RULE_TAG, HOOKS )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(NESTED_RULE_TAG), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
  PP_DEBUG_CRITICAL_COND_THROW( LEN_TRIM(NESTED_RULE_TAG).GT.LEN(THIS%TAG_), ERRFLAG_TAG_TOO_LONG )
  THIS%TAG_ = NESTED_RULE_TAG
  DEALLOCATE( NESTED_RULE_TAG, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE_NESTED_RULE_TAG )
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_ENCODER_CFG) YAML_GET_SUBCONFIGURATION( CFG, 'encoder', THIS%ENCODER_CFG_, HOOKS )
  ...
  BLOCK
    ...
    CASE(ERRFLAG_UNABLE_TO_DEALLOCATE_RULE_NAME)
      ...
      IF ( ALLOCATED(ERRMSG) ) THEN
        ...
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE(ERRFLAG_UNABLE_TO_DEALLOCATE_NESTED_RULE_TAG)
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'RULE_GET_ENCODER_CFG'
PP_THREAD_SAFE FUNCTION RULE_GET_ENCODER_CFG( THIS, NAME, TAG, SAMPLE, ENCODER_CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(NESTED_RULE_T), TARGET,        INTENT(INOUT) :: THIS
  CHARACTER(LEN=256),                  INTENT(OUT)   :: NAME
  CHARACTER(LEN=256),                  INTENT(OUT)   :: TAG
  CHARACTER(LEN=32),                   INTENT(OUT)   :: SAMPLE
  TYPE(YAML_CONFIGURATION_T), POINTER, INTENT(OUT)   :: ENCODER_CFG
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  !> Get the encoder
  ENCODER_CFG => THIS%ENCODER_CFG_

  !> Get the tag
  TAG = TRIM(ADJUSTL(THIS%TAG_))

  !> Get the name
  NAME = TRIM(ADJUSTL(THIS%NAME_))

  !> Get the sample
  SAMPLE = TRIM(ADJUSTL(THIS%SAMPLE_NAME_))
...
END FUNCTION RULE_GET_ENCODER_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  !> Error handling
  PP_DEBUG_CRITICAL_COND_THROW( .NOT. ASSOCIATED(THIS%FILTER_), ERRFLAG_FILTER_NOT_ASSOCIATED )
  ! PP_DEBUG_CRITICAL_COND_THROW( .NOT. ASSOCIATED(THIS%ENCODER_CFG_), ERRFLAG_ENCODER_NOT_ASSOCIATED )
  ...
  !> Print the encoder
  ! PP_TRYCALL(ERRFLAG_UNABLE_TO_PRINT_ENCODER) THIS%ENCODER_%PRINT( UNIT, OFFSET+2, HOOKS )
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_DESTROY_ENCODER) YAML_DELETE_CONFIGURATION( THIS%ENCODER_CFG_, HOOKS )
