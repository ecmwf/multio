...
!>
!> @brief Module containing definitions and procedures for direction filters.
!>
!> This module defines the `FILTER_DIRECTION_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of direction filters within the system. Direction filters allow for
...
#define PP_FILE_NAME 'filter_direction_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_DIRECTION_MOD'
MODULE FILTER_DIRECTION_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_DIRECTION_T
  ...
  !> @details If `.TRUE.`, the `DIRECTIONS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  INTEGER(KIND=JPIB_K) :: DIRECTION_
  ...
  TYPE(KEYSET_INT64_T) :: DIRECTIONS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_DIRECTION_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_DIRECTION_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_DIRECTION_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_DIRECTION_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_DIRECTION_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_DIRECTION_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_DIRECTION_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_DIRECTION_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_DIRECTION_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_DIRECTION_FREE
...
PUBLIC :: FILTER_DIRECTION_T

CONTAINS


!>
!> @brief Initializes the `FILTER_DIRECTION_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_DIRECTION_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_DIRECTION_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_DIRECTION_INIT'
FUNCTION FILTER_DIRECTION_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_DIRECTION_T),  INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%DIRECTIONS_%INIT( HOOKS )
  ...
  !> Read the direction to be used in the filter
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%DIRECTIONS_, HOOKS )
...
END FUNCTION FILTER_DIRECTION_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_DIRECTION_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_DIRECTION_MATCH'
FUNCTION FILTER_DIRECTION_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%DIRECTIONS_%MATCH( MSG%DIRECTION, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%DIRECTION_ .EQ. MSG%DIRECTION)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%DIRECTION_ .EQ. MSG%DIRECTION)
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%DIRECTION .GT. THIS%DIRECTION_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%DIRECTION .GE. THIS%DIRECTION_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%DIRECTION .LE. THIS%DIRECTION_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%DIRECTION .LT. THIS%DIRECTION_ )
...
END FUNCTION FILTER_DIRECTION_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_DIRECTION_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_DIRECTION_PRINT'
FUNCTION FILTER_DIRECTION_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'DIRECTION FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_DIRECTION_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_DIRECTION_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_DIRECTION_FREE'
FUNCTION FILTER_DIRECTION_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%DIRECTIONS_%FREE( HOOKS )
...
END FUNCTION FILTER_DIRECTION_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_DIRECTION_MOD
