...
SUBROUTINE DUMP_WRITE_ATM_DP( THIS, YDMSG, VALUES )
...
IMPLICIT NONE
  ...
  ! Write values
  IF ( THIS%WRITE_VALUES_ ) THEN
    CALL VAL_WRITE_DP( THIS%VAL_UNIT_, VALUES, VALADDR )
  ELSE
...
SUBROUTINE DUMP_WRITE_ATM_SP( THIS, YDMSG, VALUES )
...
IMPLICIT NONE
  ...
  ! Write values
  IF ( THIS%WRITE_VALUES_ ) THEN
    CALL VAL_WRITE_SP( THIS%VAL_UNIT_, VALUES, VALADDR )
  ELSE
    ! Negative address means no value saved
    VALADDR = -1_INT64
  ENDIF
...
SUBROUTINE DUMP_WRITE_WAM_DP( THIS, YDMSG, VALUES )
...
SUBROUTINE DUMP_WRITE_WAM_SP( THIS, YDMSG, VALUES )
