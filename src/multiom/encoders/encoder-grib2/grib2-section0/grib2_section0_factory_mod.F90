...
#define PP_SECTION_NAME 'GRIB2_SECTION0_FACTORY_MOD'
...
!>
!> Key to be used to recover the section0 number from the YAML configuration.
CHARACTER(LEN=*), PARAMETER :: SECTION0_KEY='template-number'

!>
!> Generic interface to the make encoder function.
INTERFACE MAKE_GRIB2_SECTION0
  MODULE PROCEDURE MAKE_GRIB2_SECTION0_CFG
  MODULE PROCEDURE MAKE_GRIB2_SECTION0_LAZY
END INTERFACE

!>
!> Public symbols (dataTypes)
PUBLIC :: MAKE_GRIB2_SECTION0
PUBLIC :: DESTROY_GRIB2_SECTION0
...
!> to the `GRIB2_SECTION0` object and configures it using the provided model parameters,
!> ID, and YAML configuration. If verbose mode is enabled, additional debug information
!> is output during the process.
!>
!> @param [inout] GRIB2_SECTION0 The GRIB2 Section 0 object that will be created or initialized.
...
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION0_CFG'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION0_CFG( GRIB2_SECTION0, DUMMY, &
&               CFG, OPT, FACTORIES, HOOKS ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  USE :: DATAKINDS_DEF_MOD,        ONLY: JPIB_K
  USE :: GRIB_SECTION_BASE_MOD,    ONLY: GRIB_SECTION_BASE_A
  USE :: GRIB2_SECTION0_000_MOD,   ONLY: GRIB2_SECTION0_000_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB2_SECTION0
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) READ_GRIB2_SECTION0_TYPE_FROM_CFG( CFG, ID, HOOKS )
  ...
  CASE( 0 )
    ...
    ALLOCATE( GRIB2_SECTION0_000_T::GRIB2_SECTION0, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION0%INIT( CFG, OPT, FACTORIES, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_READ_SECTION_TYPE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read section0 type' )
    ...
    CASE (ERRFLAG_ALLOCATION_ERROR)
      ...
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(TMP))//' : '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
...
END FUNCTION MAKE_GRIB2_SECTION0_CFG
...
!> is output during the process.
!>
!> @param [inout] GRIB2_SECTION0 The GRIB2 Section 0 object that will be created or ini
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION0_LAZY'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION0_LAZY( GRIB2_SECTION0, DUMMY, &
...
#if defined( PP_HAS_GET_GRIB2_SECTION0_ID_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_SECTION0_ID_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_GET_SECTION0_ID=0_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_0=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INITIALIZATION_ERROR=3_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SECTION0_ID) GET_GRIB2_SECTION0_ID_FROM_MESSAGE( MSG, PAR, OPT, ID, HOOKS )
  ...
  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION0%INIT( MSG, PAR, OPT, FACTORIES, HOOKS )
#else
  PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNABLE_TO_GET_SECTION0_ID )
#endif
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! Handle different errors
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_UNABLE_TO_GET_SECTION0_ID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get section0 ID' )
    CASE (ERRFLAG_UNKNOWN_SECTION_0)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown section0 number: '//TRIM(ADJUSTL(TMP)) )
    CASE (ERRFLAG_ALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error allocating section0 number: '//TRIM(ADJUSTL(TMP)) )
...
END FUNCTION MAKE_GRIB2_SECTION0_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Destroys a GRIB2 Section 0 structure.
!>
!> This function takes a GRIB2 Section 0 object (`GRIB2_SECTION0`) and performs
...
!>
!> @section interface
!> @param[in,out] GRIB2_SECTION0 The GRIB2 Section 0 object to be destroyed.
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see MAKE_GRIB2_SECTION0
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_GRIB2_SECTION0'
PP_THREAD_SAFE FUNCTION DESTROY_GRIB2_SECTION0( GRIB2_SECTION0, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(GRIB2_SECTION0), ERRFLAG_NOT_ASSOCIATED )

  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  GRIB2_SECTION0%FREE( OPT, DESTRUCTORS, HOOKS )

  ! Initialize the section
  DEALLOCATE( GRIB2_SECTION0, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
...
END FUNCTION DESTROY_GRIB2_SECTION0
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Read from the YAML configuration the type of the GRIB2 Section 0 object.
!>
!> @section interface
!> @param [in]     CFG             YAML configuration object used to configure the GRIB2 Section 0 object.
!> @param [out]    SECTION0_TYPE   Identifier of the section0 type read from the configuration.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'READ_GRIB2_SECTION0_TYPE_FROM_CFG'
PP_THREAD_SAFE FUNCTION READ_GRIB2_SECTION0_TYPE_FROM_CFG( CFG, SECTION0_TYPE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(YAML_CONFIGURATION_T), INTENT(IN)    :: CFG
  INTEGER(KIND=JPIB_K),       INTENT(OUT)   :: SECTION0_TYPE
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  CHARACTER(LEN=:), ALLOCATABLE :: CSECTION0_TYPE
  LOGICAL :: HAS_SECTION0
  ...
  ! Initialization
  SECTION0_TYPE = -1_JPIB_K

  !> Check if configuration has the SECTION0 key
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, SECTION0_KEY, HAS_SECTION0, HOOKS )

  !> Read the SECTION0 template number
  IF ( HAS_SECTION0 ) THEN
    PP_TRYCALL(ERRFLAG_READ_ERROR) YAML_READ_STRING( CFG, SECTION0_KEY, CSECTION0_TYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSECTION0_TYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL( ERRFLAG_STRING_IS_INTEGER ) STRING_IS_INTEGER( CSECTION0_TYPE, SECTION0_IS_INTEGER, HOOKS )
    IF ( SECTION0_IS_INTEGER ) THEN
       PP_TRYCALL( ERRFLAG_STRING_TO_INTEGER ) STRING_TO_INTEGER( CSECTION0_TYPE, SECTION0_TYPE, HOOKS )
    ELSE
    ...
    ENDIF
    DEALLOCATE(CSECTION0_TYPE, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )
  ELSE
    ! TODO
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ALLOCATED_AFTER_READ)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'SECTION0 ctype not allocated after reading' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating SECTION0 number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
...
END FUNCTION READ_GRIB2_SECTION0_TYPE_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
