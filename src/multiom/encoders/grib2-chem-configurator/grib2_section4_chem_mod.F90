...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT => GRIB2_SECTION4_CHEM_INIT
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_CHEM_INIT'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_CHEM_INIT( THIS, &
...
END FUNCTION GRIB2_SECTION4_CHEM_INIT
...
IMPLICIT NONE
  ...
  !   ENDIF
  ! ENDIF
  IF ( MSG%WAVELENGTH .NE. REPEAT('*', 32) ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'wavelength', MSG%WAVELENGTH)
  ENDIF
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_CHEM_FREE( THIS, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
