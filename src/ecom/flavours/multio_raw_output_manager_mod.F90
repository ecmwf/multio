...
#define PP_FILE_NAME 'multio_raw_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'MULTIO_RAW_OUTPUT_MANAGER_MOD'
MODULE MULTIO_RAW_MOD
...
!>
!> The `MULTIO_RAW_OUTPUT_MANAGER_T` type is a derived type that extends the
...
!>
TYPE, EXTENDS(OUTPUT_MANAGER_BASE_A) :: MULTIO_RAW_OUTPUT_MANAGER_T
...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: SETUP => MULTIO_RAW_SETUP

  !> @brief Read teh configuration from YAML using fckit
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: READ_CFG_FROM_YAML => MULTIO_RAW_READ_CFG_FROM_YAML

  !> @brief Write fields for the output manager
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_ATM_DP => MULTIO_RAW_WRITE_ATM_DP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_ATM_SP => MULTIO_RAW_WRITE_ATM_SP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_WAM_DP => MULTIO_RAW_WRITE_WAM_DP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_WAM_SP => MULTIO_RAW_WRITE_WAM_SP

  !> @brief Notify that a step is finished
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_STEP => MULTIO_RAW_FLUSH_STEP

  !> @brief Notify that a step is finished and it was the last step
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_LAST_STEP => MULTIO_RAW_FLUSH_LAST_STEP

  !> @brief Notify that a step is finished and it is necessary to dump a restart
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_STEP_AND_TRIGGER_RESTART => MULTIO_RAW_FLUSH_STEP_AND_TRIGGER_RESTART

  !> @brief Finalise ancd cleanup teh output manager
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FINALISE => MULTIO_RAW_FINALISE

END TYPE

! Whitelist of public symbols
PUBLIC :: MULTIO_RAW_OUTPUT_MANAGER_T

CONTAINS

#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_READ_CFG_FROM_YAML'
SUBROUTINE MULTIO_RAW_READ_CFG_FROM_YAML( THIS, CFG )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MULTIO_RAW_OUTPUT_MANAGER_T), INTENT(INOUT) :: THIS
  TYPE(FCKIT_CONFIGURATION),          INTENT(IN)    :: CFG

  ! Local variables
  TYPE(FCKIT_CONFIGURATION) :: MULTIO_RAW_CFG
  ...
  IF ( CFG%GET( 'multio-raw-output-manager', MULTIO_RAW_CFG ) ) THEN

    IF ( ALLOCATED(CLTMP) ) DEALLOCATE(CLTMP)
    IF ( MULTIO_RAW_CFG%GET( 'multio-plans-file', CLTMP  ) ) THEN
    ...
    IF ( MULTIO_RAW_CFG%GET( 'verbose', LTMP  ) ) THEN
      THIS%VERBOSE_ = LTMP
    ELSE
      THIS%VERBOSE_ = .FALSE.
    ENDIF
    ...
    ! Read the verbose flag from YAML file
    IF ( MULTIO_RAW_CFG%GET( 'profile', LTMP  ) ) THEN
      THIS%PROFILE_ = LTMP
    ELSE
      THIS%PROFILE_ = .FALSE.
    ENDIF
...
PP_ERROR_HANDLER
  ...
  ErrorHandler: BLOCK
    ...
    CASE (5)
      PP_DEBUG_CREATE_ERROR_MSG( STR, '"multio-raw-output-manager" key expected in YAML' )
    CASE DEFAULT
...
END SUBROUTINE MULTIO_RAW_READ_CFG_FROM_YAML
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_SETUP'
SUBROUTINE MULTIO_RAW_SETUP( THIS, YAMLFNAME, PROCESSOR_TOPO, MODEL_PARAMS )
...
IMPLICIT NONE
  ...
  CALL MAKE_ENCODERS( CFG, MODEL_PARAMS, 'MULTIO' )
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) 'multio_raw_output_manager_', PROCESSOR_TOPO%MYPROC_IO, '.log'
...
END SUBROUTINE MULTIO_RAW_SETUP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_WRITE_ATM_DP'
SUBROUTINE MULTIO_RAW_WRITE_ATM_DP( THIS, YDMSG, VALUES )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MULTIO_RAW_OUTPUT_MANAGER_T), TARGET, INTENT(INOUT) :: THIS
  ...
  CLASS(METADATA_BASE_A), POINTER :: PMMD
  ...
  PMMD => THIS%MMD_
  PP_METADATA_INIT_LOGGING( PMMD, YDMSG%ISTEP_, YDMSG%PARAM_ID_, YDMSG%IUID_, YDMSG%IPREF_, YDMSG%IREPRES_ )
  ...
  IF ( ENCODE_ATM( THIS%MODEL_PAR_, GRIB_INFO, TIME_HIST, YDMSG, PMMD ) ) THEN

    ! Write Edition
    CALL THIS%MMD_%SET( 'gribEdition', GRIB_INFO%REQUESTED_ENCODING_ )

    ! Write the field to multio
    CALL THIS%MMD_%SET( 'globalSize', YDMSG%NVALUES_ )

    CERR = THIS%MIO_%WRITE_FIELD( THIS%MMD_%GET_MULTIO_METADATA(), VALUES(1:YDMSG%NVALUES_) )
  ...
  IF ( THIS%MMD_%INITIALIZED() ) THEN
    ...
    PP_METADATA_FINALISE_LOGGING( PMMD )
  ...
  ErrorHandler: BLOCK
    ...
    CASE (2)
      ...
      IF ( ALLOCATED( MIO_ERR_STR ) ) THEN
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'Unable to write to multio: '//TRIM(ADJUSTL(MIO_ERR_STR)) )
        DEALLOCATE( MIO_ERR_STR )
      ELSE
        PP_DEBUG_CREATE_ERROR_MSG( STR, 'Unable to write to multio' )
      ENDIF
    CASE DEFAULT
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Unhandled error' )
    END SELECT
...
END SUBROUTINE MULTIO_RAW_WRITE_ATM_DP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_WRITE_ATM_SP'
SUBROUTINE MULTIO_RAW_WRITE_ATM_SP( THIS, YDMSG, VALUES )
...
END SUBROUTINE MULTIO_RAW_WRITE_ATM_SP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_WRITE_WAM_DP'
SUBROUTINE MULTIO_RAW_WRITE_WAM_DP( THIS, YDMSG, VALUES )
...
IMPLICIT NONE
  ...
  !
  ! Encode throws an error if an error happens, and return false if the field does not need to be emitted
  IF ( ENCODE_WAM( THIS%MODEL_PAR_, GRIB_INFO, TIME_HIST, YDMSG, PMMD ) ) THEN
...
END SUBROUTINE MULTIO_RAW_WRITE_WAM_DP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_WRITE_WAM_SP'
SUBROUTINE MULTIO_RAW_WRITE_WAM_SP( THIS, YDMSG, VALUES )
...
END SUBROUTINE MULTIO_RAW_WRITE_WAM_SP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_FLUSH_STEP'
SUBROUTINE MULTIO_RAW_FLUSH_STEP( THIS, KSTEP )
...
IMPLICIT NONE
  ...
  ! Exit point on success
  RETURN
END SUBROUTINE MULTIO_RAW_FLUSH_STEP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_FLUSH_LAST_STEP'
SUBROUTINE MULTIO_RAW_FLUSH_LAST_STEP( THIS, KSTEP )
...
END SUBROUTINE MULTIO_RAW_FLUSH_LAST_STEP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_FLUSH_STEP_AND_TRIGGER_RESTART'
SUBROUTINE MULTIO_RAW_FLUSH_STEP_AND_TRIGGER_RESTART( THIS, KSTEP )
...
END SUBROUTINE MULTIO_RAW_FLUSH_STEP_AND_TRIGGER_RESTART
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MULTIO_RAW_FINALISE'
SUBROUTINE MULTIO_RAW_FINALISE( THIS )
...
END SUBROUTINE MULTIO_RAW_FINALISE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE MULTIO_RAW_MOD
