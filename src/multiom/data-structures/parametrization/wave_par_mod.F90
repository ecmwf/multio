...
PRIVATE
  ...
  TYPE :: WAVE_PAR_T
    ...
    INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_DIRECTIONS = UNDEF_PARAM_E
    INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_FREQUENCIES = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTIONS = UNDEF_PARAM_E
  WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCIES = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  WAVE_PAR_TO%SCALE_FACTOR_OF_WAVE_DIRECTIONS = WAVE_PAR_FROM%SCALE_FACTOR_OF_WAVE_DIRECTIONS
  WAVE_PAR_TO%SCALE_FACTOR_OF_WAVE_FREQUENCIES = WAVE_PAR_FROM%SCALE_FACTOR_OF_WAVE_FREQUENCIES
...
IMPLICIT NONE
  ...
  IF ( HAS_KEY  ) THEN
    ...
    ! Read SCALE_FACTOR_OF_WAVE_DIRECTIONS
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( WAVE_CONFIGURATION, 'scale-factor-of-wave-directions', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( WAVE_CONFIGURATION, 'scale-factor-of-wave-directions', WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTIONS, HOOKS )
    ELSE
      WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTIONS = UNDEF_PARAM_E
    ENDIF

    ! Read SCALE_FACTOR_OF_WAVE_FREQUENCIES
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( WAVE_CONFIGURATION, 'scale-factor-of-wave-frequencies', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( WAVE_CONFIGURATION, 'scale-factor-of-wave-frequencies', WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCIES, HOOKS )
    ELSE
      WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCIES = UNDEF_PARAM_E
    ENDIF
...
IMPLICIT NONE
  ...
  IF ( ANY(CONDITIONS) ) THEN
    ...
    IF ( WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTIONS .NE. UNDEF_PARAM_E ) THEN
      ! convert integer to string
      CTMP = REPEAT(' ', MAX_STR_LEN)
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_STRING) TO_STRING(WAVE_PAR%SCALE_FACTOR_OF_WAVE_DIRECTIONS, CTMP, HOOKS )

      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'scale-factor-of-wave-directions: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF

    IF ( WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCIES .NE. UNDEF_PARAM_E ) THEN
      ! convert integer to string
      CTMP = REPEAT(' ', MAX_STR_LEN)
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_STRING) TO_STRING(WAVE_PAR%SCALE_FACTOR_OF_WAVE_FREQUENCIES, CTMP, HOOKS )

      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'scale-factor-of-wave-frequencies: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF
