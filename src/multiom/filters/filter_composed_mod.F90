...
PP_THREAD_SAFE FUNCTION FILTER_COMPOSED_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  NestedFiltersEvaluationLoop: DO I = 1, SIZE(THIS%FILTERS_)
    IF ( PRESENT(PAR) ) THEN
      PP_TRYCALL(ERRFLAG_EVALUTE_NESTED_FILTER) THIS%FILTERS_(I)%FILTER_%MATCH( MSG, THIS%CONDITIONS_(I), HOOKS, PAR=PAR )
    ELSE
      PP_TRYCALL(ERRFLAG_EVALUTE_NESTED_FILTER) THIS%FILTERS_(I)%FILTER_%MATCH( MSG, THIS%CONDITIONS_(I), HOOKS )
    ENDIF
