...
mode_t parse_mode(const char *mode_str) {
    ...
    if (strlen(mode_str) != 9) return -1; // Invalid length

    // User permissions
    if (mode_str[0] == 'r') mode |= S_IRUSR;
    if (mode_str[1] == 'w') mode |= S_IWUSR;
    if (mode_str[2] == 'x') mode |= S_IXUSR;

    // Group permissions
    if (mode_str[3] == 'r') mode |= S_IRGRP;
    if (mode_str[4] == 'w') mode |= S_IWGRP;
    if (mode_str[5] == 'x') mode |= S_IXGRP;

    // Others permissions
    if (mode_str[6] == 'r') mode |= S_IROTH;
    if (mode_str[7] == 'w') mode |= S_IWOTH;
    if (mode_str[8] == 'x') mode |= S_IXOTH;
