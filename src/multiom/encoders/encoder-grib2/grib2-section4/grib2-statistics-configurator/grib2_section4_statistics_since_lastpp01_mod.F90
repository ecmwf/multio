...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRLFAG_GET_NUMBER_OF_TIMERANGES=2_JPIB_K
  ...
  ! Get the number of timeranges from the input mars message
  PP_TRYCALL(ERRLFLAG_GET_NUMBER_OF_TIMERANGES) GET_NUMBER_OF_TIMELOOPS_FROM_MARS( MSG, THIS%NUMBER_OF_TIME_RANGES_, HOOKS )

  ! Allocate the time ranges
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'numberOfTimeRanges', THIS%NUMBER_OF_TIME_RANGES_ )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE ( ERRLFAG_GET_NUMBER_OF_TIMERANGES )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error getting number of time ranges' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
IMPLICIT NONE
  ...
  !> Error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_METADATA=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GET_NUMBER_OF_TIMERANGES=2_JPIB_K
  ...
  ! Get the number of timeranges from the input mars message
  PP_TRYCALL(ERRLFLAG_GET_NUMBER_OF_TIMERANGES) GET_NUMBER_OF_TIMELOOPS_FROM_MARS( MSG, NUMBER_OF_TIME_RANGES, HOOKS )
...
&  MSG, PAR, TIME_HIST, CURR_TIME, OPT, METADATA, HOOKS ) RESULT(RET)
  ...
  USE :: ENUMERATORS_MOD,                   ONLY: UNDEF_PARAM_E
  USE :: GRIB2_SECTION4_STATTYPE_UTILS_MOD, ONLY: GET_NUMBER_OF_TIMELOOPS_FROM_MARS
  USE :: GRIB2_SECTION4_STATTYPE_UTILS_MOD, ONLY: PARSE_STATYPE
...
IMPLICIT NONE
  ...
  !> Local variables (Assumuing never more than 8 time ranges)
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: TYPE_OF_STATISTICAL_PROCESSING
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: TYPE_OF_TIME_INCREMENT
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: INDICATOR_OF_UNIT_FOR_TIME_RANGE
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: LENGTH_OF_TIME_RANGE
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: INDICATOR_OF_UNIT_FOR_TIME_INCREMENT
  INTEGER(KIND=JPIB_K), DIMENSION(8) :: LENGTH_OF_TIME_INCREMENT
  ...
  TYPE_OF_STATISTICAL_PROCESSING       = -1_JPIB_K
  TYPE_OF_TIME_INCREMENT               = -1_JPIB_K
  INDICATOR_OF_UNIT_FOR_TIME_RANGE     = -1_JPIB_K
  LENGTH_OF_TIME_RANGE                 = -1_JPIB_K
  INDICATOR_OF_UNIT_FOR_TIME_INCREMENT = -1_JPIB_K
  LENGTH_OF_TIME_INCREMENT             = -1_JPIB_K


  IF ( NUMBER_OF_TIME_RANGES .EQ. 1 ) THEN
  ...
  ELSEIF ( NUMBER_OF_TIME_RANGES .GT. 1 ) THEN

    ! PArse the stattype
    PP_TRYCALL(ERRLFLAG_GET_NUMBER_OF_TIMERANGES) PARSE_STATTYPE( MSG, PAR, TYPE_OF_STATISTICAL_PROCESSING, LENGTH_OF_TIMERANGES_IN_SECONDS, HOOKS )

    ! Set the metadata
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'typeOfStatisticalProcessing', TYPE_OF_STATISTICAL_PROCESSING(1:NUMBER_OF_TIME_RANGES) )
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'timeIncrement', LENGTH_OF_TIMERANGES_IN_SECONDS(1:NUMBER_OF_TIME_RANGES) )
  ELSE

    PP_DEBUG_CRITICAL_THROW( ERRFLAG_INVALID_CONFIGURATION )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UDEFINED_TYPE_OF_STATISTICAL_PROCESSING)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'undefined type of statistical processing' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
