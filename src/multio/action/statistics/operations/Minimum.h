...
public:
    ...
    Minimum(const std::string& name, std::size_t size, const OperationWindow& win, const StatisticsConfiguration& cfg) :
        OperationWithData<T>{name, "minimum", size, true, win, cfg, std::numeric_limits<T>::max()} {}
