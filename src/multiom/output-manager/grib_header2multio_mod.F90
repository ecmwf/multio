...
TYPE, EXTENDS(OUTPUT_MANAGER_BASE_A) :: GRIB_HEADER2MULTIO_OUTPUT_MANAGER_T
  ...
  INTEGER(KIND=JPIB_K) :: DEFAULT_SINK_ = SINK_MESSAGE_TO_FILE_E
...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_SINK_FROM_YAML => GRIB_HEADER2MULTIO_READ_SINK_FROM_YAML
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: SINK_DP => GRIB_HEADER2MULTIO_SINK_DP
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: SINK_SP => GRIB_HEADER2MULTIO_SINK_SP
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_SINK=12_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_DESTROY_CFG=13_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SINK) THIS%READ_SINK_FROM_YAML( GRIB_HEADER2MULTIO_CFG, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_READ_SINK)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read the sink from the configuration file' )
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_READ_SINK_FROM_YAML'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FROM_YAML( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local variables
  CHARACTER(LEN=:), ALLOCATABLE :: CSINKTYPE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_HAS_KEY=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GET_SUBCFG=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CONVERT_STRING_TO_ENUM=4_JPIB_K
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'default-sink', HAS_KEY, HOOKS )

  ! Read or apply the default configuration
  IF ( HAS_KEY ) THEN

    IF ( ALLOCATED(CSINKTYPE) ) THEN
      DEALLOCATE(CSINKTYPE, STAT=STAT, ERRMSG=ERRMSG)
      PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE )
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'default-sink', CSINKTYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSINKTYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL(ERRFLAG_CONVERT_STRING_TO_ENUM) CSINK2ISINK( CSINKTYPE, THIS%DEFAULT_SINK_, HOOKS )
    IF ( ALLOCATED(CSINKTYPE) ) THEN
      DEALLOCATE( CSINKTYPE, STAT=STAT, ERRMSG=ERRMSG )
      PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE )
    ENDIF
  ...
  ELSE
    THIS%DEFAULT_SINK_ = SINK_MESSAGE_TO_FILE_E
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_GET_SUBCFG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get the "verbose" sub-configuration for the output manager' )
    CASE (ERRFLAG_CONVERT_STRING_TO_ENUM)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to convert the string to the enum' )
...
END FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FROM_YAML
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_GET_MARS_FROM_ATM( THIS, YDMSG, MSG, PAR, IS_VALID, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_GET_MARS_FROM_WAM( THIS, YDMSG, MSG, PAR, IS_VALID, HOOKS ) RESULT(RET)
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_SINK_DP'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_SINK_DP( THIS, MSG, PAR, &
&  NUNDEF, XUNDEF, MD, VALUES_DP, HOOKS ) RESULT(RET)
  ...
  USE :: HOOKS_MOD,           ONLY: HOOKS_T
  USE :: GRIB_METADATA_MOD,   ONLY: GRIB_METADATA_T
  USE :: MULTIO_UTILS_MOD,    ONLY: MULTIO_WRITE_BINARY_GRIB

  USE :: ENUMERATORS_MOD, ONLY: SINK_NONE_E
  USE :: ENUMERATORS_MOD, ONLY: SINK_HEADER_TO_FILE_E
  USE :: ENUMERATORS_MOD, ONLY: SINK_MESSAGE_TO_FILE_E
  USE :: ENUMERATORS_MOD, ONLY: SINK_MESSAGE_TO_FDB_E
  USE :: ENUMERATORS_MOD, ONLY: SINK_GRIB_HEADER_TO_MULTIO_E
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_WRITE_BINARY=9_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SINK=10_JPIB_K
  ...
  CASE (SINK_HEADER_TO_FILE_E)
    ...
    ! Open the grib file
    CALL GRIB_OPEN_FILE( FID, 'allFieldHeaders.grib', 'a', STATUS=KRET )
    PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, 2 )

    ! Paranoid verification
    SELECT TYPE(GRIB_METADATA => MD)
  ...
  CASE (SINK_MESSAGE_TO_FDB_E)
    ...
    CLASS IS (GRIB_METADATA_T)
      ...
      PP_TRYCALL(ERRFLAG_UNABLE_WRITE_BINARY)  MULTIO_WRITE_BINARY_GRIB( THIS%MIO_, THIS%MESSAGE_DATA_, HANDLE, HOOKS )
...
!    CALL MULTIO_WRITE_VALUES_DP( THIS%MIO_, MMD, VALUES_SP )
!
!    ! Destroy the multio metadata
!    CALL THIS%MMD_%DESTROY()
  CASE DEFAULT
    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOWN_SINK )
  END SELECT
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_WRITE_BINARY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to write binary to multio' )
    CASE (ERRFLAG_UNKNOWN_SINK)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown sink' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
...
END FUNCTION GRIB_HEADER2MULTIO_SINK_DP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_SINK_SP'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_SINK_SP( THIS, MSG, PAR, &
...
IMPLICIT NONE
  ...
  CASE (SINK_MESSAGE_TO_FILE_E)
    ...
    ! Set values into the grib handle
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET) MD%SET( 'values', VALUES_SP, HOOKS )
...
END FUNCTION GRIB_HEADER2MULTIO_SINK_SP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
