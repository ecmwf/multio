...
IMPLICIT NONE
  ...
  IF ( CFG%GET( TOLOWER(MULTIO_RAW_OMNAME), MULTIO_RAW_CFG ) ) THEN
    ...
    IF ( MULTIO_RAW_CFG%GET( 'report', LTMP  ) ) THEN
...
IMPLICIT NONE
  ...
  CALL MAKE_ENCODERS( CFG, MODEL_PARAMS, 'MULTIO', THIS%VERBOSE_, THIS%MIO_ )
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) TOLOWER(MULTIO_RAW_OMNAME)//'-output-manager-', PROCESSOR_TOPO%MYPROC_IO, '.log'
...
SUBROUTINE MULTIO_RAW_WRITE_ATM_DP( THIS, YDMSG, VALUES_DP )
...
&                                      YDMSG%IREPRES_,  YDMSG%IUID_, ENCODING_INFO )
  ...
  IF ( ENCODE_ATM( THIS%MODEL_PAR_, ENCODING_INFO, YDMSG, TIME_HIST, PMMD ) ) THEN
    ...
    ! Write Edition
    CALL THIS%MMD_%SET( 'gribEdition', ENCODING_INFO%GRIB_INFO%REQUESTED_ENCODING )
...
SUBROUTINE MULTIO_RAW_WRITE_ATM_SP( THIS, YDMSG, VALUES_SP )
...
SUBROUTINE MULTIO_RAW_WRITE_WAM_DP( THIS, YDMSG, VALUES_DP )
...
&                                      YDMSG%IREPRES_,  YDMSG%IUID_, ENCODING_INFO )
  ...
  !
  ! Encode throws an error if an error happens, and return false if the field does not need to be emitted
  IF ( ENCODE_WAM( THIS%MODEL_PAR_, ENCODING_INFO, YDMSG, TIME_HIST, PMMD ) ) THEN
...
SUBROUTINE MULTIO_RAW_WRITE_WAM_SP( THIS, YDMSG, VALUES_SP )
