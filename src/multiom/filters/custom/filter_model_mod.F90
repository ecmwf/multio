...
!>
!> @brief Module containing definitions and procedures for model filters.
!>
!> This module defines the `FILTER_MODEL_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of model filters within the system. Model filters allow for
...
#define PP_FILE_NAME 'filter_model_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_MODEL_MOD'
MODULE FILTER_MODEL_MOD
...
!>
!> @brief Filter type for managing level types (MODEL).
!>
!> This derived type `FILTER_MODEL_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_MODEL_T
  ...
  INTEGER(KIND=JPIB_K) :: MODEL_

  !> Holds the keyset of level types for filtering.
  TYPE(KEYSET_INT64_T) :: MODELS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_MODEL_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_MODEL_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_MODEL_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_MODEL_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_MODEL_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_MODEL_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_MODEL_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_MODEL_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CMODEL2IMODEL
...
!>
!> @see FILTER_MODEL_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_MODEL_INIT'
FUNCTION FILTER_MODEL_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_MODEL_T),      INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CMODEL2IMODEL
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%MODELS_%INIT( HOOKS )
  THIS%USE_KEYSET_ = .FALSE.

  !> Associate the filter procedure
  P_CMODEL2IMODEL => CMODEL2IMODEL
  ...
  IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%MODELS_, P_CMODEL2IMODEL, HOOKS )
    THIS%USE_KEYSET_ = .TRUE.

  ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%MODEL_, P_CMODEL2IMODEL, HOOKS )
...
END FUNCTION FILTER_MODEL_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_MODEL_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_MODEL_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_MODEL_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_MODEL_MATCH'
FUNCTION FILTER_MODEL_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%MODELS_%MATCH( MSG%MODEL, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%MODEL_ .EQ. MSG%MODEL)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%MODEL_ .EQ. MSG%MODEL)
    ENDIF
...
END FUNCTION FILTER_MODEL_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_MODEL_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_MODEL_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_MODEL_PRINT'
FUNCTION FILTER_MODEL_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'MODEL FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_MODEL_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_MODEL_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_MODEL_FREE'
FUNCTION FILTER_MODEL_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%MODELS_%FREE( HOOKS )
...
END FUNCTION FILTER_MODEL_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_MODEL_MOD
