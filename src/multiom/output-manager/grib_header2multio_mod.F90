...
IMPLICIT NONE
  ...
  ! Local variables
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CSINK2ISINK
  LOGICAL :: HAS_KEY
  ...
  ! Read or apply the default configuration
  IF ( HAS_KEY ) THEN
    P_CSINK2ISINK => CSINK2ISINK
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_INTEGER_WITH_FILTER( CFG, 'default-sink', THIS%DEFAULT_SINK_, P_CSINK2ISINK, HOOKS )
  ELSE
