...
Message MpiTransport::receive() {
    ...
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.totReturnTiming_};

    do {
        while (not msgPack_.empty()) {
            eckit::AutoTiming retTiming{statistics_.localTimer_, statistics_.returnTiming_};
        ...
        if (auto strm = streamQueue_.front()) {
            while (strm->position() < strm->size()) {
                eckit::AutoTiming decodeTiming{statistics_.localTimer_, statistics_.decodeTiming_};
...
void MpiTransport::send(const Message& msg) {
    ...
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.sendTiming_};
...
void MpiTransport::listen() {
    ...
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.pushToQueueTiming_};
...
eckit::mpi::Status MpiTransport::probe() {
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.probeTiming_};
...
size_t MpiTransport::blockingReceive(eckit::mpi::Status& status, MpiBuffer& buffer) {
    ...
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.receiveTiming_};
...
void MpiTransport::encodeMessage(eckit::Stream& strm, const Message& msg) {
    eckit::AutoTiming timing{statistics_.localTimer_, statistics_.encodeTiming_};
