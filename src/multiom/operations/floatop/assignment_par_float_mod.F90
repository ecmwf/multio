!>
!> @file assignment_par_float_mod.F90
!>
!> @brief Module containing definitions and procedures for variableeter floatops.
!>
!> This module defines the `ASSIGNMENT_PAR_FLOAT_T` type, along with its associated
...
#define PP_FILE_NAME 'assignment_par_float_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'ASSIGNMENT_PAR_FLOAT_MOD'
MODULE ASSIGNMENT_PAR_FLOAT_MOD
...
!>
TYPE, EXTENDS(ASSIGNMENT_BASE_A) :: ASSIGNMENT_PAR_FLOAT_T
...
CONTAINS
  ...
  !> @details This procedure sets up the `ASSIGNMENT_PAR_FLOAT_T` type, initializing its components.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_PAR_FLOAT_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => ASSIGNMENT_PAR_FLOAT_INIT
  ...
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_PAR_FLOAT_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => ASSIGNMENT_PAR_FLOAT_EVAL

  !> @brief PRINTs the floatop's details for debugging or logging.
  !> @details Outputs the floatop's configuration and current state.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_PAR_FLOAT_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => ASSIGNMENT_PAR_FLOAT_PRINT

  !> @brief Frees resources allocated for the floatop.
  !> @details Cleans up the `ASSIGNMENT_PAR_FLOAT_T` type, deallocating any resources used by the floatop.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_PAR_FLOAT_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => ASSIGNMENT_PAR_FLOAT_FREE
...
PUBLIC :: ASSIGNMENT_PAR_FLOAT_T

CONTAINS


!>
!> @brief Initializes the `ASSIGNMENT_PAR_FLOAT_T` floatop from a YAML configuration.
!>
!> This function initializes the `ASSIGNMENT_PAR_FLOAT_T` type based on the provided YAML
...
!>
!> @variable [inout] THIS The floatop object (`ASSIGNMENT_PAR_FLOAT_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'ASSIGNMENT_PAR_FLOAT_INIT'
FUNCTION ASSIGNMENT_PAR_FLOAT_INIT( THIS, CFG, HOOKS ) RESULT(RET)
  ...
  USE :: PARAMETRIZATION_ENUMERATORS_MOD, ONLY: CPARFLOATFLDS2IPARFLOATFLDS
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(ASSIGNMENT_PAR_FLOAT_T),            INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CPARFLOATFLDS2IPARFLOATFLDS
  LOGICAL :: HAS_NAME
  LOGICAL :: HAS_TYPE
  ...
  P_CPARFLOATFLDS2IPARFLOATFLDS => CPARFLOATFLDS2IPARFLOATFLDS
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SOURCE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'name', THIS%VARIABLE_ID_, P_CPARFLOATFLDS2IPARFLOATFLDS, HOOKS )
...
END FUNCTION ASSIGNMENT_PAR_FLOAT_INIT
...
!>
!> @variable [inout] THIS  The floatop object (`ASSIGNMENT_PAR_FLOAT_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_PAR_FLOAT_EVAL'
FUNCTION ASSIGNMENT_PAR_FLOAT_EVAL( THIS, IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_PAR_FLOAT_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS    The floatop object (`ASSIGNMENT_PAR_FLOAT_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_PAR_FLOAT_PRINT'
FUNCTION ASSIGNMENT_PAR_FLOAT_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_ENUM_TO_STRING) IPARFLOATFLDS2CPARFLOATFLDS( THIS%VARIABLE_ID_, VARIABLE_NAME, HOOKS )
...
END FUNCTION ASSIGNMENT_PAR_FLOAT_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS   The floatop object (`ASSIGNMENT_PAR_FLOAT_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_PAR_FLOAT_FREE'
FUNCTION ASSIGNMENT_PAR_FLOAT_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_PAR_FLOAT_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE ASSIGNMENT_PAR_FLOAT_MOD
