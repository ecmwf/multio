...
CHARACTER(LEN=*), PARAMETER :: TYPE_OF_LEVEL = 'cloudBase'
INTEGER(KIND=JPIB_K), PARAMETER :: TYPE_OF_FIRST_FIXED_SURFACE = 2_JPIB_K
...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT => G2S4_CLOUDBASE_INIT
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: CHECK => G2S4_CLOUDBASE_CHECK
END TYPE
...
#define PP_PROCEDURE_NAME 'G2S4_CLOUDBASE_INIT'
PP_THREAD_SAFE FUNCTION G2S4_CLOUDBASE_INIT( THIS, &
...
END FUNCTION G2S4_CLOUDBASE_INIT
...
IMPLICIT NONE
  ...
  PP_LOG_DEVELOP_STR( 'Allocate cloudBase' )
...
IMPLICIT NONE
  ...
  PP_LOG_DEVELOP_STR( 'Preset cloudBase' )
...
IMPLICIT NONE
  ...
  PP_LOG_DEVELOP_STR( 'Runtime cloudBase' )
...
PP_THREAD_SAFE FUNCTION G2S4_CLOUDBASE_FREE( THIS, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_CLOUDBASE_CHECK'
PP_THREAD_SAFE FUNCTION G2S4_CLOUDBASE_CHECK( THIS, &
&  MSG, PAR, OPT, METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! According to the options decide where to set the levels (preset or runlevel)
  PP_LOG_INFO( 'check typeOfLevel: cloudBase' )
...
END FUNCTION G2S4_CLOUDBASE_CHECK
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
