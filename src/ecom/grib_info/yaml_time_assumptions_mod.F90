...
TYPE :: TIME_ASSUMPTIONS_T
  ...
  INTEGER(KIND=JPIB_K) :: TYPE_OF_TIME_RANGE=-99
  INTEGER(KIND=JPIB_K) :: LENGTH_OF_TIME_RANGE_IN_SECONDS=-99
...
TYPE :: RULE_T
  ...
  TYPE(TIME_ASSUMPTIONS_T) :: ASSUMPTIONS
END TYPE
...
PUBLIC :: MATCH_TIME_ASSUMPTION_RULES
...
IMPLICIT NONE
  ...
  IF ( CFG%GET( 'time-assumption', TIME_ASSUMPTION_CFG ) ) THEN
    ...
    CALL READ_TIME_ASSUMPTION( TIME_ASSUMPTION_CFG, RULE%ASSUMPTIONS, VERBOSE )
...
IMPLICIT NONE
  ...
  IF ( CFG%GET( 'emit-step-zero', LTMP  ) ) THEN
    ...
    ELSEIF (TIME_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .EQ. TYPE_OF_TIME_RANGE_INSTANT_E ) THEN
      IF ( LTMP) THEN
        TIME_ASSUMPTION%EMIT_STEP_ZERO = LTMP
      ELSE
        PP_DEBUG_CRITICAL_THROW( 1 )
    ...
    ENDIF
  ELSE
    IF (TIME_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .EQ. TYPE_OF_TIME_RANGE_INSTANT_E ) THEN
...
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MATCH_TIME_ASSUMPTION_RULES'
SUBROUTINE MATCH_TIME_ASSUMPTION_RULES( PARAM_ID, LEV_TYPE, REPRES, LEVEL, IS_ENSAMBLE, ASSUMPTIONS )
...
IMPLICIT NONE
  ...
  TYPE(TIME_ASSUMPTIONS_T), INTENT(OUT) :: ASSUMPTIONS
...
&                   SPECIAL_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQ. IS_ENSAMBLE ] )
  ...
  ! Loggin rules match
  IF ( CNT  .EQ. 1 ) THEN
    WRITE(*,*) ' + TIME_ASSUMPTIONS_RULES_LOG: applied rule: '//TRIM(ADJUSTL(RMATCH))//') to field: '//TRIM(ADJUSTL(FLDSTR))
  ELSE
    WRITE(*,*) ' + TIME_ASSUMPTIONS_RULES_LOG: applied default rule to field: '//TRIM(ADJUSTL(FLDSTR))
  ENDIF
...
END SUBROUTINE MATCH_TIME_ASSUMPTION_RULES
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
