...
IMPLICIT NONE
  ...
  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME='geo-'//TRIM(ADJUSTL(SH_NAME))//'-pentagonalResolutionJ'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME='geo-'//TRIM(ADJUSTL(SH_NAME))//'-pentagonalResolutionK'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME='geo-'//TRIM(ADJUSTL(SH_NAME))//'-pentagonalResolutionM'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
  ...
  ERR = MIOH%WRITE_PARAMETRIZATION( 'misc-wave-freq', MODEL_PARAMS%WAM_%FR )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOH%WRITE_PARAMETRIZATION( 'misc-wave-dir', MODEL_PARAMS%WAM_%TH )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOH%WRITE_PARAMETRIZATION( 'misc-lengthOfTimeStepInSeconds', INT(MODEL_PARAMS%SIM_%TSTEP, KIND=JPIB_K) )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
