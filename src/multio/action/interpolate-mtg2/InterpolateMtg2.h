...
struct MIROptions {
    ...
    std::optional<bool> caching;         /// will used generated key from cache-path
...
struct OutputConfig {
    ...
    bool enable;       /// Separate option to enable or disable the output

    std::optional<std::string> cachePath
        = {};  /// Used to build a cache key for the special case if interpolation=matrix is passed
    MIROptions options = {};
...
///  The interpolate-mtg2 option allows multiplexing conversions to a list of specificed output grids
struct Options {
