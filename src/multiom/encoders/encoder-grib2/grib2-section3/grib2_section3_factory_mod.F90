...
#define PP_SECTION_NAME 'GRIB2_SECTION3_FACTORY_MOD'
...
!>
!> Key to be used to recover the section3 number from the YAML configuration.
CHARACTER(LEN=*), PARAMETER :: SECTION3_KEY='template-number'


!>
!> Generic interface to the make encoder function.
INTERFACE MAKE_GRIB2_SECTION3
  MODULE PROCEDURE MAKE_GRIB2_SECTION3_CFG
  MODULE PROCEDURE MAKE_GRIB2_SECTION3_LAZY
END INTERFACE

!>
!> Public symbols (dataTypes)
PUBLIC :: MAKE_GRIB2_SECTION3
PUBLIC :: DESTROY_GRIB2_SECTION3
CONTAINS
...
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION3_CFG'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION3_CFG( GRIB2_SECTION3, &
&               CFG, OPT, HOOKS ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  USE :: DATAKINDS_DEF_MOD,        ONLY: JPIB_K
  USE :: GRIB_SECTION_BASE_MOD,    ONLY: GRIB_SECTION_BASE_A
  USE :: GRIB2_SECTION3_000_MOD,   ONLY: GRIB2_SECTION3_000_T
  USE :: GRIB2_SECTION3_040_MOD,   ONLY: GRIB2_SECTION3_040_T
  USE :: GRIB2_SECTION3_050_MOD,   ONLY: GRIB2_SECTION3_050_T
  USE :: GRIB2_SECTION3_101_MOD,   ONLY: GRIB2_SECTION3_101_T
  USE :: GRIB2_SECTION3_150_MOD,   ONLY: GRIB2_SECTION3_150_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB2_SECTION3
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) READ_GRIB2_SECTION3_TYPE_FROM_CFG( CFG, ID, HOOKS )
  ...
  CASE( 0 )
    ...
    ALLOCATE( GRIB2_SECTION3_000_T::GRIB2_SECTION3, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 40 )

    ALLOCATE( GRIB2_SECTION3_040_T::GRIB2_SECTION3, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 50 )

    ALLOCATE( GRIB2_SECTION3_050_T::GRIB2_SECTION3, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 101 )

    ALLOCATE( GRIB2_SECTION3_101_T::GRIB2_SECTION3, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 150 )

    ALLOCATE( GRIB2_SECTION3_150_T::GRIB2_SECTION3, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION3%INIT( CFG, OPT, HOOKS )
...
END FUNCTION MAKE_GRIB2_SECTION3_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION3_LAZY'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION3_LAZY( GRIB2_SECTION3, &
...
#if defined( PP_HAS_GET_GRIB2_SECTION3_ID_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_SECTION3_ID_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_GET_SECTION3_ID=0_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_3=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INITIALIZATION_ERROR=3_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SECTION3_ID) GET_GRIB2_SECTION3_ID_FROM_MESSAGE( MSG, PAR, OPT, ID, HOOKS )
  ...
  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION3%INIT( MSG, PAR, OPT, HOOKS )
#endif
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! Handle different errors
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_UNABLE_TO_GET_SECTION3_ID)
...
END FUNCTION MAKE_GRIB2_SECTION3_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Destroys a GRIB2 Section 3 structure.
!>
!> This function takes a GRIB2 Section 3 object (`GRIB2_SECTION3`) and performs
...
!>
!> @section interface
!> @param[in,out] GRIB2_SECTION3 The GRIB2 Section 3 object to be destroyed.
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see MAKE_GRIB2_SECTION3
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_GRIB2_SECTION3'
PP_THREAD_SAFE FUNCTION DESTROY_GRIB2_SECTION3( GRIB2_SECTION3, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(GRIB2_SECTION3), ERRFLAG_NOT_ASSOCIATED )

  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  GRIB2_SECTION3%FREE( OPT, HOOKS )

  ! Initialize the section
  DEALLOCATE( GRIB2_SECTION3, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'section3 to destroy is not associated' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating section3 number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_FREE_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error destructing section3' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION DESTROY_GRIB2_SECTION3
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Read from the YAML configuration the type of the GRIB2 Section 3 object.
!>
!> @section interface
!> @param [in]     CFG             YAML configuration object used to configure the GRIB2 Section 3 object.
!> @param [out]    SECTION3_TYPE   Identifier of the section3 type read from the configuration.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'READ_GRIB2_SECTION3_TYPE_FROM_CFG'
PP_THREAD_SAFE FUNCTION READ_GRIB2_SECTION3_TYPE_FROM_CFG( CFG, SECTION3_TYPE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(YAML_CONFIGURATION_T), INTENT(IN)    :: CFG
  INTEGER(KIND=JPIB_K),       INTENT(OUT)   :: SECTION3_TYPE
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  CHARACTER(LEN=:), ALLOCATABLE :: CSECTION3_TYPE
  LOGICAL :: HAS_SECTION3
  LOGICAL :: SECTION3_IS_INTEGER
  ...
  ! Initialization
  SECTION3_TYPE = -1_JPIB_K

  !> Check if configuration has the SECTION3 key
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, SECTION3_KEY, HAS_SECTION3, HOOKS )

  !> Read the SECTION3 template number
  IF ( HAS_SECTION3 ) THEN
    PP_TRYCALL(ERRFLAG_READ_ERROR) YAML_READ_STRING( CFG, SECTION3_KEY, CSECTION3_TYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSECTION3_TYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL( ERRFLAG_STRING_IS_INTEGER ) STRING_IS_INTEGER( CSECTION3_TYPE, SECTION3_IS_INTEGER, HOOKS )
    IF ( SECTION3_IS_INTEGER ) THEN
       PP_TRYCALL( ERRFLAG_STRING_TO_INTEGER ) STRING_TO_INTEGER( CSECTION3_TYPE, SECTION3_TYPE, HOOKS )
    ELSE
      ! TODO: Somehow decide automatically whcih SECTION3 to use. String can be
      !       a label for the method to use.
      PP_DEBUG_CRITICAL_THROW( ERRFLAG_KEY_IS_NOT_INTEGER )
    ENDIF
    DEALLOCATE(CSECTION3_TYPE, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )
  ELSE
    ! TODO
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ALLOCATED_AFTER_READ)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'SECTION3 ctype not allocated after reading' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating SECTION3 number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
...
END FUNCTION READ_GRIB2_SECTION3_TYPE_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
