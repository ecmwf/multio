...
#define PP_SECTION_NAME 'GRIB2_SECTION2_FACTORY_MOD'
MODULE GRIB2_SECTION2_FACTORY_MOD
...
!>
!> Key to be used to recover the section2 number from the YAML configuration.
CHARACTER(LEN=*), PARAMETER :: SECTION2_KEY='template-number'



!>
!> Generic interface to the make encoder function.
INTERFACE MAKE_GRIB2_SECTION2
  MODULE PROCEDURE MAKE_GRIB2_SECTION2_CFG
  MODULE PROCEDURE MAKE_GRIB2_SECTION2_LAZY
END INTERFACE

!>
!> Public symbols (dataTypes)
PUBLIC :: MAKE_GRIB2_SECTION2
PUBLIC :: DESTROY_GRIB2_SECTION2
...
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION2_CFG'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION2_CFG( GRIB2_SECTION2, &
&               CFG, OPT, HOOKS ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  USE :: DATAKINDS_DEF_MOD,              ONLY: JPIB_K
  USE :: GRIB_SECTION_BASE_MOD,          ONLY: GRIB_SECTION_BASE_A
  USE :: GRIB2_SECTION2_DESTINE_001_MOD, ONLY: GRIB2_SECTION2_DESTINE_001_T
  USE :: GRIB2_SECTION2_DESTINE_036_MOD, ONLY: GRIB2_SECTION2_DESTINE_036_T
  USE :: GRIB2_SECTION2_001_MOD,         ONLY: GRIB2_SECTION2_001_T
  USE :: GRIB2_SECTION2_014_MOD,         ONLY: GRIB2_SECTION2_014_T
  USE :: GRIB2_SECTION2_015_MOD,         ONLY: GRIB2_SECTION2_015_T
  USE :: GRIB2_SECTION2_024_MOD,         ONLY: GRIB2_SECTION2_024_T
  USE :: GRIB2_SECTION2_036_MOD,         ONLY: GRIB2_SECTION2_036_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB2_SECTION2
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_2=1_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) READ_GRIB2_SECTION2_TYPE_FROM_CFG( CFG, ID, HOOKS )
  ...
  CASE (1)
    ...
    ALLOCATE( GRIB2_SECTION2_001_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (14)

    ALLOCATE( GRIB2_SECTION2_014_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (15)

    ALLOCATE( GRIB2_SECTION2_015_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (24)

    ALLOCATE( GRIB2_SECTION2_024_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (36)

    ALLOCATE( GRIB2_SECTION2_036_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (1001)

    ALLOCATE( GRIB2_SECTION2_DESTINE_001_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE (1036)

    ALLOCATE( GRIB2_SECTION2_DESTINE_036_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE DEFAULT

    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOWN_SECTION_2 )

  END SELECT

  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION2%INIT( CFG, OPT, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_READ_SECTION_TYPE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read section2 type' )
    CASE (ERRFLAG_UNKNOWN_SECTION_2)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown section2 number: '//TRIM(ADJUSTL(TMP)) )
    CASE (ERRFLAG_ALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error allocating section2 number: '//TRIM(ADJUSTL(TMP)) )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_INITIALIZATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error initializing section2 number: '//TRIM(ADJUSTL(TMP)) )
    CASE DEFAULT
...
END FUNCTION MAKE_GRIB2_SECTION2_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_SECTION2_LAZY'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_SECTION2_LAZY( GRIB2_SECTION2, &
...
#if defined( PP_HAS_GET_GRIB2_SECTION2_ID_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_SECTION2_ID_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
   ...
   ! Local error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_GET_SECTION2_ID=0_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SECTION_2=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ALLOCATION_ERROR=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INITIALIZATION_ERROR=3_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_SECTION2_ID) GET_GRIB2_SECTION2_ID_FROM_MESSAGE( MSG, PAR, OPT, ID, HOOKS )
  ...
  CASE (1036)
    ...
    ALLOCATE( GRIB2_SECTION2_DESTINE_1036_T::GRIB2_SECTION2, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
  ...
  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_SECTION2%INIT( MSG, PAR, OPT, HOOKS )
#else
  PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNABLE_TO_GET_SECTION2_ID )
#endif
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! Handle different errors
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_UNABLE_TO_GET_SECTION2_ID)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get section2 ID' )
    CASE (ERRFLAG_UNKNOWN_SECTION_2)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown section2 number: '//TRIM(ADJUSTL(TMP)) )
    CASE (ERRFLAG_ALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error allocating section2 number: '//TRIM(ADJUSTL(TMP)) )
...
END FUNCTION MAKE_GRIB2_SECTION2_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

!>
!> @brief Destroys a GRIB2 Section 2 structure.
!>
!> This function takes a GRIB2 Section 2 object (`GRIB2_SECTION2`) and performs
...
!>
!> @section interface
!> @param[in,out] GRIB2_SECTION2 The GRIB2 Section 2 object to be destroyed.
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see MAKE_GRIB2_SECTION2
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_GRIB2_SECTION2'
PP_THREAD_SAFE FUNCTION DESTROY_GRIB2_SECTION2( GRIB2_SECTION2, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(GRIB2_SECTION2), ERRFLAG_NOT_ASSOCIATED )

  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  GRIB2_SECTION2%FREE( OPT, HOOKS )

  ! Initialize the section
  DEALLOCATE( GRIB2_SECTION2, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
...
END FUNCTION DESTROY_GRIB2_SECTION2
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Read from the YAML configuration the type of the GRIB2 Section 2 object.
!>
!> @section interface
!> @param [in]     CFG             YAML configuration object used to configure the GRIB2 Section 2 object.
!> @param [out]    SECTION2_TYPE   Identifier of the section2 type read from the configuration.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'READ_GRIB2_SECTION2_TYPE_FROM_CFG'
PP_THREAD_SAFE FUNCTION READ_GRIB2_SECTION2_TYPE_FROM_CFG( CFG, SECTION2_TYPE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(YAML_CONFIGURATION_T), INTENT(IN)    :: CFG
  INTEGER(KIND=JPIB_K),       INTENT(OUT)   :: SECTION2_TYPE
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  CHARACTER(LEN=:), ALLOCATABLE :: CSECTION2_TYPE
  LOGICAL :: HAS_SECTION2
  LOGICAL :: SECTION2_IS_INTEGER
  ...
  ! Initialization
  SECTION2_TYPE = -1_JPIB_K

  !> Check if configuration has the SECTION2 key
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, SECTION2_KEY, HAS_SECTION2, HOOKS )

  !> Read the SECTION2 template number
  IF ( HAS_SECTION2 ) THEN
    PP_TRYCALL(ERRFLAG_READ_ERROR) YAML_READ_STRING( CFG, SECTION2_KEY, CSECTION2_TYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSECTION2_TYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL( ERRFLAG_STRING_IS_INTEGER ) STRING_IS_INTEGER( CSECTION2_TYPE, SECTION2_IS_INTEGER, HOOKS )
    IF ( SECTION2_IS_INTEGER ) THEN
       PP_TRYCALL( ERRFLAG_STRING_TO_INTEGER ) STRING_TO_INTEGER( CSECTION2_TYPE, SECTION2_TYPE, HOOKS )
    ELSE
      ! TODO: Somehow decide automatically whcih SECTION2 to use. String can be
      !       a label for the method to use.
      PP_DEBUG_CRITICAL_THROW( ERRFLAG_KEY_IS_NOT_INTEGER )
    ENDIF
    DEALLOCATE(CSECTION2_TYPE, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )
  ELSE
    ! TODO
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ALLOCATED_AFTER_READ)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'SECTION2 ctype not allocated after reading' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating SECTION2 number' )
...
END FUNCTION READ_GRIB2_SECTION2_TYPE_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE GRIB2_SECTION2_FACTORY_MOD
