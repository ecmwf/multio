...
IMPLICIT NONE
  ...
  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME=TRIM(ADJUSTL(GG_NAME))//'.latitudeOfFirstGridPointInDegrees'
  ...
  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME=TRIM(ADJUSTL(GG_NAME))//'.directionIncrementInDegrees'
  ...
  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME=TRIM(ADJUSTL(SH_NAME))//'.pentagonalResolutionJ'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME=TRIM(ADJUSTL(SH_NAME))//'.pentagonalResolutionK'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  KEY_NAME = REPEAT(' ', 128)
  KEY_NAME=TRIM(ADJUSTL(SH_NAME))//'.pentagonalResolutionM'
  ERR = MIOMD%SET( TRIM(ADJUSTL(KEY_NAME)), MODEL_PARAMS%GEO_%ISMAX )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
  ...
  ERR = MIOMD%SET( "ensemble.typeOfEnsembleSimulation", 1_JPIB_K )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOMD%SET( "ensemble.numberOfForecastsInEnsemble", MODEL_PARAMS%SIM_%NTOTENS )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOMD%SET( "ensemble.systemNumber", MODEL_PARAMS%SIM_%NSYSTEM )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOMD%SET( "ensemble.methodNumber", MODEL_PARAMS%SIM_%NMETHOD )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )


  !> Analysis parametrization
  ERR = MIOMD%SET( "analysis.lengthOfTimeWindow", MODEL_PARAMS%SIM_%NWINSIZE )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )


  !> Level Parametrization
  ERR = MIOH%WRITE_PARAMETRIZATION( "level.pv", MODEL_PARAMS%GEO_%ZVERT(1:2*(MODEL_PARAMS%GEO_%IFLEV+1)) )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  !> Wave parametrization
  ERR = MIOH%WRITE_PARAMETRIZATION( "wave.freq", MODEL_PARAMS%WAM_%FR )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOH%WRITE_PARAMETRIZATION( "wave.dir", MODEL_PARAMS%WAM_%TH )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
