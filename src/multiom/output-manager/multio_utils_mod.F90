...
#define PP_PROCEDURE_NAME 'MULTIO_NEW_FROM_FILE'
PP_THREAD_SAFE FUNCTION MULTIO_NEW_FROM_FILE( MIO_HANDLE, CONFIG_FILE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CONFIG_FILE_DONT_EXIST=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CREATE_MIOCFG=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SET_FAILURE_HDL=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CREATE_MIOHDL=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_DESTROY_MIOCFG=5_JPIB_K
  ...
  INQUIRE(FILE=TRIM(ADJUSTL(CONFIG_FILE)), EXIST=FILE_EXIST )
  PP_DEBUG_CRITICAL_COND_THROW(.NOT.FILE_EXIST, ERRFLAG_CONFIG_FILE_DONT_EXIST )


  ! Logging
  WRITE(*,*) 'Multio Plans file :: ', TRIM(ADJUSTL(CONFIG_FILE))
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_CONFIG_FILE_DONT_EXIST)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to find MultIO config file' )
    ...
    CASE (ERRFLAG_UNABLE_TO_CREATE_MIOCFG)
      ...
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to create MultIO configuration from config file' )
...
END FUNCTION MULTIO_NEW_FROM_FILE
...
IMPLICIT NONE
  ...
  ERR = MIOMD%SET( 'misc-typeOfEnsembleForecast', 1_JPIB_K )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )

  ERR = MIOMD%SET( 'misc-numberOfForecastsInEnsemble', MODEL_PARAMS%SIM_%NTOTENS )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )


  !> Analysis parametrization
  ERR = MIOMD%SET( 'misc-lengthOfTimeWindow', MODEL_PARAMS%SIM_%NWINSIZE )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
  ...
  ERR = MIOMD%SET( 'misc-timeIncrementInSeconds', ITMP )
  PP_DEBUG_DEVELOP_COND_THROW( ERR.NE.MULTIO_SUCCESS,  ERRFLAG_UNABLE_TO_WRITE_PARAMETRIZATION )
...
IMPLICIT NONE
  ...
  ! Handle "timespan"
  IF ( MARS%TIMESPAN .NE. UNDEF_PARAM_E ) THEN
    ERR = MIOMD%SET( 'timespan', MARS%TIMESPAN )
    PP_DEBUG_CRITICAL_COND_THROW(ERR.NE.MULTIO_SUCCESS, ERRFLAG_UNABLE_TO_WRITE_MULTIO_METADATA)
  ENDIF
  ...
  IF ( MARS%SYSTEM .NE. UNDEF_PARAM_E ) THEN
    ERR = MIOMD%SET( 'system', MARS%SYSTEM )
    PP_DEBUG_CRITICAL_COND_THROW(ERR.NE.MULTIO_SUCCESS, ERRFLAG_UNABLE_TO_WRITE_MULTIO_METADATA)
  ENDIF

  IF ( MARS%METHOD .NE. UNDEF_PARAM_E ) THEN
    ERR = MIOMD%SET( 'method', MARS%METHOD )
    PP_DEBUG_CRITICAL_COND_THROW(ERR.NE.MULTIO_SUCCESS, ERRFLAG_UNABLE_TO_WRITE_MULTIO_METADATA)
  ENDIF
  ...
  IF ( PAR%TYPE_OF_PROCESSED_DATA .NE. UNDEF_PARAM_E ) THEN
    ...
    ERR = MIOMD%SET( 'misc-typeOfProcessedData', PAR%TYPE_OF_PROCESSED_DATA )
    PP_DEBUG_CRITICAL_COND_THROW(ERR.NE.MULTIO_SUCCESS, ERRFLAG_UNABLE_TO_WRITE_MULTIO_METADATA)
  ENDIF
  ...
  ! Handle "scaleFactorOfCentralVaveNumber"
  IF ( PAR%SATELLITE%SCALED_FACTOR_OF_CENTRAL_VAWENUMBER .NE. UNDEF_PARAM_E ) THEN
    ERR = MIOMD%SET( 'misc-scaleFactorOfCentralWaveNumber', PAR%SATELLITE%SCALED_FACTOR_OF_CENTRAL_VAWENUMBER )
    PP_DEBUG_CRITICAL_COND_THROW(ERR.NE.MULTIO_SUCCESS, ERRFLAG_UNABLE_TO_WRITE_MULTIO_METADATA)
  ENDIF
