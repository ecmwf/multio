...
!>
!> @brief Module containing definitions and procedures for origin filters.
!>
!> This module defines the `FILTER_ORIGIN_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of origin filters within the system. Origin filters allow for
...
#define PP_FILE_NAME 'filter_origin_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_ORIGIN_MOD'
MODULE FILTER_ORIGIN_MOD
...
!>
!> @brief Filter type for managing level types (ORIGIN).
!>
!> This derived type `FILTER_ORIGIN_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_ORIGIN_T
  ...
  INTEGER(KIND=JPIB_K) :: ORIGIN_

  !> Holds the keyset of level types for filtering.
  TYPE(KEYSET_INT64_T) :: ORIGINS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_ORIGIN_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_ORIGIN_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_ORIGIN_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_ORIGIN_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_ORIGIN_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_ORIGIN_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_ORIGIN_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_ORIGIN_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CORIGIN2IORIGIN
...
!>
!> @see FILTER_ORIGIN_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ORIGIN_INIT'
FUNCTION FILTER_ORIGIN_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_ORIGIN_T),     INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CORIGIN2IORIGIN
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%ORIGINS_%INIT( HOOKS )
  THIS%USE_KEYSET_ = .FALSE.

  !> Associate the filter procedure
  P_CORIGIN2IORIGIN => CORIGIN2IORIGIN
  ...
  IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%ORIGINS_, P_CORIGIN2IORIGIN, HOOKS )
    THIS%USE_KEYSET_ = .TRUE.

  ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%ORIGIN_, P_CORIGIN2IORIGIN, HOOKS )
...
END FUNCTION FILTER_ORIGIN_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_ORIGIN_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_ORIGIN_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_ORIGIN_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'KEY_EQ'
FUNCTION FILTER_ORIGIN_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%ORIGINS_%MATCH( MSG%ORIGIN, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%ORIGIN_ .EQ. MSG%ORIGIN)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%ORIGIN_ .EQ. MSG%ORIGIN)
    ENDIF
...
END FUNCTION FILTER_ORIGIN_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_ORIGIN_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_ORIGIN_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ORIGIN_PRINT'
FUNCTION FILTER_ORIGIN_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'ORIGIN FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_ORIGIN_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_ORIGIN_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_ORIGIN_FREE'
FUNCTION FILTER_ORIGIN_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%ORIGINS_%FREE( HOOKS )
...
END FUNCTION FILTER_ORIGIN_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_ORIGIN_MOD
