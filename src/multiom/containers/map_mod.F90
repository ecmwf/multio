...
INTERFACE
  PP_THREAD_SAFE FUNCTION MAP_FUNCTION_@TEMPLATE_KEY_NAME@_IF( KEY, HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD, ONLY: JPIB_K
  ...
  END FUNCTION MAP_FUNCTION_@TEMPLATE_KEY_NAME@_IF
END INTERFACE

TYPE, ABSTRACT :: MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_A
CONTAINS
PROCEDURE(MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_IF), DEFERRED, PUBLIC, PASS :: APPLY
END TYPE

ABSTRACT INTERFACE
  PP_THREAD_SAFE FUNCTION MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_IF( THIS, KEY, HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD, ONLY: JPIB_K
    USE :: HOOKS_MOD,         ONLY: HOOKS_T
    !> Templated use
    @TEMPLATE_USE_KEY@
    IMPORT :: MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_A
  IMPLICIT NONE
    CLASS(MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_A), INTENT(INOUT) :: THIS
    @TEMPLATE_KEY_DUMMY_TYPE@,                   INTENT(IN)    :: KEY
    TYPE(HOOKS_T),                               INTENT(INOUT) :: HOOKS
    INTEGER(KIND=JPIB_K) :: RET
  END FUNCTION MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_IF
END INTERFACE
...
PUBLIC :: MAP_FUNCTION_@TEMPLATE_KEY_NAME@_IF
PUBLIC :: MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_A
...
PP_THREAD_SAFE FUNCTION MAP_INSERT_NODE( ROOT, KEY, VALUE, INSERTED, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ASSOCIATED( ROOT, NIL  ) ) THEN
    ...
    INSERTION_POINT => NIL
    PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_NODE) MAP_NODE_INIT( ROOT, INSERTION_POINT, KEY, VALUE, HOOKS )
  ...
  ELSE
    ...
    IF ( .NOT.FOUND ) THEN
      ...
      IF ( KEY_LT ) THEN
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_INITIALIZE_NODE) MAP_NODE_INIT( INSERTION_POINT%LEFT, INSERTION_POINT, KEY, VALUE, HOOKS )
      ...
      ELSE
        ...
        PP_TRYCALL(ERRFLAG_UNABLE_TO_ALLOCATE_NODE) MAP_NODE_INIT( INSERTION_POINT%RIGHT, INSERTION_POINT, KEY, VALUE, HOOKS )
...
PP_THREAD_SAFE FUNCTION MAP_NODE_INIT( THIS, PARENT, KEY, VALUE, HOOKS ) RESULT(RET)
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_DEALLOCATE_NODE)
      ...
      ELSE
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating map node: '//TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
...
IMPLICIT NONE
  ...
  TYPE(MAP_@TEMPLATE_KEY_NAME@_@TEMPLATE_VALUE_NAME@_NODE_T), POINTER,           INTENT(IN)    :: ROOT
  TYPE(MAP_@TEMPLATE_KEY_NAME@_@TEMPLATE_VALUE_NAME@_NODE_T), POINTER,           INTENT(IN)    :: CURRENT
  PROCEDURE(MAP_FUNCTION_@TEMPLATE_KEY_NAME@_IF), POINTER, INTENT(IN)    :: FUNCTION
  TYPE(HOOKS_T),                                              INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  TYPE(MAP_@TEMPLATE_KEY_NAME@_@TEMPLATE_VALUE_NAME@_NODE_T),     POINTER, INTENT(IN)    :: ROOT
  TYPE(MAP_@TEMPLATE_KEY_NAME@_@TEMPLATE_VALUE_NAME@_NODE_T),     POINTER, INTENT(IN)    :: CURRENT
  CLASS(MAP_FUNCTOR_@TEMPLATE_KEY_NAME@_A), POINTER, INTENT(IN)    :: FUNCTOR
  TYPE(HOOKS_T),                                     INTENT(INOUT) :: HOOKS
...
PP_THREAD_SAFE FUNCTION MAP_INSERT( THIS, KEY, VALUE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MAP_INSERT_NODE) MAP_INSERT_NODE( THIS%ROOT, KEY, VALUE, INSERTED, HOOKS )
...
PP_THREAD_SAFE FUNCTION MAP_REMOVE( THIS, KEY, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.ROOT_IS_LEAF ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MAP_REMOVE_NODE) MAP_REMOVE_NODE( THIS%ROOT, KEY, FOUND, HOOKS )
    IF ( FOUND ) THEN
      THIS%SIZE_ = THIS%SIZE_ - 1
    ENDIF
  ENDIF
