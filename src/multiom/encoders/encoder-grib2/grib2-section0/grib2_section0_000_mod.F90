!>
!> @file grib2_section0_000_mod.F90
!>
!> @brief Module for managing GRIB2 Section 0 operations.
!>
!> The `GRIB2_SECTION0_000_MOD` module contains procedures to initialize, allocate,
!> preset, run, and clean up the resources associated with GRIB2 Section 0 objects.
...
!>   - Initialization of GRIB2 Section 0 objects.
...
!>   - @see GRIB2_SECTION0_000_INIT
!>   - @see GRIB2_SECTION0_000_ALLOCATE
!>   - @see GRIB2_SECTION0_000_PRESET
!>   - @see GRIB2_SECTION0_000_RUNTIME
!>   - @see GRIB2_SECTION0_000_TO_BE_ENCODED
!>   - @see GRIB2_SECTION0_000_FREE
!>
...
#define PP_FILE_NAME 'grib2_section0_000_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION0_000_MOD'
MODULE GRIB2_SECTION0_000_MOD
...
!>
!> @brief Type definition for GRIB2 Section 0 handler.
!>
!> The `GRIB2_SECTION0_000_T` type extends the base class `GRIB_SECTION_BASE_A` and
!> provides concrete implementations of initialization, allocation, preset, runtime,
!> encoding checks, and cleanup operations for GRIB2 Section 0 objects.
!>
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION0_000_T
...
CONTAINS
  ...
  !>
  !> @brief Initializes the GRIB2 Section 0 object.
  !>
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION0_000_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION0_000_INIT_LAZY

  !>
  !> @brief Allocates resources for the GRIB2 Section 0 object.
  !>
  !> This procedure allocates memory and other necessary resources for
  !> the object based on provided parameters.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOCATE => GRIB2_SECTION0_000_ALLOCATE

  !>
  !> @brief Presets the parameters of the GRIB2 Section 0 object.
  !>
  !> This procedure configures the internal parameters of the object
  !> before runtime execution.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRESET => GRIB2_SECTION0_000_PRESET

  !>
  !> @brief Manages the runtime execution of GRIB2 Section 0 operations.
  !>
  !> This procedure handles operations and computations during runtime,
  !> making use of time and metadata information.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RUNTIME => GRIB2_SECTION0_000_RUNTIME

  !>
  !> @brief Determines if the GRIB2 Section 0 object needs to be encoded.
  !>
  !> This procedure checks whether the object should be encoded based
  !> on the provided parameters and internal state.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION0_000_TO_BE_ENCODED

  !>
  !> @brief Frees resources allocated for the GRIB2 Section 0 object.
  !>
  !> This procedure deallocates resources and performs cleanup after
  !> the object has been used.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => GRIB2_SECTION0_000_FREE
...
PUBLIC :: GRIB2_SECTION0_000_T

CONTAINS

!>
!> @brief Initializes GRIB2 Section 0 for a given object using the provided parameters.
!>
!> This function initializes a GRIB2 Section 0 object (`THIS`) using YAML configuration data (`CFG`).
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_@XXX@_T` representing the GRIB section being initialized.
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_INIT_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_INIT_CFG( THIS, &
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(GRIB2_SECTION0_000_T),  INTENT(INOUT) :: THIS
  ...
  THIS%TYPE_ = 'SECTION'
  THIS%SUBTYPE_ = 'INDICATOR_SECTION'
  THIS%KIND_ = '0.0'
...
END FUNCTION GRIB2_SECTION0_000_INIT_CFG
...
!> @brief Initializes GRIB2 Section 0 for a given object using the provided parameters.
!>
!> This function initializes a GRIB2 Section 0 object (`THIS`) using the provided model message (`MSG`)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_INIT_LAZY'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_INIT_LAZY( THIS, &
...
END FUNCTION GRIB2_SECTION0_000_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Allocates resources for GRIB2 Section 0 using the provided parameters.
!>
!> This function allocates resources for a GRIB2 Section 0 object (`THIS`) using the provided model parameters (`PARAMS`),
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION4_@XXX@_T` representing the GRIB section to allocate resources for.
!>   @param [in]    MSG      All the mars keywords needed to describe the field `FORTRAN_MESSAGE_T`.
!>   @param [in]    PAR      All information outside mars keywords needed to describe the field `PARAMETRIZATION_T`.
!>   @param [in]    OPT      The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_ALLOCATE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_ALLOCATE( THIS, &
...
END FUNCTION GRIB2_SECTION0_000_ALLOCATE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Presets GRIB2 Section 0 using the provided parameters and message data.
!>
!> This function presets a GRIB2 Section 0 object (`THIS`) using the provided model parameters (`PARAMS`),
!> message structure (`MSG`), and metadata (`METADATA`). The process can be run in verbose mode if specified.
!> The function is thread-safe and returns an error code indicating the success or failure of the preset operation.
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION0_000_T` representing the GRIB section to be preset.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_PRESET'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_PRESET( THIS, &
&  MSG, PAR, OPT, METADATA, HOOKS ) RESULT(RET)
...
#if defined( PP_HAS_GET_DISCIPLINE_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_DISCIPLINE_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  ! Extract discipline from message
  PP_TRYCALL(ERRFLAG_GET_DISCIPLINE_FROM_MESSAGE) GET_DISCIPLINE_FROM_MESSAGE( MSG, PAR, OPT, DISCIPLINE, HOOKS )

  ! Set discipline
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'discipline', DISCIPLINE )

#else
  ! Logging informations
  PP_LOG_TODO( 'Preset GRIB2 Section: '//TRIM(ADJUSTL(THIS%TYPE_))//'::'//TRIM(ADJUSTL(THIS%SUBTYPE_))//': "discipline" needs to be extracted from mars' )
#endif
...
END FUNCTION GRIB2_SECTION0_000_PRESET
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Executes runtime processing for GRIB2 Section 0 using provided parameters, message data, and time history.
!>
!> This function performs runtime operations for a GRIB2 Section 0 object (`THIS`) using the provided model parameters (`PARAMS`),
...
!> @section interface
!>
!> @param [in]    THIS          An object of type `GRIB2_SECTION0_000_T` representing the GRIB section to be allocated.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_RUNTIME'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_RUNTIME( THIS, &
...
IMPLICIT NONE
  ...
  !> Local variables
  INTEGER(KIND=JPIB_K) :: DISCIPLINE
  ...
  ! Logging informations
  PP_LOG_INFO( 'Runtime GRIB2 Section: '//TRIM(ADJUSTL(THIS%TYPE_))//'::'//TRIM(ADJUSTL(THIS%SUBTYPE_))//': Nothing to be done' )
...
END FUNCTION GRIB2_SECTION0_000_RUNTIME
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prepares GRIB2 Section 0 for encoding based on provided parameters, message data, and time history.
!>
!> This function determines whether GRIB2 Section 0 (`THIS`) is ready to be encoded. It processes the provided model parameters
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION0_000_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_TO_BE_ENCODED( THIS, &
...
IMPLICIT NONE
  ...
  ! Logging informations
  PP_LOG_TODO( 'ToBeEncoded GRIB2 Section: '//TRIM(ADJUSTL(THIS%TYPE_))//'::'//TRIM(ADJUSTL(THIS%SUBTYPE_))//': Nothing to be done' )
  TO_BE_ENCODED = .TRUE.
...
END FUNCTION GRIB2_SECTION0_000_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Frees resources associated with GRIB2 Section 0 object.
!>
!> This function deallocates and cleans up resources associated with the GRIB2 Section 0 object (`THIS`).
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_@XXX@_T` representing the GRIB section to be freed.
!>   @param [in]    OPT   The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION0_000_FREE'
!! PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_FREE(  THIS, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
PP_THREAD_SAFE FUNCTION GRIB2_SECTION0_000_FREE(  THIS, OPT, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION0_000_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE GRIB2_SECTION0_000_MOD
