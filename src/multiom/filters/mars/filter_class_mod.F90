...
!>
!> This module defines the `FILTER_CLASS_T` type, along with its associated
...
#define PP_SECTION_NAME 'FILTER_CLASS_MOD'
MODULE FILTER_CLASS_MOD
...
!>
!> @brief Filter type for managing level types (CLASS).
!>
!> This derived type `FILTER_CLASS_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_CLASS_T
  ...
  INTEGER(KIND=JPIB_K) :: CLASS_

  !> Holds the keyset of level types for filtering.
  TYPE(KEYSET_INT64_T) :: CLASSS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_CLASS_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_CLASS_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_CLASS_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_CLASS_FREE
...
!>
!> @brief Initializes a level type filter (`FILTER_CLASS_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_CLASS_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_CLASS_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CCLASS2ICLASS
...
!>
!> @see FILTER_CLASS_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CLASS_INIT'
FUNCTION FILTER_CLASS_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_CLASS_T),    INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CCLASS2ICLASS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%CLASSS_%INIT( HOOKS )
  THIS%USE_KEYSET_ = .FALSE.

  !> Associate the filter procedure
  P_CCLASS2ICLASS => CCLASS2ICLASS
  ...
  IF ( THIS%FILTER_TYPE_ .NE. FLT_INT_HAS_E .AND. THIS%FILTER_TYPE_ .NE. FLT_INT_LACKS_E ) THEN
    ...
    IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%CLASSS_, P_CCLASS2ICLASS, HOOKS )
      THIS%USE_KEYSET_ = .TRUE.

    ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%CLASS_, P_CCLASS2ICLASS, HOOKS )
...
END FUNCTION FILTER_CLASS_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_CLASS_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_CLASS_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_CLASS_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CLASS_MATCH'
FUNCTION FILTER_CLASS_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%CLASSS_%MATCH( MSG%CLASS, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%CLASS_ .EQ. MSG%CLASS)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%CLASS_ .EQ. MSG%CLASS)
    ENDIF
...
END FUNCTION FILTER_CLASS_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_CLASS_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_CLASS_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CLASS_PRINT'
FUNCTION FILTER_CLASS_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'CLASS FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_CLASS_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_CLASS_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CLASS_FREE'
FUNCTION FILTER_CLASS_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%CLASSS_%FREE( HOOKS )
...
END FUNCTION FILTER_CLASS_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_CLASS_MOD
