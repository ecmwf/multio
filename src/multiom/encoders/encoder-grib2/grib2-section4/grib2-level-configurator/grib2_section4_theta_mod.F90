!>
!> @file grib2_section4_theta_mod.F90
!>
!> @brief Module for managing GRIB2 Section 4 level configuration operations.
!>
!> The `G2S4_THETA_MOD` module contains procedures to initialize, allocate,
...
!>   - @see G2S4_THETA_INIT
!>   - @see G2S4_THETA_ALLOC
!>   - @see G2S4_THETA_PRESET
!>   - @see G2S4_THETA_RT
!>   - @see G2S4_THETA_TBE
!>   - @see G2S4_THETA_FREE
!>
...
#define PP_FILE_NAME 'grib2_section4_theta_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION4_THETA_MOD'
MODULE GRIB2_SECTION4_THETA_MOD
...
CHARACTER(LEN=*), PARAMETER :: TYPE_OF_LEVEL = 'theta'
INTEGER(KIND=JPIB_K), PARAMETER :: TYPE_OF_FIRST_FIXED_SURFACE = 107_JPIB_K
...
!>
!> The `GRIB2_SECTION4_THETA_T` type extends the base class `GRIB_SECTION_BASE_A` and
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION4_THETA_T
...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => G2S4_THETA_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => G2S4_THETA_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOCATE => G2S4_THETA_ALLOC
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRESET => G2S4_THETA_PRESET
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RUNTIME => G2S4_THETA_RT
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => G2S4_THETA_TBE
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => G2S4_THETA_FREE
  ...
  !>
  PROCEDURE, PRIVATE, PASS, NON_OVERRIDABLE :: SET_LEVELS => G2S4_THETA_SET_LEVELS
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: CHECK => G2S4_THETA_CHECK
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section being initialized.
...
#define PP_PROCEDURE_NAME 'G2S4_THETA_INIT_CFG'
PP_THREAD_SAFE FUNCTION G2S4_THETA_INIT_CFG( THIS, &
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(GRIB2_SECTION4_THETA_T),  INTENT(INOUT) :: THIS
...
END FUNCTION G2S4_THETA_INIT_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_INIT_LAZY'
PP_THREAD_SAFE FUNCTION G2S4_THETA_INIT_LAZY( THIS, &
...
END FUNCTION G2S4_THETA_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section to allocate resources for.
!>   @param [in]    MSG      All the mars keywords needed to describe the field `FORTRAN_MESSAGE_T`.
!>   @param [in]    PAR      All information outside mars keywords needed to describe the field `PARAMETRIZATION_T`.
!>   @param [in]    OPT      The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_ALLOC'
PP_THREAD_SAFE FUNCTION G2S4_THETA_ALLOC( THIS, &
...
IMPLICIT NONE
  ...
  ! Logging
  PP_LOG_DEVELOP_STR( 'Allocate: theta' )
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section to be preset.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_PRESET'
PP_THREAD_SAFE FUNCTION G2S4_THETA_PRESET( THIS, &
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Logging
  PP_LOG_DEVELOP_STR( 'Preset: theta' )
...
END FUNCTION G2S4_THETA_PRESET
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS      An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section for runlevel execution.
!>   @param [in]    MSG       The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR       The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_RT'
PP_THREAD_SAFE FUNCTION G2S4_THETA_RT( THIS, &
...
IMPLICIT NONE
  ...
  ! Logging
  PP_LOG_DEVELOP_STR( 'Runtime: theta' )
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
!>   @param [in]    TIME_HIST     The time history object of type `TIME_HISTORY_T` providing historical level data.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_TBE'
PP_THREAD_SAFE FUNCTION G2S4_THETA_TBE( THIS, &
...
!>
!> @section interface
!>   @param [inout] THIS  An object of type `GRIB2_SECTION4_THETA_T` representing the GRIB section to be freed.
!>   @param [in]    OPT   The encoder options structure of type `ENCODER_OPTIONS_T`.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_FREE'
PP_THREAD_SAFE FUNCTION G2S4_THETA_FREE( THIS, OPT, HOOKS ) RESULT(RET)
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_SET_LEVELS'
PP_THREAD_SAFE FUNCTION G2S4_THETA_SET_LEVELS( THIS, &
&  MSG, PAR, OPT, METADATA, HOOKS ) RESULT(RET)
...
!&       SCALED_VALUE_OF_SECOND_FIXED_SURFACES = 0_JPIB_K, &
...
END FUNCTION G2S4_THETA_SET_LEVELS
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'G2S4_THETA_CHECK'
PP_THREAD_SAFE FUNCTION G2S4_THETA_CHECK( THIS, &
&  MSG, PAR, OPT, METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! According to the options decide where to set the levels (preset or runlevel)
  PP_LOG_INFO( 'check typeOfLevel: theta' )
...
END FUNCTION G2S4_THETA_CHECK
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE GRIB2_SECTION4_THETA_MOD
