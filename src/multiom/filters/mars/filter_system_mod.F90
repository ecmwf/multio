...
!>
!> This module defines the `FILTER_SYSTEM_T` type, along with its associated
...
#define PP_FILE_NAME 'filter_system_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_SYSTEM_MOD'
MODULE FILTER_SYSTEM_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_SYSTEM_T
  ...
  !> @details If `.TRUE.`, the `SYSTEMS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: SYSTEMS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_SYSTEM_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_SYSTEM_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_SYSTEM_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_SYSTEM_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_SYSTEM_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_SYSTEM_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_SYSTEM_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_SYSTEM_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_SYSTEM_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_SYSTEM_FREE
...
PUBLIC :: FILTER_SYSTEM_T

CONTAINS


!>
!> @brief Initializes the `FILTER_SYSTEM_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_SYSTEM_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_SYSTEM_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_SYSTEM_INIT'
FUNCTION FILTER_SYSTEM_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_SYSTEM_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%SYSTEMS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%SYSTEMS_, HOOKS )
...
END FUNCTION FILTER_SYSTEM_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_SYSTEM_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_SYSTEM_MATCH'
FUNCTION FILTER_SYSTEM_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Match the filter
  IF ( MSG%SYSTEM .EQ. UNDEF_PARAM_E ) THEN
  ...
  ELSE
    ...
    CASE( FLT_INT_MATCH_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%SYSTEMS_%MATCH( MSG%SYSTEM, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%SYSTEM_ .EQ. MSG%SYSTEM)
      ENDIF
    ...
    CASE( FLT_INT_IGNORE_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%SYSTEM_ .EQ. MSG%SYSTEM)
      ENDIF

    CASE( FLT_INT_GT_E )

      MATCH = (MSG%SYSTEM .GT. THIS%SYSTEM_ )

    CASE( FLT_INT_GE_E )

      MATCH = (MSG%SYSTEM .GE. THIS%SYSTEM_ )

    CASE( FLT_INT_LE_E )

      MATCH = (MSG%SYSTEM .LE. THIS%SYSTEM_ )

    CASE( FLT_INT_LT_E )

      MATCH = (MSG%SYSTEM .LT. THIS%SYSTEM_ )

    CASE( FLT_INT_HAS_E )

      MATCH = .NOT.(MSG%SYSTEM .EQ. THIS%SYSTEM_ )

    CASE( FLT_INT_LACKS_E )

      MATCH = (MSG%SYSTEM .EQ. THIS%SYSTEM_ )
...
!      PP_DEBUG_PUSH_MSG_TO_FRAME( '"system" filter can be used only with "levtype"="pl"' )
...
END FUNCTION FILTER_SYSTEM_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_SYSTEM_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_SYSTEM_PRINT'
FUNCTION FILTER_SYSTEM_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'SYSTEM FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_SYSTEM_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_SYSTEM_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_SYSTEM_FREE'
FUNCTION FILTER_SYSTEM_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%SYSTEMS_%FREE( HOOKS )
...
END FUNCTION FILTER_SYSTEM_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_SYSTEM_MOD
