...
#include "multio/action/utils/GridInfo.h"
...
const std::map<const std::string, const long> ops_to_code{
    {"instant", 0000}, {"average", 1000}, {"accumulate", 2000},
...
GribEncoder::GribEncoder(codes_handle* handle,
                         const eckit::LocalConfiguration& config)
    : metkit::grib::GribHandle{handle},
      config_{config} /*, encodeBitsPerValue_(config)*/ {
  for (auto const& subtype :
       {"T grid", "U grid", "V grid", "W grid", "F grid"}) {
    grids().insert(
        std::make_pair(subtype, std::unique_ptr<GridInfo>{new GridInfo{}}));
  }
}
...
bool GribEncoder::setGridInfo(message::Message msg) {
  ...
  ASSERT(coordSet_.find(msg.metadata().getString("nemoParam")) !=
         end(coordSet_));
...
QueriedMarsFields setMarsFields(GribEncoder& g,
                                const eckit::Configuration& md) {
  ...
  withFirstOf(ValueSetter{g, "level"}, LookUpLong(md, "level"),
              LookUpLong(md, "levelist"));
  withFirstOf(ValueSetter{g, "date"}, LookUpLong(md, "date"),
              LookUpLong(md, "dataDate"));
  withFirstOf(ValueSetter{g, "time"}, LookUpLong(md, "time"),
              LookUpLong(md, "dataTime"));
  withFirstOf(ValueSetter{g, "step"}, LookUpLong(md, "step"),
              LookUpLong(md, "startStep"));

  ret.paramId = firstOf(
      LookUpLong(md, "paramId"),
      LookUpLong(
          md, "param"));  // param might be a string, separated by . for GRIB1.
                          // String to long convertion should get it right
  if (ret.paramId) {
    g.setValue("paramId", *ret.paramId);
  }
  withFirstOf(ValueSetter{g, "class"}, LookUpString(md, "class"),
              LookUpString(md, "marsClass"));
  withFirstOf(ValueSetter{g, "stream"}, LookUpString(md, "stream"),
              LookUpString(md, "marsStream"));
  withFirstOf(ValueSetter{g, "expver"}, LookUpString(md, "expver"),
              LookUpString(md, "experimentVersionNumber"));
  ...
  withFirstOf(ValueSetter{g, "complexPacking"},
              LookUpLong(md, "complexPacking"));
  if (md.has("gridType") && md.getString("gridType") == "sh") {
    withFirstOf(ValueSetter{g, "pentagonalResolutionParameterJ"},
                LookUpLong(md, "pentagonalResolutionParameterJ"),
                LookUpLong(md, "J"));
    withFirstOf(ValueSetter{g, "pentagonalResolutionParameterK"},
                LookUpLong(md, "pentagonalResolutionParameterK"),
                LookUpLong(md, "K"));
    withFirstOf(ValueSetter{g, "pentagonalResolutionParameterM"},
                LookUpLong(md, "pentagonalResolutionParameterM"),
                LookUpLong(md, "M"));
    // withFirstOf(ValueSetter{g, "unpackedSubsetPrecision"}, LookUpLong(md,
    // "unpackedSubsetPrecision"));
    withFirstOf(ValueSetter{g, "subSetJ"}, LookUpLong(md, "subSetJ"),
                LookUpLong(md, "JS"));
    withFirstOf(ValueSetter{g, "subSetK"}, LookUpLong(md, "subSetK"),
                LookUpLong(md, "KS"));
    withFirstOf(ValueSetter{g, "subSetM"}, LookUpLong(md, "subSetM"),
                LookUpLong(md, "MS"));
  }
...
void setEncodingSpecificFields(GribEncoder& g, const message::Message& msg) {
  ...
  // [&g, &msg]() {
  //     const auto& md = msg.metadata();
  //     auto paramId = firstOf(LookUpLong(md, "paramId"), LookUpLong(md,
  //     "param")); if (paramId) {
  //         // TODO may convert from string - check if possible at all?
  //         auto lv = firstOf(LookUpString(md, "levtype"), LookUpString(md,
  //         "indicatorOfTypeOfLevel")); if (!lv) {
  ...
  //         int bitsPerValue = g.getBitsPerValue(*paramId, *lv, *minmax.first,
  //         *minmax.second); return eckit::Optional<long>{bitsPerValue};
  //     }
  //     return eckit::Optional<long>{};
  // });
}
...
void setDateAndStatisticalFields(GribEncoder& g, const eckit::Configuration& md,
  ...
  auto date = firstOf(LookUpLong(md, (queriedMarsFields.type &&
                                      (*queriedMarsFields.type == "an"))
                                         ? "currentDate"
                                         : "startDate"),
  ...
  auto time = firstOf(LookUpLong(md, (queriedMarsFields.type &&
                                      (*queriedMarsFields.type == "an"))
                                         ? "currentTime"
                                         : "startTime"),
  ...
  if (operation) {
    ...
    if (*queriedMarsFields.type == "fc" && *operation == "instant") {
      ...
      withFirstOf(ValueSetter{g, "stepRange"},
                  LookUpLong(md, "stepRangeInHours"));
    }

    eckit::Optional<long> curDate;
    if (*operation != "instant" &&
        (curDate = firstOf(LookUpLong(md, "currentDate")))) {
...
void GribEncoder::setOceanMetadata(const message::Message& msg) {
  ...
  if (queriedMarsFields.type) {
    setValue("typeOfGeneratingProcess",
             type_of_generating_process.at(*queriedMarsFields.type));
  }
  applyOverwrites(*this, msg.metadata());
...
void GribEncoder::setOceanCoordMetadata(const message::Metadata& md,
                                        const eckit::Configuration& runConfig) {
  // Set run-specific md
...
void codesCheckRelaxed(int ret, const std::string& name) {
  if (ret == CODES_READ_ONLY) {
    ...
    eckit::Log::info() << "Multio GribEncoder: Ignoring readonly field " << name
                       << std::endl;
    return;
  }
  if (ret != 0) {
...
void GribEncoder::setValue(const std::string& key, long value) {
  LOG_DEBUG_LIB(LibMultio) << "*** Setting value " << value << " for key "
                           << key << std::endl;
...
void GribEncoder::setValue(const std::string& key, const std::string& value) {
  ...
  codesCheckRelaxed(codes_set_string(raw(), key.c_str(), value.c_str(), &sz),
                    key);
}
...
void GribEncoder::setValue(const std::string& key, const unsigned char* value) {
  ...
  for (int i = 0; i < DIGEST_LENGTH; ++i) {
    oss << ((i == 0) ? "" : "-") << std::hex << std::setfill('0')
        << std::setw(2) << static_cast<short>(value[i]);
  }
  LOG_DEBUG_LIB(LibMultio) << "*** Setting value " << oss.str() << " for key "
                           << key << std::endl;
...
void GribEncoder::setValue(const std::string& key, bool value) {
  LOG_DEBUG_LIB(LibMultio) << "*** Setting value " << value << "("
                           << static_cast<long>(value) << ") for key " << key
                           << std::endl;
  codesCheckRelaxed(
      codes_set_long(raw(), key.c_str(), static_cast<long>(value)), key);
}
...
message::Message GribEncoder::encodeOceanLongitudes(
...
message::Message GribEncoder::setFieldValues(const message::Message& msg) {
  ...
  return Message{Message::Header{Message::Tag::Grib, Peer{msg.source().group()},
                                 Peer{msg.destination()}},
                 std::move(buf)};
}

message::Message GribEncoder::setFieldValues(const double* values,
                                             size_t count) {
  ...
  return Message{Message::Header{Message::Tag::Grib, Peer{}, Peer{}},
                 std::move(buf)};
}
