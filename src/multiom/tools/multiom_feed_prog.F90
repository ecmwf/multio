...
&                A%MSG_ID_, A%MSG_ADDR_, YLATMMSG, CFG%BIG_ENDIAN_READ, CFG%VERBOSE, HOOKS )
            ...
            ! Read the values
            PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VAL_SP) READ_VAL_SP( TRIM(CFG%INPUT_DIR), CFG%SKIP_VAL, A%PROC_ID_, A%MSG_ID_, &
...
&                       A%MSG_ADDR_, YLATMMSG, CFG%BIG_ENDIAN_READ, CFG%VERBOSE, HOOKS )
            ...
            ! Rdad the values
            PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VAL_DP) READ_VAL_DP( TRIM(CFG%INPUT_DIR), CFG%SKIP_VAL, A%PROC_ID_, A%MSG_ID_, A%VAL_ADDR_, &
&                  A%VAL_LB_, A%VAL_UB_, VALUES_DP, CFG%BIG_ENDIAN_READ, CFG%VERBOSE, HOOKS )
        ...
        ELSE
          ...
          IF ( YLATMMSG%IPREVPP_ .GE. 0 ) THEN
            WRITE(OUTPUT_UNIT,'(A,I10,I10,I5,I5,I10,I10,I10,A4,A20,A10)',IOSTAT=WRITE_STATUS) &
&             'Atm. message:       ', A%PARAM_ID_, A%U_ID_, A%STEP_ID_, YLATMMSG%IPREVPP_, A%PROC_ID_, A%REPRES_ID_, A%PREFIX_ID_, &
...
&                A%MSG_ID_, A%MSG_ADDR_, YLWAMMSG, CFG%BIG_ENDIAN_READ, CFG%VERBOSE, HOOKS )
            ...
            ! Read the values
            PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VAL_SP) READ_VAL_SP( TRIM(CFG%INPUT_DIR), CFG%SKIP_VAL, A%PROC_ID_, A%MSG_ID_, A%VAL_ADDR_, &
