...
INTEGER(KIND=JPIB_K), PARAMETER :: CMDARG_STRLEN=2048_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: ARGSTRING_LEN=2048_JPIB_K

TYPE :: OPTIONS_T
  INTEGER(KIND=JPIB_K) :: MODIFIER=0
  CHARACTER(LEN=CMDARG_STRLEN) :: NAME=REPEAT(' ',CMDARG_STRLEN)
  LOGICAL :: MATCH = .TRUE.
  CHARACTER(LEN=CMDARG_STRLEN), DIMENSION(:), POINTER :: VALUES => NULL()
ENDTYPE
...
TYPE :: ARG_T
  CHARACTER(LEN=CMDARG_STRLEN) :: VAL=REPEAT(' ',CMDARG_STRLEN)
...
CONTAINS
  ...
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT         => COMMAND_ARGUMENTS_TOKENIZER_INIT
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INITIALIZED  => COMMAND_ARGUMENTS_TOKENIZER_INITIALIZED
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: SIZE         => COMMAND_ARGUMENTS_TOKENIZER_SIZE
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT        => COMMAND_ARGUMENTS_TOKENIZER_PRINT
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: GET_MODIFIER => COMMAND_ARGUMENTS_TOKENIZER_GET_MODIFIER
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: GET_NAME     => COMMAND_ARGUMENTS_TOKENIZER_GET_NAME
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: GET_MATCH    => COMMAND_ARGUMENTS_TOKENIZER_GET_MATCH
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: GET_VALUES   => COMMAND_ARGUMENTS_TOKENIZER_GET_VALUES
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE         => COMMAND_ARGUMENTS_TOKENIZER_FREE
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=CMDARG_STRLEN) :: NAME
  ...
  IF ( ARGCOUNT .GT. 0 ) THEN
    ...
    DO WHILE( ASSOCIATED(CURR) )
      ...
      PREV%DATA_%NAME = REPEAT(' ',CMDARG_STRLEN)
...
IMPLICIT NONE
  ...
  CLASS(COMMAND_ARGUMENTS_TOKENIZER_T), INTENT(INOUT) :: THIS
  INTEGER(KIND=JPIB_K),                 INTENT(IN)    :: ID
  CHARACTER(LEN=CMDARG_STRLEN),         INTENT(OUT)   :: NAME
  TYPE(HOOKS_T),                        INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  CLASS(COMMAND_ARGUMENTS_TOKENIZER_T),                INTENT(INOUT) :: THIS
  INTEGER(KIND=JPIB_K),                                INTENT(IN)    :: ID
  CHARACTER(LEN=CMDARG_STRLEN), DIMENSION(:), POINTER, INTENT(OUT)   :: VALUES
  TYPE(HOOKS_T),                                       INTENT(INOUT) :: HOOKS
  ...
  IF ( ALLOCATED(THIS%ARGUMENTS_) ) THEN
    PP_DEBUG_CRITICAL_COND_THROW( ID.LT.1, ERRFLAG_OUT_OF_BOUNDS )
    PP_DEBUG_CRITICAL_COND_THROW( ID.GT.SIZE(THIS%ARGUMENTS_), ERRFLAG_OUT_OF_BOUNDS )
    IF ( ASSOCIATED(THIS%ARGUMENTS_(ID)%VALUES) ) THEN
...
IMPLICIT NONE
  ...
  IF ( ALLOCATED(THIS%ARGUMENTS_) ) THEN
    DO I = 1, SIZE(THIS%ARGUMENTS_)
      ...
      THIS%ARGUMENTS_(I)%NAME = REPEAT(' ',CMDARG_STRLEN)
      IF ( ASSOCIATED(THIS%ARGUMENTS_(I)%VALUES) ) THEN
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIM_K) :: ARGLEN
  INTEGER(KIND=JPIM_K) :: IDX
  INTEGER(KIND=JPIB_K) :: ARGCOUNT
  ...
  IF (COMMAND_ARGID(1) .LE. ARGCOUNT) THEN
    ...
    IDX = COMMAND_ARGID(1)
    CALL GET_COMMAND_ARGUMENT( IDX, VALUE=ARGSTRING, STATUS=STAT )
    ARGLEN = LEN_TRIM(ARGSTRING)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_SKIP_WHITE=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_PUSH_CHAR=7_JPIB_K
  ...
  IF ( .NOT. EOC ) THEN
    ...
    IF ( .NOT.EOC .AND. .NOT.CHAR_VALUE.EQ.'-') THEN
  ...
  ! Push back the modifier character
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_SKIP_WHITE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to skip white characters' )
...
IMPLICIT NONE
  ...
  IS_VALID = ( C .NE. ',' .AND. C .NE. ' ' .AND. C .NE. CHAR(0) .AND. &
&              C .NE. '[' .AND. C .NE. ']' )
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CALL_IS_VALID_FLAG=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_NOT_IS_FLAG=6_JPIB_K
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.IS_VALID, ERRFLAG_NOT_IS_FLAG )
  PP_TRYCALL(ERRFLAG_PUSH_CHAR) PUSH_CHAR(COMMAND_ARGID, HOOKS  )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_CALL_IS_VALID_FLAG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to call IS_VALID_FLAG' )
    CASE (ERRFLAG_NOT_IS_FLAG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Not a valid flag character after modifier "-": "'//CHAR_VALUE//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
#undef PP_PROCEDURE_TYPE
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GET_MATCH_FLAG'
PP_THREAD_SAFE FUNCTION GET_MATCH_FLAG( COMMAND_ARGID, MATCH, EOC, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_POP_CHAR=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_PUSH_CHAR=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_SKIP_WHITE=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_SEQUENCE=6_JPIB_K
  ...
  IF ( CHAR_VALUE .EQ. '^' ) THEN
    ...
    IF ( .NOT. EOC ) THEN
      PP_TRYCALL(ERRFLAG_SKIP_WHITE) SKIP_WHITE( COMMAND_ARGID, CHAR_VALUE, EOC, HOOKS )
    ELSE
      PP_DEBUG_CRITICAL_THROW( ERRFLAG_WRONG_SEQUENCE )
    ENDIF
  ELSE
    IF ( .NOT.EOC ) THEN
      PP_TRYCALL(ERRFLAG_PUSH_CHAR) PUSH_CHAR(COMMAND_ARGID, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_SKIP_WHITE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to skip white characters' )
    CASE (ERRFLAG_WRONG_SEQUENCE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Wrong sequence of characters' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_CHARACTER=10_JPIB_K
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_INVALID_CHARACTER)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Invalid character "'//CHAR_VALUE//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=1) :: CHAR_VALUE
  CHARACTER(LEN=1) :: SEP
  CHARACTER(LEN=CMDARG_STRLEN) :: VALUE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_POP=12_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_MATCH_FLAG=13_JPIB_K
  ...
  IF ( .NOT.EOC ) THEN
    PP_TRYCALL(ERRFLAG_MATCH_FLAG) GET_MATCH_FLAG( COMMAND_ARGID, MATCH, EOC, HOOKS )
  ELSE
    MATCH = .TRUE.
  ENDIF
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_POP) POP_CHAR( COMMAND_ARGID, CHAR_VALUE, EOC, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_POP)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to pop character' )
    CASE (ERRFLAG_MATCH_FLAG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read match flag' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
  ...
  IF ( .NOT.EOC ) THEN
    ...
    DO WHILE(LOOP)
      ...
      IF ( IS_VALID ) THEN
        ...
        I = I + 1_JPIB_K
        PP_DEBUG_CRITICAL_COND_THROW( I.GT.CMDARG_STRLEN, ERRFLAG_OUT_OF_BOUNDS )
      ...
      ELSEIF ( CHAR_VALUE .EQ. ',' .OR. CHAR_VALUE .EQ. ']' ) THEN
        ...
        IF ( LIST%SIZE .EQ. 0 ) THEN
          ...
          PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_ALLOCATE )
          LIST%HEAD%VAL = REPEAT( ' ', CMDARG_STRLEN )
        ...
        ELSE
          ...
          PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_ALLOCATE )
          LIST%TAIL%NEXT%VAL = REPEAT( ' ', CMDARG_STRLEN )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  INTEGER(KIND=JPIB_K), DIMENSION(2), INTENT(INOUT) :: COMMAND_ARGID
  CHARACTER(LEN=CMDARG_STRLEN),          INTENT(OUT)   :: VALUE
  CHARACTER(LEN=1),                   INTENT(OUT)   :: SEP
  LOGICAL,                            INTENT(OUT)   :: EOC
  TYPE(HOOKS_T),                      INTENT(INOUT) :: HOOKS
