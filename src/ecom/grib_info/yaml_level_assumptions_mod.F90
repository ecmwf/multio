...
#define PP_SECTION_NAME 'YAML_LEVEL_ASSUMPTIONS_MOD'
MODULE YAML_LEVEL_ASSUMPTIONS_MOD
...
TYPE :: LEVEL_ASSUMPTIONS_T
  INTEGER(KIND=JPIB_K) :: PRODUCT_DEFINITION_TEMPLATE_NUMBER=-99
  INTEGER(KIND=JPIB_K) :: TYPE_OF_LEVEL_RANGE=-99
  INTEGER(KIND=JPIB_K) :: LENGTH_OF_LEVEL_RANGE_IN_SECONDS=-99
  INTEGER(KIND=JPIB_K) :: TYPE_OF_STATISTICAL_PROCESSING=-99
...
TYPE :: RULE_T
  ...
  TYPE(LEVEL_ASSUMPTIONS_T) :: ASSUMPTIONS
END TYPE
...
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'FREE_LEVEL_ASSUMPTION_RULES'
SUBROUTINE FREE_LEVEL_ASSUMPTION_RULES()
...
END SUBROUTINE FREE_LEVEL_ASSUMPTION_RULES
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  IF ( CFG%GET( 'time-assumption', LEVEL_ASSUMPTION_CFG ) ) THEN
    ...
    CALL READ_LEVEL_ASSUMPTION( LEVEL_ASSUMPTION_CFG, RULE%ASSUMPTIONS, VERBOSE )
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=8), DIMENSION(6)  :: LKEY
  ...
  CNT = 0
  DO I = 1, 6
    IF ( CFG%HAS( LKEY(I) ) ) THEN
      CNT = CNT + 1
      LID = I
    ENDIF
  ENDDO
  PP_DEBUG_CRITICAL_COND_THROW( CNT.GT.1, 7 )

  ! Read rules for level
  IF ( CNT .EQ. 1 ) THEN
    IF ( CFG%GET( LKEY(LID), ATMP  ) ) THEN

      IF ( ALLOCATED(ATMP) ) THEN

        PP_DEBUG_DEVELOP_COND_THROW( SIZE(ATMP).LE.0, 7 )
        PP_DEBUG_DEVELOP_COND_THROW( SIZE(ATMP).GT.1 .AND. LID.EQ.1, 7 )
        PP_DEBUG_DEVELOP_COND_THROW( SIZE(ATMP).GT.1 .AND. LID.EQ.2, 7 )
        PP_DEBUG_DEVELOP_COND_THROW( SIZE(ATMP).GT.1 .AND. LID.EQ.5, 7 )
        PP_DEBUG_DEVELOP_COND_THROW( SIZE(ATMP).GT.1 .AND. LID.EQ.6, 7 )
        ...
        DO I = 1, SIZE(ATMP)
          ...
          IF ( VERBOSE ) THEN
            WRITE(ERROR_UNIT,*) LKEY(LID)//': ', I, MATCHER%LEVEL(I), ATMP(I), SIZE(MATCHER%LEVEL)
          ENDIF
        ENDDO
...
SUBROUTINE READ_LEVEL_ASSUMPTION( CFG, LEVEL_ASSUMPTION, VERBOSE )
  ...
  ! Symbols imported from other modules within the project.
  USE :: OM_CORE_MOD, ONLY: JPIB_K
  USE :: OM_CORE_MOD, ONLY: TYPE_OF_LEVEL_RANGE_INSTANT_E
  USE :: OM_CORE_MOD, ONLY: TYPE_OF_LEVEL_RANGE_FROM_STEP0_E
  USE :: OM_CORE_MOD, ONLY: TYPE_OF_LEVEL_RANGE_FROM_LASTPP_E
  USE :: OM_CORE_MOD, ONLY: TYPE_OF_LEVEL_RANGE_FIXED_SIZE_E
...
IMPLICIT NONE
  ...
  CALL READ_PRODUCT_DEFINITION_TEMPLATE_NUMBER_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )

  ! Read the type of time range
  CALL READ_TYPE_OF_LEVEL_RANGE_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )

  ! Read the type of statistical process
  CALL READ_TYPE_OF_STATISTICAL_PROCESS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )

  ! Read the length of time range
  IF ( CFG%HAS( 'length-of-time-range-in-seconds' ) ) THEN
    CALL READ_LENGTH_OF_LEVEL_RANGE_IN_SECONDS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
  ELSEIF ( CFG%HAS( 'length-of-time-range-in-hours' ) ) THEN
    CALL READ_LENGTH_OF_LEVEL_RANGE_IN_HOURS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
  ELSE
    SELECT CASE (LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE)
    CASE (TYPE_OF_LEVEL_RANGE_INSTANT_E)
      LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = 0
    CASE (TYPE_OF_LEVEL_RANGE_FROM_STEP0_E)
      LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = -1
    CASE (TYPE_OF_LEVEL_RANGE_FROM_LASTPP_E)
      LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = -2
    CASE DEFAULT
      PP_DEBUG_CRITICAL_THROW( 1 )
    END SELECT
  ENDIF
  ...
  CALL READ_EMIT_STEP_ZERO_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
SUBROUTINE READ_PRODUCT_DEFINITION_TEMPLATE_NUMBER_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(FCKIT_CONFIGURATION), INTENT(IN)    :: CFG
  TYPE(LEVEL_ASSUMPTIONS_T),  INTENT(INOUT) :: LEVEL_ASSUMPTION
  LOGICAL,                   INTENT(IN)    :: VERBOSE
  ...
  IF ( CFG%GET( 'product-definition-template-number', ITMP  ) ) THEN
    LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER = ITMP
    IF ( ITMP .NE. 0 .AND. &
&        ITMP .NE. 1 .AND. &
&        ITMP .NE. 8 .AND. &
&        ITMP .NE. 11 ) THEN
    ...
    ENDIF
    IF ( VERBOSE ) THEN
      WRITE(ERROR_UNIT,*) 'ProductDefinitionTemplateNumber: ',  LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER
    ENDIF
  ELSE
    PP_DEBUG_CRITICAL_THROW( 1 )
  ENDIF
...
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'READ_TYPE_OF_LEVEL_RANGE_CFG'
SUBROUTINE READ_TYPE_OF_LEVEL_RANGE_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
IMPLICIT NONE
  ...
  IF ( CFG%GET( 'type-of-time-range', CLTMP  ) ) THEN
    ...
    IF ( IS_INTEGER(CLTMP) ) THEN
      READ(CLTMP,* ) ITMP
      IF ( ALL( ITMP .NE. [ TYPE_OF_LEVEL_RANGE_INSTANT_E,     &
&                           TYPE_OF_LEVEL_RANGE_FROM_STEP0_E,  &
&                           TYPE_OF_LEVEL_RANGE_FROM_LASTPP_E, &
&                           TYPE_OF_LEVEL_RANGE_FIXED_SIZE_E ] ) ) THEN
        PP_DEBUG_CRITICAL_THROW( 1 )
      ENDIF
    ELSE
      SELECT CASE( TOLOWER(CLTMP) )
      CASE ( 'instant' )
        ITMP = TYPE_OF_LEVEL_RANGE_INSTANT_E
      CASE ( 'from-step-zero' )
        ITMP = TYPE_OF_LEVEL_RANGE_FROM_STEP0_E
      CASE ( 'from-last-pp' )
        ITMP = TYPE_OF_LEVEL_RANGE_FROM_LASTPP_E
      CASE ( 'fixed-size' )
        ITMP = TYPE_OF_LEVEL_RANGE_FIXED_SIZE_E
      CASE DEFAULT
        PP_DEBUG_CRITICAL_THROW( 2 )
      END SELECT
    ENDIF
    ! Consistency checks
    IF ( LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER .EQ. 0 .OR. &
&        LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER .EQ. 1 ) THEN
      IF ( ITMP .EQ. TYPE_OF_LEVEL_RANGE_INSTANT_E ) THEN
        LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE = ITMP
      ELSE
        PP_DEBUG_CRITICAL_THROW( 3 )
      ENDIF
    ELSE
    ...
    ENDIF
    IF ( VERBOSE ) THEN
      WRITE(ERROR_UNIT,*) 'TypeOfTimeRange: ',  LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE
    ENDIF
  ...
  ELSE
    IF (ALLOCATED(CLTMP)) DEALLOCATE(CLTMP)
      IF ( LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER .EQ. 0 .OR. &
&          LEVEL_ASSUMPTION%PRODUCT_DEFINITION_TEMPLATE_NUMBER .EQ. 1 ) THEN
         LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE =  TYPE_OF_LEVEL_RANGE_INSTANT_E
    ELSE
...
END SUBROUTINE READ_TYPE_OF_LEVEL_RANGE_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
SUBROUTINE READ_TYPE_OF_STATISTICAL_PROCESS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
&                           TYPE_OF_STATISTICAL_PROCESS_MODE_E ] ) ) THEN
    ...
    IF ( ITMP .EQ. TYPE_OF_STATISTICAL_PROCESS_INSTANT_E ) THEN
      IF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_INSTANT_E ) THEN
        LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING = ITMP
      ELSE
        PP_DEBUG_CRITICAL_THROW( 3 )
      ENDIF
    ELSE
    ...
    ENDIF
    IF ( VERBOSE ) THEN
      WRITE(ERROR_UNIT,*) 'TypeOfStatisticalProcessing: ',  LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING
    ENDIF
    IF (ALLOCATED(CLTMP)) DEALLOCATE(CLTMP)
  ELSE
    IF (ALLOCATED(CLTMP)) DEALLOCATE(CLTMP)
...
PP_ERROR_HANDLER
  ...
  ErrorHandler: BLOCK
    ...
    CASE (2)
      CTTR = REPEAT(' ',1024)
      CTTR='["instant","average","accumul","min","max","severity","mode"]'
...
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'READ_LENGTH_OF_LEVEL_RANGE_IN_SECONDS_CFG'
SUBROUTINE READ_LENGTH_OF_LEVEL_RANGE_IN_SECONDS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
IF ( CFG%GET( 'length-of-time-range-in-seconds', ITMP  ) ) THEN
    ...
    PP_DEBUG_CRITICAL_COND_THROW( MOD(ITMP, 3600).NE.0, 0 )
    ! Consistency checks
    IF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_INSTANT_E ) THEN
      IF ( ITMP .NE. 0 ) THEN
        PP_DEBUG_CRITICAL_THROW( 1 )
      ELSE
        LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = ITMP
      ENDIF
    ELSEIF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_FROM_STEP0_E ) THEN
      IF ( ITMP .NE. -1 ) THEN
        PP_DEBUG_CRITICAL_THROW( 2 )
      ELSE
        LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = ITMP
      ENDIF
    ELSEIF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_FROM_LASTPP_E ) THEN
      IF ( ITMP .NE. -2 ) THEN
        PP_DEBUG_CRITICAL_THROW( 3 )
      ELSE
        LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = ITMP
      ENDIF
    ELSEIF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_FIXED_SIZE_E ) THEN
      IF ( ITMP .LE. 0 ) THEN
        PP_DEBUG_CRITICAL_THROW( 4 )
      ELSE
        LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = ITMP
    ...
    ENDIF
    IF ( VERBOSE ) THEN
      WRITE(ERROR_UNIT,*) 'LengthOfTimeRange: ',  LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS
    ENDIF
  ELSE
    SELECT CASE (LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE)
    CASE (TYPE_OF_LEVEL_RANGE_INSTANT_E)
...
END SUBROUTINE READ_LENGTH_OF_LEVEL_RANGE_IN_SECONDS_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'READ_LENGTH_OF_LEVEL_RANGE_IN_HOURS_CFG'
SUBROUTINE READ_LENGTH_OF_LEVEL_RANGE_IN_HOURS_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
IF ( CFG%GET( 'length-of-time-range-in-hours', ITMP  ) ) THEN
    ...
    ELSEIF ( LEVEL_ASSUMPTION%TYPE_OF_LEVEL_RANGE .EQ. TYPE_OF_LEVEL_RANGE_FIXED_SIZE_E ) THEN
      IF ( ITMP .LE. 0 ) THEN
        PP_DEBUG_CRITICAL_THROW( 4 )
      ELSE
        LEVEL_ASSUMPTION%LENGTH_OF_LEVEL_RANGE_IN_SECONDS = ITMP*3600
      ENDIF
    ELSE
      PP_DEBUG_CRITICAL_THROW( 5 )
    ENDIF
...
END SUBROUTINE READ_LENGTH_OF_LEVEL_RANGE_IN_HOURS_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
SUBROUTINE READ_EMIT_STEP_ZERO_CFG( CFG, LEVEL_ASSUMPTION, VERBOSE )
...
IMPLICIT NONE
  ...
  IF ( CFG%GET( 'emit-step-zero', LTMP  ) ) THEN
    IF (LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .EQ. TYPE_OF_STATISTICAL_PROCESS_ACCUMUL_E ) THEN
      LEVEL_ASSUMPTION%EMIT_STEP_ZERO = LTMP
    ELSEIF (LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .EQ. TYPE_OF_LEVEL_RANGE_INSTANT_E ) THEN
      IF ( LTMP) THEN
        LEVEL_ASSUMPTION%EMIT_STEP_ZERO = LTMP
      ELSE
        PP_DEBUG_CRITICAL_THROW( 1 )
    ...
    ENDIF
    IF ( VERBOSE ) THEN
      WRITE(ERROR_UNIT,*) 'EmitStepZero: ',  LEVEL_ASSUMPTION%EMIT_STEP_ZERO
    ENDIF
  ELSE
    IF (LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .EQ. TYPE_OF_LEVEL_RANGE_INSTANT_E ) THEN
      LEVEL_ASSUMPTION%EMIT_STEP_ZERO = .TRUE.
    ELSE
      IF ( LEVEL_ASSUMPTION%TYPE_OF_STATISTICAL_PROCESSING .NE. TYPE_OF_STATISTICAL_PROCESS_ACCUMUL_E ) THEN
        LEVEL_ASSUMPTION%EMIT_STEP_ZERO = .FALSE.
      ELSE
        PP_DEBUG_CRITICAL_THROW( 3 )
      ENDIF
    ENDIF
...
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'MATCH_LEVEL_ASSUMPTION_RULES'
SUBROUTINE MATCH_LEVEL_ASSUMPTION_RULES( PARAM_ID, LEV_TYPE, REPRES, LEVEL, IS_ENSAMBLE, ASSUMPTIONS )
...
IMPLICIT NONE
  ...
  TYPE(LEVEL_ASSUMPTIONS_T), INTENT(OUT) :: ASSUMPTIONS
...
&                   SPECIAL_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQ. IS_ENSAMBLE ] )
    ...
    LTMP = SPECIAL_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQ. IS_ENSAMBLE
    MATCH = MATCH .AND. LTMP
  ...
  &                   DEFAULT_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQ. IS_ENSAMBLE ] )
      ...
      LTMP = DEFAULT_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQ. IS_ENSAMBLE
      MATCH = MATCH .AND. LTMP
  ...
  ELSE
    ...
    ASSUMPTIONS = SPECIAL_ASSUMPTIONS(RID)%ASSUMPTIONS
  ENDIF
  ...
  IF ( CNT.LT.1 ) THEN
    PP_DEBUG_CRITICAL_THROW( 5 )
  ELSE
    ASSUMPTIONS = DEFAULT_ASSUMPTIONS(RID)%ASSUMPTIONS
  ENDIF


  ! Loggin rules match
  IF ( CNT  .EQ. 1 ) THEN
    WRITE(*,*) ' + LEVEL_ASSUMPTIONS_RULES_LOG: applied rule: '//TRIM(ADJUSTL(RMATCH))//') to field: '//TRIM(ADJUSTL(FLDSTR))
  ELSE
    WRITE(*,*) ' + LEVEL_ASSUMPTIONS_RULES_LOG: applied default rule to field: '//TRIM(ADJUSTL(FLDSTR))
  ENDIF
...
END SUBROUTINE MATCH_LEVEL_ASSUMPTION_RULES
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE YAML_LEVEL_ASSUMPTIONS_MOD
