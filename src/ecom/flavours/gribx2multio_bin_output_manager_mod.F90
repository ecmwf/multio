...
#define PP_FILE_NAME 'gribx2multio_bin_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIBX2MULTIO_BIN_OUTPUT_MANAGER_MOD'
MODULE GRIBX2MULTIO_BIN_MOD
...
!>
!> The `GRIBX2MULTIO_BIN_OUTPUT_MANAGER_T` type is a derived type that extends the
...
!>
TYPE, EXTENDS(OUTPUT_MANAGER_BASE_A) :: GRIBX2MULTIO_BIN_OUTPUT_MANAGER_T
...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: SETUP => GRIBX2MULTIO_BIN_SETUP

  !> @brief Read teh configuration from YAML using fckit
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: READ_CFG_FROM_YAML => GRIBX2MULTIO_BIN_READ_CFG_FROM_YAML

  !> @brief Write fields for the output manager
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_ATM_DP => GRIBX2MULTIO_BIN_WRITE_ATM_DP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_ATM_SP => GRIBX2MULTIO_BIN_WRITE_ATM_SP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_WAM_DP => GRIBX2MULTIO_BIN_WRITE_WAM_DP
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: WRITE_WAM_SP => GRIBX2MULTIO_BIN_WRITE_WAM_SP

  !> @brief Notify that a step is finished
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_STEP => GRIBX2MULTIO_BIN_FLUSH_STEP

  !> @brief Notify that a step is finished and it was the last step
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_LAST_STEP => GRIBX2MULTIO_BIN_FLUSH_LAST_STEP

  !> @brief Notify that a step is finished and it is necessary to dump a restart
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FLUSH_STEP_AND_TRIGGER_RESTART => GRIBX2MULTIO_BIN_FLUSH_STEP_AND_TRIGGER_RESTART

  !> @brief Finalise ancd cleanup teh output manager
  PROCEDURE, NON_OVERRIDABLE, PASS, PUBLIC :: FINALISE => GRIBX2MULTIO_BIN_FINALISE

END TYPE

! Whitelist of public symbols
PUBLIC :: GRIBX2MULTIO_BIN_OUTPUT_MANAGER_T
PUBLIC :: GRIBX2MULTIO_BIN_OMNAME

CONTAINS

#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_READ_CFG_FROM_YAML'
SUBROUTINE GRIBX2MULTIO_BIN_READ_CFG_FROM_YAML( THIS, CFG )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIBX2MULTIO_BIN_OUTPUT_MANAGER_T), INTENT(INOUT) :: THIS
  TYPE(FCKIT_CONFIGURATION),                INTENT(IN)    :: CFG

  ! Local variables
  INTEGER(KIND=JPIM_K) :: KRET
  TYPE(FCKIT_CONFIGURATION) :: GRIBX2MULTIO_BIN_CFG
  CHARACTER(LEN=:), ALLOCATABLE :: CLTMP
  CHARACTER(LEN=1024) :: CTMP_PATH
  LOGICAL :: LTMP
  LOGICAL :: EX
  ...
  IF ( CFG%GET( TOLOWER(GRIBX2MULTIO_BIN_OMNAME), GRIBX2MULTIO_BIN_CFG ) ) THEN

    IF ( ALLOCATED(CLTMP) ) DEALLOCATE(CLTMP)
    IF ( GRIBX2MULTIO_BIN_CFG%GET( 'multio-plans-file', CLTMP  ) ) THEN
    ...
    IF ( ALLOCATED(CLTMP) ) DEALLOCATE(CLTMP)
    IF ( GRIBX2MULTIO_BIN_CFG%GET( 'samples-path', CLTMP  ) ) THEN
      PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CLTMP), 1 )
      PP_DEBUG_CRITICAL_COND_THROW( LEN(CLTMP).GT.LEN(CTMP_PATH), 2 )
      CTMP_PATH=REPEAT( ' ', LEN(CTMP_PATH) )
      CALL OM_REPLACE_ENVVAR_IN_STRING( CLTMP, CTMP_PATH )
      IF ( ALLOCATED(CLTMP) ) DEALLOCATE( CLTMP )
      CALL GRIB_SET_SAMPLES_PATH( TRIM(CTMP_PATH), STATUS=KRET )
    ...
    IF ( GRIBX2MULTIO_BIN_CFG%GET( 'verbose', LTMP  ) ) THEN
      THIS%VERBOSE_ = LTMP
    ELSE
      THIS%VERBOSE_ = .FALSE.
    ENDIF
    ...
    ! Read the verbose flag from YAML file
    IF ( GRIBX2MULTIO_BIN_CFG%GET( 'profile', LTMP  ) ) THEN
      THIS%PROFILE_ = LTMP
    ELSE
      THIS%PROFILE_ = .FALSE.
    ENDIF
    ...
    IF ( GRIBX2MULTIO_BIN_CFG%GET( 'report', LTMP  ) ) THEN
    ...
    CALL GRIBX2MULTIO_BIN_CFG%FINAL()
  ELSE
...
PP_ERROR_HANDLER
  ...
  ErrorHandler: BLOCK
    ...
    CASE (6)
      PP_DEBUG_CREATE_ERROR_MSG( STR, 'Unable to read "samples-path"' )
    CASE (7)
...
END SUBROUTINE GRIBX2MULTIO_BIN_READ_CFG_FROM_YAML
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_SETUP'
SUBROUTINE GRIBX2MULTIO_BIN_SETUP( THIS, YAMLFNAME, PROCESSOR_TOPO, MODEL_PARAMS )
...
IMPLICIT NONE
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) TOLOWER(GRIBX2MULTIO_BIN_OMNAME)//'-output-manager-', PROCESSOR_TOPO%MYPROC_IO, '.log'
...
END SUBROUTINE GRIBX2MULTIO_BIN_SETUP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_WRITE_ATM_DP'
SUBROUTINE GRIBX2MULTIO_BIN_WRITE_ATM_DP( THIS, YDMSG, VALUES_DP )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIBX2MULTIO_BIN_OUTPUT_MANAGER_T), TARGET, INTENT(INOUT) :: THIS
  TYPE(OM_ATM_MSG_T),                               INTENT(IN)    :: YDMSG
  REAL(KIND=JPRD_K), DIMENSION(:),                  INTENT(IN)    :: VALUES_DP

  ! Local variables
  INTEGER(KIND=JPIB_K) :: DATA_LENGTH
  INTEGER(KIND=JPIB_K) :: KRET
  INTEGER(KIND=JPIB_K) :: STAT
...
END SUBROUTINE GRIBX2MULTIO_BIN_WRITE_ATM_DP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_WRITE_ATM_SP'
SUBROUTINE GRIBX2MULTIO_BIN_WRITE_ATM_SP( THIS, YDMSG, VALUES_SP )
...
END SUBROUTINE GRIBX2MULTIO_BIN_WRITE_ATM_SP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_WRITE_WAM_DP'
SUBROUTINE GRIBX2MULTIO_BIN_WRITE_WAM_DP( THIS, YDMSG, VALUES_DP )
...
&                                      YDMSG%IREPRES_,  YDMSG%IUID_, ENCODING_INFO )
  ...
  !
  ! Encode throws an error if an error happens, and return false if the field does not need to be emitted
  IF ( ENCODE_WAM( THIS%MODEL_PAR_, ENCODING_INFO, YDMSG, TIME_HIST, PGMD ) ) THEN
...
END SUBROUTINE GRIBX2MULTIO_BIN_WRITE_WAM_DP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_WRITE_WAM_SP'
SUBROUTINE GRIBX2MULTIO_BIN_WRITE_WAM_SP( THIS, YDMSG, VALUES_SP )
...
END SUBROUTINE GRIBX2MULTIO_BIN_WRITE_WAM_SP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_FLUSH_STEP'
SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_STEP( THIS, KSTEP )
...
IMPLICIT NONE
  ...
  ! Exit point on success
  RETURN
END SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_STEP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_FLUSH_LAST_STEP'
SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_LAST_STEP( THIS, KSTEP )
...
END SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_LAST_STEP
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_FLUSH_STEP_AND_TRIGGER_RESTART'
SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_STEP_AND_TRIGGER_RESTART( THIS, KSTEP )
...
END SUBROUTINE GRIBX2MULTIO_BIN_FLUSH_STEP_AND_TRIGGER_RESTART
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'SUBROUTINE'
#define PP_PROCEDURE_NAME 'GRIBX2MULTIO_BIN_FINALISE'
SUBROUTINE GRIBX2MULTIO_BIN_FINALISE( THIS )
...
END SUBROUTINE GRIBX2MULTIO_BIN_FINALISE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE GRIBX2MULTIO_BIN_MOD
