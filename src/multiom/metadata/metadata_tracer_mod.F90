...
IMPLICIT NONE
  ...
  TYPE :: ENCODE_STACK_FRAME_NODE
    ...
    TYPE(ENCODE_STACK_FRAME_NODE), POINTER :: NEXT => NULL()
  ...
  !>
  !> The `ENCODE_STACK_T` type provides functionality to manage a stack of error frames,
  ...
  TYPE :: ENCODE_STACK_T
    ...
    !> Error frames
    TYPE(ENCODE_STACK_FRAME_NODE), POINTER :: FRAME_STACK => NULL()

  CONTAINS

    !> Initialize the ENCODE_STACK_T structure
    PROCEDURE, PRIVATE, PASS, NON_OVERRIDABLE :: INIT => ENCODE_STACK_INIT_DEFAULT

    !> Copy the error stack to another ENCODE_STACK_T structure
    PROCEDURE, PRIVATE, PASS, NON_OVERRIDABLE :: COPY_TO    => ENCODE_STACK_COPY_TO

    !> Free the error stack (both frames and messages)
    PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => ENCODE_STACK_FREE

    !> Add an error frame to the stack
    PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PUSH_FRAME => ENCODE_STACK_PUSH_FRAME

    !> Add an error message to the current frame
    PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PUSH_MSG => ENCODE_STACK_PUSH_MSG
    ...
    PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT_STACK => ENCODE_STACK_PRINT_STACK
...
IMPLICIT NONE
  ...
  !> Reset frame stack
  !> It it is already emty it will not do anything
...
!>
#define PP_PROCEDURE_TYPE 'PP_THREAD_SAFE FUNCTION'
#define PP_PROCEDURE_NAME 'ENCODE_STACK_PUSH_MSG'
PP_THREAD_SAFE FUNCTION  ENCODE_STACK_PUSH_MSG( THIS, MSG, HOOKS ) RESULT(RET)
...
END FUNCTION ENCODE_STACK_PUSH_MSG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
#define PP_PROCEDURE_TYPE 'PP_THREAD_SAFE FUNCTION'
#define PP_PROCEDURE_NAME 'ENCODE_STACK_PRINT_STACK'
PP_THREAD_SAFE FUNCTION  ENCODE_STACK_PRINT_STACK( THIS, UNIT, HOOKS ) RESULT(RET)
...
END FUNCTION ENCODE_STACK_PRINT_STACK
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE ENCODE_STACK_MOD
