...
!>   - `ASSIGNMENT_BASE_A`: An abstract class that defines the interface for all
...
#define PP_FILE_NAME 'assignment_base_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'ASSIGNMENT_BASE_MOD'
MODULE ASSIGNMENT_BASE_MOD
IMPLICIT NONE
...
!> methods.
!>
TYPE, ABSTRACT :: ASSIGNMENT_BASE_A
CONTAINS
...
!>
!> @brief Container for any assignemtn object of type `ASSIGNMENT_BASE_A`.
!>
!> This container type, `ASSIGNMENT_CONTAINER_T`, is used maily for
!> constructing arrays of assignments
TYPE :: ASSIGNMENT_CONTAINER_T
...
!>
!> @section interface
!> - @param [inout] THIS  - The operation object to be initialized (must be of type `ASSIGNMENT_BASE_A`).
...
!> - @dependency [TYPE] ASSIGNMENT_BASE_A
!>
...
PP_THREAD_SAFE FUNCTION OPERATION_INIT_IF( THIS, CFG, HOOKS ) RESULT( RET )
  ...
  IMPORT :: ASSIGNMENT_BASE_A

IMPLICIT NONE

  !> Dummy arguments
  CLASS(ASSIGNMENT_BASE_A), INTENT(INOUT) :: THIS
...
!>
!> @section interface
!> - @param [inout] THIS   - The operation object (must be of type `ASSIGNMENT_BASE_A`) that performs the matching.
...
!> None.
!>
PP_THREAD_SAFE FUNCTION OPERATION_EVAL_IF( THIS, IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS ) RESULT(RET)
...
!> related to the `ASSIGNMENT_BASE_A` object. It ensures that the operation object is
...
!>
!> This function outputs the current state of the `ASSIGNMENT_BASE_A` object to the
...
!>
!> @brief Container structure that holds a operation object of type `ASSIGNMENT_BASE_A`.
!>
!> This container type, `OPERATION_CONTAINER_T`, is used to manage a operation object
!> that is based on the abstract class `ASSIGNMENT_BASE_A`. The operation object is
...
!>
TYPE :: OPERATION_CONTAINER_T

  CLASS(ASSIGNMENT_BASE_A), POINTER :: OPERATION_ => NULL()

END TYPE

!> White list of public symbols
PUBLIC :: ASSIGNMENT_BASE_A
PUBLIC :: ASSIGNMENT_CONTAINER_T

END MODULE ASSIGNMENT_BASE_MOD
