!>
!> @file grib2_section4_param_factory_mod.F90
!>
!> @brief Module containing the factory function for creating or initializing GRIB2 Param Configurator objects.
!>
!> The `GRIB2_SECTION4_PARAM_FACTORY_MOD` provides a factory function that creates or initializes
...
!>   - @dependency [TYPE] GRIB2_PARAM_CONFIGURATOR_000_MOD::GRIB2_PARAM_CONFIGURATOR_000_T
...
#define PP_SECTION_NAME 'GRIB2_SECTION4_PARAM_FACTORY_MOD'
...
CHARACTER(LEN=*), PARAMETER :: PARAM_CONFIGURATOR_KEY='type'


!>
!> Generic interface to the make encoder function.
INTERFACE MAKE_GRIB2_PARAM_CONFIGURATOR
  MODULE PROCEDURE MAKE_GRIB2_PARAM_CONFIGURATOR_CFG
  MODULE PROCEDURE MAKE_GRIB2_PARAM_CONFIGURATOR_LAZY
END INTERFACE

!>
!> Public symbols (dataTypes)
PUBLIC :: MAKE_GRIB2_PARAM_CONFIGURATOR
...
!> based on the provided parameters. It assigns the proper type (`GRIB2_PARAM_CONFIGURATOR_000_T`)
!> to the `GRIB2_PARAM_CONFIGURATOR` object and configures it using the provided model parameters,
!> ID, and YAML configuration. If verbose mode is enabled, additional debug information
!> is output during the process.
!>
!> @param [inout] GRIB2_PARAM_CONFIGURATOR The GRIB2 Param Configurator object that will be created or initialized.
...
!>   - `GRIB2_PARAM_CONFIGURATOR_000_T`
!>
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see GRIB2_PARAM_CONFIGURATOR_000_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_PARAM_CONFIGURATOR_CFG'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_PARAM_CONFIGURATOR_CFG( GRIB2_PARAM_CONFIGURATOR, SEC4ID, &
&               CFG, OPT, FACTORIES, HOOKS ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  USE :: DATAKINDS_DEF_MOD,               ONLY: JPIB_K
  USE :: GRIB_SECTION_BASE_MOD,           ONLY: GRIB_SECTION_BASE_A
  USE :: GRIB2_SECTION4_PARAM_MOD,        ONLY: GRIB2_SECTION4_PARAM_T
  USE :: GRIB2_SECTION4_PARAMID_MOD,      ONLY: GRIB2_SECTION4_PARAMID_T
  USE :: GRIB2_SECTION4_PARAMIDECMF_MOD,  ONLY: GRIB2_SECTION4_PARAMIDECMF_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB2_PARAM_CONFIGURATOR
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) READ_GRIB2_PARAM_CONFIGURATOR_TYPE_FROM_CFG( CFG, SEC4ID, ID, HOOKS )
  ...
  CASE( 0 )
    ...
    ALLOCATE( GRIB2_SECTION4_PARAM_T::GRIB2_PARAM_CONFIGURATOR, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 1 )

    ALLOCATE( GRIB2_SECTION4_PARAMID_T::GRIB2_PARAM_CONFIGURATOR, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 2 )

    ALLOCATE( GRIB2_SECTION4_PARAMIDECMF_T::GRIB2_PARAM_CONFIGURATOR, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_PARAM_CONFIGURATOR%INIT( CFG, OPT, FACTORIES, HOOKS )
...
END FUNCTION MAKE_GRIB2_PARAM_CONFIGURATOR_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_PARAM_CONFIGURATOR_LAZY'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_PARAM_CONFIGURATOR_LAZY( GRIB2_PARAM_CONFIGURATOR, SEC4ID, &
&               MSG, PAR, OPT, FACTORIES, HOOKS ) RESULT(RET)
  ...
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_PARAM_CONFIGURATOR_ID_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) GET_GRIB2_PARAM_CONFIGURATOR_ID_FROM_MESSAGE( MSG, PAR, SEC4ID, OPT, ID, HOOKS )
  ...
  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_PARAM_CONFIGURATOR%INIT( MSG, PAR, OPT, FACTORIES, HOOKS )
...
END FUNCTION MAKE_GRIB2_PARAM_CONFIGURATOR_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> This function takes a GRIB2 Param Configurator object (`GRIB2_PARAM_CONFIGURATOR`) and performs
...
!>
!> @section interface
!> @param[in,out] GRIB2_PARAM_CONFIGURATOR The GRIB2 Param Configurator object to be destroyed.
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see MAKE_GRIB2_PARAM_CONFIGURATOR
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_GRIB2_PARAM_CONFIGURATOR'
PP_THREAD_SAFE FUNCTION DESTROY_GRIB2_PARAM_CONFIGURATOR( GRIB2_PARAM_CONFIGURATOR, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(GRIB2_PARAM_CONFIGURATOR), ERRFLAG_NOT_ASSOCIATED )

  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  GRIB2_PARAM_CONFIGURATOR%FREE( OPT, DESTRUCTORS, HOOKS )

  ! Initialize the section
  DEALLOCATE( GRIB2_PARAM_CONFIGURATOR, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )

  ! Nullify the pointer after deallocation
  GRIB2_PARAM_CONFIGURATOR => NULL()
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'param_configurator to destroy is not associated' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating param_configurator number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_FREE_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error destructing param_configurator' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION DESTROY_GRIB2_PARAM_CONFIGURATOR
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!> @section interface
!> @param [in]     CFG             YAML configuration object used to configure the GRIB2 Param Configurator object.
!> @param [out]    PARAM_CONFIGURATOR_TYPE   Identifier of the param_configurator type read from the configuration.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'READ_GRIB2_PARAM_CONFIGURATOR_TYPE_FROM_CFG'
PP_THREAD_SAFE FUNCTION READ_GRIB2_PARAM_CONFIGURATOR_TYPE_FROM_CFG( CFG, &
&  SEC4ID, PARAM_CONFIGURATOR_TYPE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(YAML_CONFIGURATION_T), INTENT(IN)    :: CFG
  INTEGER(KINd=JPIB_K),       INTENT(IN)    :: SEC4ID
  INTEGER(KIND=JPIB_K),       INTENT(OUT)   :: PARAM_CONFIGURATOR_TYPE
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  CHARACTER(LEN=:), ALLOCATABLE :: CPARAM_CONFIGURATOR_TYPE
  LOGICAL :: HAS_PARAM_CONFIGURATOR
  LOGICAL :: PARAM_CONFIGURATOR_IS_INTEGER
  ...
  ! Initialization
  PARAM_CONFIGURATOR_TYPE = -1_JPIB_K

  !> Check if configuration has the PARAM_CONFIGURATOR key
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, PARAM_CONFIGURATOR_KEY, HAS_PARAM_CONFIGURATOR, HOOKS )

  !> Read the PARAM_CONFIGURATOR template number
  IF ( HAS_PARAM_CONFIGURATOR ) THEN
    PP_TRYCALL(ERRFLAG_READ_ERROR) YAML_READ_STRING( CFG, PARAM_CONFIGURATOR_KEY, CPARAM_CONFIGURATOR_TYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CPARAM_CONFIGURATOR_TYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL( ERRFLAG_STRING_IS_INTEGER ) STRING_IS_INTEGER( CPARAM_CONFIGURATOR_TYPE, PARAM_CONFIGURATOR_IS_INTEGER, HOOKS )
    IF ( PARAM_CONFIGURATOR_IS_INTEGER ) THEN
       PP_TRYCALL( ERRFLAG_STRING_TO_INTEGER ) STRING_TO_INTEGER( CPARAM_CONFIGURATOR_TYPE, PARAM_CONFIGURATOR_TYPE, HOOKS )
    ELSE
      SELECT CASE (CPARAM_CONFIGURATOR_TYPE)
      CASE ( 'param' )
        PARAM_CONFIGURATOR_TYPE = 0_JPIB_K
      CASE ( 'paramId' )
        PARAM_CONFIGURATOR_TYPE = 1_JPIB_K
      CASE ( 'paramIdECMF' )
        PARAM_CONFIGURATOR_TYPE = 2_JPIB_K
      CASE DEFAULT
        PP_DEBUG_CRITICAL_THROW( ERRFLAG_KEY_IS_NOT_INTEGER )
      END SELECT
    ENDIF
    DEALLOCATE(CPARAM_CONFIGURATOR_TYPE, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )
  ELSE
    ! TODO
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ALLOCATED_AFTER_READ)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'PARAM_CONFIGURATOR ctype not allocated after reading' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating PARAM_CONFIGURATOR number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
...
END FUNCTION READ_GRIB2_PARAM_CONFIGURATOR_TYPE_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
