!>
!> @file intop_parametrization_mod.F90
!>
!> @brief Module containing definitions and procedures for parametrizationeter intops.
!>
!> This module defines the `INTOP_PARAMETRIZATION_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of parametrizationeter intops within the system. PARAMETRIZATIONeter intops allow for
...
#define PP_FILE_NAME 'intop_parametrization_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'INTOP_PARAMETRIZATION_MOD'
MODULE INTOP_PARAMETRIZATION_MOD
...
!>
!> This derived type extends `INTOP_BASE_A` and is used for intoping based on parametrizationeters.
...
!>
TYPE, EXTENDS(INTOP_BASE_A) :: INTOP_PARAMETRIZATION_T
...
CONTAINS
  ...
  !> @brief Initializes the intop parametrizationeter type.
  !> @details This procedure sets up the `INTOP_PARAMETRIZATION_T` type, initializing its components.
  !>
  !> @parametrization [in] this The instance of `INTOP_PARAMETRIZATION_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => INTOP_PARAMETRIZATION_INIT
  ...
  !>
  !> @parametrization [in] this The instance of `INTOP_PARAMETRIZATION_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => INTOP_PARAMETRIZATION_EVAL

  !> @brief Prints the intop's details for debugging or logging.
  !> @details Outputs the intop's configuration and current state.
  !>
  !> @parametrization [in] this The instance of `INTOP_PARAMETRIZATION_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => INTOP_PARAMETRIZATION_PRINT

  !> @brief Frees resources allocated for the intop.
  !> @details Cleans up the `INTOP_PARAMETRIZATION_T` type, deallocating any resources used by the intop.
  !>
  !> @parametrization [in] this The instance of `INTOP_PARAMETRIZATION_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => INTOP_PARAMETRIZATION_FREE
...
PUBLIC :: INTOP_PARAMETRIZATION_T

CONTAINS


!>
!> @brief Initializes the `INTOP_PARAMETRIZATION_T` intop from a YAML configuration.
!>
!> This function initializes the `INTOP_PARAMETRIZATION_T` type based on the provided YAML
!> configuration (`CFG`) and applies any hooks specified in `HOOKS`. The function
!> reads the necessary parametrizationeters from the configuration and sets up the intop structure.
!>
!> @parametrization [inout] THIS The intop object (`INTOP_PARAMETRIZATION_T`) that will be initialized.
!> @parametrization [in]    CFG  The YAML configuration object containing the intop settings.
!> @parametrization [in]    OPT The generic options to be used to initialize the intop.
...
#define PP_PROCEDURE_NAME 'INTOP_PARAMETRIZATION_INIT'
FUNCTION INTOP_PARAMETRIZATION_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(INTOP_PARAMETRIZATION_T),    INTENT(INOUT) :: THIS
...
END FUNCTION INTOP_PARAMETRIZATION_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a intop parametrizationeter with a parametrization and parametrizationeter.
!>
...
!>
!> @parametrization [inout] THIS  The intop object (`INTOP_PARAMETRIZATION_T`) used for matching.
!> @parametrization [in]    MSG   The parametrization (`FORTRAN_MESSAGE_T`) that is checked against the intop.
!> @parametrization [in]    PAR   The parametrizationeter object (`PARAMETRIZATIONETRIZATION_T`) used in the matching process.
!> @parametrization [out]   MATCH Logical flag indicating whether the parametrization and parametrizationeter match the intop's criteria.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_PARAMETRIZATION_EVAL'
FUNCTION INTOP_PARAMETRIZATION_EVAL( THIS, MSG, PAR, RESULT, HOOKS ) RESULT(RET)
...
END FUNCTION INTOP_PARAMETRIZATION_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @parametrization [inout] THIS    The intop object (`INTOP_PARAMETRIZATION_T`) whose details are to be printed.
!> @parametrization [in]    UNIT    The output unit (file or console) where the intop's details will be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_PARAMETRIZATION_PRINT'
FUNCTION INTOP_PARAMETRIZATION_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Local parametrizations
  CHARACTER(LEN=16) :: PARAMETRIZATION_NAME
  ...
  PARAMETRIZATION_NAME = REPEAT(' ',16)

  !> Convert IDs to names
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_ENUM_TO_STRING) IPARINTFLDS2CPARINTFLDS( THIS%PARAMETRIZATION_ID_, PARAMETRIZATION_NAME, HOOKS )

  !> Print the parametrization intop
...
END FUNCTION INTOP_PARAMETRIZATION_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @parametrization [inout] THIS   The intop object (`INTOP_PARAMETRIZATION_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_PARAMETRIZATION_FREE'
FUNCTION INTOP_PARAMETRIZATION_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION INTOP_PARAMETRIZATION_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE INTOP_PARAMETRIZATION_MOD
