...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'dump_values', HAS_KEY, HOOKS )

  ! Read or apply the default configuration
  IF ( HAS_KEY ) THEN
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_LOGICAL( CFG, 'dump_values', THIS%DUMP_VALUES_, HOOKS )
  ELSE
    THIS%DUMP_VALUES_ = .FALSE.
  ENDIF
...
&                                     PROCESSOR_TOPO%MYPROC_IO, THIS%TID_, HOOKS )
  ...
  ! Parameters
  PP_TRYCALL(ERRFLAG_CREATE_PAR_FILE_NAME) PAR_CREATE_NAME( THIS%OUT_DIR_, PROCESSOR_TOPO%MYPROC_IO, PAR_FNAME, HOOKS )
  PP_TRYCALL(ERRFLAG_WOPEN_PAR_FILE) PAR_WOPEN( TRIM(PAR_FNAME), PAR_UNIT, HOOKS )
  ...
  ! Messages
  THIS%CNT_ = 0
  PP_TRYCALL(ERRFLAG_CREATE_MSG_FILE_NAME) MSG_CREATE_NAME( THIS%OUT_DIR_, THIS%CNT_, PROCESSOR_TOPO%MYPROC_IO, MSG_FNAME, HOOKS )
  PP_TRYCALL(ERRFLAG_WOPEN_MSG_FILE) MSG_WOPEN( TRIM(MSG_FNAME), THIS%MSG_UNIT_, HOOKS )

  ! Values
  IF ( THIS%DUMP_VALUES_ ) THEN
    PP_TRYCALL(ERRFLAG_CREATE_VAL_FILE_NAME) VAL_CREATE_NAME( THIS%OUT_DIR_, THIS%CNT_, PROCESSOR_TOPO%MYPROC_IO, VAL_FNAME, HOOKS )
    PP_TRYCALL(ERRFLAG_WOPEN_VAL_FILE) VAL_WOPEN( TRIM(VAL_FNAME), THIS%VAL_UNIT_, HOOKS )
  ENDIF
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_MSG_CREATE_NAME) MSG_CREATE_NAME( THIS%OUT_DIR_, THIS%CNT_, THIS%TOPOLOGY_%MYPROC_IO, MSG_FNAME, HOOKS )
  PP_TRYCALL(ERRFLAG_MSG_WOPEN) MSG_WOPEN( TRIM(MSG_FNAME), THIS%MSG_UNIT_, HOOKS )


  ! Values
  IF ( THIS%DUMP_VALUES_ ) THEN
    PP_TRYCALL(ERRFLAG_VAL_CLOSE)  VAL_CLOSE( THIS%VAL_UNIT_, HOOKS )
    PP_TRYCALL(ERRFLAG_VAL_CREATE_NAME)  VAL_CREATE_NAME( THIS%OUT_DIR_, THIS%CNT_, THIS%TOPOLOGY_%MYPROC_IO, VAL_FNAME, HOOKS )
    PP_TRYCALL(ERRFLAG_VAL_WOPEN)  VAL_WOPEN( TRIM(VAL_FNAME), THIS%VAL_UNIT_, HOOKS )
  ENDIF
