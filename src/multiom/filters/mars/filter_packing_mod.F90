...
!>
!> @brief Module containing definitions and procedures for packing filters.
!>
!> This module defines the `FILTER_PACKING_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of packing filters within the system. PACKING filters allow for
...
#define PP_FILE_NAME 'filter_packing_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_PACKING_MOD'
MODULE FILTER_PACKING_MOD
...
!>
!> @brief Filter type for managing level types (PACKING).
!>
!> This derived type `FILTER_PACKING_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_PACKING_T
  ...
  INTEGER(KIND=JPIB_K) :: PACKING_

  !> Holds the keyset of level types for filtering.
  TYPE(KEYSET_INT64_T) :: PACKINGS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_PACKING_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_PACKING_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_PACKING_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_PACKING_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_PACKING_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_PACKING_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_PACKING_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_PACKING_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CPACKING2IPACKING
...
!>
!> @see FILTER_PACKING_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PACKING_INIT'
FUNCTION FILTER_PACKING_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_PACKING_T),     INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CPACKING2IPACKING
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%PACKINGS_%INIT( HOOKS )
  THIS%USE_KEYSET_ = .FALSE.

  !> Associate the filter procedure
  P_CPACKING2IPACKING => CPACKING2IPACKING
  ...
  IF ( THIS%FILTER_TYPE_ .NE. FLT_INT_HAS_E .AND. THIS%FILTER_TYPE_ .NE. FLT_INT_LACKS_E ) THEN
    ...
    IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%PACKINGS_, P_CPACKING2IPACKING, HOOKS )
      THIS%USE_KEYSET_ = .TRUE.

    ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%PACKING_, P_CPACKING2IPACKING, HOOKS )
...
END FUNCTION FILTER_PACKING_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_PACKING_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_PACKING_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_PACKING_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PACKING_MATCH'
FUNCTION FILTER_PACKING_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = (MSG%PACKING .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = (MSG%PACKING .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )

    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%PACKINGS_%MATCH( MSG%PACKING, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%PACKING_ .EQ. MSG%PACKING)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%PACKING_ .EQ. MSG%PACKING)
    ENDIF
...
END FUNCTION FILTER_PACKING_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_PACKING_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_PACKING_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PACKING_PRINT'
FUNCTION FILTER_PACKING_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'PACKING FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_PACKING_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_PACKING_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_PACKING_FREE'
FUNCTION FILTER_PACKING_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%PACKINGS_%FREE( HOOKS )
...
END FUNCTION FILTER_PACKING_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_PACKING_MOD
