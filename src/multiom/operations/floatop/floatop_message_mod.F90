!>
!> @file floatop_message_mod.F90
!>
!> @brief Module containing definitions and procedures for messageeter floatops.
!>
!> This module defines the `FLOATOP_MESSAGE_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of messageeter floatops within the system. MESSAGEeter floatops allow for
...
#define PP_FILE_NAME 'floatop_message_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FLOATOP_MESSAGE_MOD'
MODULE FLOATOP_MESSAGE_MOD
...
!>
!> This derived type extends `FLOATOP_BASE_A` and is used for floatoping based on messageeters.
...
!>
TYPE, EXTENDS(FLOATOP_BASE_A) :: FLOATOP_MESSAGE_T
...
CONTAINS
  ...
  !> @brief Initializes the floatop messageeter type.
  !> @details This procedure sets up the `FLOATOP_MESSAGE_T` type, initializing its components.
  !>
  !> @message [in] this The instance of `FLOATOP_MESSAGE_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FLOATOP_MESSAGE_INIT
  ...
  !>
  !> @message [in] this The instance of `FLOATOP_MESSAGE_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => FLOATOP_MESSAGE_EVAL

  !> @brief Prints the floatop's details for debugging or logging.
  !> @details Outputs the floatop's configuration and current state.
  !>
  !> @message [in] this The instance of `FLOATOP_MESSAGE_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FLOATOP_MESSAGE_PRINT

  !> @brief Frees resources allocated for the floatop.
  !> @details Cleans up the `FLOATOP_MESSAGE_T` type, deallocating any resources used by the floatop.
  !>
  !> @message [in] this The instance of `FLOATOP_MESSAGE_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FLOATOP_MESSAGE_FREE
...
PUBLIC :: FLOATOP_MESSAGE_T

CONTAINS


!>
!> @brief Initializes the `FLOATOP_MESSAGE_T` floatop from a YAML configuration.
!>
!> This function initializes the `FLOATOP_MESSAGE_T` type based on the provided YAML
!> configuration (`CFG`) and applies any hooks specified in `HOOKS`. The function
!> reads the necessary messageeters from the configuration and sets up the floatop structure.
!>
!> @message [inout] THIS The floatop object (`FLOATOP_MESSAGE_T`) that will be initialized.
!> @message [in]    CFG  The YAML configuration object containing the floatop settings.
!> @message [in]    OPT The generic options to be used to initialize the floatop.
...
#define PP_PROCEDURE_NAME 'FLOATOP_MESSAGE_INIT'
FUNCTION FLOATOP_MESSAGE_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FLOATOP_MESSAGE_T),    INTENT(INOUT) :: THIS
...
END FUNCTION FLOATOP_MESSAGE_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a floatop messageeter with a message and messageeter.
!>
...
!>
!> @message [inout] THIS  The floatop object (`FLOATOP_MESSAGE_T`) used for matching.
!> @message [in]    MSG   The message (`FORTRAN_MESSAGE_T`) that is checked against the floatop.
!> @message [in]    PAR   The messageeter object (`MESSAGEETRIZATION_T`) used in the matching process.
!> @message [out]   MATCH Logical flag indicating whether the message and messageeter match the floatop's criteria.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FLOATOP_MESSAGE_EVAL'
FUNCTION FLOATOP_MESSAGE_EVAL( THIS, MSG, PAR, RESULT, HOOKS ) RESULT(RET)
...
END FUNCTION FLOATOP_MESSAGE_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @message [inout] THIS    The floatop object (`FLOATOP_MESSAGE_T`) whose details are to be printed.
!> @message [in]    UNIT    The output unit (file or console) where the floatop's details will be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FLOATOP_MESSAGE_PRINT'
FUNCTION FLOATOP_MESSAGE_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
END FUNCTION FLOATOP_MESSAGE_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @message [inout] THIS   The floatop object (`FLOATOP_MESSAGE_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FLOATOP_MESSAGE_FREE'
FUNCTION FLOATOP_MESSAGE_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION FLOATOP_MESSAGE_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FLOATOP_MESSAGE_MOD
