...
template<typename T>
std::vector<T> expectedPayloadInverseDifference(std::vector<std::vector<T>> payloads) {
    ...
    std::transform(payloads[0].begin(), payloads[0].end(), payloads[payloads.size() - 1].begin(), expected.begin(),
                   [](double vf, double vl) { return vf - vl; });
    return expected;
}
...
CASE("Statistics Action Inverse Difference Random") {
    ...
    auto payloadExpected = expectedPayloadInverseDifference(payloads);
