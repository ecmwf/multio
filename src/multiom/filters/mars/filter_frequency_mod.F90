!>
!> @file filter_frequency_mod.F90
!>
!> @brief Module containing definitions and procedures for frequency filters.
!>
!> This module defines the `FILTER_FREQUENCY_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of frequency filters within the system. Frequency filters allow for
...
#define PP_FILE_NAME 'filter_frequency_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_FREQUENCY_MOD'
MODULE FILTER_FREQUENCY_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_FREQUENCY_T
  ...
  !> @details If `.TRUE.`, the `FREQUENCIES_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  INTEGER(KIND=JPIB_K) :: FREQUENCY_
  ...
  TYPE(KEYSET_INT64_T) :: FREQUENCIES_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_FREQUENCY_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_FREQUENCY_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_FREQUENCY_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_FREQUENCY_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_FREQUENCY_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_FREQUENCY_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_FREQUENCY_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_FREQUENCY_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_FREQUENCY_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_FREQUENCY_FREE
...
PUBLIC :: FILTER_FREQUENCY_T

CONTAINS


!>
!> @brief Initializes the `FILTER_FREQUENCY_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_FREQUENCY_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_FREQUENCY_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_FREQUENCY_INIT'
FUNCTION FILTER_FREQUENCY_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_FREQUENCY_T),  INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%FREQUENCIES_%INIT( HOOKS )
  ...
  !> Read the frequency to be used in the filter
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%FREQUENCIES_, HOOKS )
...
END FUNCTION FILTER_FREQUENCY_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_FREQUENCY_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_FREQUENCY_MATCH'
FUNCTION FILTER_FREQUENCY_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%FREQUENCIES_%MATCH( MSG%FREQUENCY, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%FREQUENCY_ .EQ. MSG%FREQUENCY)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%FREQUENCY_ .EQ. MSG%FREQUENCY)
    ENDIF

  CASE( FLT_INT_GT_E )

    MATCH = (MSG%FREQUENCY .GT. THIS%FREQUENCY_ )

  CASE( FLT_INT_GE_E )

    MATCH = (MSG%FREQUENCY .GE. THIS%FREQUENCY_ )

  CASE( FLT_INT_LE_E )

    MATCH = (MSG%FREQUENCY .LE. THIS%FREQUENCY_ )

  CASE( FLT_INT_LT_E )

    MATCH = (MSG%FREQUENCY .LT. THIS%FREQUENCY_ )
...
END FUNCTION FILTER_FREQUENCY_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_FREQUENCY_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_FREQUENCY_PRINT'
FUNCTION FILTER_FREQUENCY_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'FREQUENCY FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_FREQUENCY_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_FREQUENCY_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_FREQUENCY_FREE'
FUNCTION FILTER_FREQUENCY_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%FREQUENCIES_%FREE( HOOKS )
...
END FUNCTION FILTER_FREQUENCY_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_FREQUENCY_MOD
