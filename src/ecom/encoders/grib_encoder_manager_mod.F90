...
MODULE GRIB_ENCODER_MANAGER_MOD
  ...
  USE :: GRIB_ENCODERS_FACTORY_MOD, ONLY: MAKE_FCN_IF
  USE :: GRIB_ENCODERS_FACTORY_MOD, ONLY: DESTROY_FCN_IF
...
!> @brief Unit used to log fields that do not need to be encoded
INTEGER(KIND=JPIB_K) :: LOG_UNIT = -99
LOGICAL :: LMESSAGEHIST = .FALSE.
...
SUBROUTINE UTILS_INIT( CFG, MODEL_PARAMS )
  ...
  USE :: GENERAL_RUNTIME_UTILS_MOD,     ONLY: GENERAL_RUNTIME_INIT
  ...
  USE :: COMPRESSION_RUNTIME_UTILS_MOD, ONLY: COMPRESSION_RUNTIME_INIT
...
IMPLICIT NONE
  ...
  CALL GENERAL_RUNTIME_INIT( CFG, MODEL_PARAMS )
  ...
  CALL COMPRESSION_RUNTIME_INIT( CFG, MODEL_PARAMS )
...
SUBROUTINE UTILS_FREE(  )
  ...
  USE :: GENERAL_RUNTIME_UTILS_MOD,     ONLY: GENERAL_RUNTIME_FREE
  ...
  USE :: COMPRESSION_RUNTIME_UTILS_MOD, ONLY: COMPRESSION_RUNTIME_FREE
...
SUBROUTINE MAKE_ENCODERS( CFG, MODEL_PARAMS, METADATA_KIND, VERBOSE, MIOH )
  ...
  USE :: GRIB_ENCODERS_FACTORY_MOD, ONLY: MAKE_ENCODER
  USE :: GRIB_ENCODERS_FACTORY_MOD, ONLY: MAKE_FCN_IF
  USE :: GRIB_ENCODERS_FACTORY_MOD, ONLY: DESTROY_FCN_IF
...
IMPLICIT NONE
  ...
  ! Logging fields to be encoded
  IF ( VERBOSE ) THEN
    LMESSAGEHIST=.TRUE.
    OPEN(NEWUNIT=LOG_UNIT, FILE='check_valid_message_history.log', STATUS='UNKNOWN', ACTION='WRITE', FORM='FORMATTED')
  ELSE
    LMESSAGEHIST=.FALSE.
  ENDIF
  ...
  IF ( .NOT.ALLOCATED(ENCODERS) ) THEN
    ...
    IF ( PRESENT(MIOH) ) THEN
      ...
      IF ( LGENABLE_GRIB1_GG_P ) THEN
        CALL MAKE_ENCODER( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF

      IF ( LGENABLE_GRIB1_GG_T ) THEN
        CALL MAKE_ENCODER( THETA_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( THETA_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_GG_V ) THEN
        CALL MAKE_ENCODER( VORTICITY_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( VORTICITY_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_GG_S ) THEN
        CALL MAKE_ENCODER( SURFACE_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( SURFACE_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_GG_M ) THEN
        CALL MAKE_ENCODER( MODEL_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( MODEL_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_GG_WI ) THEN
        CALL MAKE_ENCODER( WAVE_INT_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( WAVE_INT_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_GG_WS ) THEN
        CALL MAKE_ENCODER( WAVE_SPEC_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( WAVE_SPEC_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      ...
      IF ( LGENABLE_GRIB1_SH_P ) THEN
        CALL MAKE_ENCODER( PRESSURE_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( PRESSURE_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_SH_T ) THEN
        CALL MAKE_ENCODER( THETA_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( THETA_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_SH_V ) THEN
        CALL MAKE_ENCODER( VORTICITY_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( VORTICITY_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_SH_S ) THEN
        CALL MAKE_ENCODER( SURFACE_E, REPRES_SPECTRAL_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( SURFACE_E, REPRES_SPECTRAL_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB1_SH_M ) THEN
        CALL MAKE_ENCODER( MODEL_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( MODEL_LEVEL_E, REPRES_SPECTRAL_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF

      ! Grib2 gridded encoders
      IF ( LGENABLE_GRIB2_GG_P ) THEN
        CALL MAKE_ENCODER( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_T ) THEN
        CALL MAKE_ENCODER( THETA_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( THETA_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_V ) THEN
        CALL MAKE_ENCODER( VORTICITY_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( VORTICITY_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_S ) THEN
        CALL MAKE_ENCODER( SURFACE_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( SURFACE_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_M ) THEN
        CALL MAKE_ENCODER( MODEL_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( MODEL_LEVEL_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_WI ) THEN
        CALL MAKE_ENCODER( WAVE_INT_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( WAVE_INT_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_GG_WS ) THEN
        CALL MAKE_ENCODER( WAVE_SPEC_E, REPRES_GRIDDED_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( WAVE_SPEC_E, REPRES_GRIDDED_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF

      ! Grib2 spherical harmonics encoders
      IF ( LGENABLE_GRIB2_SH_P ) THEN
        CALL MAKE_ENCODER( PRESSURE_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( PRESSURE_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_SH_T ) THEN
        CALL MAKE_ENCODER( THETA_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( THETA_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_SH_V ) THEN
        CALL MAKE_ENCODER( VORTICITY_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( VORTICITY_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_SH_S ) THEN
        CALL MAKE_ENCODER( SURFACE_E, REPRES_SPECTRAL_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( SURFACE_E, REPRES_SPECTRAL_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
      IF ( LGENABLE_GRIB2_SH_M ) THEN
        CALL MAKE_ENCODER( MODEL_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( MODEL_LEVEL_E, REPRES_SPECTRAL_E, GRIB2_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P, MIOH=MIOH )
      ENDIF
    ...
    ELSE
      ...
      IF ( LGENABLE_GRIB1_GG_P ) THEN
        CALL MAKE_ENCODER( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E, MAKE_FCN_P, DESTROY_FCN_P )
        EIDX = ENCODER_IDX( PRESSURE_LEVEL_E, REPRES_GRIDDED_E, GRIB1_E )
        CALL ENCODERS( EIDX )%CREATE( CFG, MODEL_PARAMS, METADATA_KIND, MAKE_FCN_P, DESTROY_FCN_P )
      ENDIF
...
IMPLICIT NONE
  ...
  ! Logging fields to be encoded
  INQUIRE(UNIT=LOG_UNIT, OPENED=EX)
  IF ( EX ) THEN
    CLOSE(UNIT=LOG_UNIT)
  ENDIF
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=*),                   INTENT(IN)    :: METADATA_KIND
  PROCEDURE(MAKE_FCN_IF),    POINTER, INTENT(IN)    :: MAKE_FCN
  PROCEDURE(DESTROY_FCN_IF), POINTER, INTENT(IN)    :: DESTROY_FCN
  TYPE(MULTIO_HANDLE), OPTIONAL,      INTENT(IN)    :: MIOH
...
FUNCTION ENCODE_ATM( MODEL_PARAMS, ENCODING_INFO, MSG, TIME_HIST, METADATA ) RESULT(EX)
...
IMPLICIT NONE
  ...
  ! Get the previous emission step for the current message
  IF ( ENCODING_INFO%TIME_HISTORY%IS_EMPTY() ) THEN
    PREVSTEP = -1;
  ELSE
    TMP = ENCODING_INFO%TIME_HISTORY%GET( 1_JPIB_K, PREVSTEP )
  ENDIF

  IF ( COMPUTE_CURRENT_TIME( MODEL_PARAMS, ENCODING_INFO, MSG%ISTEP_, TIME_HIST, CURR_TIME ) ) THEN

    ! Get the encoder index
    IDX = ENCODER_IDX( ENCODING_INFO%GRIB_INFO, MSG )

    ! Call the encoder
    EX = ENCODERS(IDX)%ENCODE_ATM( MODEL_PARAMS, ENCODING_INFO%GRIB_INFO, TIME_HIST, CURR_TIME, MSG, METADATA )

    IF ( LMESSAGEHIST ) THEN
      IF ( ENCODING_INFO%TIME_HISTORY%IS_EMPTY() ) THEN
        WRITE(LOG_UNIT,'(2L10,9I10)') &
&        .TRUE., &
&        ENCODING_INFO%GRIB_INFO%IS_STEP0_VALID_, &
&        MSG%PARAM_ID_, &
&        MSG%IUID_, &
&        MSG%IPREF_, &
&        PREVSTEP, &
&        MSG%ISTEP_, &
&        ENCODING_INFO%GRIB_INFO%TYPE_OF_STATISTICAL_PROCESS_, &
&        ENCODING_INFO%GRIB_INFO%TYPE_OF_TIME_RANGE_, &
&        ENCODING_INFO%GRIB_INFO%OVERALL_LENGTH_OF_TIME_RANGE_
      ELSE
        WRITE(LOG_UNIT,'(2L10,9I10)') &
&        .TRUE., &
&        ENCODING_INFO%GRIB_INFO%IS_STEP0_VALID_, &
...
FUNCTION ENCODE_WAM( MODEL_PARAMS, ENCODING_INFO, MSG, TIME_HIST, METADATA ) RESULT(EX)
...
IMPLICIT NONE
  ...
  IF ( COMPUTE_CURRENT_TIME( MODEL_PARAMS, ENCODING_INFO, MSG%ISTEP_, TIME_HIST, CURR_TIME ) ) THEN
    ...
    ! Call the encoder
    EX = ENCODERS(IDX)%ENCODE_WAM( MODEL_PARAMS, ENCODING_INFO%GRIB_INFO, TIME_HIST, CURR_TIME, MSG, METADATA )
...
&        ENCODING_INFO%GRIB_INFO%OVERALL_LENGTH_OF_TIME_RANGE_
      ENDIF
    ENDIF
    ! TODO: Log encoded and not encoded fields
    EX = .FALSE.
  ENDIF
