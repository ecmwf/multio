...
PUBLIC :: F_C_INJECT_CHECKSUM_WRAPPER
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'F_C_INJECT_CHECKSUM_WRAPPER'
PP_THREAD_SAFE FUNCTION F_C_INJECT_CHECKSUM_WRAPPER( WRAPPER, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTERFACE
    ...
    IMPLICIT NONE
      ...
      INTEGER(C_INT) :: RET
    END FUNCTION C_HASH_INT8_ARRAY
  END INTERFACE

  ! Local error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRAPPER_NOT_ASSOCIATED=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ARRAY_NOT_ASSOCIATED=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_COMPUTE_HASH=3_JPIB_K
  ...
  ! Get the fortran pointer to the c-object
  PP_TRYCALL(ERRFLAG_COMPUTE_HASH) C_HASH_INT8_ARRAY( &
&        WRAPPER(1)%BYTES, &
&        WRAPPER(1)%BUF_SIZE, &
&        C_LOC(WRAPPER(1)%HASH) )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_COMPUTE_HASH)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to compute hash' )
...
END FUNCTION F_C_INJECT_CHECKSUM_WRAPPER
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION F_C_GET_INFO_WRAPPER( WRAPPER, OBJ_ID, OBJ_SZ, BUF_SZ, HASH, HOOKS, VERIFY_CHECKSUM ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(HOOKS_T),        INTENT(INOUT) :: HOOKS
  LOGICAL, OPTIONAL,    INTENT(IN)    :: VERIFY_CHECKSUM

  ! Function result
  INTEGER(KIND=JPIB_K) :: RET

  ! Local variables
  TYPE(F_C_WRAPPER_T), POINTER, DIMENSION(:) :: LOC_WRAPPER
  LOGICAL :: LOC_VERIFY_CHECKSUM
  INTEGER(KIND=C_SIZE_T), TARGET :: CURR_CHECKSUM
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_COMPUTE_CHECKSUM=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_CHECKSUM=4_JPIB_K
  ...
  ! Optional arguments
  IF ( PRESENT(VERIFY_CHECKSUM) ) THEN
    LOC_VERIFY_CHECKSUM = VERIFY_CHECKSUM
  ELSE
    LOC_VERIFY_CHECKSUM = .FALSE.
  ENDIF
  ...
  ! Extract the wrapper
  LOC_WRAPPER => NULL()
  CALL C_F_POINTER( WRAPPER, LOC_WRAPPER, [1] )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(LOC_WRAPPER), ERRFLAG_UNABLE_TO_EXTRACT_WRAPPER )

  ! Assign output variables
  OBJ_ID = INT(LOC_WRAPPER(1)%OBJECT_ID, KIND=JPIB_K )
  OBJ_SZ = INT(LOC_WRAPPER(1)%OBJ_SIZE, KIND=JPIB_K )
  BUF_SZ = INT(LOC_WRAPPER(1)%BUF_SIZE, KIND=JPIB_K )
  HASH   = INT(LOC_WRAPPER(1)%HASH, KIND=JPIB_K )

  IF ( LOC_VERIFY_CHECKSUM ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_COMPUTE_CHECKSUM) C_HASH_INT8_ARRAY( &
&        LOC_WRAPPER(1)%BYTES, &
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CHARACTER(LEN=32) :: REF_CHECKSUM
    CHARACTER(LEN=32) :: CUR_CHECKSUM
    ...
    CASE (ERRFLAG_UNABLE_TO_EXTRACT_WRAPPER)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to extract the wrapper' )
    CASE (ERRFLAG_UNABLE_TO_COMPUTE_CHECKSUM)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to compute checksum' )
    CASE (ERRFLAG_INVALID_CHECKSUM)
      REF_CHECKSUM = REPEAT(' ', 32)
      CUR_CHECKSUM = REPEAT(' ', 32)
      WRITE(REF_CHECKSUM, '(I32)', IOSTAT=STAT) LOC_WRAPPER(1)%HASH
      WRITE(CUR_CHECKSUM, '(I32)', IOSTAT=STAT) CURR_CHECKSUM
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Invalid checksum' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Expected checksum: "'//TRIM(ADJUSTL(REF_CHECKSUM))//'"' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Computed checksum: "'//TRIM(ADJUSTL(CUR_CHECKSUM))//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
