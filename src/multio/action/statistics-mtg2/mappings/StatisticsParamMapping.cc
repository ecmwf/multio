...
std::optional<std::int64_t> StatisticsParamMapping::getMapping(std::int64_t param,
                                                               std::int64_t typeOfStatisticalProcessing) const {
...
void StatisticsParamMapping::applyMapping(message::Metadata& metadata, std::int64_t typeOfStatisticalProcessing,
                                          bool strict) const {
    // Read param directly from metadata (always int64 from IFS model messages)
    auto paramOpt = metadata.getOpt<std::int64_t>("param");
    if (paramOpt.has_value()) {
        auto paramOld = *paramOpt;
        auto paramNew = getMapping(paramOld, typeOfStatisticalProcessing);
        if (paramNew.has_value()) {
            metadata.set("param", *paramNew);
            return;
        }
        if (!strict) {
            return;
        ...
        os << "Mapping for param=" << paramOld << " and typeOfStatisticalProcessing=" << typeOfStatisticalProcessing
...
void StatisticsParamMapping::applyMapping(message::Metadata& metadata, const std::string& opname, bool strict) const {
    if (auto search = opname_to_typeOfStatisticalProcessing.find(opname);
        search != opname_to_typeOfStatisticalProcessing.end()) {
