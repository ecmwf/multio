...
CONTAINS
  ...
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: GENERATING_RULE =>  CACHED_ENCODER_GENERATING_RULE
  ...
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRINT_RULE      =>  CACHED_ENCODER_PRINT_RULE
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_DUMP_SAMPLE=9_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SAFE_LOAD1=10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SAFE_LOAD2=11_JPIB_K
  ...
  ELSE
    ...
    IF ( MSG%REPRES .EQ. REPRES_SPHERICALHARMONICS_E ) THEN
      ...
      !!  // Hack suggested by Shahram (Modified)
      ...
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'numberOfDataPoints', 6_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'numberOfValues', 6_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'bitsPerValue', 16_JPIB_K )
      !! PP_TRYCALL(ERRFLAG_SET_METADATA) THIS%METADATA_%SET( 'values', FAKE_VALUES, HOOKS )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'typeOfFirstFixedSurface', 105_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'scaleFactorOfFirstFixedSurface', 0_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'scaledValueOfFirstFixedSurface', 0_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'gridDefinitionTemplateNumber', 50_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'J', 1_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'K', 1_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'M', 1_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'spectralType', 1_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'spectralMode', 1_JPIB_K )
      PP_METADATA_SET( THIS%METADATA_, ERRFLAG_SET_METADATA, 'dataRepresentationTemplateNumber', 51_JPIB_K )

    ELSEIF ( MSG%REPRES .EQ. REPRES_GAUSSIANGRID_E ) THEN

      ! Hack to convert the regular_gg sample to reduced_gg (add pl array)
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_SAFE_LOAD1) THIS%METADATA_%SAFE_LOAD( HOOKS )
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_SAFE_LOAD2) THIS%METADATA_%SAFE_LOAD( HOOKS )

  PP_LOG_DEVELOP_STR( ' * Sample is now done' )
  ...
  ! PP_TRYCALL(ERRFLAG_DUMP_SAMPLE) THIS%METADATA_%DUMP_SAMPLE( 'xxx.grib', HOOKS )

  ! WRITE(*,'(A,I8)') 'CACHED_ENCODER_INIT :: Created a new encoder from rule : "'//TRIM(ADJUSTL(NAME))//'"'
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE(ERRFLAG_UNABLE_TO_SAFE_LOAD1)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error safe loading the sample before preset' )
    CASE(ERRFLAG_UNABLE_TO_SAFE_LOAD2)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error safe loading the sample after preset' )
    ...
    CASE(ERRFLAG_DUMP_SAMPLE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error dumping sample' )
    CASE DEFAULT
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'CACHED_ENCODER_GENERATING_RULE'
PP_THREAD_SAFE FUNCTION  CACHED_ENCODER_GENERATING_RULE( THIS, TAG, NAME, HOOKS ) RESULT(RET)
...
END FUNCTION  CACHED_ENCODER_GENERATING_RULE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CREATE_NAME = 7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_OPEN_LOG_FILE = 8_JPIB_K
  ...
  ! Construct the name of the logging file
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CREATE_NAME) MSG%TO_STRING( FNAME, HOOKS, &
&            PREFIX=OPT%DUMP_PATH, EXTENSION='yaml' )

  ! Open the logging file
  OPEN( NEWUNIT=NEW_UNIT, FILE=TRIM(FNAME), &
&       STATUS='UNKNOWN', ACTION='WRITE', IOSTAT=WRITE_STATUS )
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_OPEN_LOG_FILE )

  ! Print the message
  PP_TRYCALL(ERRFLAG_UNABLE_TO_DUMP_MSG) MSG%WRITE_TO_YAML( NEW_UNIT, OFFSET, HOOKS )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_DUMP_PAR) PAR%WRITE_TO_YAML( NEW_UNIT, OFFSET, HOOKS )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  ...
  IF ( PRESENT(SEPARATOR) ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_PRINT) THIS%ENCODER_%PRINT( NEW_UNIT, OFFSET, OPT, HOOKS, SEPARATOR )
  ELSE
    PP_TRYCALL(ERRFLAG_UNABLE_TO_PRINT) THIS%ENCODER_%PRINT( NEW_UNIT, OFFSET, OPT, HOOKS )
  ENDIF
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  WRITE(NEW_UNIT,*,IOSTAT=WRITE_STATUS) REPEAT(' ',OFFSET)
  ...
  CLOSE(UNIT=NEW_UNIT, IOSTAT=WRITE_STATUS)
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )

  ! Deallocate the filename
  IF ( ALLOCATED(FNAME) ) THEN
    DEALLOCATE(FNAME, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS .NE. 0, ERRFLAG_UNABLE_TO_WRITE )
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE(ERRFLAG_UNABLE_TO_CREATE_NAME)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error creating name' )
    CASE (ERRFLAG_UNABLE_TO_DEALLOCATE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating' )
    ...
    CASE ( ERRFLAG_UNABLE_TO_OPEN_LOG_FILE )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error opening log file' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'file: "'//TRIM(ADJUSTL(FNAME))//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'CACHED_ENCODER_PRINT_RULE'
PP_THREAD_SAFE FUNCTION  CACHED_ENCODER_PRINT_RULE( THIS, UNIT, HOOKS ) RESULT(RET)
...
END FUNCTION  CACHED_ENCODER_PRINT_RULE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
&      MSG, PAR, THIS%TIME_HISTORY_, CURR_TIME, OPT, METADATA, HOOKS )
