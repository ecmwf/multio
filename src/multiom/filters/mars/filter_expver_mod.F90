...
!>
!> This module defines the `FILTER_EXPVER_T` type, along with its associated
...
#define PP_SECTION_NAME 'FILTER_EXPVER_MOD'
MODULE FILTER_EXPVER_MOD
...
!>
!> @brief Filter type for managing level types (EXPVER).
!>
!> This derived type `FILTER_EXPVER_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_EXPVER_T
  ...
  CHARACTER(LEN=:), ALLOCATABLE :: EXPVER_

  !> Holds the keyset of level types for filtering.
  CHARACTER(LEN=:), DIMENSION(:), ALLOCATABLE :: EXPVERS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_EXPVER_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_EXPVER_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_EXPVER_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_EXPVER_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_EXPVER_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_EXPVER_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_EXPVER_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_EXPVER_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CEXPVER2IEXPVER
...
!>
!> @see FILTER_EXPVER_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_EXPVER_INIT'
FUNCTION FILTER_EXPVER_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
  ...
  USE :: YAML_CORE_UTILS_MOD, ONLY: YAML_READ_STRING_ARRAY
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_EXPVER_T),    INTENT(INOUT) :: THIS
  ...
  IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_STRING_ARRAY( CFG, 'values', THIS%EXPVERS_, HOOKS )
    THIS%USE_KEYSET_ = .TRUE.

  ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_STRING( CFG, 'value',THIS%EXPVER_, HOOKS )
...
END FUNCTION FILTER_EXPVER_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_EXPVER_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_EXPVER_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_EXPVER_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_EXPVER_MATCH'
FUNCTION FILTER_EXPVER_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE( FLT_INT_MATCH_E )
    IF ( THIS%USE_KEYSET_ ) THEN
      MATCH = .FALSE.
      DO I = 1, SIZE(THIS%EXPVERS_)
        IF (THIS%EXPVERS_(I) .EQ. MSG%EXPVER) THEN
          MATCH = .TRUE.
          EXIT
        ENDIF
      ENDDO
    ELSE
      MATCH = (THIS%EXPVER_ .EQ. MSG%EXPVER)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      ENDDO
    ELSE
      MATCH = .NOT.(THIS%EXPVER_ .EQ. MSG%EXPVER)
    ENDIF
...
END FUNCTION FILTER_EXPVER_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_EXPVER_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_EXPVER_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_EXPVER_PRINT'
FUNCTION FILTER_EXPVER_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'EXPVER FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_EXPVER_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_EXPVER_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_EXPVER_FREE'
FUNCTION FILTER_EXPVER_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_DELETE = 2_JPIB_K
  ...
  ! Free expver array
  IF ( ALLOCATED(THIS%EXPVERS_) ) THEN
    DEALLOCATE( THIS%EXPVERS_, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_UNABLE_TO_DELETE )
  ENDIF

  ! Free expver
  IF ( ALLOCATED(THIS%EXPVER_) ) THEN
    DEALLOCATE( THIS%EXPVER_, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_UNABLE_TO_DELETE )
  ENDIF
...
END FUNCTION FILTER_EXPVER_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_EXPVER_MOD
