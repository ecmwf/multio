...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION1_DATA_TYPE_DEFAULT_TO_BE_ENCODED
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_CFG( THIS, &
...
END FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_LAZY'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_LAZY( THIS, &
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE ( ERRFLAG_WRONG_PATH )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'default configurators cannot have lazy constructors' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
&  MSG, PAR, OPT,  METADATA, HOOKS ) RESULT(RET)
  ...
  USE :: ENUMERATORS_MOD,          ONLY: CLASS_D1_E
...
IMPLICIT NONE
  ...
  ! Set the production status of processed data (N.B. This part allocates section2!!!!!!)
  IF ( MSG%CLASS .EQ. CLASS_D1_E ) THEN
    ! Allocate the section2 for destine (12 for Climate DT, 13 for testing)
     PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'productionStatusOfProcessedData', 12_JPIB_K )
  ENDIF
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION1_DATA_TYPE_DEFAULT_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
!>   @param [in]    TIME_HIST     The time history object of type `TIME_HISTORY_T` providing historical time data.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION1_DATA_TYPE_DEFAULT_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_TO_BE_ENCODED( THIS, &
...
END FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION1_DATA_TYPE_DEFAULT_FREE( THIS, OPT, HOOKS ) RESULT(RET)
