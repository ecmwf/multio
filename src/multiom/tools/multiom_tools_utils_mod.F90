...
INTEGER(KIND=JPIB_K) :: MSG_COUNT=0_JPIB_K
INTEGER(KIND=JPIB_K) :: TIMESTEP_IN_SECONDS=3600_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: HOURS2SECONDS=3600_JPIB_K

! Format of the print
INTEGER(KIND=JPIB_K), PARAMETER :: CELL_SIZE = 16_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: NCELLS = 9_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: SEP_SIZE = 3_JPIB_K
CHARACTER(LEN=*), PARAMETER :: PRINT_FORMAT = '(1x," | ",A," | ",A," | ",A," | ",A," | ",A," | ",A," |" ,A," | ",A," | ",A," | ")'
...
PUBLIC :: INITIALIZE_DIST_OUTPUT_MANAGER
PUBLIC :: INITIALIZE_TABLE_OF_CONTENTS
PUBLIC :: SHOW_PARAMS
PUBLIC :: MODIFY_PARAMS
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MODIFY_PARAMS'
PP_THREAD_SAFE FUNCTION MODIFY_PARAMS( CMDARGS, PROCID, BIG_ENDIAN_READ, VERBOSE, NPROC_IO, HOOKS ) RESULT(RET)
  ...
  USE :: MULTIOM_API, ONLY: PAR_ROPEN
  USE :: MULTIOM_API, ONLY: PAR_READ
  USE :: MULTIOM_API, ONLY: PAR_WOPEN
  USE :: MULTIOM_API, ONLY: PAR_WRITE
  USE :: MULTIOM_API, ONLY: PAR_CLOSE
  USE :: MULTIOM_API, ONLY: JPIB_K
  USE :: MULTIOM_API, ONLY: HOOKS_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(COMMAND_LINE_ARGS_T), INTENT(IN)    :: CMDARGS
  INTEGER(KIND=JPIB_K),      INTENT(IN)    :: PROCID
  LOGICAL,                   INTENT(IN)    :: BIG_ENDIAN_READ
  LOGICAL,                   INTENT(IN)    :: VERBOSE
  INTEGER(KIND=JPIB_K),      INTENT(OUT)   :: NPROC_IO
  TYPE(HOOKS_T),             INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_PAR_CLOSE = 5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GET_DUMP_DIRECTORY = 6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WOPEN_PAR_FILE = 7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRITE_PAR_FILE = 8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_CLOSE_PAR_FILE = 9_JPIB_K
  ...
  ! Modify parameters
  NPROC_IO = PARAMETERS%SIM_%NPROC_IO
  PARAMETERS%SIM_%CFCLASS = 'd1'
  PARAMETERS%WAM_%YCLASS = 'd1'
  PARAMETERS%SIM_%NSTREAM = 1025_JPIB_K ! OPER

  ! Parameters
  PAR_FNAME = REPEAT( ' ', 128 )
  WRITE(PAR_FNAME,'(A,A,I6.6,A)', IOSTAT=WRITE_STATUS) './hacked_par.', PROC_ID,'.par'
  PP_TRYCALL(ERRFLAG_WOPEN_PAR_FILE) PAR_WOPEN( TRIM(ADJUSTL(PAR_FNAME)), PARUNIT, HOOKS )
  PP_TRYCALL(ERRFLAG_WRITE_PAR_FILE) PAR_WRITE( PARAMETERS, PARUNIT, HOOKS )
  PP_TRYCALL(ERRFLAG_CLOSE_PAR_FILE) PAR_CLOSE( PARUNIT, HOOKS )
...
PP_THREAD_SAFE FUNCTION INITIALIZE_OUTPUT_MANAGER( CMDARGS, TOPOLOGY, &
& PARAMETERS, OUTPUT_MANAGER, TOC, BIG_ENDIAN_READ, VERBOSE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TIMESTEP_IN_SECONDS = INT( PARAMETERS%SIM_%TSTEP, KIND=JPIB_K )
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INITIALIZE_DIST_OUTPUT_MANAGER'
PP_THREAD_SAFE FUNCTION INITIALIZE_DIST_OUTPUT_MANAGER( CMDARGS, MPI_RANK, MPI_SIZE, TOPOLOGY, &
& PARAMETERS, OUTPUT_MANAGER, TOC, BIG_ENDIAN_READ, VERBOSE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  INTEGER(KIND=JPIB_K), DIMENSION(1) :: PROCESSOR_LIST
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_MAKE_OUTPUT_MANAGER = 10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_ALL_TOC = 11_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_NUM_PROCESSORS = 12_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_WRONG_RANK = 13_JPIB_K
  ...
  ! Error handling
  PP_DEBUG_CRITICAL_COND_THROW( MPI_SIZE .NE. NUMBER_OF_PROCESSORS, ERRFLAG_WRONG_NUM_PROCESSORS )
  PP_DEBUG_CRITICAL_COND_THROW( MPI_RANK .LT. 0 .OR. MPI_RANK .GE. MPI_SIZE, ERRFLAG_WRONG_RANK )

  PROCESSOR_LIST(1) = MPI_RANK + 1_JPIB_K
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CHARACTER(LEN=32) :: CRANK
    CHARACTER(LEN=32) :: CSIZE
    CHARACTER(LEN=32) :: CNPROCS
    ...
    CASE(ERRFLAG_UNABLE_TO_READ_ALL_TOC)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read all table of contents' )
    CASE(ERRFLAG_WRONG_NUM_PROCESSORS)
      CRANK = REPEAT( ' ', 32 )
      WRITE(CRANK,'(I0)') MPI_RANK
      CSIZE = REPEAT( ' ', 32 )
      CNPROCS = REPEAT( ' ', 32 )
      WRITE(CSIZE,'(I0)') MPI_SIZE
      WRITE(CNPROCS,'(I0)') NUMBER_OF_PROCESSORS
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Wrong number of processors' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'MPI rank: ' // TRIM(ADJUSTL(CRANK)) )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'MPI size: ' // TRIM(ADJUSTL(CSIZE)) )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Number of processors in parameter file: ' // TRIM(ADJUSTL(CNPROCS)) )
    CASE(ERRFLAG_WRONG_RANK)
      CRANK = REPEAT( ' ', 32 )
      WRITE(CRANK,'(I0)') MPI_RANK
      CSIZE = REPEAT( ' ', 32 )
      WRITE(CSIZE,'(I0)') MPI_SIZE
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Wrong rank' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'MPI rank: ' // TRIM(ADJUSTL(CRANK)) )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'MPI size: ' // TRIM(ADJUSTL(CSIZE)) )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
...
END FUNCTION INITIALIZE_DIST_OUTPUT_MANAGER
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
&    MESSAGE%ILEVG_, &
&    TOC_ENTRY%STEP_ID_*TIMESTEP_IN_SECONDS/HOURS2SECONDS, &
&    MATCH, &
&    HOOKS )
...
&                TOC_ENTRY%PARAM_ID_, TOC_ENTRY%U_ID_,TOC_ENTRY%REPRES_ID_, ILEVTYPE, HOOKS )
    ...
    CPROCID = REPEAT(' ',32)
    WRITE(CPROCID,'(I32)', IOSTAT=WRITE_STAT) TOC_ENTRY%PROC_ID_
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_UNABLE_TO_WRITE_TO_STRING )
    ...
    CSTEP = REPEAT(' ',32)
    WRITE(CSTEP,'(I32)', IOSTAT=WRITE_STAT) TOC_ENTRY%STEP_ID_*TIMESTEP_IN_SECONDS/HOURS2SECONDS
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_UNABLE_TO_WRITE_TO_STRING )
...
&   NORMALIZE_STRING( TRIM(ADJUSTL(CPROCID)) ) , &
...
IMPLICIT NONE
  ...
  CSTEP = REPEAT(' ',32)
  CMSG_COUNT = REPEAT(' ',32)
  WRITE(CSTEP,'(I32)') TOC_ENTRY%STEP_*TIMESTEP_IN_SECONDS/HOURS2SECONDS
  WRITE(CMSG_COUNT,'(I32)') MSG_COUNT
  MSG_COUNT = 0_JPIB_K


  ! Print the header separator
  PP_TRYCALL(ERRFLAG_UNABLE_TO_WRITE_TO_STRING) PRINT_HEADER_PVT( CMDARGS, OUTPUT_UNIT, HOOKS )
  WRITE( UNIT=OUTPUT_UNIT, FMT='(1x,A)', IOSTAT=WRITE_STAT ) 'Flush step: '//TRIM(ADJUSTL(CSTEP))//' Message count: '//TRIM(ADJUSTL(CMSG_COUNT))
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_UNABLE_TO_WRITE_TO_STRING )
  WRITE( UNIT=OUTPUT_UNIT, FMT='(A)', IOSTAT=WRITE_STAT ) ' '
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_UNABLE_TO_WRITE_TO_STRING )
  WRITE( UNIT=OUTPUT_UNIT, FMT='(A)', IOSTAT=WRITE_STAT ) ' '
