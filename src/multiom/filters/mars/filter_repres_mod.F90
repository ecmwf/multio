...
!>
!> @brief Module containing definitions and procedures for repres filters.
!>
!> This module defines the `FILTER_REPRES_T` type, along with its associated
!> procedures and helper functions that facilitate the creation, management, and
!> utilization of repres filters within the system. Repres filters allow for
...
#define PP_FILE_NAME 'filter_repres_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_REPRES_MOD'
MODULE FILTER_REPRES_MOD
...
!>
!> @brief Filter type for managing level types (REPRES).
!>
!> This derived type `FILTER_REPRES_T` is used to manage level-type-specific filtering.
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_REPRES_T
  ...
  INTEGER(KIND=JPIB_K) :: REPRES_

  !> Holds the keyset of level types for filtering.
  TYPE(KEYSET_INT64_T) :: REPRESS_

CONTAINS

  !> Initializes the level type filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_REPRES_INIT

  !> Matches the filter with the input data based on level type.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_REPRES_MATCH

  !> Prints the filter’s current state and configuration.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_REPRES_PRINT

  !> Frees the resources allocated by the filter.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_REPRES_FREE

END TYPE

!> Whitlist of public symbols
PUBLIC :: FILTER_REPRES_T

CONTAINS


!>
!> @brief Initializes a level type filter (`FILTER_REPRES_T`) from a YAML configuration.
!>
!> This function initializes the `FILTER_REPRES_T` object (`THIS`) by reading relevant
...
!>
!> @section interface
!> @param [inout] THIS The filter object of type `FILTER_REPRES_T` to be initialized.
...
!> @dependency [PROCEDURE] ENUMERATORS_MOD::CREPRES2IREPRES
...
!>
!> @see FILTER_REPRES_T, YAML_CONFIGURATION_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_REPRES_INIT'
FUNCTION FILTER_REPRES_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_REPRES_T),     INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%REPRESS_%INIT( HOOKS )
  ...
  IF ( THIS%FILTER_TYPE_ .NE. FLT_INT_HAS_E .AND. THIS%FILTER_TYPE_ .NE. FLT_INT_LACKS_E ) THEN
    ...
    IF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUES .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_FILTER( CFG, 'values', THIS%REPRESS_, P_CREPRES2IREPRES, HOOKS )
      THIS%USE_KEYSET_ = .TRUE.

    ELSEIF ( THIS%FILTER_TYPE_ .EQ. FLT_INT_MATCH_E .AND. HAS_VALUE .AND. .NOT.HAS_VALUES ) THEN

      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'value',THIS%REPRES_, P_CREPRES2IREPRES, HOOKS )
...
END FUNCTION FILTER_REPRES_INIT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Matches a given parameterization (`PAR`) against the filter level type (`FILTER_REPRES_T`).
!>
...
!>
!> @section interface
!> @param [inout] THIS The level type filter object (`FILTER_REPRES_T`) for the match check.
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_REPRES_T, PARAMETRIZATION_T, FORTRAN_MESSAGE_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_REPRES_MATCH'
FUNCTION FILTER_REPRES_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = (MSG%REPRES .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = (MSG%REPRES .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )

    IF ( THIS%USE_KEYSET_ ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%REPRESS_%MATCH( MSG%REPRES, MATCH, HOOKS )
    ELSE
      MATCH = (THIS%REPRES_ .EQ. MSG%REPRES)
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    IF ( THIS%USE_KEYSET_ ) THEN
      ...
      MATCH = .NOT.MATCH
    ELSE
      MATCH = .NOT.(THIS%REPRES_ .EQ. MSG%REPRES)
    ENDIF
...
END FUNCTION FILTER_REPRES_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Prints the details of a level type filter (`FILTER_REPRES_T`) to the specified output unit.
!>
...
!> @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see FILTER_REPRES_T, HOOKS_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_REPRES_PRINT'
FUNCTION FILTER_REPRES_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'REPRES FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_REPRES_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


!>
!> @brief Releases resources allocated for a level type filter (`FILTER_REPRES_T`).
!>
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_REPRES_FREE'
FUNCTION FILTER_REPRES_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%REPRESS_%FREE( HOOKS )
...
END FUNCTION FILTER_REPRES_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


END MODULE FILTER_REPRES_MOD
