...
Param2ParamMap getMappings() {
    ...
    for (auto& mappings : mappingConf.getSubConfigurations()) {
        ...
        auto paramOut = mappings.getInt64("paramOut");
...
void AverageRate::executeImpl(message::Message msg) {
    ...
    auto param = md.get<std::int64_t>(dm::legacy::Param);
...
void AverageRate::compute(message::Message& msg) const {
    ...
    auto timespan = dm::parseEntry(dm::TIMESPAN, msg.metadata());
    ASSERT(timespan.isSet());
    const double c = 1.0 / timespan.get().toSeconds();

    std::transform(data, data + size, data,
                   [c](Precision value) { return static_cast<Precision>(value * c); });
}
