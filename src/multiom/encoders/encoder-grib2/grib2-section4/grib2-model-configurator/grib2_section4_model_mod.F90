...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION4_MODEL_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION4_MODEL_INIT_LAZY
  ...
  !>
  !> @brief Determines if the GRIB2 Section 4 model configuration object needs to be encoded.
  !>
  !> This procedure checks whether the object should be encoded based
  !> on the provided parameters and internal state.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION4_MODEL_TO_BE_ENCODED
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_MODEL_INIT_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_MODEL_INIT_CFG( THIS, &
...
END FUNCTION GRIB2_SECTION4_MODEL_INIT_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_MODEL_INIT_LAZY'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_MODEL_INIT_LAZY( THIS, &
...
END FUNCTION GRIB2_SECTION4_MODEL_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  ! ! TODO Presetting this will cause problems (eccodes key/value not found) ... usually this should be 
  ! ! preset through section2 and pdt...
  IF (MSG%MODEL.NE.UNDEF_PARAM_E) THEN
      PP_TRYCALL(ERRFLAG_CONVERT_MODEL) METADATA%DUMP_SAMPLE( 'sample_before_model.grib', HOOKS )
...
!>
!> @brief Prepares GRIB2 Section 4 model configuration for encoding based on provided parameters, message data, and time history.
!>
!> This function determines whether GRIB2 Section 4 model configuration (`THIS`) is ready to be encoded. It processes the provided model parameter
!> (`MODEL`), message structure (`MSG`), current time (`CURR_TIME`), time history (`TIME_HISTORY`), and updates the
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION4_MODEL_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
!>   @param [in]    TIME_HIST     The time history object of type `TIME_HISTORY_T` providing historical time data.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION4_MODEL_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_MODEL_TO_BE_ENCODED( THIS, &
...
IMPLICIT NONE
  ...
  ! Not condition applicable for section 4 model configuration to avoid encoding the field
  TO_BE_ENCODED = .TRUE.
...
END FUNCTION GRIB2_SECTION4_MODEL_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION GRIB2_SECTION4_MODEL_FREE( THIS, OPT, HOOKS ) RESULT(RET)
