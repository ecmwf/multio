...
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION4_DIRFREQ_DEFAULT_T
  ...
  INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_DIRECTIONS_=2_JPIB_K

  !> Scaled factors for wave frequencies
  INTEGER(KIND=JPIB_K) :: SCALE_FACTOR_OF_WAVE_FREQUENCIES_=6_JPIB_K
...
&       MSG, PAR, OPT, THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_, &
&       THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_, HOOKS )
#else
...
IMPLICIT NONE
  ...
  IF ( PAR%WAVE%SCALE_FACTOR_OF_WAVE_DIRECTIONS .NE. UNDEF_PARAM_E ) THEN
    THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_ = PAR%WAVE%SCALE_FACTOR_OF_WAVE_DIRECTIONS
  ENDIF
  IF ( PAR%WAVE%SCALE_FACTOR_OF_WAVE_FREQUENCIES .NE. UNDEF_PARAM_E ) THEN
    THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_ = PAR%WAVE%SCALE_FACTOR_OF_WAVE_FREQUENCIES
  ENDIF
  ...
  DO I = 1, N_DIRS
    SCALED_VALUES_OF_WAVE_DIRECTIONS(I) = NINT(PAR%WAVE%DIRS_(I)*10**THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_*RAD2DEG)
  ENDDO
  ...
  DO I = 1, N_FREQ
    SCALED_VALUES_OF_WAVE_FREQUENCIES(I) = NINT(PAR%WAVE%FREQ_(I)*10**THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_)
  ENDDO

  !> Set the metadata
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'numberOfWaveDirections', N_DIRS )
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaleFactorOfWaveDirections', THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_ )
  ...
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA,  'scaleFactorOfWaveFrequencies', THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_ )
...
IMPLICIT NONE
  ...
  !> Local variables
  LOGICAL :: HAS_SCALE_FACTOR_OF_WAVE_DIRECTIONS
  LOGICAL :: HAS_SCALE_FACTOR_OF_WAVE_FREQUENCIES
...
&     'scale-factor-of-wave-directions',  HAS_SCALE_FACTOR_OF_WAVE_DIRECTIONS, HOOKS )

  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( CFG, &
&     'scale-factor-of-wave-frequencies', HAS_SCALE_FACTOR_OF_WAVE_FREQUENCIES, HOOKS )

  !> Read the encoder configuration
  IF ( HAS_SCALE_FACTOR_OF_WAVE_DIRECTIONS ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( CFG, &
&      'scale-factor-of-wave-directions', THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_, HOOKS )
  ELSE
    THIS%SCALE_FACTOR_OF_WAVE_DIRECTIONS_ = 2_JPIB_K
  ENDIF

  IF ( HAS_SCALE_FACTOR_OF_WAVE_FREQUENCIES ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( CFG, &
&      'scale-factor-of-wave-frequencies', THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_, HOOKS )
  ELSE
    THIS%SCALE_FACTOR_OF_WAVE_FREQUENCIES_ = 6_JPIB_K
  ENDIF
