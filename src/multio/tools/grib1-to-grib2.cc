...
void handleParamId(metkit::codes::CodesHandle& h, dm::FullMarsRecord& mars) {
    ...
    switch (initParamId) {
        case 55:
            mars.param.set(228004);
            mars.timespan.set(24);
            break;
        case 56:
            mars.param.set(235168);
            break;
        case 130232:
            mars.param.set(235135);
            break;
        case 151130:
            mars.param.set(262100);
            break;
        case 151131:
            mars.param.set(262140);
            break;
        case 151132:
            mars.param.set(262139);
            break;
        case 151145:
            mars.param.set(262124);
            break;
        case 151148:
            mars.param.set(3067);
            break;
        case 151163:
            mars.param.set(262104);
            break;
        case 151164:
            mars.param.set(262144);
            break;
        case 151175:
            mars.param.set(262118);
            break;
        case 172146:
            mars.param.set(235033);
            break;
        case 172147:
            mars.param.set(235034);
            break;
        case 172169:
            mars.param.set(235035);
            break;
        case 172175:
            mars.param.set(235036);
            break;
        case 172176:
            mars.param.set(235037);
            break;
        case 172177:
            mars.param.set(235038);
            break;
        case 172178:
            mars.param.set(235039);
            break;
        case 172179:
            mars.param.set(235040);
            break;
        case 174098:
            mars.param.set(262000);
            break;
        case 72:
            mars.param.set(260087);
            break;
        case 73:
            mars.param.set(260097);
            break;
        case 172050:
            mars.param.set(235026);
            break;
        case 172145:
            mars.param.set(235032);
            break;
        case 172189:
            mars.param.set(235189);
            break;
        case 172195:
            mars.param.set(235045);
            break;
        case 172196:
            mars.param.set(235046);
            break;
        case 172197:
            mars.param.set(235047);
            break;
        default:
            mars.param.set(initParamId);
    }
}
...
bool needTimespan(long paramId) {
    ...
    // This is just to fix some grib1 fields that are encoded wrongly as instant fields rather than as a proper time range. For these fields, we have to set the timespan explicitly.
    static const std::unordered_set<long> paramIdsWithTimespan{
        8, 9, 20, 44, 45, 47, 50, 57, 58, 142, 143, 144, 146, 145, 147, 169, 175, 176,
        177, 178, 179, 180,181,182, 189, 195, 196, 197, 205, 208, 209, 210,
        211, 212, 213, 228, 228021, 228022, 228080, 228082, 228129, 228130, 228216, 228251};
...
void mapGrib1ToGrib2(KeySet& marsKeys, metkit::codes::CodesHandle& h, dm::FullMarsRecord& mars, dm::MiscRecord& misc,
    ...
    mars.chem = dm::parseEntry(dm::CHEM, h);
    ...
    if (mars.levtype.isSet()) {
        ...
        if (h.has("level")) {
            ...
            mars.levelist.set(*levelist);
        }
    }

    mars.direction = dm::parseEntry(dm::DIRECTION, h);
    mars.frequency = dm::parseEntry(dm::FREQUENCY, h);
    mars.date = dm::parseEntry(dm::DATE, h);
    mars.hdate = dm::parseEntry(dm::HDATE, h);

    mars.method = dm::parseEntry(dm::METHOD, h);
    mars.system = dm::parseEntry(dm::SYSTEM, h);
...
//                 throw std::runtime_error("Invalid step range: endStep must be greater than or equal to startStep");
//             }
//             if ( operationWindow == 0 ) {
...
Grib1ToGrib2::Grib1ToGrib2(int argc, char** argv) : multio::MultioTool{argc, argv} {
    ...
    options_.push_back(new eckit::option::SimpleOption<std::string>(
        ...
        "How to handle fields with time span equal to zero (these fields should not exist in grib2 so it is not possible to try-to-handle). Values: \"log-and-ignore\" (default), \"ignore\", \"copy\""));
...
void Grib1ToGrib2::execute(const eckit::option::CmdArgs& args) {
    ...
    while ((msg = reader.next())) {
        ...
        if (isMessageValid != 1) {
            std::string errorMsg = "Message " + std::to_string(msgIndex) + " is not valid according to the GRIB1 metadata. This likely means the message is malformed and may fail to convert to GRIB2. ";
        ...
        if (edition == "2" && copyGrib2Messages_) {
            ...
            if (( marsConfig.has("timespan") && marsConfig.getLong("timespan") > 0 ) || !marsConfig.has("timespan") ) {
                ...
                std::cout << "Encoding message #" << msgIndex << " to GRIB2 (total GRIB2 messages so far: " << msgIndex << ")"
                          << std::endl;
                auto preparedHandle = encoder.encode(values, marsConfig, miscConfig);

                long isMessageValid = preparedHandle->getLong("isMessageValid");
                if (isMessageValid != 1) {
                    std::cerr << "WARNING: Re-encoded message #" << msgIndex << " is not valid according to the GRIB2 metadata. This likely means the message is malformed and may fail to convert to GRIB2. ";
                }
