...
#define PP_SECTION_NAME 'MULTIOM_ENCODER_MOD'
MODULE MULTIOM_ENCODER_MOD
...
CONTAINS
  ...
  ! PROCEDURE, PUBLIC, NON_OVERRIDABLE, PASS :: DUMP_CACHED_SAMPLE => MULTIO_ENCODER_DUMP_CACHED_SAMPLE
...
& ENCODER_OPTIONS, MAPPING_OPTIONS, FILTER_OPTIONS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(MULTIOM_ENCODERS_T),       INTENT(INOUT) :: THIS
...
IMPLICIT NONE
  ...
  !> When working without cache search all the time through all the mappings
  MAPPER => NULL()
  PP_TRYCALL(ERRFLAG_MAPPING_CACHE_INIT) MAKE_MAPPERS_COLLECTION( &
&   MSG, PAR, THIS%MAPPING_RULES, THIS%MAPPING_OPTIONS, THIS%FILTER_OPTIONS, MAPPER, HOOKS )
...
&     MSG, PAR, MAPPED_MSG, MAPPED_PAR, MAPPING_TAG, MAPPING_NAME, HOOKS )
    ...
    !> When working without cache search all the time through all the encoders
    PP_TRYCALL(ERRFLAG_ENCODER_CACHE_INIT) MAKE_ENCODER_COLLECTION( &
&     MAPPED_MSG, MAPPED_PAR, THIS%METADATA, THIS%ENCODER_RULES, THIS%ENCODER_OPTIONS, THIS%FILTER_OPTIONS, ENCODERS, HOOKS )
...
END MODULE MULTIOM_ENCODER_MOD
