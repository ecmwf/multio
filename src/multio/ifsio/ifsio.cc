...
class MIO : public util::FailureAware<ComponentTag::Client> {
 public:
  ...
  int encodeBitsPerValue(int paramid, const std::string& levtype, double min,
                         double max) {
    ASSERT(bpv_);
 ...
 private:
  ...
  MIO()
      : MIO(configureFromEnv().setComponentTag(
            multio::util::ComponentTag::Client)) {}
  ...
  ConfigurationContext configureFromEnv() {
    ...
    if (::getenv("MULTIO_PLANS")) {
      ...
      return ConfigurationContext(
          eckit::LocalConfiguration(eckit::YAMLConfiguration(cfg)), cfg, cfg);
    }
    ...
    if (::getenv("MULTIO_CONFIG")) {
      ...
      return configureFromSinks(ConfigurationContext(
          eckit::LocalConfiguration(eckit::YAMLConfiguration(cfg)), cfg, cfg));
    }
    ...
    std::cout << "MultIO initialising with $MULTIO_SINKS " << oss.str()
              << std::endl;

    std::istringstream iss(oss.str());
    return configureFromSinks(ConfigurationContext(
        eckit::LocalConfiguration(eckit::YAMLConfiguration(iss)), "", ""));
  }
  ...
  ~MIO() {
    if (dirty_) {
      ...
      if (abort_on_error) {
        std::cout
            << "ERROR - MultIO finished without a final call to imultio_flush"
            << std::endl;
        std::cerr
            << "ERROR - MultIO finished without a final call to imultio_flush"
            << std::endl;
        ...
        std::cout
            << "WARNING - MultIO finished without a final call to imultio_flush"
            << std::endl;
    }
  }
...
fortint imultio_write_(const void* data, const fortint* words) {
  try {
    ...
    multio::message::Metadata metadata;
    multio::message::Message message{
        multio::message::Message::Header{Message::Tag::Grib, Peer{}, Peer{},
                                         std::move(metadata)},
        std::move(payload)};
    MIO::instance().dispatch(message);
...
fortint imultio_write_raw_(const void* configuration, const void* data,
                           const fortint* words) {
  try {
    eckit::AutoLock<MIO> lock(MIO::instance());
    ...
    const eckit::Configuration* conf =
        reinterpret_cast<const eckit::Configuration*>(configuration);
    ...
    multio::message::Message message{
        multio::message::Message::Header{Message::Tag::Field, Peer{}, Peer{},
                                         std::move(metadata)},
        std::move(payload)};
    MIO::instance().dispatch(message);
...
fortint imultio_encode_bitspervalue_(fortint* bitspervalue,
                                     ...
                                     const char* levtype, const double* min,
                                     const double* max, int levtype_len) {
  try {
    std::string slevtype(levtype, levtype + levtype_len);
    eckit::AutoLock<MIO> lock(MIO::instance());
    *bitspervalue =
        MIO::instance().encodeBitsPerValue(*paramid, slevtype, *min, *max);
  } catch (std::exception& e) {
    return ifsio_handle_error(e);
  }
  return 0;
