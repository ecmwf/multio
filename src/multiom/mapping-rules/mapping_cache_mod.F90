...
#define PP_FILE_NAME 'mapping_cache_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'MAPPING_CACHE_MOD'
MODULE MAPPING_CACHE_MOD
...
INTERFACE
  PP_THREAD_SAFE FUNCTION FUNCTION_MAPPING_CACHE_IF( KEY, CACHE_OPT, HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD,   ONLY: JPIB_K
  ...
  END FUNCTION FUNCTION_MAPPING_CACHE_IF
END INTERFACE

TYPE, ABSTRACT :: FUNCTOR_MAPPING_CACHE_A
CONTAINS
PROCEDURE(FUNCTOR_MAPPING_CACHE_IF), DEFERRED, PUBLIC, PASS :: APPLY
END TYPE

ABSTRACT INTERFACE
  PP_THREAD_SAFE FUNCTION FUNCTOR_MAPPING_CACHE_IF( THIS, KEY, CACHE_OPT, HOOKS ) RESULT(RET)
    ! Symbols imported from other modules within the project.
    USE :: DATAKINDS_DEF_MOD,   ONLY: JPIB_K
    USE :: HOOKS_MOD,           ONLY: HOOKS_T
    USE :: FORTRAN_MESSAGE_MOD, ONLY: FORTRAN_MESSAGE_T
    USE :: CACHE_UTILS_MOD,     ONLY: CACHE_OPTIONS_T
    IMPORT :: FUNCTOR_MAPPING_CACHE_A
  IMPLICIT NONE
    CLASS(FUNCTOR_MAPPING_CACHE_A), INTENT(INOUT) :: THIS
    TYPE(FORTRAN_MESSAGE_T),        INTENT(IN)    :: KEY
    TYPE(CACHE_OPTIONS_T),          INTENT(IN)    :: CACHE_OPT
    TYPE(HOOKS_T),                  INTENT(INOUT) :: HOOKS
    INTEGER(KIND=JPIB_K) :: RET
  END FUNCTION FUNCTOR_MAPPING_CACHE_IF
END INTERFACE


!> @brief Datatype used to to store a node of the tree
TYPE :: MAPPING_CACHE_NODE_T

  !> Key
  TYPE(FORTRAN_MESSAGE_T) :: KEY

  !> Value
  TYPE(CACHED_MAPPER_COLLECTION_T), POINTER :: MAPPERS_ => NULL()
  ...
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: PARENT => NULL()

  !> Pointer to the lef subtree
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: LEFT   => NULL()

  !> Pointer to the right subtree
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: RIGHT  => NULL()

END TYPE


!> @brief Datatype used to to store the entire map
TYPE :: MAPPING_CACHE_T

  !> Pointer to the class
  PRIVATE

  !> Pointer to the root node
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: ROOT => NULL()
...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: ACCESS_OR_CREATE => MAPPING_CACHE_ACCESS_OR_CREATE
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE             => MAPPING_CACHE_FREE
  ! PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: DUMP             => MAPPING_CACHE_DUMP
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: APPLY_FUNCTION   => MAPPING_CACHE_APPLY_FUNCTION
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: APPLY_FUNCTOR    => MAPPING_CACHE_APPLY_FUNCTOR

END TYPE


!> @brief Node used as terminal symbol
TYPE(MAPPING_CACHE_NODE_T), TARGET :: NIL

!> Whitelist of public symbols
PUBLIC :: MAPPING_CACHE_T
PUBLIC :: FUNCTION_MAPPING_CACHE_IF
PUBLIC :: FUNCTOR_MAPPING_CACHE_A
...
PP_THREAD_SAFE FUNCTION KEY_EQUAL_TO( KEY1, KEY2, CACHE_OPTION, IS_EQUAL, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(CACHE_OPTIONS_T),   INTENT(IN)    :: CACHE_OPTION
  ...
  PP_TRYCALL(ERRFLAG_COMPARE_ERROR) KEY1%IS_EQUAL_TO( &
&    KEY2, CACHE_OPTION, IS_EQUAL, HOOKS )
...
PP_THREAD_SAFE FUNCTION KEY_LOWER_THAN( KEY1, KEY2, CACHE_OPTION, IS_LOWER_THAN, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_COMPARE_ERROR) KEY1%IS_LOWER_THAN( &
&    KEY2, CACHE_OPTION, IS_LOWER_THAN, HOOKS )
...
PP_THREAD_SAFE FUNCTION ACCESS_OR_CREATE_NODE( ROOT, KEY, PAR, MAPPING_RULES, &
&     MAPPERS, INSERTED, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER,       INTENT(INOUT) :: ROOT
  TYPE(FORTRAN_MESSAGE_T),                   INTENT(IN)    :: KEY
  TYPE(PARAMETRIZATION_T),                   INTENT(IN)    :: PAR
  TYPE(MAPPING_RULES_COLLECTION_T),          INTENT(IN)    :: MAPPING_RULES
  TYPE(CACHED_MAPPER_COLLECTION_T), POINTER, INTENT(OUT)   :: MAPPERS
  ...
  ! Local variables
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: INSERTION_POINT
  LOGICAL :: FOUND
  LOGICAL :: KEY_LT
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_PRINT_RULES=10_JPIB_K
...
&       PAR, MAPPING_RULES, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS )
    MAPPERS => ROOT%MAPPERS_
...
&             KEY, PAR, MAPPING_RULES, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS )
        MAPPERS => INSERTION_POINT%LEFT%MAPPERS_
...
&             KEY, PAR, MAPPING_RULES, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS )
        MAPPERS => INSERTION_POINT%RIGHT%MAPPERS_
    ...
    ELSE
      ...
      ! WRITE(*,*) 'Node already in the map'
      INSERTED =.FALSE.
      MAPPERS => INSERTION_POINT%MAPPERS_
    ENDIF
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED( MAPPERS ), ERRFLAG_ENCODERS_NOT_ASSOCIATED )

  ! Print the rule used for encoding the field
  IF ( CACHE_OPT%VERBOSE ) THEN
    ! WRITE(6_JPIB_K,*) '+ MAPPING WITH RULES :: '
    PP_TRYCALL(ERRFLAG_UNABLE_TO_PRINT_RULES) INSERTION_POINT%MAPPERS_%PRINT_RULES( 6_JPIB_K, HOOKS )
  ENDIF
...
PP_THREAD_SAFE FUNCTION DELETE_NODE( ROOT, KEY, CACHE_OPT, MAPPING_OPT, FOUND, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( .NOT.ROOT_IS_LEAF ) THEN
    ...
    IF ( FOUND ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_REMOVE_NODE) REMOVE_NODE( ROOT, INSERTION_POINT, CACHE_OPT, MAPPING_OPT, HOOKS )
    ENDIF
  ENDIF
...
RECURSIVE FUNCTION FREE_NODE( CURRENT, CACHE_OPT, MAPPING_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: CURRENT
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(MAPPING_OPTIONS_T),             INTENT(IN)    :: MAPPING_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  IF ( .NOT.ASSOCIATED( CURRENT, NIL ) ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_NODE) FREE_NODE( CURRENT%LEFT, CACHE_OPT, MAPPING_OPT, HOOKS )

    ! Deallocate right subtree.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_NODE) FREE_NODE( CURRENT%RIGHT, CACHE_OPT, MAPPING_OPT, HOOKS )

    ! Free memory
    PP_TRYCALL(ERRFLAG_UNABLE_TO_DEALLOCATE_NODE) DEALLOCATE_NODE( CURRENT, CACHE_OPT, MAPPING_OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION NODE_INIT( THIS, PARENT, KEY, PAR, &
        MAPPING_RULES, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS ) RESULT(RET)
  ...
  USE :: MAPPING_UTILS_MOD,            ONLY: MAKE_MAPPERS_COLLECTION
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T),                INTENT(INOUT) :: THIS
  TYPE(MAPPING_CACHE_NODE_T), POINTER,       INTENT(IN)    :: PARENT
  TYPE(FORTRAN_MESSAGE_T),                   INTENT(IN)    :: KEY
  TYPE(PARAMETRIZATION_T),                   INTENT(IN)    :: PAR
  TYPE(MAPPING_RULES_COLLECTION_T),          INTENT(IN)    :: MAPPING_RULES
  TYPE(CACHE_OPTIONS_T),                     INTENT(IN)    :: CACHE_OPT
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CREATE_MAPPING=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_ALLOCATE_MAPPERS=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_INITIALIZE_NODE=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_MAPPING_COLLECTION_INIT=5_JPIB_K
  ...
  ALLOCATE( THIS%MAPPERS_, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_UNABLE_TO_ALLOCATE_MAPPERS )

  !> Initialize the mappers
  PP_TRYCALL(ERRFLAG_MAPPING_COLLECTION_INIT) MAKE_MAPPERS_COLLECTION( &
&   KEY, PAR, MAPPING_RULES, MAPPING_OPT, FILTER_OPT, THIS%MAPPERS_, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_MAPPING_COLLECTION_INIT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'mapping collection init' )
    CASE (ERRFLAG_UNABLE_TO_ALLOCATE_MAPPERS)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unable to allocate node' )
...
PP_THREAD_SAFE FUNCTION DEALLOCATE_NODE( X, CACHE_OPT, MAPPING_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: X
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(MAPPING_OPTIONS_T),             INTENT(IN)    :: MAPPING_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  IF ( ASSOCIATED( X ) ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_VAL) X%MAPPERS_%FREE( MAPPING_OPT, HOOKS )

    ! Free mappers
    IF ( ASSOCIATED( X%MAPPERS_ ) ) THEN
      DEALLOCATE( X%MAPPERS_, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: CUR
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: Y
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: X
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(MAPPING_CACHE_NODE_T), TARGET,  INTENT(INOUT) :: X_
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(IN)    :: X
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(IN)    :: X
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(OUT)   :: Y
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  LOGICAL :: IS_LEAF
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: X_
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: NODE_1
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: NODE_2
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  TYPE(CACHED_MAPPER_COLLECTION_T), POINTER :: MAPPERS
  ...
  MAPPERS => NODE_1%MAPPERS_
  NODE_1%MAPPERS_ => NODE_2%MAPPERS_
  NODE_2%MAPPERS_ => MAPPERS
  MAPPERS => NULL()
...
PP_THREAD_SAFE FUNCTION REMOVE_NODE( ROOT, Z, CACHE_OPT, MAPPING_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: ROOT
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(INOUT) :: Z
  TYPE(CACHE_OPTIONS_T),               INTENT(IN)    :: CACHE_OPT
  TYPE(MAPPING_OPTIONS_T),             INTENT(IN)    :: MAPPING_OPT
  TYPE(HOOKS_T),                       INTENT(INOUT) :: HOOKS
  ...
  ! Free memory.
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_DEALLOCATE_NODE) DEALLOCATE_NODE( Y, CACHE_OPT, MAPPING_OPT, HOOKS )
  Y => NULL()
...
IMPLICIT NONE
  ...
  ! Local variables
  LOGICAL :: IS_LEAF
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: W
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(IN)    :: ROOT
  TYPE(MAPPING_CACHE_NODE_T), POINTER, INTENT(IN)    :: CURRENT
...
IMPLICIT NONE
  ...
  TYPE(MAPPING_CACHE_NODE_T), POINTER,           INTENT(IN)    :: ROOT
  TYPE(MAPPING_CACHE_NODE_T), POINTER,           INTENT(IN)    :: CURRENT
  PROCEDURE(FUNCTION_MAPPING_CACHE_IF), POINTER, INTENT(IN)    :: PFUNCTION
  TYPE(CACHE_OPTIONS_T),                         INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                                 INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  TYPE(MAPPING_CACHE_NODE_T),     POINTER, INTENT(IN)    :: ROOT
  TYPE(MAPPING_CACHE_NODE_T),     POINTER, INTENT(IN)    :: CURRENT
  CLASS(FUNCTOR_MAPPING_CACHE_A), POINTER, INTENT(IN)    :: FUNCTOR
  TYPE(CACHE_OPTIONS_T),                   INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                           INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), INTENT(INOUT) :: ROOT
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MAPPING_CACHE_NODE_T), INTENT(INOUT) :: ROOT
  INTEGER(KIND=JPIB_K),        INTENT(IN)    :: UNIT
  TYPE(CACHE_OPTIONS_T),       INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),               INTENT(INOUT) :: HOOKS
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  TYPE(MAPPING_CACHE_NODE_T), POINTER,   INTENT(INOUT) :: NODE
  ...
  ! Local variables
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: Y
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: PREV
  LOGICAL :: IS_LEAF
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_INIT( ENCODERS_MAP, CACHE_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MAPPING_CACHE_T), INTENT(INOUT) :: ENCODERS_MAP
  TYPE(CACHE_OPTIONS_T),  INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),          INTENT(INOUT) :: HOOKS
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_FREE( ENCODERS_MAP, CACHE_OPT, MAPPING_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_FREE) FREE_NODE( ENCODERS_MAP%ROOT, CACHE_OPT, MAPPING_OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_MINIMUM( ENCODERS_MAP, KEY, CACHE_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ENCODERS_MAP%SIZE .GT. 0 ) THEN
    ...
    ! Recursive deletion of the tree.
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_MINIMUM) MINIMUM( ENCODERS_MAP%ROOT, Y, CACHE_OPT, HOOKS )
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_MAXIMUM( ENCODERS_MAP, KEY, CACHE_OPT, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_GET_SORTED_KEYS( ENCODERS_MAP, SORTED_KEYS, CACHE_OPT, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAPPING_CACHE_ACCESS_OR_CREATE'
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_ACCESS_OR_CREATE( THIS, KEY, PAR, MAPPING_RULES, &
&               MAPPERS, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MAPPING_CACHE_T),                    INTENT(INOUT) :: THIS
  TYPE(FORTRAN_MESSAGE_T),                   INTENT(IN)    :: KEY
  TYPE(PARAMETRIZATION_T),                   INTENT(IN)    :: PAR
  TYPE(MAPPING_RULES_COLLECTION_T),          INTENT(IN)    :: MAPPING_RULES
  TYPE(CACHED_MAPPER_COLLECTION_T), POINTER, INTENT(OUT)   :: MAPPERS
...
&     MAPPING_RULES, MAPPERS, INSERTED, CACHE_OPT, MAPPING_OPT, FILTER_OPT, HOOKS )
...
END FUNCTION MAPPING_CACHE_ACCESS_OR_CREATE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_MATCH( THIS, KEY, CACHE_OPT, LMATCH, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Local variables
  LOGICAL :: ROOT_IS_LEAF
  TYPE(MAPPING_CACHE_NODE_T), POINTER :: SEARCHED_NODE
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_REMOVE( THIS, KEY, CACHE_OPT, MAPPING_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( LMATCH ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_REMOVE_NODE) REMOVE_NODE( THIS%ROOT, SEARCHED_NODE, CACHE_OPT, MAPPING_OPT, HOOKS )
    THIS%SIZE = THIS%SIZE - 1
  ENDIF
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_LIST( THIS, UNIT, PREFIX, CACHE_OPT, HOOKS ) RESULT(RET)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAPPING_CACHE_APPLY_FUNCTION'
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_APPLY_FUNCTION( THIS, FUNCTION, CACHE_OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(MAPPING_CACHE_T),                        INTENT(INOUT) :: THIS
  PROCEDURE(FUNCTION_MAPPING_CACHE_IF), POINTER, INTENT(IN)    :: FUNCTION
  TYPE(CACHE_OPTIONS_T),                         INTENT(IN)    :: CACHE_OPT
  TYPE(HOOKS_T),                                 INTENT(INOUT) :: HOOKS
...
END FUNCTION MAPPING_CACHE_APPLY_FUNCTION
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAPPING_CACHE_APPLY_FUNCTOR'
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_APPLY_FUNCTOR( THIS, FUNCTOR, CACHE_OPT, HOOKS ) RESULT(RET)
...
END FUNCTION MAPPING_CACHE_APPLY_FUNCTOR
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
PP_THREAD_SAFE FUNCTION MAPPING_CACHE_PRINT( THIS, NAME, IDX, CACHE_OPT, HOOKS ) RESULT(RET)
