...
!>
!> This module defines the `FILTER_CHEM_T` type, along with its associated
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_CHEM_T
  ...
  !> @details If `.TRUE.`, the `CHEMS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: CHEMS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_CHEM_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_CHEM_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_CHEM_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_CHEM_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_CHEM_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_CHEM_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_CHEM_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_CHEM_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_CHEM_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_CHEM_FREE
...
PUBLIC :: FILTER_CHEM_T

CONTAINS


!>
!> @brief Initializes the `FILTER_CHEM_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_CHEM_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_CHEM_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_CHEM_INIT'
FUNCTION FILTER_CHEM_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_CHEM_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%CHEMS_%INIT( HOOKS )
...
&            .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%CHEMS_, HOOKS )
...
!>
!> @param [inout] THIS  The filter object (`FILTER_CHEM_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CHEM_MATCH'
FUNCTION FILTER_CHEM_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = (MSG%CHEM .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = (MSG%CHEM .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )

    IF ( MSG%CHEM .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%CHEMS_%MATCH( MSG%CHEM, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%CHEM_ .EQ. MSG%CHEM)
      ENDIF
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    ELSE
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%CHEM_ .EQ. MSG%CHEM)
      ENDIF
    ENDIF
...
END FUNCTION FILTER_CHEM_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_CHEM_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CHEM_PRINT'
FUNCTION FILTER_CHEM_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'CHEM FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_CHEM_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_CHEM_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_CHEM_FREE'
FUNCTION FILTER_CHEM_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%CHEMS_%FREE( HOOKS )
...
END FUNCTION FILTER_CHEM_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_CHEM_MOD
