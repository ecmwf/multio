...
PUBLIC :: STATISTICAL_PROCESSING_T
...
&       PARSE_STATYPE( MSG, STATTYPE, NBLOCKS, NTIME_LOOPS, HOOKS )
  ...
  PP_DEBUG_CRITICAL_COND_THROW( NTIME_LOOPS .GT. 2_JPIB_K, ERRFLAG_UNEXPECTED_NUMBER_OF_TIMELOOPS )

  ! Fill time description
  TIME_DESCRIPTION%NUMBER_OF_TIME_RANGES = NTIME_LOOPS
  DO I = 1, NTIME_LOOPS
    IF ( I .EQ. NTIME_LOOPS ) THEN  ! Last / inner loop
      TIME_DESCRIPTION%TYPE_OF_STATISTICAL_PROCESSING(I) = OUTER_OP
      ...
      TIME_DESCRIPTION%LENGTH_OF_TIME_RANGE(I) = MSG%TIMESPAN ! TimeSpan is in hours
      ...
      TIME_DESCRIPTION%LENGTH_OF_TIME_INCREMENT(I) = INT(PAR%TIME%LENGTH_OF_TIME_STEP_IN_SECONDS_, KIND=JPIB_K)

    ELSEIF ( I .LT. NTIME_LOOPS ) THEN  ! Iterate over stattypes
      TIME_DESCRIPTION%TYPE_OF_STATISTICAL_PROCESSING(I) = STATTYPE(I,2)
      TIME_DESCRIPTION%TYPE_OF_TIME_INCREMENT(I) = 2_JPIB_K ! Should be always 2 for multIO
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_TIME_ENUM) INDICATOR_OF_UNIT_FOR_TIME( 'h', TIME_DESCRIPTION%INDICATOR_OF_UNIT_FOR_TIME_RANGE(I), HOOKS )
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PERIOD_IN_HOURS) PERIOD_TO_HOURS( STATTYPE(I,1), TIME_DESCRIPTION%LENGTH_OF_TIME_RANGE(I), END_YEAR, END_MONTH, HOOKS )
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_TIME_ENUM) INDICATOR_OF_UNIT_FOR_TIME( 's',  TIME_DESCRIPTION%INDICATOR_OF_UNIT_FOR_TIME_INCREMENT(I), HOOKS )

      ! Set the timeIncrement from lengthOfTimeRange of the next inner-loop
      IF ( I .EQ. NTIME_LOOPS-1_JPIB_K ) THEN
        ! If we iterate the last stattype, timeIncrement is timespan of the inner loop (from param/timespan)
        TIME_DESCRIPTION%LENGTH_OF_TIME_INCREMENT(I) = MSG%TIMESPAN*3600_JPIB_K
      ELSE
        ! Otherwise, timeIncrement is timespan of the next stattype
        PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_PERIOD_IN_HOURS) PERIOD_TO_HOURS( STATTYPE(I+1,1), TMP, END_YEAR, END_MONTH, HOOKS )
        TIME_DESCRIPTION%LENGTH_OF_TIME_INCREMENT(I) = TMP*3600_JPIB_K
      ENDIF
    ENDIF
  ENDDO
