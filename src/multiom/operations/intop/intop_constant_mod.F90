!>
!> @file intop_constant_mod.F90
!>
!> @brief Module containing definitions and procedures for constanteter operations.
!>
!> This module defines the `INTOP_CONSTANT_T` type, along with its associated
...
#define PP_FILE_NAME 'intop_constant_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'INTOP_CONSTANT_MOD'
MODULE INTOP_CONSTANT_MOD
...
!>
!> This derived type extends `INTOP_BASE_A` and is used for operationing based on constanteters.
...
!>
TYPE, EXTENDS(INTOP_BASE_A) :: INTOP_CONSTANT_T
...
CONTAINS
  ...
  !> @details This procedure sets up the `INTOP_CONSTANT_T` type, initializing its components.
  !>
  !> @constant [in] this The instance of `INTOP_CONSTANT_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => INTOP_CONSTANT_INIT
  ...
  !>
  !> @constant [in] this The instance of `INTOP_CONSTANT_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => INTOP_CONSTANT_EVAL
  ...
  !>
  !> @constant [in] this The instance of `INTOP_CONSTANT_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => INTOP_CONSTANT_PRINT

  !> @brief Frees resources allocated for the operation.
  !> @details Cleans up the `INTOP_CONSTANT_T` type, deallocating any resources used by the operation.
  !>
  !> @constant [in] this The instance of `INTOP_CONSTANT_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => INTOP_CONSTANT_FREE
...
PUBLIC :: INTOP_CONSTANT_T

CONTAINS


!>
!> @brief Initializes the `INTOP_CONSTANT_T` operation from a YAML configuration.
!>
!> This function initializes the `INTOP_CONSTANT_T` type based on the provided YAML
...
!>
!> @constant [inout] THIS The operation object (`INTOP_CONSTANT_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'INTOP_CONSTANT_INIT'
FUNCTION INTOP_CONSTANT_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(INTOP_CONSTANT_T),    INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_INTEGER) STRING_IS_INTEGER( CTMP, IS_INTEGER, HOOKS )
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.IS_INTEGER, ERRFLAG_NOT_INTEGER )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TO_INTEGER) STRING_TO_INTEGER( CTMP, THIS%CONSTANT_, HOOKS )
...
END FUNCTION INTOP_CONSTANT_INIT
...
!>
!> @constant [inout] THIS  The operation object (`INTOP_CONSTANT_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_CONSTANT_EVAL'
FUNCTION INTOP_CONSTANT_EVAL( THIS, MSG, PAR, RESULT, HOOKS ) RESULT(RET)
...
END FUNCTION INTOP_CONSTANT_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @constant [inout] THIS    The operation object (`INTOP_CONSTANT_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_CONSTANT_PRINT'
FUNCTION INTOP_CONSTANT_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
END FUNCTION INTOP_CONSTANT_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @constant [inout] THIS   The operation object (`INTOP_CONSTANT_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'INTOP_CONSTANT_FREE'
FUNCTION INTOP_CONSTANT_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION INTOP_CONSTANT_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE INTOP_CONSTANT_MOD
