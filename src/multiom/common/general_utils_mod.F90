...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), DIMENSION(2) :: NENVLN
  INTEGER(KIND=JPIB_K) :: STAT
  LOGICAL, DIMENSION(2) :: IS_DEFINED
  LOGICAL :: READ_ENV
  CHARACTER(LEN=32) :: OMYAML_NAME
  LOGICAL :: EX
  ...
  READ_ENV = .FALSE.
  OMYAML_NAME = REPEAT(' ',LEN(OMYAML_NAME))

  ! Read Output Manager Type
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_ENVVAR) ENVVAR_IS_DEFINED( 'OUTPUT_MANAGER_YAML', IS_DEFINED(1), HOOKS, NDLEN=NENVLN(1) )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_ENVVAR) ENVVAR_IS_DEFINED( 'MULTIO_OUTPUT_MANAGER_YAML', IS_DEFINED(2), HOOKS, NDLEN=NENVLN(2) )

  ! Search the proper name of the environment variable
  IF ( IS_DEFINED(1) .AND. .NOT.IS_DEFINED(2) ) THEN
    OMYAML_NAME = 'OUTPUT_MANAGER_YAML'
    PP_DEBUG_CRITICAL_COND_THROW( NENVLN(1).GT.LEN(OMYAML), ERRFLAG_ENVVAR_TOO_LONG )
    READ_ENV = .TRUE.
  ELSEIF ( IS_DEFINED(2) ) THEN
    OMYAML_NAME = 'MULTIO_OUTPUT_MANAGER_YAML'
    PP_DEBUG_CRITICAL_COND_THROW( NENVLN(2).GT.LEN(OMYAML), ERRFLAG_ENVVAR_TOO_LONG )
    READ_ENV = .TRUE.
  ENDIF
  ...
  IF ( READ_ENV ) THEN
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_ENVVAR) READ_ENVVAR( TRIM(ADJUSTL(OMYAML_NAME)), CTMP, NENVLN(1), HOOKS )
...
IMPLICIT NONE
  ...
  OMTYPE_NAME = REPEAT(' ',LEN(OMTYPE_NAME))
  READ_ENV = .FALSE.

  ! Read Output Manager Type
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_ENVVAR) ENVVAR_IS_DEFINED( 'OUTPUT_MANAGER_TYPE', IS_DEFINED(1), HOOKS, NDLEN=NENVLN(1) )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CHECK_ENVVAR) ENVVAR_IS_DEFINED( 'MULTIO_OUTPUT_MANAGER_TYPE', IS_DEFINED(2), HOOKS, NDLEN=NENVLN(2) )

  ! Search the proper name of the environment variable
  IF ( IS_DEFINED(1) .AND. .NOT.IS_DEFINED(2) ) THEN
    OMTYPE_NAME = 'OUTPUT_MANAGER_TYPE'
    PP_DEBUG_CRITICAL_COND_THROW( NENVLN(1).GT.LEN(OMTYPE), ERRFLAG_ENVVAR_TOO_LONG )
    READ_ENV = .TRUE.
  ELSEIF ( IS_DEFINED(2) ) THEN
    OMTYPE_NAME = 'MULTIO_OUTPUT_MANAGER_TYPE'
    PP_DEBUG_CRITICAL_COND_THROW( NENVLN(2).GT.LEN(OMTYPE), ERRFLAG_ENVVAR_TOO_LONG )
    READ_ENV = .TRUE.
  ENDIF
  ...
  IF ( READ_ENV ) THEN
    ...
    ! Read the environment variable
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_ENVVAR) READ_ENVVAR( TRIM(ADJUSTL(OMTYPE_NAME)), LOC_OMTYPE, NENVLN(1), HOOKS )
