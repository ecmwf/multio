...
IMPLICIT NONE
  ...
  ! Create the filename
  WRITE( FILENAME, '(A,A,I6.6,A,I6.6,A)', IOSTAT=STAT ) TRIM(ADJUSTL(DIRECTORY)), '/profile_', PROC_ID, '_', THREAD_ID ,'.dat'
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_WRITE_FILENAME)
      ...
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'directory: '//TRIM(ADJUSTL(DIRECTORY)) )
      CTMP = REPEAT( ' ', 32)
      WRITE(CTMP,'(I32)') PROC_ID
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'proc id: '//TRIM(ADJUSTL(CTMP)) )
      CTMP = REPEAT( ' ', 32)
      WRITE(CTMP,'(I32)') THREAD_ID
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'thread id: '//TRIM(ADJUSTL(CTMP)) )
