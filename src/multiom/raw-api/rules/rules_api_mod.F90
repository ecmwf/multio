...
IMPLICIT NONE
  ...
  CHARACTER(LEN=FLEN+8) :: RULES_FNAME
  LOGICAL :: EXISTS
...
& C_MARS_DICT_P, C_RULE_NAME_PP ) &
 BIND(C,NAME='multio_grib2_rules_search') RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(C_PTR), VALUE, INTENT(IN) :: C_RULE_NAME_PP
  ...
  TYPE(C_PTR), DIMENSION(:), POINTER :: C_RULE_NAME_P

  CHARACTER(LEN=8192) :: F_RULE_NAME
  ...
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_RULE_ALREADY_ASSOCIATED = 3_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_EXTRACT_MARS_DICTIONARY = 4_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_DEREFERENCE_DOUBLE_POINTER = 5_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_SEARCH_RULE = 6_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_CONVERT_TO_CSTRING = 7_JPIB_K
  ...
  C_RULE_NAME_P => NULL()
  PP_TRYCALL(ERRFLAG_DEREFERENCE_DOUBLE_POINTER) DEREFERENCE_DOUBLE_C_POINTER( &
&   C_RULE_NAME_PP, 1_JPIB_K, C_RULE_NAME_P, HOOKS )

  !> Error handling
  PP_DEBUG_CRITICAL_COND_THROW( C_ASSOCIATED(C_RULE_NAME_P(1)), ERRFLAG_RULE_ALREADY_ASSOCIATED )

  !> Get the rules object info
  F_RULE_NAME = REPEAT(' ', LEN(F_RULE_NAME))
  PP_TRYCALL(ERRFLAG_SEARCH_RULE) SEARCH_RULE( C_RULES_P, &
&    F_MARS_DICT, F_RULE_NAME, &
&    HOOKS )

  !> Populate output pointers
  PP_TRYCALL(ERRFLAG_CONVERT_TO_CSTRING) CONVERT_TO_C_STRING( &
    TRIM(ADJUSTL(F_RULE_NAME)), C_RULE_NAME_P(1), HOOKS )
...
PP_THREAD_SAFE FUNCTION MULTIO_GRIB2_RULES_PRINT( C_RULES_P, &
& C_UNIT, FLEN  ) &
 BIND(C,NAME='multio_grib2_rules_print_f') RESULT(RET)
  ...
  USE :: HOOKS_MOD,             ONLY: HOOKS_T
  USE :: API_GENERAL_UTILS_MOD, ONLY: COPY_CPTR_TO_F_STRING
  USE :: RULES_WRAPPER_MOD,     ONLY: PRINT_RULES
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  TYPE(C_PTR),         VALUE, INTENT(IN) :: C_RULES_P
  TYPE(C_PTR),         VALUE, INTENT(IN) :: C_UNIT
  INTEGER(KIND=C_INT), VALUE, INTENT(IN) :: FLEN
  ...
  !> Local error flags
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_RULES_NOT_ASSOCIATED = 1_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_UNABLE_TO_READ_FUNIT = 2_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_UNABLE_TO_OPEN_FUNIT = 3_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_UNABLE_TO_PRINT = 4_JPIB_K
  INTEGER(KIND=C_INT), PARAMETER :: ERRFLAG_UNABLE_TO_CLOSE_FUNIT = 5_JPIB_K
  ...
  !> Get the type of the encoder to be created
  F_UNIT = REPEAT(' ', LEN(F_UNIT))
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_FUNIT) COPY_CPTR_TO_F_STRING( &
&        C_UNIT, INT(FLEN,C_SIZE_T), F_UNIT, HOOKS, TOLOWER=.FALSE. )

  !> Open Unit
  SELECT CASE( TRIM(ADJUSTL(F_UNIT)) )
  CASE ('stdout')
    UNIT = OUTPUT_UNIT
    OFFSET = 0_JPIB_K
    NEED_CLOSE = .FALSE.
  CASE ('stderr')
    ...
    NEED_CLOSE = .FALSE.
  CASE DEFAULT
    !> Convert the string to an integer unit number
    OPEN( UNIT=UNIT, FILE=TRIM(ADJUSTL(F_UNIT)), STATUS='NEW', ACTION='WRITE', IOSTAT=RET )
    PP_DEBUG_CRITICAL_COND_THROW( RET .NE. 0_JPIB_K, ERRFLAG_UNABLE_TO_OPEN_FUNIT )
    OFFSET = 0_JPIB_K
    NEED_CLOSE = .TRUE.
  END SELECT

  !> Print Rules
  PP_TRYCALL(ERRFLAG_UNABLE_TO_PRINT) PRINT_RULES( C_RULES_P, &
&   OFFSET, UNIT, HOOKS)

  !> Close Unit
  FLUSH( UNIT=UNIT )
  IF ( NEED_CLOSE ) THEN
    CLOSE( UNIT=UNIT, IOSTAT=RET )
    PP_DEBUG_CRITICAL_COND_THROW( RET .NE. 0_JPIB_K, ERRFLAG_UNABLE_TO_CLOSE_FUNIT )
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_RULES_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Rules not associated' )
    CASE (ERRFLAG_UNABLE_TO_READ_FUNIT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read file unit name' )
    CASE (ERRFLAG_UNABLE_TO_OPEN_FUNIT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to open file unit' )
    CASE (ERRFLAG_UNABLE_TO_PRINT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to print rules' )
    CASE (ERRFLAG_UNABLE_TO_CLOSE_FUNIT)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to close file unit' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown error' )
    END SELECT
