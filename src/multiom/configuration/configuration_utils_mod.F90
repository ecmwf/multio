...
PUBLIC :: STRING_TO_REAL32
PUBLIC :: STRING_TO_REAL64
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_SEARCH_SEPARATOR_FIRST_NUM_ZERO=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_SEARCH_SEPARATOR=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_CHAR_IN_SEPARATOR=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_OOB_IN_OUT_VALUES=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNEXPECTED=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_FAILED_READ_INTEGER=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GLB_IDX_LOWER_THAN_1=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GLB_IDX_BIGGER_THAN_STRING=9_JPIB_K
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: SEARCH_SEPARATOR_FIRST_NUMBER_IS_ZERO_STATE=5
  INTEGER(KIND=JPIB_K), PARAMETER :: SEARCH_SEPARATOR_STATE=6
  INTEGER(KIND=JPIB_K), PARAMETER :: VALID_VALUE_STATE=7
  INTEGER(KIND=JPIB_K), PARAMETER :: LAST_VALID_VALUE_STATE=8
  INTEGER(KIND=JPIB_K), PARAMETER :: EXIT_STATE=9
  INTEGER(KIND=JPIB_K), PARAMETER :: ERROR_STATE=10
  ...
  StatesLoop: DO WHILE (LOOP)
    ...
    CASE (CHECK_FIRST_NUMBER_NO_PULL_STATE)
      ...
      ELSEIF ( GLB_IDX.LT.LEN_TRIM(STRING) .AND. (C.EQ.'0') ) THEN
        STATE = SEARCH_SEPARATOR_FIRST_NUMBER_IS_ZERO_STATE
    ...
    !> Check if the character after a single 0 as the first number is the separator character
    CASE (SEARCH_SEPARATOR_FIRST_NUMBER_IS_ZERO_STATE)
      ...
      IF ( GLB_IDX .LT. 1 ) THEN
        ...
        CYCLE StatesLoop
      ENDIF
      IF ( GLB_IDX .GE. LEN_TRIM(STRING) ) THEN
        ERR_IDX = ERRFLAG_GLB_IDX_BIGGER_THAN_STRING
        STATE=ERROR_STATE
        CYCLE StatesLoop
      ENDIF
      ...
      ELSE
        ERR_IDX = ERRFLAG_INVALID_CHAR_IN_SEARCH_SEPARATOR_FIRST_NUM_ZERO
        STATE = ERROR_STATE
      ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_INVALID_CHAR_IN_SEARCH_SEPARATOR_FIRST_NUM_ZERO)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Invalid character in search separator after zero: '//TRIM(ADJUSTL(TMPSTR)) )
...
IMPLICIT NONE
  ...
  ! Error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CALL_READ_REAL = 1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_ALLOCATE = 2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_DEALLOCATE = 3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_ERROR_READING_REAL_ARRAY = 4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_INVALID_FLAG = 5_JPIB_K
  ...
  main : DO
    ...
    IF ( BUF .EQ. '[' .AND. CNT .EQ. 0 ) THEN
      ...
      PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STAT .NE. 0, ERRFLAG_UNABLE_TO_ALLOCATE )
      THIS => HEAD
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_READ_REAL) PARSE_REAL64( UNIT, POS, TERM, THIS%PAR, S, HOOKS )
      R = .FALSE.
      PP_DEBUG_CRITICAL_COND_THROW( .NOT.EX, ERRFLAG_INVALID_FLAG)
      ! IF (.NOT. EX ) EXIT main
      CYCLE main
    ...
    IF ( BUF .NE. '[' .AND. &
      ...
      PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STAT .NE. 0, ERRFLAG_UNABLE_TO_ALLOCATE )
      THIS => HEAD
      PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_READ_REAL) PARSE_REAL64( UNIT, POS, TERM, THIS%PAR, S, HOOKS, BUF )
      R = .FALSE.
      PP_DEBUG_CRITICAL_COND_THROW( .NOT.EX, ERRFLAG_INVALID_FLAG)
      ! IF (.NOT. EX ) EXIT main
      CYCLE main
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! HAndle different errors
    SELECT CASE(ERRIDX)
    CASE(ERRFLAG_UNABLE_TO_CALL_READ_REAL)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to call read real' )
    CASE(ERRFLAG_UNABLE_TO_ALLOCATE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to allocate' )
      IF ( ALLOCATED( ERRMSG ) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error message: '//TRIM(ADJUSTL(ERRMSG)) )
    ...
    CASE(ERRFLAG_UNABLE_TO_DEALLOCATE)
      ...
      IF ( ALLOCATED( ERRMSG ) ) THEN
        ...
        DEALLOCATE( ERRMSG, STAT=DEALLOC_STAT )
      END IF
    CASE(ERRFLAG_ERROR_READING_REAL_ARRAY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error reading real array' )
    CASE(ERRFLAG_INVALID_FLAG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Invalid flag' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
