...
INTERFACE IFS2MARS_SET_MODEL
MODULE PROCEDURE IFS2MARS_SET_MODEL_ATM
MODULE PROCEDURE IFS2MARS_SET_MODEL_WAM
END INTERFACE
...
PUBLIC :: IFS2MARS_SET_DATETIME       ! DATE/TIME/STEP/TIMESPAN
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'IFS2MARS_SET_IDENTIFICATION'
PP_THREAD_SAFE FUNCTION IFS2MARS_SET_IDENTIFICATION( ATM_MSG, IFS_PAR, MSG, PAR, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(OM_BASE_MSG_A),    INTENT(IN)    :: ATM_MSG
  TYPE(MODEL_PAR_T),       INTENT(IN)    :: IFS_PAR
  TYPE(FORTRAN_MESSAGE_T), INTENT(INOUT) :: MSG
  TYPE(PARAMETRIZATION_T), INTENT(INOUT) :: PAR
  TYPE(HOOKS_T),           INTENT(INOUT) :: HOOKS
  ...
  ! Local variables
  INTEGER(KIND=JPIB_K) :: IFS_TYPE

  ! Local error flags
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOW_TYPE_OF_PROCESSED_DATA=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CONVERT_TYPE=2_JPIB_K
  ...
  ! Set the message type
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_TYPE) CTYPE2ITYPE( IFS_PAR%SIM_%CTYPE, IFS_TYPE, HOOKS )

  ! TODO MIVAL: this check is very naive, it should be replaced with a more complex one
  SELECT CASE ( IFS_TYPE )
  CASE ( TYPE_FC_E )
    PAR%TYPE_OF_PROCESSED_DATA = TYPE_OF_PROCESSED_DATA_FC_E
  CASE ( TYPE_AN_E )
    PAR%TYPE_OF_PROCESSED_DATA = TYPE_OF_PROCESSED_DATA_FC_E
  CASE ( TYPE_CF_E )
    PAR%TYPE_OF_PROCESSED_DATA = TYPE_OF_PROCESSED_DATA_CF_E
  CASE ( TYPE_PF_E )
    PAR%TYPE_OF_PROCESSED_DATA = TYPE_OF_PROCESSED_DATA_PF_E
  CASE ( TYPE_SSD_E, TYPE_GSD_E )
    PAR%TYPE_OF_PROCESSED_DATA = TYPE_OF_PROCESSED_DATA_SO_E
  CASE DEFAULT
    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOW_TYPE_OF_PROCESSED_DATA )
  END SELECT
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! HAndle different errors
    SELECT CASE(ERRIDX)
    CASE(ERRFLAG_UNKNOW_TYPE_OF_PROCESSED_DATA)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown type of processed data' )
    CASE(ERRFLAG_UNABLE_TO_CONVERT_TYPE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to convert type of processed data' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
...
END FUNCTION IFS2MARS_SET_IDENTIFICATION
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'IFS2MARS_SET_MODEL_ATM'
PP_THREAD_SAFE FUNCTION IFS2MARS_SET_MODEL_ATM( ATM_MSG, IFS_PAR, MSG, PAR, HOOKS ) RESULT(RET)
...
END FUNCTION IFS2MARS_SET_MODEL_ATM
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'IFS2MARS_SET_MODEL_WAM'
PP_THREAD_SAFE FUNCTION IFS2MARS_SET_MODEL_WAM( WAM_MSG, IFS_PAR, MSG, PAR, HOOKS ) RESULT(RET)
...
END FUNCTION IFS2MARS_SET_MODEL_WAM
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CHECK_TYPE=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_SET_ANALYSIS=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_COMPUTE_TIMESPAN=8_JPIB_K
  ...
  ! Compute time proc
  PP_TRYCALL(ERRFLAG_COMPUTE_TIMESPAN) COMPUTE_TIME_PROC( IFS_MSG%ISTEP_, IFS_PAR%SIM_%TSTEP, &
...
&  IFS_MSG%IPREVPP_, IFS_MSG%PARAM_ID_, LOC_IS_VALID, ISECSTART, ISECEND, ISTEPTYPE, HOOKS )
  ...
  IF ( ISTEPTYPE .NE. TYPE_OF_STATISTICAL_PROCESS_INSTANT_E ) THEN
     MSG%TIMESPAN = (ISECEND - ISECSTART)/HOURS2SECONDS
  ENDIF

  ! If it is analysis DATE/TIME/STEP should be set to the analysis time
  IF ( IFS_PAR%SIM_%CTYPE .EQ. 'an' .OR. (IFS_PAR%SIM_%CTYPE .EQ. 'fc' .AND. MSG%STREAM .EQ. STREAM_CLTE_E) ) THEN
    PP_TRYCALL(ERRFLAG_UNABLE_TO_SET_ANALYSIS) IFS2MARS_SET_ANALYSIS_TIME( &
&     DATE, TIME, STEP, TIMESTEP_IN_SECONDS, MSG, PAR, HOOKS )
  ELSE
    MSG%DATE = DATE
...
&      MSG%LEVTYPE .EQ. LEVTYPE_ML_E ) THEN
      ...
      PAR%DATA_REPRESENTATION%LAPLACIAN_SCALE_FACTOR_ = 0.5_JPRD_K
...
&          MSG%LEVTYPE .EQ. LEVTYPE_ML_E ) THEN
    ...
    ELSE
      ...
      PAR%DATA_REPRESENTATION%LAPLACIAN_SCALE_FACTOR_ = 0.0_JPRD_K
    ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NEED_FIT_SPECTRUM)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error computing LNFT' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
& CDTYPE, KPREVPP, KGRIBCD, LDVALID, ISECSTART, ISECEND, ISTEPTYPE, HOOKS ) RESULT(RET)
  ...
  ! Grib codes
  USE :: GRIB_CODES_MOD, ONLY: NGRBAVG_LICGI
  USE :: GRIB_CODES_MOD, ONLY: NGRBAVG_LITOTI
  ...
  USE :: GRIB_CODES_MOD, ONLY: NGRBCSFWE
  USE :: GRIB_CODES_MOD, ONLY: NGRBDSRP
  USE :: GRIB_CODES_MOD, ONLY: NGRBESWE
  USE :: GRIB_CODES_MOD, ONLY: NGRBEVA
  USE :: GRIB_CODES_MOD, ONLY: NGRBEWSS
  USE :: GRIB_CODES_MOD, ONLY: NGRBFDIR
  USE :: GRIB_CODES_MOD, ONLY: NGRBFREQ_PTYPE
  USE :: GRIB_CODES_MOD, ONLY: NGRBFZRAWE
  USE :: GRIB_CODES_MOD, ONLY: NGRBGPP
  USE :: GRIB_CODES_MOD, ONLY: NGRBGWD
  USE :: GRIB_CODES_MOD, ONLY: NGRBLGWS
  USE :: GRIB_CODES_MOD, ONLY: NGRBLSFWE
  USE :: GRIB_CODES_MOD, ONLY: NGRBLSP
  USE :: GRIB_CODES_MOD, ONLY: NGRBLSPF
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAX_2T
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAX_CAPES
  USE :: GRIB_CODES_MOD, ONLY: NGRBMIN_2T
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAXERA
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAX_I10FG
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAX_MUCAPE
  USE :: GRIB_CODES_MOD, ONLY: NGRBMAX_TPRATE
  USE :: GRIB_CODES_MOD, ONLY: NGRBMGWS
  USE :: GRIB_CODES_MOD, ONLY: NGRBMINERA
  USE :: GRIB_CODES_MOD, ONLY: NGRBMIN_TPRATE
  USE :: GRIB_CODES_MOD, ONLY: NGRBNEE
  USE :: GRIB_CODES_MOD, ONLY: NGRBNSSS
  USE :: GRIB_CODES_MOD, ONLY: NGRBPAR
  USE :: GRIB_CODES_MOD, ONLY: NGRBPARCS
  USE :: GRIB_CODES_MOD, ONLY: NGRBPEVA
  USE :: GRIB_CODES_MOD, ONLY: NGRBREC
  USE :: GRIB_CODES_MOD, ONLY: NGRBROWE
  USE :: GRIB_CODES_MOD, ONLY: NGRBSEV_PTYPE
...
IMPLICIT NONE
  ...
  ! TODO: Remove 49 and 201 with later version of IFS / Dump!
  CASE(NGRBMAX_2T,NGRBMAX_I10FG,NGRBMAX_TPRATE,NGRBMAX_MUCAPE,NGRBMAX_CAPES,49,201)
    ISTEPTYPE = TYPE_OF_STATISTICAL_PROCESS_MAX_E

  ! TODO: Remove 202 with later version of IFS / Dump!
  CASE(NGRBMIN_2T,NGRBMIN_TPRATE,202)
  ...
  CASE(NGRBCH4WET, &
    ...
    & NGRBCDIR,NGRBSSRDC,NGRBSTRDC,NGRBSURFRO,NGRBSSURFRO,NGRBEVA,NGRBPEVA, &
    & NGRBESWE,NGRBSNOM,NGRBLSP,NGRBCP,NGRBSF,NGRBROWE,NGRBTP,NGRBCSFWE,NGRBLSFWE,&
    & NGRBPARCS,NGRBPAR,NGRBSUND,NGRBNEE,NGRBGPP,NGRBREC,NGRBFZRAWE,NGRBDSRP,&
  ...
  CASE(NGRBAVG_LITOTI,NGRBAVG_LICGI)
