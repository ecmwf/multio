...
!>
!> This module defines the `FILTER_HDATE_T` type, along with its associated
...
#define PP_FILE_NAME 'filter_hdate_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_HDATE_MOD'
MODULE FILTER_HDATE_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_HDATE_T
  ...
  !> @details If `.TRUE.`, the `HDATES_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: HDATES_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_HDATE_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_HDATE_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_HDATE_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_HDATE_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_HDATE_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_HDATE_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_HDATE_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_HDATE_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_HDATE_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_HDATE_FREE
...
PUBLIC :: FILTER_HDATE_T

CONTAINS


!>
!> @brief Initializes the `FILTER_HDATE_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_HDATE_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_HDATE_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_HDATE_INIT'
FUNCTION FILTER_HDATE_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_HDATE_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%HDATES_%INIT( HOOKS )
...
&            .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
      ...
      !> Read all the subconfigurations
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%HDATES_, HOOKS )
...
END FUNCTION FILTER_HDATE_INIT
...
!>
!> @param [inout] THIS  The filter object (`FILTER_HDATE_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_HDATE_MATCH'
FUNCTION FILTER_HDATE_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CASE ( FLT_INT_HAS_E )
    ...
    MATCH = (MSG%HDATE .NE. UNDEF_PARAM_E)

  CASE ( FLT_INT_LACKS_E )

    MATCH = (MSG%HDATE .EQ. UNDEF_PARAM_E)

  CASE( FLT_INT_MATCH_E )

    IF ( MSG%HDATE .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%HDATES_%MATCH( MSG%HDATE, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%HDATE_ .EQ. MSG%HDATE)
      ENDIF
    ENDIF
  ...
  CASE( FLT_INT_IGNORE_E )
    ...
    ELSE
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%HDATE_ .EQ. MSG%HDATE)
      ENDIF
    ENDIF
  ...
  CASE( FLT_INT_GT_E )
    ...
    IF ( MSG%HDATE .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      MATCH = (MSG%HDATE .GT. THIS%HDATE_ )
    ENDIF
  ...
  CASE( FLT_INT_GE_E )
    ...
    IF ( MSG%HDATE .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      MATCH = (MSG%HDATE .GE. THIS%HDATE_ )
    ENDIF
  ...
  CASE( FLT_INT_LE_E )
    ...
    IF ( MSG%HDATE .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      MATCH = (MSG%HDATE .LE. THIS%HDATE_ )
    ENDIF
  ...
  CASE( FLT_INT_LT_E )
    ...
    IF ( MSG%HDATE .EQ. UNDEF_PARAM_E ) THEN
      MATCH = .FALSE.
    ELSE
      MATCH = (MSG%HDATE .LT. THIS%HDATE_ )
    ENDIF
...
END FUNCTION FILTER_HDATE_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_HDATE_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_HDATE_PRINT'
FUNCTION FILTER_HDATE_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'HDATE FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_HDATE_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_HDATE_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_HDATE_FREE'
FUNCTION FILTER_HDATE_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%HDATES_%FREE( HOOKS )
...
END FUNCTION FILTER_HDATE_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_HDATE_MOD
