...
public:
    ...
    std::pair<typename std::unordered_map<std::string, multio::MaestroCdo>::iterator, bool> insert(
        std::pair<std::string, multio::MaestroCdo>&& val) {
    ...
    size_t erase(const std::string& cdo_name) { return map_.erase(cdo_name); }
