!>
!> @file grib2_section1_tables_factory_mod.F90
!>
!> @brief Module containing the factory function for creating or initializing GRIB2 Param Configurator objects.
!>
!> The `GRIB2_SECTION1_TABLES_FACTORY_MOD` provides a factory function that creates or initializes
...
!>   - @dependency [TYPE] GRIB2_TABLES_CONFIGURATOR_000_MOD::GRIB2_TABLES_CONFIGURATOR_000_T
...
#define PP_FILE_NAME 'grib2_section1_tables_factory_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION1_TABLES_FACTORY_MOD'
MODULE GRIB2_SECTION1_TABLES_FACTORY_MOD
...
!>
!> Key to be used to recover the tables_configurator number from the YAML configuration.
CHARACTER(LEN=*), PARAMETER :: TABLES_CONFIGURATOR_KEY='type'


!>
!> Generic interface to the make encoder function.
INTERFACE MAKE_GRIB2_TABLES_CONFIGURATOR
  MODULE PROCEDURE MAKE_GRIB2_TABLES_CONFIGURATOR_CFG
  MODULE PROCEDURE MAKE_GRIB2_TABLES_CONFIGURATOR_LAZY
END INTERFACE

!>
!> Public symbols (dataTypes)
PUBLIC :: MAKE_GRIB2_TABLES_CONFIGURATOR
PUBLIC :: DESTROY_GRIB2_TABLES_CONFIGURATOR
...
!> based on the provided parameters. It assigns the proper type (`GRIB2_TABLES_CONFIGURATOR_000_T`)
!> to the `GRIB2_TABLES_CONFIGURATOR` object and configures it using the provided model parameters,
!> ID, and YAML configuration. If verbose mode is enabled, additional debug information
!> is output during the process.
!>
!> @param [inout] GRIB2_TABLES_CONFIGURATOR The GRIB2 Param Configurator object that will be created or initialized.
...
!>   - `GRIB2_TABLES_CONFIGURATOR_000_T`
!>
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see GRIB2_TABLES_CONFIGURATOR_000_T
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_TABLES_CONFIGURATOR_CFG'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_TABLES_CONFIGURATOR_CFG( GRIB2_TABLES_CONFIGURATOR, SEC4ID, &
&               CFG, OPT, FACTORIES, HOOKS ) RESULT(RET)

  !> Symbols imported from other modules within the project.
  USE :: DATAKINDS_DEF_MOD,                 ONLY: JPIB_K
  USE :: GRIB_SECTION_BASE_MOD,             ONLY: GRIB_SECTION_BASE_A
  USE :: GRIB2_SECTION1_TABLES_DEFAULT_MOD, ONLY: GRIB2_SECTION1_TABLES_DEFAULT_T
  USE :: GRIB2_SECTION1_TABLES_CUSTOM_MOD,  ONLY: GRIB2_SECTION1_TABLES_CUSTOM_T
...
IMPLICIT NONE
  ...
  ! Dummy arguments
  CLASS(GRIB_SECTION_BASE_A), POINTER, INTENT(INOUT) :: GRIB2_TABLES_CONFIGURATOR
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) READ_GRIB2_TABLES_CONFIGURATOR_TYPE_FROM_CFG( CFG, SEC4ID, ID, HOOKS )
  ...
  CASE( 0 )
    ...
    ALLOCATE( GRIB2_SECTION1_TABLES_DEFAULT_T::GRIB2_TABLES_CONFIGURATOR, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )

  CASE( 1 )

    ALLOCATE( GRIB2_SECTION1_TABLES_CUSTOM_T::GRIB2_TABLES_CONFIGURATOR, STAT=ALLOC_STATUS, ERRMSG=ERRMSG )
    PP_DEBUG_CRITICAL_COND_THROW( ALLOC_STATUS.NE.0, ERRFLAG_ALLOCATION_ERROR )
  ...
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_TABLES_CONFIGURATOR%INIT( CFG, OPT, FACTORIES, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_READ_SECTION_TYPE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get tables_configurator ID' )
    CASE (ERRFLAG_UNKNOWN_SECTION_0)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown tables_configurator number: '//TRIM(ADJUSTL(TMP)) )
    CASE (ERRFLAG_ALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error allocating tables_configurator number: '//TRIM(ADJUSTL(TMP)) )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_INITIALIZATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error initializing tables_configurator number: '//TRIM(ADJUSTL(TMP)) )
    CASE DEFAULT
...
END FUNCTION MAKE_GRIB2_TABLES_CONFIGURATOR_CFG
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'MAKE_GRIB2_TABLES_CONFIGURATOR_LAZY'
PP_THREAD_SAFE FUNCTION MAKE_GRIB2_TABLES_CONFIGURATOR_LAZY( GRIB2_TABLES_CONFIGURATOR, SEC4ID, &
...
#if defined( PP_HAS_GET_GRIB2_TABLES_CONFIGURATOR_ID_FROM_MESSAGE )
  !> Symbols imported from the mapping module
  USE :: MARS2GRIB_MAPPING_MOD, ONLY: GET_GRIB2_TABLES_CONFIGURATOR_ID_FROM_MESSAGE
#endif
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SECTION_TYPE) GET_GRIB2_TABLES_CONFIGURATOR_ID_FROM_MESSAGE( MSG, PAR, SEC4ID, OPT, ID, HOOKS )
  ...
  !> Initialization of the section
  PP_TRYCALL(ERRFLAG_INITIALIZATION_ERROR)  GRIB2_TABLES_CONFIGURATOR%INIT( MSG, PAR, OPT, FACTORIES, HOOKS )
...
END FUNCTION MAKE_GRIB2_TABLES_CONFIGURATOR_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> This function takes a GRIB2 Param Configurator object (`GRIB2_TABLES_CONFIGURATOR`) and performs
...
!>
!> @section interface
!> @param[in,out] GRIB2_TABLES_CONFIGURATOR The GRIB2 Param Configurator object to be destroyed.
...
!>   - @dependency [*] PP_TRACE_USE_VARS::*
!>
!> @see MAKE_GRIB2_TABLES_CONFIGURATOR
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'DESTROY_GRIB2_TABLES_CONFIGURATOR'
PP_THREAD_SAFE FUNCTION DESTROY_GRIB2_TABLES_CONFIGURATOR( GRIB2_TABLES_CONFIGURATOR, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_DEBUG_CRITICAL_COND_THROW( .NOT.ASSOCIATED(GRIB2_TABLES_CONFIGURATOR), ERRFLAG_NOT_ASSOCIATED )

  !> Free the section
  PP_TRYCALL(ERRFLAG_FREE_ERROR)  GRIB2_TABLES_CONFIGURATOR%FREE( OPT, DESTRUCTORS, HOOKS )

  ! Initialize the section
  DEALLOCATE( GRIB2_TABLES_CONFIGURATOR, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG )
  PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )

  ! Nullify the pointer after deallocation
  GRIB2_TABLES_CONFIGURATOR => NULL()
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ASSOCIATED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'tables_configurator to destroy is not associated' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating tables_configurator number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
        DEALLOCATE(ERRMSG)
      ENDIF
    CASE (ERRFLAG_FREE_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error destructing tables_configurator' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION DESTROY_GRIB2_TABLES_CONFIGURATOR
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!> @section interface
!> @param [in]     CFG             YAML configuration object used to configure the GRIB2 Param Configurator object.
!> @param [out]    TABLES_CONFIGURATOR_TYPE   Identifier of the tables_configurator type read from the configuration.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'READ_GRIB2_TABLES_CONFIGURATOR_TYPE_FROM_CFG'
PP_THREAD_SAFE FUNCTION READ_GRIB2_TABLES_CONFIGURATOR_TYPE_FROM_CFG( CFG, &
&  SEC4ID, TABLES_CONFIGURATOR_TYPE, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  TYPE(YAML_CONFIGURATION_T), INTENT(IN)    :: CFG
  INTEGER(KINd=JPIB_K),       INTENT(IN)    :: SEC4ID
  INTEGER(KIND=JPIB_K),       INTENT(OUT)   :: TABLES_CONFIGURATOR_TYPE
  TYPE(HOOKS_T),              INTENT(INOUT) :: HOOKS
  ...
  INTEGER(KIND=JPIB_K) :: DEALLOC_STATUS
  CHARACTER(LEN=:), ALLOCATABLE :: ERRMSG
  CHARACTER(LEN=:), ALLOCATABLE :: CTABLES_CONFIGURATOR_TYPE
  LOGICAL :: HAS_TABLES_CONFIGURATOR
  LOGICAL :: TABLES_CONFIGURATOR_IS_INTEGER
  ...
  ! Initialization
  TABLES_CONFIGURATOR_TYPE = -1_JPIB_K

  !> Check if configuration has the TABLES_CONFIGURATOR key
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, TABLES_CONFIGURATOR_KEY, HAS_TABLES_CONFIGURATOR, HOOKS )

  !> Read the TABLES_CONFIGURATOR template number
  IF ( HAS_TABLES_CONFIGURATOR ) THEN
    PP_TRYCALL(ERRFLAG_READ_ERROR) YAML_READ_STRING( CFG, TABLES_CONFIGURATOR_KEY, CTABLES_CONFIGURATOR_TYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CTABLES_CONFIGURATOR_TYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_TRYCALL( ERRFLAG_STRING_IS_INTEGER ) STRING_IS_INTEGER( CTABLES_CONFIGURATOR_TYPE, TABLES_CONFIGURATOR_IS_INTEGER, HOOKS )
    IF ( TABLES_CONFIGURATOR_IS_INTEGER ) THEN
       PP_TRYCALL( ERRFLAG_STRING_TO_INTEGER ) STRING_TO_INTEGER( CTABLES_CONFIGURATOR_TYPE, TABLES_CONFIGURATOR_TYPE, HOOKS )
    ELSE
      SELECT CASE (CTABLES_CONFIGURATOR_TYPE)
      CASE ( 'default' )
        TABLES_CONFIGURATOR_TYPE = 0_JPIB_K
      CASE ( 'custom' )
        TABLES_CONFIGURATOR_TYPE = 1_JPIB_K
      CASE DEFAULT
      ...
      END SELECT
    ENDIF
    DEALLOCATE(CTABLES_CONFIGURATOR_TYPE, STAT=DEALLOC_STATUS, ERRMSG=ERRMSG)
    PP_DEBUG_CRITICAL_COND_THROW( DEALLOC_STATUS.NE.0, ERRFLAG_DEALLOCATION_ERROR )
  ELSE
    ! TODO
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_NOT_ALLOCATED_AFTER_READ)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'TABLES_CONFIGURATOR ctype not allocated after reading' )
    CASE (ERRFLAG_DEALLOCATION_ERROR)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error deallocating TABLES_CONFIGURATOR number' )
      IF ( ALLOCATED(ERRMSG) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( TRIM(ADJUSTL(ERRMSG)) )
...
END FUNCTION READ_GRIB2_TABLES_CONFIGURATOR_TYPE_FROM_CFG
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE GRIB2_SECTION1_TABLES_FACTORY_MOD
