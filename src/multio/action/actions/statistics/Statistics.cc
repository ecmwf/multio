...
#include "multio/action/utils/TemporalStatistics.h"
...
void Statistics::executeImpl(message::Message msg) const {
  ...
  {
    util::ScopedTiming timing{statistics_.localTimer_,
                              statistics_.actionTiming_};

    LOG_DEBUG_LIB(LibMultio)
        << "*** " << msg.destination() << " -- metadata: " << md << std::endl;

    // Create a unique key for the fieldStats_ map
    os << msg.metadata().getString("param") << msg.metadata().getLong("level")
       << msg.source();
    ...
    md.set("stepRange",
           fieldStats_.at(os.str())->stepRange(md.getLong("step")));
    md.set("currentDate",
           fieldStats_.at(os.str())->current().endPoint().date().yyyymmdd());
    md.set("currentTime",
           fieldStats_.at(os.str())->current().endPoint().time().hhmmss());
    ...
    if (md.has("step") && md.has("timeStep")) {
      ...
      auto stepRangeInHours =
          std::to_string(prevStep) + "-" + std::to_string(stepInHours);
...
void Statistics::print(std::ostream& os) const {
  os << "Statistics(output frequency = " << timeSpan_
     << ", unit = " << timeUnit_ << ", operations = ";
  bool first = true;
