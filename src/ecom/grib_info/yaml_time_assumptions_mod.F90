...
IMPLICIT NONE
  ...
  PP_LOG_DEVELOP_STR( 'Reading file: '//TRIM(ADJUSTL(YAMLFNAME)) )
...
&                   SPECIAL_ASSUMPTIONS(I)%MATCHER%IS_ENSAMBLE .EQV. IS_ENSAMBLE ] )
  ...
  ! Logging
  PP_LOG_DEVELOP_COND_STR( (CNT.EQ.1), '(A128)', ' + ASSUMPTIONS_RULES_LOG: applied rule: '//TRIM(ADJUSTL(RMATCH))//') to field: '//TRIM(ADJUSTL(FLDSTR)) )
  PP_LOG_DEVELOP_COND_STR( (CNT.NE.1), '(A128)', ' + ASSUMPTIONS_RULES_LOG: applied default rule to field: '//TRIM(ADJUSTL(FLDSTR)) )
