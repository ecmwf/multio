...
!>   - `STRINGOP_BASE_A`: An abstract class that defines the interface for all
...
#define PP_FILE_NAME 'stringop_base_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'STRINGOP_BASE_MOD'
MODULE STRINGOP_BASE_MOD
IMPLICIT NONE
...
!>
!> @section interface
!> - @param [inout] THIS  - The operation object to be initialized (must be of type `STRINGOP_BASE_A`).
...
!> - @dependency [TYPE] STRINGOP_BASE_A
!>
...
PP_THREAD_SAFE FUNCTION OPERATION_INIT_IF( THIS, CFG, HOOKS ) RESULT( RET )
  ...
  IMPORT :: STRINGOP_BASE_A

IMPLICIT NONE

  !> Dummy arguments
  CLASS(STRINGOP_BASE_A), INTENT(INOUT) :: THIS
...
!>
!> @section interface
!> - @param [inout] THIS   - The operation object (must be of type `STRINGOP_BASE_A`) that performs the matching.
...
!> related to the `STRINGOP_BASE_A` object. It ensures that the operation object is
...
!>
!> This function outputs the current state of the `STRINGOP_BASE_A` object to the
...
!>
!> @brief Container structure that holds a operation object of type `STRINGOP_BASE_A`.
!>
!> This container type, `OPERATION_CONTAINER_T`, is used to manage a operation object
!> that is based on the abstract class `STRINGOP_BASE_A`. The operation object is
...
!>
TYPE :: STRINGOP_CONTAINER_T

  CLASS(STRINGOP_BASE_A), POINTER :: OPERATION_ => NULL()

END TYPE

!> White list of public symbols
PUBLIC :: STRINGOP_BASE_A
PUBLIC :: STRINGOP_CONTAINER_T

END MODULE STRINGOP_BASE_MOD
