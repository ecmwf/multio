...
# TODO pgeier
# Refactor:
#   - Remove key checking - just assume groups of keys in PDTs
#   - Use type annotiations
...
def evalToFlatKeys(v, groups, default=None, doExtend=True, **kwargs):
    ...
    def extend(g, **kwargs):
        ...
        return evalToFlatKeys(
            groups[g], groups, default=None, doExtend=doExtend, **kwargs
        )
    ...
    def extendList(l, default=None, **kwargs):
        ...
        for li in l:
            extendOrAppend(
                res,
                evalToFlatKeys(
                    li, groups=groups, default=default, doExtend=doExtend, **kwargs
                ),
            )
        return res
    ...
    if isinstance(v, list):
        ...
        if "groups" in v.keys():
            ...
            return extend(
                ...
                **{
                    **kwargs,
                    **{k: v[k] for k in (set(v.keys()).difference({"extend"}))},
                },
            )
            ...
            if v["append"] not in kwargs.keys():
                ...
                return evalToFlatKeys(
                    kwargs[v["append"]], groups, doExtend=doExtend, **kwargs
                )
...
def flattenGroups(g, groups, default="extend", **kwargs):
    ...
    def extendList(l, default=None, **kwargs):
        ...
        for li in l:
            extendOrAppend(
                res, flattenGroups(li, groups=groups, default=default, **kwargs)
            )
        return res
    ...
    if isinstance(g, list):
        ...
        if "groups" in g.keys():
            ...
            return extend(
                ...
                **{
                    **kwargs,
                    **{k: g[k] for k in (set(g.keys()).difference({"extend"}))},
                },
            )
            ...
            if g["append"] not in kwargs.keys():
                ...
                return flattenGroups(
                    kwargs[g["append"]], groups, default=None, **kwargs
                )
...
def flattenGroupsToArcs(
    fromValue, g, groups, default="extend", listNestedOnExtension=True, **kwargs
):
    """
    ...
    def extend(f, g, **kwargs):
        ...
        extendOrAppend(
            ...
            flattenGroupsToArcs(
                ...
                groups,
                default=None,
                listNestedOnExtension=listNestedOnExtension,
                **kwargs,
            ),
        )
        return res
        ...
        if "groups" in g.keys():
            ...
            if listNestedOnExtension:
                ...
                for k in set(g.keys()).difference({"extend"}):
                    extendOrAppend(
                        ret, extendList(fromValue, g[k], default=None, **kwargs)
                    )
                return ret
            else:
                return extend(
...
def valueListForCategory(c):
    ...
    if isinstance(c, list):
        ...
        if "values" in c.keys():
            return c["values"]
        raise ValueError(
            "valueListForCategory: no key 'values' in category dict: {}".format(c)
        )
    else:
        raise ValueError(
            "valueListForCategory: category is no list and no dict: {}".format(c)
        )
...
def categoryValueGroupsAndSubDefinitions(entry):
    ...
    if isinstance(entry, str):
        ...
        if len(entry) == 1:
            ...
            return (
                ...
                else ([entry["name"]] if entry["name"] is not None else None),
                entry["categories"] if "categories" in entry.keys() else None,
            )
        else:
            raise ValueError(
                "categoryValueGroupsAndSubDefinitions: value dict must have exactly one mapping or the keys 'name' and 'groups' (optionally 'categories'): {}".format(
                    entry
                )
            )
    ...
    raise ValueError(
        "categoryValueGroupsAndSubDefinitions: value is no str, no dict and not None: {}".format(
            entry
        )
    )
    ...
    if isinstance(v, list):
        ...
        if "default" in v.keys():
            ...
            if isinstance(vd, str):
                ...
                raise ValueError(
                    "defaultCategoryValue: default value is no string: {}".format(vd)
                )
        if "values" in v.keys():
            return defaultFromList(v["values"])
        else:
            raise ValueError(
                "defaultCategoryValue: no key 'default' or 'values' in category dict: {}".format(
                    v
                )
            )
        ...
        if cName not in finishedCategories:
            ...
            if cName in categorySelection.keys():
                ...
                if not isinstance(cv, str):
                    raise ValueError(
                        "groupsFromCategorySelection: mapped value in categorySelection for category {} is supposed to be a string: {}".format(
                            cName, cv
                        )
                    )
            else:
            ...
            for v in valueListForCategory(cDef):
                ...
                if groups is not None and set(groups).issubset(groupSet):
                    if alreadyMatched is not None:
                        raise ValueError(
                            "checkPDTSForUnqiueSetPerCategory Already matched pdt: {}, cName: {}, groups: {} - alreadyMatched: {}".format(
                                pdtId, cName, groups, alreadyMatched
                            )
                        )
...
def generateAllCategoryCombinations(categories, emitWithDefaultExcluded=False):
    ...
    def generateForSingleCatgeory(cName, cDef):
        ...
        valWithGroups = [
            categoryValueGroupsAndSubDefinitions(v) for v in valueListForCategory(cDef)
        ]
        ...
        for cValName, igroups, subDefs in valWithGroups:
            ...
            (sel1, groups1, egroups1) = (
                {cName: cValName},
                set([] if igroups is None else igroups),
                set([] if egroups is None else egroups),
            )

            if subDefs is None:
                # If the value is default, also emit a selector without the value
                if emitWithDefaultExcluded and (
                    (cValName == cDefValName)
                    or (cValName is None and cDefValName is None)
                ):
    ...
    def combineCategories(cats):
        ...
        for sel1, groups1, egroups1 in generateForSingleCatgeory(cName, cDef):
            for sel2, groups2, egroups2 in combineCategories(
                {k: v for (k, v) in cats.items() if k != cName}
            ):
    ...
    def generateForSingleCatgeory(cName, cDef, pCat=None, pCatVal=None):
        ...
        for cValName, igroups, subDefs in valWithGroups:
            ...
            if valueTosubValueArcs and pCat is not None:
                yield {
                    "source": "{}::{}".format(pCat, pCatVal),
                    "target": "{}::{}".format(cName, cValName),
                }
...
def pdtsMatchingGroupSet(pdt, groupSet, exclusiveGroupSet=set([])):
    ...
    return {
        pdtId: pdtVal
        for (pdtId, pdtVal) in pdt.items()
        if (len(set(pdtVal["groupSet"]).intersection(exclusiveGroupSet)) == 0)
        and set(pdtVal["groupSet"]).issuperset(groupSet)
    }
...
def checkIfAllPDTHaveAtLeastOneSelector(categorySelectorsWithMappedPdt):
    ...
    if mappedPdtIds != allPdtIds:
        raise ValueError(
            "checkIfAllPDTHaveAtLeastOneSelector: The following pdt have not been mapped from a selector: {}".format(
                allPdtIds.difference(mappedPdtIds)
            )
        )
...
def listCategorySelectorsWithMappedPdt(pdt, categories):
    return list(
        (
            (sel, groups, egroups, matchedPdt)
            for (sel, groups, egroups, matchedPdt) in (
                (
                    sel,
                    groups,
                    egroups,
                    pdtsWithFewestKeysMatchingGroupSet(pdt, groups, egroups),
                )
                for (sel, groups, egroups) in generateAllCategoryCombinations(
                    categories
                )
            )
            if matchedPdt is not None
        )
    )
...
def readPDT(fname="input/pdt.yaml"):
    ...
    with open(fname) as f:
        ...
        pdt = {
            k: {
                **v,
                "keySet": evalToFlatKeys(v["keys"], groups),
                "groupSet": flattenGroups(v["groups"], groups),
                "arcs": flattenGroupsToArcs(
                    k, v["groups"], groups, listNestedOnExtension=True
                ),
            }
            for (k, v) in pdt["pdt"].items()
        }
        # print(pdt)
...
keyMappingsForKey = functools.reduce(
    lambda acc, kTup: {
        **acc,
        kTup[1]: (acc[kTup[1]] + [kTup[0]] if kTup[1] in acc.keys() else [kTup[0]]),
    },
    (
        (km, k)
        for (km, m) in keyMappings.items()
        for k in evalToFlatKeys(m, keyMappings)
    ),
    {},
)
...
for pdtId, pdtVal in pdt.items():
    if len(pdtVal["categorySelectors"]) > 1:
        raise ValueError(
            "pdt {} hase multiple categorySelectors: {}".format(
                pId, pdtVal["categorySelectors"]
            )
        )
