!>
!> @file assignment_msg_string_mod.F90
!>
!> @brief Module containing definitions and procedures for variableeter stringops.
!>
!> This module defines the `ASSIGNMENT_MSG_STRING_T` type, along with its associated
...
#define PP_FILE_NAME 'assignment_msg_string_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'ASSIGNMENT_MSG_STRING_MOD'
MODULE ASSIGNMENT_MSG_STRING_MOD
...
!>
TYPE, EXTENDS(ASSIGNMENT_BASE_A) :: ASSIGNMENT_MSG_STRING_T
...
CONTAINS
  ...
  !> @details This procedure sets up the `ASSIGNMENT_MSG_STRING_T` type, initializing its components.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_STRING_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => ASSIGNMENT_MSG_STRING_INIT
  ...
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_STRING_T`.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: EVAL => ASSIGNMENT_MSG_STRING_EVAL
  ...
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_STRING_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => ASSIGNMENT_MSG_STRING_PRINT

  !> @brief Frees resources allocated for the stringop.
  !> @details Cleans up the `ASSIGNMENT_MSG_STRING_T` type, deallocating any resources used by the stringop.
  !>
  !> @variable [in] this The instance of `ASSIGNMENT_MSG_STRING_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => ASSIGNMENT_MSG_STRING_FREE
...
PUBLIC :: ASSIGNMENT_MSG_STRING_T

CONTAINS


!>
!> @brief Initializes the `ASSIGNMENT_MSG_STRING_T` stringop from a YAML configuration.
!>
!> This function initializes the `ASSIGNMENT_MSG_STRING_T` type based on the provided YAML
...
!>
!> @variable [inout] THIS The stringop object (`ASSIGNMENT_MSG_STRING_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_STRING_INIT'
FUNCTION ASSIGNMENT_MSG_STRING_INIT( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(ASSIGNMENT_MSG_STRING_T), INTENT(INOUT) :: THIS
  ...
  PROCEDURE(FUN_C2I_IF), POINTER :: P_CMSGSTRINGFLDS2IMSGSTRINGFLDS
  LOGICAL :: HAS_NAME
  LOGICAL :: HAS_TYPE
  ...
  P_CMSGSTRINGFLDS2IMSGSTRINGFLDS => CMSGSTRINGFLDS2IMSGSTRINGFLDS
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SOURCE) YAML_READ_INTEGER_WITH_FILTER( CFG, 'name', THIS%VARIABLE_ID_, P_CMSGSTRINGFLDS2IMSGSTRINGFLDS, HOOKS )
...
END FUNCTION ASSIGNMENT_MSG_STRING_INIT
...
!>
!> @variable [inout] THIS  The stringop object (`ASSIGNMENT_MSG_STRING_T`) used for matching.
!> @variable [in]    MSG   The message (`FORTRAN_MESSAGE_T`) that is checked against the stringop.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_STRING_EVAL'
FUNCTION ASSIGNMENT_MSG_STRING_EVAL( THIS, IN_MSG, IN_PAR, OUT_MSG, OUT_PAR, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_MSG_STRING_EVAL
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS    The stringop object (`ASSIGNMENT_MSG_STRING_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_STRING_PRINT'
FUNCTION ASSIGNMENT_MSG_STRING_PRINT( THIS, UNIT, OFFSET, HOOKS, SEPARATOR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_ENUM_TO_STRING) IMSGSTRINGFLDS2CMSGSTRINGFLDS( THIS%VARIABLE_ID_, VARIABLE_NAME, HOOKS )
...
END FUNCTION ASSIGNMENT_MSG_STRING_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @variable [inout] THIS   The stringop object (`ASSIGNMENT_MSG_STRING_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'ASSIGNMENT_MSG_STRING_FREE'
FUNCTION ASSIGNMENT_MSG_STRING_FREE( THIS, HOOKS ) RESULT(RET)
...
END FUNCTION ASSIGNMENT_MSG_STRING_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE ASSIGNMENT_MSG_STRING_MOD
