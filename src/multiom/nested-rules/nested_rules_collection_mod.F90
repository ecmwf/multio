...
PP_THREAD_SAFE FUNCTION RULE_MATCH( THIS, MSG, PAR, TAG, NAME, SAMPLE, ENCODER_CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=256),              INTENT(OUT)   :: SAMPLE
  TYPE(YAML_CONFIGURATION_T),      INTENT(OUT)   :: ENCODER_CFG
  TYPE(HOOKS_T),                   INTENT(INOUT) :: HOOKS
  ...
  TYPE(NESTED_RULE_CONTAINER_T), DIMENSION(MAX_MATCHING_RULES_P), TARGET :: RULES
  TYPE(NESTED_RULE_CONTAINER_T), DIMENSION(:), POINTER :: P_RULES
