...
PRIVATE
  ...
  CONTAINS
    PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT => TIME_PAR_INIT
...
#define PP_PROCEDURE_NAME 'TIME_PAR_INIT'
PP_THREAD_SAFE FUNCTION TIME_PAR_INIT( TIME_PAR, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_FREE( TIME_PAR, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_SET_INITIAL_STEP( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_GET_INITIAL_STEP( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_SET_LENGTH_OF_TIME_STEP_IN_SECONDS( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_GET_LENGTH_OF_TIME_STEP_IN_SECONDS( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_SET_LENGTH_OF_TIME_RANGE_IN_SECONDS( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_GET_LENGTH_OF_TIME_RANGE_IN_SECONDS( TIME_PAR, VALUE, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION TIME_PAR_COPY_FROM( TIME_PAR_TO, TIME_PAR_FROM, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION READ_TIME_PAR_FROM_YAML( TIME_PAR, CONFIG, HOOKS ) RESULT(RET)
...
PP_THREAD_SAFE FUNCTION WRITE_TIME_PAR_TO_YAML( TIME_PAR, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Check if the value is valid
  CONDITIONS(1) = TIME_PAR%INITIAL_STEP_ .NE. UNDEF_PARAM_E
  CONDITIONS(2) = TIME_PAR%LENGTH_OF_TIME_STEP_IN_SECONDS_ .NE. UNDEF_PARAM_E
  CONDITIONS(3) = TIME_PAR%LENGTH_OF_TIME_RANGE_IN_SECONDS_ .NE. UNDEF_PARAM_E
  ...
  IF ( ANY(CONDITIONS) ) THEN
    ...
    IF ( CONDITIONS(1) ) THEN
      ...
      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'initial-step: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF
    ...
    IF ( CONDITIONS(2) ) THEN
      ...
      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'length-of-time-step-in-seconds: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF
    ...
    IF ( CONDITIONS(3) ) THEN
      ...
      ! Write to the unit
      WRITE( UNIT, '(A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'length-of-time-range-in-seconds: ', TRIM(ADJUSTL(CTMP))
      PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
    ENDIF
