...
IMPLICIT NONE
  ...
  PP_TRYCALL(ERRFLAG_MARS_TO_JSON) MD%DUMP_SAMPLE( 'before_sink.grib', HOOKS )
  ...
  CLASS IS (GRIB_METADATA_T)
    ...
    IF (TMP1 .NE. MSG%LEVELIST ) THEN
      ...
      IF ( ALLOCATED(JSON) ) THEN
        WRITE(*,*)     ' SINK-DP :: LEVEL1 :: ERROR LEVELIST DOES NOT MATCH :: ', TMP1, MSG%LEVELIST
        WRITE(*,'(A)') ' SINK-DP :: LEVEL1 :: ERROR LEVELIST DOES NOT MATCH :: '//TRIM(ADJUSTL(JSON))
  ...
  WRITE(*,*) ' * EXIT FROM GRIB_HEADER2MULTIO_READ_SINK_DP :: '
  WRITE(*,*) ' --------------------------------------------------------'
  WRITE(*,*) ' '
