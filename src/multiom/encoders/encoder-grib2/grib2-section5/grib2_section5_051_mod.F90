!>
!> @file grib2_section5_051_mod.F90
!>
!> @brief Module for managing GRIB2 SECTION 5 operations.
!>
!> The `GRIB2_SECTION5_051_MOD` module contains procedures to initialize, allocate,
...
!>   - @see GRIB2_SECTION5_051_INIT
!>   - @see GRIB2_SECTION5_051_ALLOCATE
!>   - @see GRIB2_SECTION5_051_PRESET
!>   - @see GRIB2_SECTION5_051_RUNTIME
!>   - @see GRIB2_SECTION5_051_TO_BE_ENCODED
!>   - @see GRIB2_SECTION5_051_FREE
!>
...
#define PP_FILE_NAME 'grib2_section5_051_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'GRIB2_SECTION5_051_MOD'
MODULE GRIB2_SECTION5_051_MOD
...
!>
!> The `GRIB2_SECTION5_051_T` type extends the base class `GRIB_SECTION_BASE_A` and
...
!> environments.
!>
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION5_051_T

  !> Default symbols visibility
  PRIVATE

  !> TODO: Add configuration for these variables
  LOGICAL :: LFORCEZ_ = .FALSE.
  LOGICAL :: LFORCELNSP_ = .FALSE.
...
CONTAINS
  ...
  !> GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_CFG => GRIB2_SECTION5_051_INIT_CFG
  ...
  !> the GRIB2 encoder.
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: INIT_LAZY => GRIB2_SECTION5_051_INIT_LAZY
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: ALLOCATE => GRIB2_SECTION5_051_ALLOCATE
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: PRESET => GRIB2_SECTION5_051_PRESET
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: RUNTIME => GRIB2_SECTION5_051_RUNTIME
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: TO_BE_ENCODED => GRIB2_SECTION5_051_TO_BE_ENCODED
  ...
  !>
  PROCEDURE, PUBLIC, PASS, NON_OVERRIDABLE :: FREE => GRIB2_SECTION5_051_FREE
...
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_INIT_CFG'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_INIT_CFG( THIS, &
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(GRIB2_SECTION5_051_T),  INTENT(INOUT) :: THIS
...
END FUNCTION GRIB2_SECTION5_051_INIT_CFG
...
!> @brief Initializes GRIB2 SECTION 5 for a given object using the provided parameters.
!>
!> This function initializes a GRIB2 SECTION 5 object (`THIS`) using the provided model message (`MSG`)
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_INIT_LAZY'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_INIT_LAZY( THIS, &
...
END FUNCTION GRIB2_SECTION5_051_INIT_LAZY
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_ALLOCATE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_ALLOCATE( THIS, &
...
IMPLICIT NONE
  ...
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'dataRepresentationTemplateNumber', 51_JPIB_K )
...
END FUNCTION GRIB2_SECTION5_051_ALLOCATE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [in]    THIS     An object of type `GRIB2_SECTION5_051_T` representing the GRIB section to be preset.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_PRESET'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_PRESET( THIS, &
&  MSG, PAR, OPT, METADATA, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  REAL(KIND=JPRD_K)    :: ZP
  REAL(KIND=JPRD_K)    :: ZBETA0
  REAL(KIND=JPRD_K)    :: ZBETA1
  ...
  !> Error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_METADATA=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_NEED_FIT_SPECTRUM=2_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_NEED_FIT_SPECTRUM) NEED_FIT_SPECTRUM(MSG%TRUNCATION, LNFT,HOOKS)

  IF ( MSG%PARAM .EQ. NGRBLNSP .AND. &
&      MSG%LEVTYPE .EQ. LEVTYPE_ML_E ) THEN
    IBITSSH = PAR%DATA_REPRESENTATION%BITS_PER_VALUE_
  ELSE
    IBITSSH = 16
  ENDIF

  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'numberOfBitsContainingEachPackedValue', PAR%DATA_REPRESENTATION%BITS_PER_VALUE_ )

  IF ( THIS%LFORCEZ_  .AND. &
&      MSG%PARAM   .EQ. NGRBZ  .AND. &
&      MSG%LEVTYPE .EQ. LEVTYPE_ML_E ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'laplacianOperator',0.5_JPRD_K)
  ELSEIF ( THIS%LFORCELNSP_    .AND. &
&          MSG%PARAM   .EQ. NGRBLNSP  .AND. &
&          MSG%LEVTYPE .EQ. LEVTYPE_ML_E ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA,'laplacianOperator',0.5_JPRD_K)
  ELSEIF ( LNFT ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'laplacianOperator',PAR%DATA_REPRESENTATION%LAPLACIAN_SCALE_FACTOR_ )
  ELSE
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'laplacianOperator', 0.0_JPRD_K )
  ENDIF

  IF ( MSG%TRUNCATION .NE. UNDEF_PARAM_E ) THEN
      IF( MSG%TRUNCATION .GE. 213 ) THEN
        ISTRUNC = 20
      ELSE
        ISTRUNC = MIN( 10 ,MSG%TRUNCATION )
      ENDIF
  ELSE
    ISTRUNC = 20
  ENDIF
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'subSetJ', ISTRUNC )
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'subSetK', ISTRUNC )
  PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'subSetM', ISTRUNC )

  ! IF ( MSG%TRUNCATION .NE. UNDEF_PARAM_E ) THEN
  !     ISPEC2 = (MSG%TRUNCATION+1)*(MSG%TRUNCATION+2)
  ! ENDIF
  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'JS', ISTRUNC )
  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'KS', ISTRUNC )
  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'MS', ISTRUNC )
  ! PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'TS', ISPEC2 )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE ( ERRFLAG_METADATA )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error using metadata' )
    CASE ( ERRFLAG_NEED_FIT_SPECTRUM )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'error in NEED_FIT_SPECTRUM' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION GRIB2_SECTION5_051_PRESET
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!> @section interface
!>
!> @param [in]    THIS          An object of type `GRIB2_SECTION5_051_T` representing the GRIB section to be allocated.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_RUNTIME'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_RUNTIME( THIS, &
...
END FUNCTION GRIB2_SECTION5_051_RUNTIME
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @section interface
!>   @param [inout] THIS          An object of type `GRIB2_SECTION5_051_T` representing the GRIB section being checked.
!>   @param [in]    MSG           The message object of type `FORTRAN_MESSAGE_T` used to handle preset-related messaging.
!>   @param [in]    PAR           The parametrization structure of type `PARAMETRIZATION_T` used for the preset operation.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_TO_BE_ENCODED'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_TO_BE_ENCODED( THIS, &
...
END FUNCTION GRIB2_SECTION5_051_TO_BE_ENCODED
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB2_SECTION5_051_FREE'
PP_THREAD_SAFE FUNCTION GRIB2_SECTION5_051_FREE(  THIS, OPT, DESTRUCTORS, HOOKS ) RESULT(RET)
...
END FUNCTION GRIB2_SECTION5_051_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE GRIB2_SECTION5_051_MOD
