...
!>
!> This module defines the `FILTER_STEP_T` type, along with its associated
...
#define PP_FILE_NAME 'filter_step_mod.F90'
#define PP_SECTION_TYPE 'MODULE'
#define PP_SECTION_NAME 'FILTER_STEP_MOD'
MODULE FILTER_STEP_MOD
...
!>
TYPE, EXTENDS(FILTER_BASE_A) :: FILTER_STEP_T
  ...
  !> @details If `.TRUE.`, the `STEPS_` keyset is used for filtering specific parameter IDs.
  LOGICAL :: USE_KEYSET_
  ...
  TYPE(KEYSET_INT64_T) :: STEPS_

CONTAINS

  !> @brief Initializes the filter parameter type.
  !> @details This procedure sets up the `FILTER_STEP_T` type, initializing its components.
  !>
  !> @param [in] this The instance of `FILTER_STEP_T` to initialize.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: INIT  => FILTER_STEP_INIT
  ...
  !>
  !> @param [in] this The instance of `FILTER_STEP_T`.
  ...
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: MATCH => FILTER_STEP_MATCH

  !> @brief Prints the filter's details for debugging or logging.
  !> @details Outputs the filter's configuration and current state.
  !>
  !> @param [in] this The instance of `FILTER_STEP_T` to print.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: PRINT => FILTER_STEP_PRINT

  !> @brief Frees resources allocated for the filter.
  !> @details Cleans up the `FILTER_STEP_T` type, deallocating any resources used by the filter.
  !>
  !> @param [in] this The instance of `FILTER_STEP_T` to free.
  PROCEDURE, NON_OVERRIDABLE, PUBLIC, PASS :: FREE  => FILTER_STEP_FREE
...
PUBLIC :: FILTER_STEP_T

CONTAINS


!>
!> @brief Initializes the `FILTER_STEP_T` filter from a YAML configuration.
!>
!> This function initializes the `FILTER_STEP_T` type based on the provided YAML
...
!>
!> @param [inout] THIS The filter object (`FILTER_STEP_T`) that will be initialized.
...
#define PP_PROCEDURE_NAME 'FILTER_STEP_INIT'
FUNCTION FILTER_STEP_INIT( THIS, CFG, OPT, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CLASS(FILTER_STEP_T),      INTENT(INOUT) :: THIS
  ...
  PP_TRYCALL(ERRFLAG_INIT_KEYSET) THIS%STEPS_%INIT( HOOKS )
...
&          .NOT.HAS_TRESHOLD .AND. .NOT.HAS_VALUE ) THEN
    ...
    !> Read all the subconfigurations
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_VALUES) YAML_READ_INTEGER_KEYSET_WITH_RANGES( CFG, 'values', THIS%STEPS_, HOOKS )
...
!>
!> @param [inout] THIS  The filter object (`FILTER_STEP_T`) used for matching.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_STEP_MATCH'
FUNCTION FILTER_STEP_MATCH( THIS, MSG, MATCH, HOOKS, PAR ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Match the filter
  IF ( MSG%STEP .EQ. UNDEF_PARAM_E ) THEN
  ...
  ELSE
    ...
    CASE( FLT_INT_MATCH_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        PP_TRYCALL(ERRFLAG_UNABLE_TO_CALL_KEYSET_MATCH) THIS%STEPS_%MATCH( MSG%STEP, MATCH, HOOKS )
      ELSE
        MATCH = (THIS%STEP_ .EQ. MSG%STEP)
      ENDIF
    ...
    CASE( FLT_INT_IGNORE_E )
      ...
      IF ( THIS%USE_KEYSET_ ) THEN
        ...
        MATCH = .NOT.MATCH
      ELSE
        MATCH = .NOT.(THIS%STEP_ .EQ. MSG%STEP)
      ENDIF

    CASE( FLT_INT_GT_E )

      MATCH = (MSG%STEP .GT. THIS%STEP_ )

    CASE( FLT_INT_GE_E )

      MATCH = (MSG%STEP .GE. THIS%STEP_ )

    CASE( FLT_INT_LE_E )

      MATCH = (MSG%STEP .LE. THIS%STEP_ )

    CASE( FLT_INT_LT_E )

      MATCH = (MSG%STEP .LT. THIS%STEP_ )
...
!      PP_DEBUG_PUSH_MSG_TO_FRAME( '"step" filter can be used only with "levtype"="pl"' )
...
END FUNCTION FILTER_STEP_MATCH
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS    The filter object (`FILTER_STEP_T`) whose details are to be printed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_STEP_PRINT'
FUNCTION FILTER_STEP_PRINT( THIS, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  WRITE(UNIT,'(A)') REPEAT(' ',OFFSET)//'STEP FILTER: '//TRIM(ADJUSTL(OPERATION_TYPE_STR))
...
END FUNCTION FILTER_STEP_PRINT
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
!>
!> @param [inout] THIS   The filter object (`FILTER_STEP_T`) whose resources are to be freed.
...
!>
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'FILTER_STEP_FREE'
FUNCTION FILTER_STEP_FREE( THIS, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Free keyset
  PP_TRYCALL(ERRFLAG_UNABLE_TO_FREE_KEYSET) THIS%STEPS_%FREE( HOOKS )
...
END FUNCTION FILTER_STEP_FREE
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE

END MODULE FILTER_STEP_MOD
