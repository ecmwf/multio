...
IMPLICIT NONE
  ...
  ! Print TIMEPROC
  IF ( THIS%TIMEPROC .NE. '********' ) THEN
    WRITE(UNIT,'(A,A)',IOSTAT=WRITE_STAT) ' - TIMEPROC....: ', TRIM(ADJUSTL(THIS%TIMEPROC))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
...
IMPLICIT NONE
  ...
  IF ( THIS%TIMEPROC .NE. '********' ) THEN
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = '"timeproc":"'//TRIM(ADJUSTL(THIS%TIMEPROC))//'"'
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 2
  ENDIF
...
IMPLICIT NONE
  ...
  IF ( THIS%PARAM .NE. UNDEF_PARAM_E ) THEN
    ...
    CNT = CNT + 1
    JSON(CNT) = REPEAT(' ',128)
    JSON(CNT) = 'paramId-'//TRIM(ADJUSTL(CTMP))
    TOTLEN = TOTLEN + LEN(TRIM(ADJUSTL(JSON(CNT)))) + 1
  ENDIF
...
IMPLICIT NONE
  ...
  IF ( OPT%USE_COMPRESSED_CACHE .AND. THIS%LEVTYPE .EQ. LEVTYPE_PL_E ) THEN
    ...
    ELSE
      TMP1 = 1
    ENDIF
    ! CONDITIONS(16) =  ( THIS%LEVELIST .EQ. OTHER%LEVELIST )
    CONDITIONS(16) =  ( TMP0 .EQ. TMP1 )
  ELSE
    CONDITIONS(16) = .TRUE.
  ENDIF
  CONDITIONS(17) =  ( THIS%DIRECTION .EQ. OTHER%DIRECTION )
  CONDITIONS(18) =  ( THIS%FREQUENCY .EQ. OTHER%FREQUENCY )
  CONDITIONS(19) =  ( THIS%REPRES .EQ. OTHER%REPRES )
  CONDITIONS(20) =  ( THIS%GRID .EQ. OTHER%GRID )
  CONDITIONS(21) =  ( THIS%TRUNCATION .EQ. OTHER%TRUNCATION )
...
IMPLICIT NONE
  ...
  IF ( OPT%USE_COMPRESSED_CACHE .AND. THIS%LEVTYPE .EQ. LEVTYPE_PL_E ) THEN
    ...
    ELSE
      TMP1 = 1
    ENDIF
    ! CONDITIONS(16,1) =  ( THIS%LEVELIST .EQ. OTHER%LEVELIST )
  ...
  CONDITIONS(19,1) =  ( THIS%REPRES .EQ. OTHER%REPRES )
  CONDITIONS(20,1) =  ( THIS%GRID .EQ. OTHER%GRID )
  CONDITIONS(21,1) =  ( THIS%TRUNCATION .EQ. OTHER%TRUNCATION )
  ...
  IF ( THIS%LEVTYPE .EQ. LEVTYPE_PL_E ) THEN
    IF ( THIS%LEVELIST .LT. 100 ) THEN
      TMP0 = 0
    ELSE
      TMP0 = 1
    ...
    ELSE
      TMP1 = 1
    ENDIF
    ! CONDITIONS(16,2) = ( THIS%LEVELIST .LE. OTHER%LEVELIST )
    CONDITIONS(16,2) = ( TMP0 .LE. TMP1 )
  ELSE
    CONDITIONS(16,2) = .TRUE.
  ENDIF
  CONDITIONS(17,2) = ( THIS%DIRECTION .LE. OTHER%DIRECTION )
  CONDITIONS(18,2) = ( THIS%FREQUENCY .LE. OTHER%FREQUENCY )
  CONDITIONS(19,2) = ( THIS%REPRES .LE. OTHER%REPRES )
  CONDITIONS(20,2) = ( THIS%GRID .LE. OTHER%GRID )
  CONDITIONS(21,2) = ( THIS%TRUNCATION .LE. OTHER%TRUNCATION )
...
IMPLICIT NONE
  ...
  IF ( HAS_MESSAGE  ) THEN
    ...
    !> Read the "param"
    PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_CONFIGURATION_HAS_KEY( MSG_CONFIG, 'paramId', HAS_KEY, HOOKS )
    IF ( HAS_KEY ) THEN
      PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_CFG) YAML_READ_INTEGER( MSG_CONFIG, 'paramId', MSG%PARAM, HOOKS )
    ELSE
...
IMPLICIT NONE
  ...
  IF ( THIS%PARAM .NE. UNDEF_PARAM_E ) THEN
    CTMP = REPEAT(' ',32)
    WRITE(CTMP,'(I8)',IOSTAT=WRITE_STAT) THIS%PARAM
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'paramId: '//TRIM(ADJUSTL(CTMP))
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
  ...
  ! Print TIMEPROC
  IF ( THIS%TIMEPROC .NE. '********' ) THEN
    WRITE(UNIT,'(A)',IOSTAT=WRITE_STAT) REPEAT(' ',OFFSET+2)//'timeproc: "'//TRIM(ADJUSTL(THIS%TIMEPROC))//'"'
    PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT .NE. 0, ERRFLAG_IOSTATUS_NOT_ZERO )
  ENDIF
