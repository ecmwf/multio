...
!> Sink file flush options
INTEGER(KIND=JPIB_K), PARAMETER :: SINK_FILE_FLUSH_NEVER_E=1_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: SINK_FILE_FLUSH_STEP_E=2_JPIB_K
INTEGER(KIND=JPIB_K), PARAMETER :: SINK_FILE_FLUSH_MESSAGE_E=3_JPIB_K
...
TYPE, EXTENDS(OUTPUT_MANAGER_BASE_A) :: GRIB_HEADER2MULTIO_OUTPUT_MANAGER_T
  ...
  CHARACTER(LEN=1024) :: SINK_FILE_PATH_ = REPEAT(' ',1024)
  CHARACTER(LEN=1024) :: SINK_FILE_NAME_ = REPEAT(' ',1024)
  LOGICAL :: PROFILE_ = .FALSE.
  LOGICAL :: SAVE_REPORT_ = .FALSE.
  LOGICAL :: VERBOSE_ = .FALSE.
  LOGICAL :: APPEND_STEP_TO_SINK_FILE_ = .FALSE.
  INTEGER(KIND=JPIB_K) :: SINK_FILE_FLUSH_FREQUENCY_ = SINK_FILE_FLUSH_NEVER_E
  ...
  INTEGER(KIND=JPIM_K) :: SINK_FILE_UNIT_ = -99
...
CONTAINS
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_SINK_FILE_PATH_FROM_YAML => GRIB_HEADER2MULTIO_READ_SINK_FILE_PATH_FROM_YAML
  ...
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_SINK_FILE_FROM_YAML => GRIB_HEADER2MULTIO_READ_SINK_FILE_FROM_YAML
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_VERBOSE_FROM_YAML => GRIB_HEADER2MULTIO_READ_VERBOSE_FROM_YAML
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_SINK_FILE_APPEND_STEP_FROM_YAML => GRIB_HEADER2MULTIO_READ_SINK_FILE_APPEND_STEP_FROM_YAML
  PROCEDURE, NON_OVERRIDABLE, PASS, PRIVATE :: READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML => GRIB_HEADER2MULTIO_READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_SINK_FILE=13_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_SINK_FILE_PATH=14_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_SINK_FILE_APPEND_STEP=15_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_READ_SINK_FILE_FLUSH_FREQUENCY=16_JPIB_K
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SINK_FILE_APPEND_STEP) THIS%READ_SINK_FILE_APPEND_STEP_FROM_YAML( GRIB_HEADER2MULTIO_CFG, HOOKS )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SINK_FILE_FLUSH_FREQUENCY) THIS%READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML( GRIB_HEADER2MULTIO_CFG, HOOKS )
  ...
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SINK_FILE) THIS%READ_SINK_FILE_FROM_YAML( GRIB_HEADER2MULTIO_CFG, HOOKS )
  PP_TRYCALL(ERRFLAG_UNABLE_TO_READ_SINK_FILE_PATH) THIS%READ_SINK_FILE_PATH_FROM_YAML( GRIB_HEADER2MULTIO_CFG, HOOKS )
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_READ_SINK_FILE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read the sink file from the configuration file' )
    CASE (ERRFLAG_UNABLE_TO_READ_SINK_FILE_APPEND_STEP)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read the sink file append step from the configuration file' )
    CASE (ERRFLAG_UNABLE_TO_READ_SINK_FILE_FLUSH_FREQUENCY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read the sink file flush frequency from the configuration file' )
    CASE (ERRFLAG_UNABLE_TO_DESTROY_CFG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to destroy the configuration object' )
    CASE (ERRFLAG_UNABLE_TO_READ_SINK_FILE_PATH)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to read the sink file path from the configuration file' )
    CASE DEFAULT
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_READ_SINK_FILE_PATH_FROM_YAML'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_PATH_FROM_YAML( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'sink-file-path', HAS_KEY, HOOKS )

  ! Read or apply the default configuration
  THIS%SINK_FILE_PATH_ = REPEAT(' ',LEN(THIS%SINK_FILE_PATH_))
  IF ( HAS_KEY ) THEN
    IF ( ALLOCATED(LOG_PATH) ) THEN
      DEALLOCATE(LOG_PATH, STAT=STAT)
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'sink-file-path', LOG_PATH, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(LOG_PATH), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_DEBUG_CRITICAL_COND_THROW( LEN(LOG_PATH).GT.LEN(THIS%SINK_FILE_PATH_), ERRFLAG_TOO_LONG )
    THIS%SINK_FILE_PATH_ = LOG_PATH(:)
    PP_TRYCALL(ERRFLAG_PATH_DONT_EXIST) IS_DIRECTORY( THIS%SINK_FILE_PATH_, IS_DIR, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.IS_DIR, ERRFLAG_PATH_DONT_EXIST )
    IF ( ALLOCATED(LOG_PATH) ) THEN
      DEALLOCATE( LOG_PATH, STAT=STAT )
    ENDIF
  ELSE
    THIS%SINK_FILE_PATH_ = './'
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_PATH_DONT_EXIST)
      ...
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Path: "'//TRIM(ADJUSTL(THIS%SINK_FILE_PATH_))//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
...
END FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_PATH_FROM_YAML
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_READ_SINK_FILE_APPEND_STEP_FROM_YAML'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_APPEND_STEP_FROM_YAML( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'append-step-to-sink-file', HAS_KEY, HOOKS )

  ! Read or apply the default configuration
  IF ( HAS_KEY ) THEN
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_LOGICAL( CFG, 'append-step-to-sink-file', THIS%APPEND_STEP_TO_SINK_FILE_, HOOKS )
  ELSE
    THIS%APPEND_STEP_TO_SINK_FILE_ = .FALSE.
  ENDIF
...
END FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_APPEND_STEP_FROM_YAML
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_READ_SINK_FILE_FROM_YAML'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_FROM_YAML( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_TOO_LONG=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_EXTENSION_NOT_ALLOWED=7_JPIB_K
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'sink-file-without-extension', HAS_KEY, HOOKS )
  ...
  IF ( HAS_KEY ) THEN
    ...
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'sink-file-without-extension', CSINKTYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSINKTYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    PP_DEBUG_CRITICAL_COND_THROW( LEN(CSINKTYPE).GT.LEN(THIS%SINK_FILE_NAME_), ERRFLAG_TOO_LONG )
    THIS%SINK_FILE_NAME_ = REPEAT(' ',LEN(THIS%SINK_FILE_NAME_))
    THIS%SINK_FILE_NAME_ = TRIM(ADJUSTL(CSINKTYPE(:)))
    PP_DEBUG_CRITICAL_COND_THROW( INDEX(THIS%SINK_FILE_NAME_, '.').GT.0, ERRFLAG_EXTENSION_NOT_ALLOWED )
    IF ( ALLOCATED(CSINKTYPE) ) THEN
      DEALLOCATE( CSINKTYPE, STAT=STAT, ERRMSG=ERRMSG )
      PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE )
    ENDIF
  ELSE
    THIS%SINK_FILE_NAME_ = REPEAT(' ',LEN(THIS%SINK_FILE_NAME_))
    THIS%SINK_FILE_NAME_ = 'allFields'
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_TOO_LONG)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'The string is too long' )
    CASE (ERRFLAG_EXTENSION_NOT_ALLOWED)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'The extension is not allowed ("." character not allowed in file name)' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'File name: "'//TRIM(ADJUSTL(THIS%SINK_FILE_NAME_))//'"' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
...
END FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_FROM_YAML
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'GRIB_HEADER2MULTIO_READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML'
PP_THREAD_SAFE FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML( THIS, CFG, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! Read from YAML file the flag to enable verbose execution
  PP_TRYCALL(ERRFLAG_HAS_KEY) YAML_CONFIGURATION_HAS_KEY( CFG, 'sink-file-flush-frequency', HAS_KEY, HOOKS )
  ...
  IF ( HAS_KEY ) THEN
    ...
    IF ( ALLOCATED(CSINKTYPE) ) THEN
      ...
      PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE )
    ENDIF
    PP_TRYCALL(ERRFLAG_GET_SUBCFG) YAML_READ_STRING_WITH_ENV_EXPANSION( CFG, 'sink-file-flush-frequency', CSINKTYPE, HOOKS )
    PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CSINKTYPE), ERRFLAG_NOT_ALLOCATED_AFTER_READ )
    SELECT CASE( TRIM(ADJUSTL(CSINKTYPE)) )
    CASE ('never')
      THIS%SINK_FILE_FLUSH_FREQUENCY_ = SINK_FILE_FLUSH_NEVER_E
    CASE ('every-step')
      THIS%SINK_FILE_FLUSH_FREQUENCY_ = SINK_FILE_FLUSH_STEP_E
    CASE ('every-message')
      THIS%SINK_FILE_FLUSH_FREQUENCY_ = SINK_FILE_FLUSH_MESSAGE_E
    CASE DEFAULT
      PP_DEBUG_CRITICAL_THROW( ERRFLAG_CONVERT_STRING_TO_ENUM )
    END SELECT
    IF ( ALLOCATED(CSINKTYPE) ) THEN
      DEALLOCATE( CSINKTYPE, STAT=STAT, ERRMSG=ERRMSG )
      PP_DEBUG_CRITICAL_COND_THROW( STAT.NE.0, ERRFLAG_UNABLE_TO_DEALLOCATE )
...
END FUNCTION GRIB_HEADER2MULTIO_READ_SINK_FILE_FLUSH_FREQUENCY_FROM_YAML
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  CHARACTER(LEN=4096) :: SINK_FILE_NAME
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_OPEN_GRIB_FILE=11_JPIB_K
  ...
  CASE (SINK_HEADER_TO_FILE_E)
    ...
    ! Create the sink file name
    SINK_FILE_NAME = REPEAT( ' ', LEN(SINK_FILE_NAME) )
    SINK_FILE_NAME = TRIM(ADJUSTL(THIS%SINK_FILE_PATH_))//'/'//TRIM(ADJUSTL(THIS%SINK_FILE_NAME_))//'-header.grib'

    ! Open the grib file
    CALL GRIB_OPEN_FILE( FID, TRIM(ADJUSTL(SINK_FILE_NAME)), 'a', STATUS=KRET )
    PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, ERRFLAG_UNABLE_TO_OPEN_GRIB_FILE )
  ...
  CASE (SINK_MESSAGE_TO_FILE_E)
    ...
    ! Create the sink file name
    SINK_FILE_NAME = REPEAT( ' ', LEN(SINK_FILE_NAME) )
    SINK_FILE_NAME = TRIM(ADJUSTL(THIS%SINK_FILE_PATH_))//'/'//TRIM(ADJUSTL(THIS%SINK_FILE_NAME_))//'.grib'
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_SINK=10_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_OPEN_GRIB_FILE=11_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_GET_JSON=12_JPIB_K
  ...
  CASE (SINK_MESSAGE_TO_FDB_E)
    ...
    PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_JSON) MSG%TO_JSON( JSON, HOOKS )
    ...
    IF (ALLOCATED(JSON)) THEN
      DEALLOCATE(JSON, STAT=DEALLOC_STAT)
    ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_WRITE_BINARY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to write binary to multio' )
      IF ( ALLOCATED(JSON) ) THEN
        PP_DEBUG_PUSH_MSG_TO_FRAME( 'mars: '//TRIM(ADJUSTL(JSON)) )
      ENDIF
    CASE (ERRFLAG_UNKNOWN_SINK)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unknown sink' )
    CASE (ERRFLAG_UNABLE_TO_OPEN_GRIB_FILE)
      ...
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to open the file' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'grib error: '//TRIM(ADJUSTL(GRIB_ERROR)) )
    CASE (ERRFLAG_UNABLE_TO_GET_JSON)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get the JSON from the message' )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unhandled error' )
    END SELECT
...
&                                                       PROCESSOR_TOPO%MYPROC_IO, THIS%TID_, HOOKS )
  ...
  ! CALL GRIB_OPEN_FILE( THIS%SINK_FILE_UNIT_, TRIM(ADJUSTL(SINK_FILE_NAME)), 'a', STATUS=KRET )
  ! PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, ERRFLAG_UNABLE_TO_OPEN_GRIB_FILE )
...
IMPLICIT NONE
  ...
  ! Close the grib file
  ! PP_LOG_STR( 'CLose sink file: '//TRIM(ADJUSTL(CTMP)) )
  ! CALL GRIB_CLOSE_FILE( THIS%SINK_FILE_UNIT_, STATUS=KRET )
  ! PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, ERRFLAG_UNABLE_TO_CLOSE_GRIB_FILE )
