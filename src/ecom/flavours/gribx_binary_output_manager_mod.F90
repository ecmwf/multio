...
IMPLICIT NONE
  ...
  IF ( CFG%GET( TOLOWER(GRIBX_BINARY_OMNAME), GRIBX_BIN_CFG ) ) THEN
    ...
    IF ( GRIBX_BIN_CFG%GET( 'path', CLTMP  ) ) THEN
      ...
      WRITE(THIS%FIELDS_FILE_,'(A,A,I8.8,A)') TRIM(CTMP_PATH), '_', THIS%TOPOLOGY_%MYPROC_IO, '.grib'
      IF ( ALLOCATED(CLTMP) ) DEALLOCATE( CLTMP )
    ELSE
    ...
    IF ( ALLOCATED(CLTMP) ) DEALLOCATE(CLTMP)
    IF ( GRIBX_BIN_CFG%GET( 'samples-path', CLTMP  ) ) THEN
      PP_DEBUG_CRITICAL_COND_THROW( .NOT.ALLOCATED(CLTMP), 1 )
...
IMPLICIT NONE
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) TOLOWER(GRIBX_BINARY_OMNAME)//'-output-manager-', PROCESSOR_TOPO%MYPROC_IO, '.log'
