...
FUNCTION ANALYSIS_PAR_FREE( ANALYSIS_PAR, HOOKS ) RESULT(RET)
...
FUNCTION ANALYSIS_PAR_COPY_FROM( ANALYSIS_PAR_TO, ANALYSIS_PAR_FROM, HOOKS ) RESULT(RET)
...
FUNCTION READ_ANALYSIS_PAR_FROM_YAML( ANALYSIS_PAR, CONFIG, HOOKS ) RESULT(RET)
...
FUNCTION WRITE_ANALYSIS_PAR_TO_YAML( ANALYSIS_PAR, UNIT, OFFSET, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  IF ( ANALYSIS_PAR%LENGTH_OF_TIME_WINDOW_ .EQ. UNDEF_PARAM_E ) THEN
    CTMP='"undefined"'
  ELSE
  ...
  ! Write to the unit
  WRITE( UNIT, '(A,A,A,A)', IOSTAT=WRITE_STAT ) REPEAT(' ', OFFSET+2), 'length-of-time-window: ', TRIM(ADJUSTL(CTMP)), ' # length of time window used in analysis'
  PP_DEBUG_CRITICAL_COND_THROW( WRITE_STAT.NE.0, ERRFLAG_WRITE_ERROR )
