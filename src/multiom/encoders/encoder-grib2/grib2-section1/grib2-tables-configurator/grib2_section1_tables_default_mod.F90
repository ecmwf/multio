...
TYPE, EXTENDS(GRIB_SECTION_BASE_A) :: GRIB2_SECTION1_TABLES_DEFAULT_T
  ...
  INTEGER(KIND=JPIB_K) :: TABLES_VERSION_ = UNDEF_PARAM_E
...
IMPLICIT NONE
  ...
  ELSE
    ...
    THIS%TABLES_VERSION_ = UNDEF_PARAM_E
  END IF
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIM_K) :: LATEST_TABLE_VERSION

  !> Error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_METADATA=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_GRIB_GET_FAILED=2_JPIB_K
  ...
  ELSEIF ( THIS%TABLES_VERSION_ .NE. UNDEF_PARAM_E ) THEN
    PP_METADATA_SET( METADATA, ERRFLAG_METADATA, 'tablesVersion', THIS%TABLES_VERSION_ )
  ELSE

    ! This case should be unreached at the moment, but the code is here to
    ! show how to lookup the latest tables version from the sample
    SELECT TYPE(GRIB_METADATA => METADATA)

    CLASS IS (GRIB_METADATA_T)

      ! Extract the latest tables version from the sample
      LATEST_TABLE_VERSION = -1_JPIB_K
      PP_TRYCALL(ERRFLAG_UNABLE_TO_GET_HANDLE) GRIB_METADATA%GET_HANDLE( HANDLE, HOOKS )
      CALL GRIB_GET( HANDLE, 'tablesVersionLatest', LATEST_TABLE_VERSION, STATUS=KRET )
      PP_DEBUG_CRITICAL_COND_THROW( KRET.NE.GRIB_SUCCESS, ERRFLAG_GRIB_GET_FAILED )
      PP_DEBUG_CRITICAL_COND_THROW( LATEST_TABLE_VERSION.LT.0, ERRFLAG_GRIB_GET_FAILED )

      ! Lazy initialization for the default tables version
      THIS%TABLES_VERSION_ = LATEST_TABLE_VERSION

    CLASS IS (CGRIB_METADATA_T)

      ! If the metadata is a CGRIB metadata, we cannot set the tables version
      PP_TRYCALL(ERRFLAG_METADATA) GRIB_METADATA%TABLES_VERSION_LATEST( THIS%TABLES_VERSION_, HOOKS )
  ...
  ! Set the tables version to the latest one
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_GRIB_GET_FAILED)
      ...
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Unable to get "latestTablesVersion" from sample' )
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'grib error: '//TRIM(ADJUSTL(GRIB_ERROR)) )
    CASE (ERRFLAG_UNABLE_TO_GET_HANDLE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unable to get handle from metadata' )
