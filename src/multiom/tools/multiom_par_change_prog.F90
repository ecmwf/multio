...
#define PP_FILE_NAME 'multiom_par_prog.F90'
#define PP_SECTION_TYPE 'PROGRAM'
#define PP_SECTION_NAME 'MULTIOM_PAR_PROG'
#define PP_PROCEDURE_TYPE 'PROGRAM'
#define PP_PROCEDURE_NAME 'MULTIOM_PAR_PROG'
PROGRAM MULTIOM_PAR_PROG
  ...
  USE :: MULTIOM_TOOLS_UTILS_MOD, ONLY: MODIFY_PARAMS
...
IMPLICIT NONE
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNABLE_TO_CHECK_ENDIANNES = 6_JPIB_K
  ...
  I = 1_JPIB_K
  WRITE(*,*) 'Parameters ', I
  PP_TRYCALL(ERRFLAG_UNABLE_TO_MAKE_TABLE_OF_CONTENTS) MODIFY_PARAMS( CMDARGS, I, BIG_ENDIAN_READ, VERBOSE, NPROC_IO, HOOKS )

  IF ( NPROC_IO .GT. 1_JPIB_K ) THEN
    DO I = 2_JPIB_K, NPROC_IO
      WRITE(*,*) 'Parameters ', I
      PP_TRYCALL(ERRFLAG_UNABLE_TO_MAKE_TABLE_OF_CONTENTS) MODIFY_PARAMS( CMDARGS, I, BIG_ENDIAN_READ, VERBOSE, NPROC_IO, HOOKS )
    ENDDO
  ENDIF
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_GET_VERBOSITY)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error getting the verbose flag' )
    CASE (ERRFLAG_UNABLE_TO_ENCODE_CMD_LINE)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error encoding the command line arguments' )
    CASE (ERRFLAG_UNABLE_TO_MAKE_TABLE_OF_CONTENTS)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'Error initializing the output manager' )
