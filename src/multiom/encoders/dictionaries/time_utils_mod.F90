...
&  MSG, PAR, TIME_HISTORY, CURR_TIME, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  ! CURR_TIME%ISEC_ = MSG%STEP*PAR%TIME%LENGTH_OF_TIME_STEP_IN_SECONDS_

  ! Add the contribution of the initial step
  CURR_TIME%ISEC_ = MSG%STEP*PAR%TIME%LENGTH_OF_TIME_STEP_IN_SECONDS_ + MSG%STEP*PAR%TIME%INITIAL_STEP_ * HOURS2SECONDS
