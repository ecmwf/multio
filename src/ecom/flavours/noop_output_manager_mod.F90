...
IMPLICIT NONE
  ...
  IF ( CFG%GET( TOLOWER(NOOP_OMNAME), NOOP_CFG ) ) THEN
...
IMPLICIT NONE
  ...
  IF ( THIS%VERBOSE_ ) THEN
    ...
    WRITE(THIS%LOG_FNAME_,'(A,I8.8,A)', IOSTAT=STAT) TOLOWER(NOOP_OMNAME)//'-output-manager-', PROCESSOR_TOPO%MYPROC_IO, '.log'
...
&                                        YDMSG%IREPRES_,  YDMSG%IUID_, ENCODING_INFO )
    ...
    CALL ENCODING_INFO%TIME_HISTORY%ENQUEUE( YDMSG%ISTEP_ )
    EX = ENCODING_INFO%TIME_HISTORY%GET_ALL( TIME_HIST%SIZE_, TIME_HIST%HIST_(1:)  )
