...
IMPLICIT NONE
  ...
  IF ( COMMAND_ARGUMENT_COUNT() .EQ. 4 ) THEN
    ! Get sample path
    CALL GET_COMMAND_ARGUMENT(1, SAMPLES_PATH, LENGTH, STATUS=RET )
    ...
    ! Get encoder rules
    CALL GET_COMMAND_ARGUMENT(2, ENCODER_FNAME, LENGTH, STATUS=RET )
    IF (RET .NE. 0) THEN
      ENCODER_FNAME = 'encoder-rules.yaml'
    END IF
    ! Get mapping rules
    CALL GET_COMMAND_ARGUMENT(3, MAPPING_FNAME, LENGTH, STATUS=RET )
    IF (RET .NE. 0) THEN
      MAPPING_FNAME = 'mapping-rules.yaml'
    END IF
    ! Get mapping rules
    CALL GET_COMMAND_ARGUMENT(4, DEBUG_FNAME, LENGTH, STATUS=RET )
    IF (RET .NE. 0) THEN
